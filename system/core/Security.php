...
class CI_Security {
	...
	public function csrf_verify()
	{
		...
		if ($exclude_uris = config_item('csrf_exclude_uris'))
		{
            ...
            // log_message('error', 'URI string => ' . $uri->uri_string());
		...
		// log_message('error','SERVER: '.json_encode($this->_csrf_cookie_name));
		$local_csrf = '';
		foreach (getallheaders() as $name => $value) {
			if ($this->_csrf_token_name == $name)
				$local_csrf = $value;
		}
		// Do the tokens exist in both the _POST and _COOKIE arrays?
		// if ( ! isset($_POST[$this->_csrf_token_name], $_COOKIE[$this->_csrf_cookie_name]))
		if ( !isset($_COOKIE[$this->_csrf_cookie_name]) || (empty($local_csrf) && !isset($_POST[$this->_csrf_token_name])))
		{
			$this->csrf_show_error();
		}
		//check if isset(post) then check eqality with post and cookied
		// Do the tokens match?
		if (isset($_POST[$this->_csrf_token_name]) && $_POST[$this->_csrf_token_name] != $_COOKIE[$this->_csrf_cookie_name] )
		{
			$this->csrf_show_error();
		}
		if (!isset($_POST[$this->_csrf_token_name]) && ($local_csrf != $_COOKIE[$this->_csrf_cookie_name]))
		{
			$this->csrf_show_error();
		}
		...
		unset($_SERVER[$this->_csrf_token_name]);
		...
		// unset($_COOKIE[$this->_csrf_cookie_name]);
		// if(config_item('csrf_regenerate')){
		// 	$this->_csrf_force_set_hash(); // will delete this function
		// }else{
		// 	$this->_csrf_set_hash();
		// }
	...
	// protected function _csrf_force_set_hash(){
