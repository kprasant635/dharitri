<div class="row p-2" >
  <div class="col-md-6">
    ...
    Whether total patta/ tenancy land held by the applicant family including the land applied for as above exceeds ceiling limit under relevant provisions of the Assam Fixation of Ceiling on Land Holdings Act,1956 ?</span><?=form_error('is_landless')?>
  </div>
  ...
  <div class="col-md-3 col-lg-3 col-sm-6 col-xs-12">
    ...
    <div class="form-check form-check-inline">
      <input
        ...
        id="is_landless1"
        value="YES"
        <?php if(!empty($additional_property)) {echo "checked";}?>
        <?php if(set_value('is_landless') == 'YES'){ echo "checked";} ?>
      />
      <label class="form-check-label" for="is_landless1">Yes</label>
    </div>
    <div class="form-check form-check-inline">
      <input
        class="form-check-input <?php if(form_error('is_landless')){echo 'lm_invalid';}?>"
        ...
        value="NO"
        <?php if(empty($additional_property)) {echo "checked";}?>
        <?php if(set_value('is_landless') == 'NO'){ echo "checked";} ?>
      />
      <label class="form-check-label" for="is_landless2">No</label>
    </div>
  ...
  <div class="col-md-3 col-lg-3 col-sm-6 col-xs-12">
    <?php if(!empty($additional_property)) { ?>
      <button class="btn btn-sm btn-warning btnServerAddProperty" onclick="openAddPropertyModalTeaGrant()" type="button"><i class="fa fa-university"></i>&nbsp;View Property</button>
    <?php } ?>

    <button class="btn btn-sm btn-warning btnJsAddProperty" onclick="openAddPropertyModalTeaGrant()" style="display:none" type="button"><i class="fa fa-university"></i>&nbsp;View Property</button>
...
<div id="myModalAdditionalPropertyTeaGrant" class="modal">
    ...
    <div class="modal-body">
      <div class="row">
        <div id="additional_property" >
          <input type="hidden" name="application_no" id="application_no" value="<?php echo $this->utilityclass->decryptJwtCase($_GET['app']); ?>">
          ...
          <div class="row">
            <div class="col-md-3 col-lg-3 col-sm-6 col-xs-12">
              ...
              <div class="form__div">
                <select name="additional_district_code"
                  class="form-control  form_select ps-3 additional_mselect property_add_reset"
                  id='additional_district'
                          ...
                          <tr id="prop<?php echo $row->id;?>" class="table_list">
                              ...
                              <td>
                                  <?php if($row->applied_flag != CITIZEN) { ?>
                                      <a href="javascript:void(0)" onclick="delTeaAddProperty(<?php echo $row->id;?>)" class="btn btn-danger" id="delProperty">Delete</a>
                                  <?php } ?>
                              </td>
                          </tr>
                      <?php }} ?>
...
<script>
  ...
  // Get the modal
  var modal_additional_property = document.getElementById("myModalAdditionalPropertyTeaGrant");
  ...
  function delTeaAddProperty(id){
    if(confirm("Are you sure you want to delete this Record?")){
      $.ajax({
        type: "POST",
        url: baseurl + "TeaGrantControllerLm/propertydel",
        async: false,
        data: {
          property_id: id
        },
  ...
  $('#submitproperty').click(function(e){
    ...
    $.ajax({
      type: "POST",
      url: baseurl + "TeaGrantControllerLm/addProperty",
      async: false,
      data: {
        ref_no                    : ref_no,
        case_no                   : case_no,
  ...
  function openAddPropertyModalTeaGrant(){
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            if(data.count > 0){
              ...
              $('.btnServerAddProperty').hide(); 
              $("#is_landless1").prop("checked", true);
            }
            if(data.count == 0){
              $('.btnServerAddProperty').hide();
              $('.btnJsAddProperty').hide();
              $("#is_landless2").prop("checked", true);
            }
          }
        });
  ...
  ////btnServerAddProperty  btnJsAddProperty
...
<script type="text/javascript">
  ...
  $(document).ready(function(){
    $("input:radio[name=is_landless]").click(function() 
    {
      if($('input:radio[name=is_landless]:checked').val() == 'YES') 
      {
        openAddPropertyModalTeaGrant();
        $("input[name='is_landless']").prop("checked", false);
      } 
      else 
      {
        modal_additional_property.style.display = "none";
