          ...
          <div class="col-12 text-center">
            ...
            <script>
                ...
                function manualPaymentDetailsSubmitHandle(){
                    ...
                    $.ajax({
                        url: baseurl + "TeaGrantControllerAdc/manualPaymentDetailsSubmitHandle",
                        type: 'POST',
...
<!--     <div class="row">
      <div class="col-md-12 panel panel-body">
        ...
        <?php //if($paid_confirm > 0) { ?>
            <h4 class="text-center mt-2">
              <button class="btn btn-success btn-sm" type="button" id="confirmPaymentForwardCo"><b>Confirm Payment & Forward to CO</b></span>
            </h4>
        <?php //} ?>
    ...
    <?php if($lraVerificationUsed == false): ?>
        <div class="row">
            <div class="col-md-12 panel panel-body">
                <h1 class="text-danger">Final verification is pending at the LRA’s end. Once the verification is complete, any changes made by the LRA will be displayed here, and the Approve button will become available !!!</h1>
            </div>
        </div>
    <?php endif; ?>
...
<script>
    ...
    $('#confirmPaymentForwardCo').click(function(){
      case_no      = '<?php echo $case_no; ?>';
      rtps_case_no = '<?php echo $case_no_rtps; ?>';
      ...
      swalWithBootstrapButtons.fire({
        title             : "Warning !!",
        html              : 'This will be the final forward to CO. Are you sure to proceed ?',
        icon              : 'warning',
        showCancelButton  : true,
        ...
        if (result . isConfirmed) {
          ...
          $.ajax({
            url: baseurl+'TeaGrantControllerAdc/paymentConfirmationForwardToCo',
            type: "POST",
            data: {case_no:case_no, rtps_case_no:rtps_case_no},
            success: function(data) 
            {
              arr = JSON.parse(data);
              $.unblockUI();
...
<script>
    ...
    function finalVerificationModal(case_no)
    {
        ...
        $.ajax({
            url: baseurl+'TeaGrantControllerAdc/getFinalVerificationData',
            type: "POST",
            data: postData,
            success: function(data) 
            {
  ...
  function finalApprove(case_no)
  {
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: baseurl+'TeaGrantControllerAdc/adcApproveLmReport',
                    type: "POST",
                    data: postData,
                    success: function(data) {
                        arr = JSON.parse(data);
...
<script type="text/javascript">
    ...
    function makePaymentDummy()
    {
      case_no = '<?php echo $case_no; ?>';
      $.ajax({
        url      : baseurl + "TeaGrantControllerAdc/makePaymentDummy",
        type     : 'POST',
