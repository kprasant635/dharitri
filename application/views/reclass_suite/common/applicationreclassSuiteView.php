            ...
            <div class="tableCard">
                ...
                <table class="table mb-0">
                    ...
                    <tr>
                        ...
                        <button type="button" onclick="editPropClass(<?=$dagspremlm->dag_no?>);" class="btn btn-sm btn-primary mt-1">
                            <strong>Change Proposed Class</strong>
                        </button>
                         <input type="hidden" name="" id="prop_lc_code<?=$dagspremlm->dag_no?>" value="<?=$dagspremlm->prop_lc_cat_id?>">
                     <?php }?>
                        </td>
                    ...
                    <div class="row p-2">
                        ...
                        <div class="col-md-6">
                            <table class="table table-bordered">
                                ...
                                <tr>
                                    <td><span id="lmfinalamountupdated"> <?=isset($premium_data_lm->final_amount)? $premium_data_lm->final_amount:0;?></span></td>
                                    <td><span id="lmdueamountupdated"> <?=isset($premium_data_lm->due_amount)? $premium_data_lm->due_amount:0;?></span></td>
                                </tr>
                            </table>
...
<script>
    ...
    function showPartitionInfo(dag_no) {
        ...
        $.ajax({
            url: baseurl+'ReclassSuiteControllerCO/getPartionInforDags',
            type: "POST",
            data: postData,
            success: function(data) {
                $.unblockUI();

                let obj = JSON.parse(data); // Parse the response (if not already an object)


                // Check if 'dag_data' exists and is an array
                if (obj.dag_data && Array.isArray(obj.dag_data)) {
                    obj.dag_data.forEach(function(item) {
                        let area = `${item.lm_area_b}B-${item.lm_area_k}K-${item.lm_area_lc}L`;

                        $('#propertyTablePart-'+dag_no).find("tr:gt(0)").remove();

                        let row = `<tr>
                                <td>${area}</td>
                                <td>${item.pdar_id}</td>
                                <td>${item.retain_old_dag == "1" ? "Yes" : "No"}</td>
                            </tr>`;

                        // Append row to the specific table
                        //$("#propertyTablePart-" + item.dag_no).append(row);

                        $('#propertyTablePart-'+item.dag_no).append(row);
                        $('.addPropertyDetailPart-'+item.dag_no).show('slow');
                        $('.viewModalPart-'+item.dag_no).hide('slow');
                        $('.closeModalPart-'+item.dag_no).show('slow');
                    });
                } else {
                    console.error("dag_data is missing or not an array:", obj);
                }
...
<?php include(APPPATH."views/reclass_suite/include/editPropClass.php"); ?>
<script src="<?php echo base_url();?>js/reclassSuite/editPropClass.js"></script>