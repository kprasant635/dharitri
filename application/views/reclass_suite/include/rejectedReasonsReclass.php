                        ...
                        foreach($rejected_list as $rej_list_key => $rej_list)
                        {
                            if($r_head->CODE == $rej_list->remark_head):
                                ...
                                ?> 
                                    <br>
                                    <h6>
                                        <span style="color:blue; cursor:pointer;" onclick="collapse('col<?=$rej_list->remark_head?>');" class="p-1">
...
<script>
    ...
    function additionalSubRemark(reject_code, dag_no_remark)
    {
        ...
        $.ajax({
            url: baseurl+'SettlementCommon/getAdditionalInputIfAny',
            type: "POST",
            data: postData,
