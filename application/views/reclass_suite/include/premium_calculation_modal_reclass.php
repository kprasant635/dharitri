                ...
                <div class="modal-body" align="">
                    <?php  $areacount=1;  foreach($dags as $dagsprem){ ?>
                        ...
                        <div class="tableCard " style="padding: 25px!important;">
                            ...
                            <div class="row">
                                <div class="form-group col-md-6 ">
                                    <label>Existing Land  Class(As per Chitha)</label>

                                </div>
                                <div class="form-group col-md-6">
                                    <input type="text" class="form-control" name="" value="<?=$dagsprem->exist_land_class_name?>" readonly>
                                </div>
                            </div>

                            <?php if($dagsprem->land_class_code=='0134'){?>
                            <div class="row">
                            <div class="form-group col-md-6">
                                <label for="title">Select Land Type for Existing Land Class <span style="color:red;">(select the newly notified Land Class corresponding to the existing land class of the dag as per Chitha record)</span>:</label>
                            </div>
                            <div class="form-group col-md-6">
                                 <input type="hidden" class="form-control" name="dag_no" value="<?=$dagsprem->dag_no?>" id="dag_no<?=$dagsprem->dag_no?>" readonly>

                                 <input type="hidden" class="form-control" name="nature_possession_selected<?=$dagsprem->dag_no?>" id="nature_possession_selected<?=$dagsprem->dag_no?>" readonly>
                            ...
                            <?php $getClassInfo = $this->utilityclass->getClassInfo($dagsprem->land_class_code);
                        ...
                        <?php }else{?>
                            <div class="row">
                            <div class="form-group col-md-6">
                                <label for="title">Select Land Type for Existing Land Class <span style="color:red;">(select the newly notified Land Class corresponding to the existing land class of the dag)</span>:</label>
                            </div>
                            <div class="form-group col-md-6">
                                 <input type="hidden" class="form-control" name="dag_no" value="<?=$dagsprem->dag_no?>" id="dag_no<?=$dagsprem->dag_no?>" readonly>
                        ...
                        <?php }}?>
                            ...
                            <!-- <div class="row">
                                ...
                                <div class="form-group col-md-6">
                                    <span id="display_area_type<?=$dagsprem->dag_no?>">First Select Area Type in Area Details</span>
                                </div>
                            </div> -->
                            <div class="row">
                                <div class="form-group col-md-6 ">
                            ...
                            <div class="row" id="msg<?=$dagsprem->dag_no?>">
                            </div>

                            <input type="hidden" name="is_penalty<?=$dagsprem->dag_no?>" id="is_penalty<?=$dagsprem->dag_no?>">

                            <input type="hidden" name="rate<?=$dagsprem->dag_no?>" id="rate<?=$dagsprem->dag_no?>">
                        ...
                        <script>
                            ...
                            <?php if((in_array($basic["dist_code"], json_decode(BARAK_VALLEY)))): ?>
                                ...
                                var nature_possession = $('#nature_possession_selected<?=$dagsprem->dag_no?>').val();
                                // alert(case_no);return;
                                var proc_lc_code= $('#proc_lc_code<?=$dagsprem->dag_no?>').val();
                                var dag_prem= $('#zonal_valuation_prem<?=$dagsprem->dag_no?>').val();
                                var newcase = case_no.replace(/\//g, "_");

                               var reclass_type = $("input[name='reclass_option_<?=$dagsprem->dag_no?>']:checked").val();
                                if(reclass_type == 'part_yes')
                                {

                                    var bigha = parseFloat($('#dag_area_b<?=$dagsprem->dag_no?>').val());
                                    var katha = parseFloat($('#dag_area_k<?=$dagsprem->dag_no?>').val());
                                    var lessa = parseFloat($('#dag_area_lc<?=$dagsprem->dag_no?>').val());

                                    var total_lessa = parseFloat((bigha * 6400) + (katha * 320) + (lessa*20));
                                }
                                ...
                                // ajax for getting land list starts here
                                $.ajax({
                                    url: baseurl + "ReclassSuite/getRate/" + typeID +'/'+ proc_lc_code +'/'+ dag_no +'/'+ newcase + '/' + nature_possession + '/' + reclass_type + '/' + total_lessa,
                                    success: function(data) {
                                        // loading.out();
                                        ...
                                        for (var i = 0; i < landpercentage.length; i++) {
                                            ...
                                            var template3 =landpercentage[i].is_penalty;
                                            var template4 =landpercentage[i].total_lessa;
                                            var template5 =landpercentage[i].rate;

                                            var premium_zonal_per_bigha = (dag_prem * landpercentage[i].rate) / 100;

                                            var premium_zonal_per_lessa = premium_zonal_per_bigha / 6400;

                                            var premium = template4 * premium_zonal_per_lessa;

                                           //console.log(template4);return;
                                            
                                        }
                                        //console.log(template);
                                        $("#percentage<?=$dagsprem->dag_no?>").html(template);
                                        $("#msg<?=$dagsprem->dag_no?>").html(template2);
                                        $("#is_penalty<?=$dagsprem->dag_no?>").val(template3);
                                        $("#total_lessa<?=$dagsprem->dag_no?>").val(template4);
                                        $('#amount_<?=$dagsprem->dag_no?>').val(premium);
                                        $('#totaldue').val('');
                                        $('#finalamount').val('');
                                        $(".premhide").hide();
                                        $("#finalsubmit").show();
                                        ...
                                        $("#lmfinalamount").text('');
                                        $("#lmdueamount").text('');
                                        $("#rate<?=$dagsprem->dag_no?>").val(template5);
                                    },
                                    error: function(error) {
                                        // loading.out();
                                    },
                            ...
                            $( "select[name='rate_type<?=$dagsprem->dag_no?>']" ).change(function () {
                                ...
                                if(reclass_type == 'part_yes')
                                {
                                    ...
                                    var total_lessa = parseFloat((bigha * 100) + (katha * 20) + lessa);
                                }
                                ...
                                $.ajax({
                                    ...
                                    success: function(data) {
                                        ...
                                        for (var i = 0; i < landpercentage.length; i++) {
                                            ...
                                            var premium_zonal_per_lessa = premium_zonal_per_bigha / 100;
