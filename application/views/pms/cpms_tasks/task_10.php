...
<?php if($data_exists_flag): ?>
    <form id="cpms_task_10_form" method="POST">
                            ...
                            <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            onkeyup="task_10_keyup('<?=$cpmsMatrix->subtask_id?>','<?=$evaluation_parameters->evaluation_from_subtask_id?>', '<?=$evaluation_parameters->evaluated_value_display_id?>')" 
                                            id="task_10_subtask_id_<?=$cpmsMatrix->subtask_id?>" 
                                            name="task_10_subtask_id_<?=$cpmsMatrix->subtask_id?>" 
                            ...
                            <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            id="task_10_evaluation_display_id_<?=$evaluation_parameters->evaluated_value_display_id?>"
                                            name="task_10_evaluation_id_<?=$evaluation_parameters->evaluated_value_display_id?>" 
                ...
                <table class="table table-bordered shadow-lg">                
                    <tbody>
                            ...
                            <tr class="text-center">
                                <td>
                                    <input type="text" name="cumalative_marks_achieved_task_10" class="form-control text-center" id="cumalative_marks_achieved_task_10" placeholder="Percentage" readonly>
                                </td>
                            </tr>
                    </tbody>
                </table>
        ...
        <div class="row mb-3">
            <div class="col-lg-12 mt-3 text-center">
                <button class="btn btn-success btn-sm" onclick="submit_task_10()"
        ...
        <div class="col-lg-12" id="task_10_form_validation_error_div" style="display:none;">
            <div class="alert alert-warning alert-dismissible" role="alert">
                <strong class="text-center" style="color:red !important"
                    id="task_10_form_validation_error_msg">
                </strong>
            </div>
        </div>       
    </form>
    <script>        
        function task_10_keyup(subtask_id, to_be_evalutaed_id, display_input_id){        
            var subtask_val = $('#task_10_subtask_id_'+ subtask_id).val();
            var to_be_evaluated_value = $('#task_10_subtask_id_'+ to_be_evalutaed_id).val();
            ...
            $('#task_10_evaluation_display_id_'+display_input_id).val(result);
            if(result >= 90){
                $('#cumalative_marks_achieved_task_10').val(10);
                return;
            }else if(result >= 80){
                $('#cumalative_marks_achieved_task_10').val(8);
                return;
            }else if(result >= 70){
                $('#cumalative_marks_achieved_task_10').val(6);
                return;
            }else if(result >= 60){
                $('#cumalative_marks_achieved_task_10').val(4);
                return;
            }else{
                $('#cumalative_marks_achieved_task_10').val(0);
                return;
            }
            
        }
        function submit_task_10(){
            event.preventDefault();
            $('#task_10_form_validation_error_msg').empty();
            $('#task_10_form_validation_error_div').hide();
            var cpms_task_10_form = $('#cpms_task_10_form').serialize();
            $.ajax({
                url: baseurl + "CPMSController/task_10_submit_handle",
                type: 'POST',
                data: cpms_task_10_form,
                dataType: 'json',
                beforeSend: function () {
                    $.blockUI({
                        message: $('#displayBox'),
                ...
                success: function (data) {
                    ...
                    if(data.result == 'VALIDATION-ERROR'){                
                        ...
                        $('#task_10_form_validation_error_div').show();
                        for (let i = 0; i < data.msg.length; i++) {
                            $('#task_10_form_validation_error_msg').append(data.msg[i]);
                        }
                        return;
                    }
                    //*******************/
...
<?php else : ?>
    <div class="alert alert-success text-center">
        ...
        <button class="btn btn-warning btn-sm mt-1" data-toggle="modal" data-target="#exampleModalCenter10"><i class="fa fa-eye" aria-hidden="true"></i> view</button>
    </div>    
    <!-- Modal -->
