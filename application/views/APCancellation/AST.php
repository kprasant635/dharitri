<div class="container-fluid login form-top">
    <div class="row">
        <div class="col-lg-12 ">
            ...
            <form class="form-horizontal" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" id="myForm"
            ...
            <div class="col-md-8">
                <div class="modal-content p-4">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-9" id="form_mouza"></div>
                    </div>
                    <div class="row">
                        <div class="col-md-3"><b>Lot No: </b></div>
                        <div class="col-md-9" id="form_lot_no"></div>
                    </div>
                    <div class="row">
...
document.addEventListener("DOMContentLoaded", function() {
    const mouzaSelectElm = document.getElementById("mouza_code");
    const lot_noSelectElm = document.getElementById("lot_no");
    ...
    function loadPattaNumber() {
        const mouza_pargona_code = mouzaSelectElm.value;
        const lot_no = lot_noSelectElm.value;
        ...
        fetch("<?=BASE_JS_LINK?>APCancellation/getPatta_no", {
                ...
                body: `mouza_pargona_code=${encodeURIComponent(mouza_pargona_code)}&lot_no=${encodeURIComponent(lot_no)}&village_id=${encodeURIComponent(villageSelect)}&patta_type_code=${encodeURIComponent(pattaTypeCode)}`
            })
    ...
    function loadDagNumber() {                
        ...
        fetch("<?=BASE_JS_LINK?>APCancellation/getAvailDags", {
                ...
                body: `mouza_pargona_code=${encodeURIComponent(mouza_pargona_code)}&lot_no=${encodeURIComponent(lot_no)}&village_id=${encodeURIComponent(villageSelect)}&patta_type_code=${encodeURIComponent(pattaTypeCode)}&patta_no=${encodeURIComponent(patta_no)}`
            })
            .then(response => response.json())
            .then(data => {
                // Clear existing options
    ...
    function loadPdar() {
        ...
        fetch("<?=BASE_JS_LINK?>APCancellation/getPdarName", {
                ...
                body: `mouza_pargona_code=${encodeURIComponent(mouza_pargona_code)}&lot_no=${encodeURIComponent(lot_no)}&village_id=${encodeURIComponent(villageSelect)}&patta_type_code=${encodeURIComponent(pattaTypeCode)}&patta_no=${encodeURIComponent(patta_no)}&APC_dag_no=${encodeURIComponent(APC_dag_no)}`
            })
            .then(response => response.json())
            .then(data => {
                const tableBody = document.querySelector("#pdarTable tbody");
...
function showForm() {
    ...
    const mouza_code = document.getElementById("mouza_code");
    if (!mouza_code || mouza_code.value.trim() === "") {
        alert("Please select a Mouza. 'Mouza' cannot be empty.");
        mouza_code.focus();
        event.preventDefault();
        return;
    }
    ...
    if (lot_no.options[lot_no.selectedIndex].text == "Select Lot") {
        alert("Please select a Lot Number. 'Lot Number' cannot be empty.");
        lot_no.focus();
        event.preventDefault();
        return;
    }
    ...
    const form_mouza = document.getElementById("form_mouza");
    form_mouza.innerText = mouza_code.options[mouza_code.selectedIndex].text;

    const form_lot_no = document.getElementById("form_lot_no");
    form_lot_no.innerText = lot_no.options[lot_no.selectedIndex].text;
