...
<div class="panel panel-info panel-form mt-5">
    <div class="panel-heading bg-info text-center">
        <h3 class="panel-title">
            <u>
                Village Land Bank - (Pending-List) : Circle - <?php echo $this->utilityclass->getCircleName($dist_code,$subdiv_code,$circle_code); ?>, 
            </u>                        
        </h3>
    </div>
        ...
        <div class="row">
            ...
            <div class="col-lg-6">
                <div class="form-group">
                    <form action="<?php echo base_url() . 'index.php/LandBankCO/PendingList' ?>" method="post" id="lb_pagination_form">
                ...
                <div class="form-group">
                    <form action="<?php echo base_url() . 'index.php/LandBankCO/searchPendingListByDag' ?>" method="post" id="lb_pagination_form">
                        ...
                        <tbody>
                            <?php foreach ($pending_list as $pending):?>                                    
                                <tr>
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-success btn-sm text-white" onclick="lbViewModal('<?=$pending->id?>', '<?=$pending->dag_no?>' )">
                                            <i class="fa fa-eye"></i>
                                            View & Forward
                                        </button>
                                    </td>
                                   <!--  <td>
                                        <button type="button" class="btn btn-success btn-sm text-white" onclick="lbApprove('<?=$pending->id?>', '<?=$pending->dag_no?>')">
                                            <i class="fa fa-check"></i>
                                            Forward
                                        </button>
                                    </td>  -->     
                                    <td>
                                        <button type="button" class="btn btn-danger btn-sm text-white" onclick="lbReject('<?=$pending->id?>','<?=$pending->dag_no?>')">
...
<script src="<?php echo base_url(); ?>application/views/js/land_bank/land_bank_co.js?v=<?php echo date('YmdHis'); ?>"></script>