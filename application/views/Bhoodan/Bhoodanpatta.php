...
<p style="text-align: center;"><u>&nbsp; অসম চৰকাৰ </u><br />
<u>উপায়ুক্তৰ কাৰ্যালয়, <?=$distName?> জিলা</u></p>

<p style="text-align: center;"><u><span style="color:#FF0000;">হুকুম নং </span>- </u><?=$notice['case_no']?> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; <u>&nbsp;দিনাংক-<?=date('d/m/Y')?></u></p>

<?php
if(strpos($dags['types'], ',') !== false){
    $agriHome="কৃষি / ঘৰৰ বাবে";
}else{
    if($dags['types']==1){
        $agriHome="ঘৰৰ বাবে";
    }else{
...
$totalArea=$this->utilityclass->Total_Bigha_Katha_Lessa($dags['bigha']*100+$dags['katha']*20+$dags['lessa']);
?>
<p><span style="font-family:times new roman,times,serif;">মিছন বসুন্ধৰা আঁচনিৰ অধীনত 
    <?php
        if($basic['service_code']==SETTLEMENT_SPECIAL_CULTIVATORS_ID){
            echo SETTLEMENT_SPECIAL_ASS;
            $agriHome="বিশেষ খেতিৰ বাবে";
        }else if($basic['service_code']==SETTLEMENT_KHAS_LAND_ID){
            echo SETTLEMENT_KHAS_ASS;
        }else if($basic['service_code']==SETTLEMENT_TRIBAL_COMMUNITY_ID){
            echo SETTLEMENT_TRIBAL_ASS;
        }else if($basic['service_code']==SETTLEMENT_PGR_VGR_LAND_ID){
            echo SETTLEMENT_PGR_ASS;
        }
        echo 'Bhoodan/Gramdan';
     ?> 
     সেৱাৰ বাবে আবেদনকাৰী <?=$applicant['applicant_name']?> পিতা/ স্বামী <?=$applicant['father_name']?> ৰ আবেদন নং <?=$notice['case_no']?> মৰ্মে তথা ভূমি উপদেষ্টা সমিতিৰ <?=date('d/m/Y',strtotime($notice['sdlac_proposal_date']))?> তাৰিখৰ বৈঠকত লোৱা প্ৰস্তাৱ নং <?=$notice['sdlac_proposal_id']?> অনুমোদন অনুসৰি আবেদিত ভূমিৰ পট্টনৰ বাবে সৰ্বমুঠ <?=$notice['total_amount']?> টকাৰ প্ৰিমিয়াম আদায় দিবলৈ <?=date('d/m/Y',strtotime($notice['payment_notice_date']))?> তাৰিখে&nbsp; <?=$notice['notice_no']?> নং জাননী জাৰি কৰা হৈছিল। </span></p>

<p><span style="font-family:times new roman,times,serif;">আবেদনকাৰীয়ে উক্ত ধনৰাশি অনলাইনযোগে <?=date('d/m/Y',strtotime($notice['payment_completed_date']))?> তাৰিখে জমা দিয়াৰ সূত্ৰে শ্ৰী&nbsp; <?=$applicant['applicant_name']?> পিতা/স্বামী <?=$applicant['father_name']?> ৰ সপক্ষে <?=$agriHome?> উদ্দেশ্যে <?=$cirName?> ৰাজহ চক্ৰৰ <?=$mouName?> মৌজাৰ <?=$villName?> ৰাজহ গাঁৱৰ <?=$dags['dags']?> নং দাগৰ মুঠ <?=$totalArea[0]?> বিঘা <?=$totalArea[1]?> কঠা 
<?=$totalArea[2]?> লেচা ভূমিত পট্টনৰ হুকুম জাৰি কৰা হ&#39;ল।<br />
</p>
<p style="text-align: justify;">হুকুমমতে <?=$cirName?> ৰাজহ চক্ৰৰ চক্ৰ বিষয়াই উক্ত জমি আবেদনকাৰী শ্ৰী <?=$applicant['applicant_name']?> ৰ নামত নথি সংশোধশ কৰিব।</p>
<?php
$data = explode(",", $qrcode);
$data = $data[1];
...
<h4 style="text-align: center;">মিয়াদি খেৰাজী পট্টা নং (PERIODIC PATTA NO.) <?=$patta[0]['new_patta_no']?></h4>
...
<table border="1" cellpadding="0" cellspacing="0" width="100%">
    <tbody>
        ...
        <?php $total=$total_lc=0;
        foreach($patta as $val){
        ?>
        <tr>
            <td style="width:136px;"><?=$val['new_dag_no']?></td>
            <td style="width:136px;"><?=$this->utilityclass->getLandClassCode($val['new_land_class_code']);?></td>
            <td style="width:160px;"><?=$val['bigha'] ."B-".$val['katha']."K-".$val['lessa']."LC"?></td>
            <td style="width:113px;"><?=$val['new_dag_revenue']?></td>
            <td style="width:136px;"></td>
        </tr>
        <?php $total=$total+($val['new_dag_revenue']);
              $total_lc=$total_lc+$val['new_local_tax'];
         } ?>
        <tr>
            <td colspan="3" style="width:433px;">মুঠ</td>
            <td style="width:113px;"><?=$total?></td>
            <td style="width:136px;">&nbsp;</td>
        </tr>
        <tr>
            <td colspan="3" style="width:433px;">স্থানীয় কৰ যোগদিয়া</td>
            <td style="width:113px;"><?=$total_lc?></td>
            <td style="width:136px;">&nbsp;</td>
        </tr>
        <tr>
            <td colspan="3" style="width:433px;">সৰ্বমুঠ</td>
            <td style="width:113px;"><?=$total_lc+$total?></td>
            <td style="width:136px;">&nbsp;</td>
        </tr>
    </tbody>
</table>
