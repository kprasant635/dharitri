...
<script>
    ...
    $(document).on('click','.viewVillageAreaDetails',function ()
    {
        ...
        $.ajax({
            url: BASE_URL + "/BhoodanController/getChithaApproveAppliedAppAreaCalculation",
            type: "post",
            dataType: "json",
            ...
            success: function (data) {
                ...
                else if (data.responseType == 2)
                {
                    ...
                    else
                    {
                        ...
                        $('.buttDanger').show();
                        $('#messageShow').show();
                        // $('#markAsSDLAC').hide(); // uncomment it when in UAT or production
                        $('#markAsSDLAC').show(); // remove later, for developing env
                        $('#approveByDc').hide();
                        $('#viewVillageAreaDetails').hide();
                        $('.generalNotice').hide();
                    }
