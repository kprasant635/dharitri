        ...
        <div class="col-md-12">
            <div>
                <form id="dsc-form">
                    ...
                    <span class="btn btn-primary" id="push_data_div<?= $dag_no ?>" style="display:none;">
                        <i class="fa fa-send"></i> <input id="push_data_to_prop_chain<?= $dag_no ?>" property-id="<?= $property_id ?>" prop-data="<?= $property_data ?>" type="button" value="Push to property chain" class=" text-white" onclick="return createPropertyChain(<?= $dag_no ?>);" />
                    </span>
                    <span class="btn btn-primary" id="map_update_div" style="display:none;">
                        <i class="fa fa-map-pin"></i><input type="button" id="push_bc_map" class="btn btn-primary text-white" value="Update Map">
                    </span>
                </form>
            </div>
        </div>
        ...
        $(document).ready(function() {
            ...
            function getDSCDetails() {
                ...
                dscSigner.certificate(function(res) {
                    ...
                    document.getElementById('push_data_to_prop_chain' + <?= $dag_no ?>).setAttribute("digi-cert", res.certificates[0].certificate);
            ...
            var initConfig = {
                ...
                "postSignCallback": function(alias, sign, key) {
                    ...
                    if ($('#btnSign').val() == ' Sign Map Data') {
                        ...
                        $('#push_data_div' + <?= $dag_no ?>).show();
                    }
                    // set attributes for property chain
                    document.getElementById('push_data_to_prop_chain' + <?= $dag_no ?>).setAttribute("prop-sign", sign);
                    document.getElementById('push_data_to_prop_chain' + <?= $dag_no ?>).setAttribute("prop-sign-key", key);
