...
function lbViewModal(lb_details_id){
     // ('#lb_approve_rmk_form').trigger("reset");
    $.ajax({
        url: baseurl + "LandBankCO/getLbDataForView",
        type: 'POST',
        ...
        success: function (data) {     
            ...
            for (let i = 0; i < data.land_bank_encroacher_details.length; i++) {
                ...
                else
                {
                    ...
                    $('#delete_enc'+i).val(data.land_bank_encroacher_details[i].id);
                }
...
function GetDynamicTextBoxForView(count,gender_list, caste_list, type_of_land_use, type_of_encroacher) {
    var row =  '<td><input type="checkbox" class="deleteEnc" id="delete_enc'+count +'" onclick="checkboxes();" name="delete_enc[]" ></td><td><input id ="lb_view_modal_en_name_'+count +'"  disabled type="text" value = "" class="form-control" /></td>'
...
function lbApprove(lb_details_id, dag_no){
    $.blockUI({
        message: $('#displayBox'),
        css: {
            border:'none',
    ...
    $('#lb_approve_rmk_lb_details_id').val(lb_details_id);
    const modal = $('#lb_approve_modal').modal({
        backdrop: 'static',
        keyboard: false,
    });
    modal.fadeIn('slow').modal('show');
...
//approve modal close
function lbApproveModalClose(){
    $('#lb_approve_rmk').text('');
    document.getElementById("lb_approve_rmk_form").reset();
    $('#lb_approve_modal').fadeOut('slow').modal('hide');
}
//approve modal submit handle
function lbApproveFormSubmit(){

    var formdata = $('#lb_approve_rmk_form').serialize();  
    $('#lb_approval_rmk_form_validation_error_msg').empty();
    $('#lb_approval_rmk_form_validation_error_div').hide();
    $.ajax({
        url: baseurl + "LandBankCO/landBankDetailsApprove",
        type: 'POST',
        data: formdata,
        dataType: 'json',
        beforeSend: function () {
...
function lbReject(lb_details_id, dag_no){
    ...
    const modal = $('#lb_revert_modal').modal({
        backdrop: 'static',
        keyboard: false,
    });
    modal.fadeIn('slow').modal('show');
...
//revert remark close 
function lbRevertModalClose(){
    $('#lb_revert_rmk').text('');
    $('#lb_revert_rmk_form_validation_error_msg').empty();
    $('#lb_revert_rmk_form_validation_error_div').hide();
    document.getElementById("lb_revert_rmk_form").reset();
    $('#lb_revert_modal').fadeOut('slow').modal('hide');
}
//revert remark submit handle
function lbRevertFormSubmit(){
    var formdata = $('#lb_revert_rmk_form').serialize();  
    $('#lb_revert_rmk_form_validation_error_msg').empty();
    $('#lb_revert_rmk_form_validation_error_div').hide();
    $.ajax({
        url: baseurl + "LandBankCO/landBankDetailsRevert",
        type: 'POST',
        data: formdata,
        dataType: 'json',
        beforeSend: function () {
