<td valign="top" height="409" width="15%"  rowspan=3 id=chitha_col_31><!--REMARKS-->
    ...
    if (isset($chithainf['sro'])) {
        ...
        foreach ($chithainf['sro'] as $key => $sr):
            ...
            if(in_array($this->session->userdata('dist_code'),json_decode(BARAK_VALLEY))){
                echo $headingsro . '&nbsp;' . '&nbsp;' . 'এই দাগৰ (' . $chithainf['sro'][$key]['dag_no'] . ' নং)  জমীত পটাদাৰ ' . $chithainf['sro'][$key]['reg_from_name'] . ' আৰু ক্ৰেতা / দানলওঁতা ' . $chithainf['sro'][$key]['reg_to_name'] . ' ৰ মাজত ' . '<br>(' . $chithainf['sro'][$key]['dag_area_b'] . 'B - ' . $chithainf['sro'][$key]['dag_area_k'] . 'K - ' . $chithainf['sro'][$key]['dag_area_lc'] . 'S - '. $chithainf['sro'][$key]['dag_area_g'] . 'G)' . ' মাটিৰ <br> ৰেঃ দলিল <br>( Deed No.) ' . $chithainf['sro'][$key]['deed_no'] . ' নং <br>' . $newDatesro . ' তাৰিখে ' . ' পঞ্জীয়ন হয় |'; 
            }
            else {
                echo $headingsro . '&nbsp;' . '&nbsp;' . 'এই দাগৰ (' . $chithainf['sro'][$key]['dag_no'] . ' নং)  জমীত পটাদাৰ ' . $chithainf['sro'][$key]['reg_from_name'] . ' আৰু ক্ৰেতা / দানলওঁতা ' . $chithainf['sro'][$key]['reg_to_name'] . ' ৰ মাজত ' . '<br>(' . $chithainf['sro'][$key]['dag_area_b'] . 'B - ' . $chithainf['sro'][$key]['dag_area_k'] . 'K - ' . $chithainf['sro'][$key]['dag_area_lc'] . 'L)' . ' মাটিৰ <br> ৰেঃ দলিল <br>( Deed No.) ' . $chithainf['sro'][$key]['deed_no'] . ' নং <br>' . $newDatesro . ' তাৰিখে ' . ' পঞ্জীয়ন হয় |'; 
            }
        ...
        <?php if (sizeof($r) > 0): ?>
            ...
            <?php if (($r['remark_type_code'] == '01') && ($r['ord_type_code'] == '03')): ?>
                ...
                <p>চক্র বিষয়া'ৰ  <br>
                    ...
                    if ($r['by_right_of'] == '11') {
                        ...
                        if(in_array($this->session->userdata('dist_code'),json_decode(BARAK_VALLEY))){
                            echo $this->utilityclass->cassnum(number_format($r['lessa'], 2)) . " ছটাক " . $this->utilityclass->cassnum(number_format($r['ganda'], 2)) . " গোণ্ডা মাটি ";
                        }
                        else {
            ...
            <?php if ($r['ord_type_code'] == '01'): ?>
                ...
                <p><?php echo $r['ord_passby_desig']; ?>ৰ  </p><p>
                    ...
                    ৰ নামত <?php echo $this->utilityclass->cassnum($r['land_area_b']) . " বিঘা  " . $this->utilityclass->cassnum($r['land_area_k']) . " কঠা  " . $this->utilityclass->cassnum(number_format($r['land_area_lc'], 2)) . " ছটাক " . $this->utilityclass->cassnum(number_format($r['land_area_g'], 2)) . " গোণ্ডা "; ?> মাটি  পৃঠক
            ...
            <?php  if (($r['remark_type_code'] == '10') && ($r['ord_type_code'] == '10')): ?>
                <p>হুকুম নং :<?=$r['history_no'];?><br>
                    উপায়ুক্ত মহোদয়ৰ <?=$r['ord_no']?> নং আৱন্টন বন্দৱস্তী গোচৰৰ  <?=date('d/m/Y',strtotime($r['date_entry']))?>  ইং তাৰিখৰ হুকুম মতে চৰকাৰী  <?=$r['old_dag']?>  নং দাগৰ  <?=$r['allottee_land_b']?> বিঘা  <?=$r['allottee_land_k']?>  কঠা  <?=$r['allottee_land_lc']?> ছটাক <?= $r['allottee_land_g'] ?> গোণ্ডা মাটিৰ  <?=$r['new_dag']?>  নং দাগ আৰু  <?=$r['new_patta']?> নং নতুন খেৰাজ ম্যাদী পট্টা ভূক্ত কৰা হল । <?=date('Y')?> চনত দৌল ভূক্ত হব । </p>
                <p><u class='text-danger'>লাট মণ্ডল :</u><br>(<?php echo $r['lm_name']; ?>)</p>
                <p><u class='text-danger'>চক্র বিষয়া  :</u><br>(<?php echo $r['username']; ?>)</p>
            <?php endif; ?>
            <?php
            if (($r['remark_type_code'] == '01') && ($r['ord_type_code'] == '04')):
                ...
                if ($howmany != null) {
                    ...
                    <p>চক্র বিষয়া'ৰ
                        ...
                        echo $this->utilityclass->cassnum($r['bigha']) . " বিঘা ";
                        echo $this->utilityclass->cassnum($r['katha']) . " কঠা ";
                        echo $this->utilityclass->cassnum(number_format($r['lessa'], 2)) . " ছটাক ";
                        echo $this->utilityclass->cassnum(number_format($r['ganda'], 2)) . " গোণ্ডা মাটি ";
                        ?>
                        <?php
                        $count = 1;
                        $howmany = sizeof($r['infav']);
    ...
    if (isset($chithainf['encro'])) {
        ...
        foreach ($chithainf['encro'] as $key => $enc):
            ...
            <p>
                <!-- ছটাক " . $firstParty[0]->area_g . " গোণ্ডা -->
                <?php echo ($key+1).'. '.$chithainf['encro'][$key]['encro_name'] . '<br>' . 'মাটি ' . '(' . $chithainf['encro'][$key]['encro_land_b'] . '-' . $chithainf['encro'][$key]['encro_land_k'] . '-' . $chithainf['encro'][$key]['encro_land_lc'] . '-' . $chithainf['encro'][$key]['encro_land_g'] . ')'. '<br>'  . $chithainf['encro'][$key]['land_used_by_encro']; ?>
        ...
        foreach ($chithainf['archeo'] as $key => $archeo): ?>
            <p>
                ...
                echo '<u>' . $chithainf['archeo'][$key]['hist_description_nme'] . ' </u><br>'
                    .'(' . $chithainf['archeo'][$key]['archeo_b'] . '-' . $chithainf['archeo'][$key]['archeo_k'] . '-' . $chithainf['archeo'][$key]['archeo_lc'] . '-' . $chithainf['archeo'][$key]['archeo_g'] . ')'. '<br>'
    ...
    <?php
    if(isset($chithainf['rs_data']) && $chithainf['rs_data'] != '') {
        if($chithainf['rs_data'] == '2') {
            echo '2nd RS Data';
        }
        else if($chithainf['rs_data'] == '3') {
            echo '3rd RS Data';
        }
    }
    ?>
