...
<div class='chitha_report container-fluid'>
	...
	<div class='col-lg-12'>
    <br>
    <?php if(isset($application_no))
    {
    echo "<h2> e-Chitha for Patta Land for Current Year ,";
    echo  (date('Y') . '-' . date('y', strtotime('+1 year'))) ;
    echo "</h2>";
    }else{ ?>
    <p align="left" style="margin-top: 0; margin-bottom: 5px"> <font size="4" face="courier">Assam Schedule XXXVII,Form No. 30</font></p>
    <div id="table_head">
        <p style="margin-top: 0; margin-bottom: 0;text-align: center;"><font face="Verdana" size="5">Chitha for Surveyed Villages / জৰীপ হোৱা গাঁৱৰ চিঠা</font></p>
        <br>
    </div>
    <?php } ?>
    <center>
        ...
        <table class="table" id='table1'>
         ...
         <?php if(isset($application_no)) echo "<p>Application No.:&nbsp; ". $application_no . "&nbsp;   Dated:".date('d/m/Y')."</p>"; ?>
    </center>
    <br/>
    ...
    foreach ($data as $chithainf):
        include_rccms_view([
        'dist_code' => $location['dist_code'], 'subdiv_code' => $location['subdiv_code'], 'cir_code' => $location['cir_code'], 'mouza_pargona_code' => $location['mouza_pargona_code'], 'lot_no' => $location['lot_no'], 'vill_townprt_code' => $location['vill_townprt_code'], 'dag_no' => $chithainf['dag_no']]
        );
        ///////////////////////////////////// map_for_property check ///////////
        $map_property = map_property_check($location['dist_code'], $location['subdiv_code'], $location['cir_code'],$location['mouza_pargona_code'], $location['lot_no'], $location['vill_townprt_code'], $chithainf['dag_no']);
	   ?>
       <?php
        if($map_property ==0 ){
       ?>
            <div class="alert alert-danger" role="alert">
                Map not updated in Bhunaksha yet for dag no <?=$chithainf['dag_no'];?>. Kindly split the map in Bhunaksha.
            </div>
       <?php
        if(MAP_FOR_PROPERTY_CONTINUE==1){
            continue;
        } 
        }
       ?>
                    ...
                    <table class="table table_black" id='table2'  >
                        <tr >
                            ...
                            <td align=center rowspan="2" style="width:150px;"> <!--AREA COL. 3-->
                                কালি <br>( বি-ক-লে )</td>
                            <td align=center rowspan="2" style="width:150px; "> &nbsp;পট্টাৰ 
                                নং আৰু প্ৰকাৰ</td>
                        ...
                        <tr >
                            ...
                            <td align='center' > 
                                <?php
                                if (in_array($chithainf['class_code_cat'], ['01','1'])) {
                            ...
                            <td align='center' > 
                                <?php
                                if (in_array($chithainf['class_code_cat'] , ['02','2','3','4','5','6','7','8','9','10'])) {
                                    echo $chithainf['land_type'];
                                }
                                ?>
                            </td>
                ...
                <div align="center" class="col-lg-12">
                    <table class="table" id='table3' STYLE="width:100%; border-collapse:collapse" cellPadding=0 cellSpacing=0 height="580">
                        ...
                        <tr>
                            ...
                            <td align="center" valign="top" height="100">কালি (বি-ক-লে)</td>
                            ...
                            <td	rowspan="3" valign="top" height="409"id=chitha_col_9 ><!--Tenant Descr.-->
                                ...
                                if (isset($chithainf['tenant'])) {
                                    foreach ($chithainf['tenant'] as $tenantdesc):
										...
										if($tenantdesc['p_flag']!='1'){
                                        echo "<p>".$tenantName . '<br>(' . $tenantsFather . ")" . '<br>' . $tenantsadd1 . '<br>' . $tenantsadd2 ."</p>";
                                        }else{
                                        echo "<strike><p style=\"color:red\">".$tenantName . '<br>(' . $tenantsFather . ")" . '<br>' . $tenantsadd1 . '<br>' . $tenantsadd2 ."</p></strike>";
                                        }
          //                               else{
                            ...
                            <td align="center" valign="top" height="100" id=crop_year_no_1><!--YEAR NO-->
                                ...
                                foreach ($chithainf['years'] as $key => $yr):
                                    ...
                                    $year_assam = $chithainf['years'][$key]['year'];
...
<!--//////////////////////////////////////////////////// property chain create button div /////////////////////////////////////////////////-->
<?php if ($this->session->userdata('user_desig_code') == 'CO') { ?>
    <!-- <button class="btn btn-danger" onclick="return updateBcMapBulk()">Click</button> -->
    <div class="d-flex justify-content-center">
        <?php if (isset($ulpin_new) && isset($ulpin_old) && isset($old_dag_create_btn)) {
            // echo $create_prop_chain_bulk_btn;
            // var_dump($old_dag_create_btn);
            echo $old_dag_create_btn;
        } ?>
    </div>

    <div class="prop_create_div text-center" style="margin:5px">
        <div class="d-flex justify-content-center">
            <?php
            if (!isset($create_prop_chain_bulk_btn)) {
                foreach ($create_prop_chain_btns as $key => $prop_create_btn) {
                    if ($key == 0) { ?>
                        <!-- starting row -->
                        ...
                        <!-- if more then 3 dags create new row -->
                        <?php
                        if ($key != 0 && $key + 1 % 3 == 0) { ?>
                        </div> <!-- close previous row -->
                        <!-- start new row -->
                        <div class="row" style="margin: 5px;">
                            <?php //echo $prop_create_btn ?>
                <?php }
                    }
                }
                ?>
...
<?php if(isset($application_no)){
    echo "<p class='red'><i>NOTE</i></p>";
    echo "<p class='red'> * This document is as per the records available in  Dharitree.In case  of any required modifications,applicant may apply in appropriate service through SewaSetu portal(https:://sewasetu.assam.gov.in)</p>";
    echo "<p class='red'> *This is a system generated signed document and does not require physical signature.</p>";
    echo "<div class='col-lg-2 pull-left'><img src=" . $link . " /></div>";
} ?>
</div>
