...
<button type="button" id="multipleUpload" class="btn btn-info btn-danger btn-sm">Upload document</button>
<br>
<table id="otherFiles" class="table table-bordered" style="display:none; margin-top:20px;">
  <tr>
    <th>File Name : </th>
    <th>Download </th>
    <th>Action</th>
...
<div class="modal" id="multipleFileUploadModal" role="dialog">
  <div class="modal-dialog" style="max-width: 80%;margin-top: 220px;">
...
<script type="text/javascript">
  ...
  $("#mulFileUpload").on('click', function(argument) {
    ...
    uploadedFile.append("application_id", $('#app_id').val());
    console.log(uploadedFile);
    uploadedFile.append("doc_name", $("#doc_name").val());
    jQuery.ajax({
      url: baseurl + "MultipleFileUpload/multipleFileSave",
      type: "POST",
      processData: false,
      contentType: false,
      dataType: "json",
      ...
      success: function(data) {
        if (data.responseType == 1) {
          ...
          let file_link = '<?php echo base_url() ?>' + "index.php/MultipleFileUpload/viewfile/" + randomPrefix + data.doc_id + randomSuffix;
  ...
  function deleteFile(doc_id) {
    ...
    $.ajax({
      type: 'post',
      url: baseurl + "MultipleFileUpload/deleteFile",
      data: {
        doc_id: doc_id
      },
      dataType: 'JSON',
