    ...
    <mark>
        <strong style="font-size: 20px;">
            <?php
                if($service == '45')
                {
                    echo "Digitalized allotment/settlement land to non individual juridical entities";
                }
    ...
    $(document).ready(function ()
    {
        ...
        function load_data(category,v_status)
        {
            ...
            var table = $('#datatable').DataTable({
                ...
                'ajax':{
                    url: base_url+'index.php/SettlementInstitutionLm/finalVerificationPagination',
                    type:'POST',
                    data: {
...
<script>
    ...
    function finalVerificationModal(case_no)
    {
        ...
        $.ajax({
            url: baseurl+'SettlementInstitutionLm/getFinalVerificationData',
            type: "POST",
            data: postData,
            success: function(data) 
            {
                ...
                for(i=0; i<arr.dagResult.length; i++)
                {
                    ...
                    for(j=0; j<arr.land_class_code.length; j++)
                    {
                        ...
                        // }
                        var selected23 = '';
                        if(arr.dagResult[i].ins_proposed_land_class ==  arr.land_class_code[j].id)
                        {
                            selected23 = 'selected';
                        }
                    ...
                    tr += '<tr>'+
                           ...
                           '</tr>';
                            ...
                            {
                                var selected1 = '';
                                if(arr.dagResult[i].old_rate_type == arr.land_type_existing[k].id)
                                {
                                    selected1 = "selected";
                                }
                            ...
                            tr += '<tr>'+
                                    ...
                                    '<th>'+
                                        ...
                                        '<select class="form-control" name="land_class_code_homestead'+arr.dagResult[i].dag_no+'"  id="land_class_code_homestead'+arr.dagResult[i].old_dag+'">'+
                                    ...
                                    '<th>'+
                                        '<label>Revenue</label>'+ 
                                        '<input type="number" id="revenue_home'+arr.dagResult[i].old_dag+'" name="revenue_home'+arr.dagResult[i].dag_no+'" placeholder="Enter Revenue" class="form-control">'+
                                    '</th>'+

                                    '<th><label>Local Tax</label> <input type="number" name="local_tax_home'+arr.dagResult[i].dag_no+'" id="local_tax_home'+arr.dagResult[i].old_dag+'" placeholder="Enter Local Tax" class="form-control" ></th>'+
                                '</tr>';
                ...
                tr +=  '<tr>'+
                            '<th colspan="4" class="bg-info">Patta Information</th>'+
                        '</tr>'+
                
                            '<tr>'+
                                // '<th>Enter Patta Type</th>'+
                                // '<td>'+
                                //     '<select name="new_patta_type" class="form-select">'+
                ...
                {
                    var selected = '';
                    if(arr.land_pupose_selected == arr.land_purpose[i].id)
                    {
                        selected = "selected";
                    }
                        ...
                        '</tr>'+
                            ...
                            '<tr>'+
                                ...
                                '<td>'+
                                    ...
                                    '</select>'+
                                '</td>'+
                                '<th>Other Details <span style="color:red">If "Others" is selected as the land purpose, please specify the purpose here.</span></th>'+
                                '<td>'+
                                    '<select name="other_details" class="form-select">'+
                                        '<option value="">--SELECT--</option><option value="Irrigation">Irrigation</option><option value="Veterinary">Veterinary</option><option value="sericulture">Sericulture</option><option value="electricity">Electricity</option><option value="handloom">Handloom</option><option value="soilconservation">Soil Conservation</option><option value="aidc">AIDC</option><option value="Ruraldevelopment">Panchayat and Rural Development</option><option value="agriculture">Agriculture</option><option value="co-operation">Co-operation</option><option value="post">Post</option><option value="homeandpolitical">Home and Political</option><option value="forest">Forest & Environment</option><option value="LocalGovernance">Local Governance</option><option value="PHE">Public Health & Engineering</option><option value="SocialWelfare">Social Welfare</option><option value="NFRailway">NF Railway </option><option value="FireEmergencyService">Fire and Emergency Service  </option><option value="Administration">Administration</option><option value="health">Health</option><option value="education">Education</option><option value="pwdr">PWD Roads</option><option value="pwdb">PWD Buildings</option><option value="transport">Transport</option><option value="Housingandurbanaffairs">Housing and Urban Affairs</option>'+
                                    '</select>'+
                                '</td>'+
                           '</tr>';
...
<script>
    ...
    $('#approvalForm').submit(function (e) {
        ...
        $.ajax({
            url: baseurl + "SettlementInstitutionLm/chithaProcessingDetails",
            type: 'POST',
