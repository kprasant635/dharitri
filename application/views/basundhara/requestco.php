<center><mark>Application Received in Basundhara </mark></center>
<table class="table" id='dataTable'>
	<thead>
	<tr>
		<th>Application No</th>
		<th>Application Date</th>
		<th>Village Name</th>
		<th>Query</th>
		<th>Action</th>
	</tr>
</thead>
<tbody>
<?php foreach($pending as $pen):

$curr_date = strtotime(date('Y-m-d'));
$co_date = strtotime(CO_DATE); //remove action button from co login
$desg_code = $this->session->userdata('user_desig_code');

?>
	<tr>
		<td><?=$pen->application_no?></td>
		<td><?=$pen->date_submission?></td>
		<td><?=$this->utilityclass->getVillageName($pen->dist_code,$pen->subdiv_code,$pen->cir_code,$pen->mouza_code,$pen->lot_no,$pen->village_code)?></td>
		<td><?=$pen->query?></td>
		<td>
			<?php if($pen->service_code=='1'){ ?>
				<?php if(($pen->allow_reapply=='A') || ($pen->allow_reapply=='Y') || ($pen->allow_reapply=='N') 
				      || ($pen->allow_reapply=='E') && date($pen->modified_at) < date('2022-05-08')){ ?>

					<?php if($curr_date <= $co_date && $desg_code=='CO'){ ?>
						<a href='<?php echo base_url() ?>index.php/basundhara/inheritanceBasu?app=<?=$pen->application_no?>' class="btn btn-sm  btn-primary"><i class='fa fa-check-square-o'></i> Action</a>
					<?php } ?>

					<?php if($pen->is_urban=='N'){ ?>
						<a href='<?php echo base_url() ?>index.php/lmmutation/fieldReport?app=<?=$pen->application_no?>' class='lmreportmut' > Report</a>
					<?php } else if($pen->is_urban=='Y'){ ?>
						<a href='<?php echo base_url() ?>index.php/lmmutation/officeReport?app=<?=$pen->application_no?>' class='lmreportmut'> Report</a>
					<?php } ?>
				<?php }else{ ?>
					<a href='<?php echo base_url() ?>index.php/basundhara/inheritanceBasuCO?app=<?=$pen->application_no?>' class="btn btn-sm  btn-info"><i class='fa fa-check-square-o'></i> View</a>
				<?php } ?>
			
			<?php }else if($pen->service_code=='2'){ ?>
				<?php if(($pen->allow_reapply=='A') || ($pen->allow_reapply=='Y') || ($pen->allow_reapply=='N') 
				      || ($pen->allow_reapply=='E') && date($pen->modified_at) < date('2022-05-08')){ ?>

					<?php if($curr_date <= $co_date && $desg_code=='CO'){ ?>
						<a href='<?php echo base_url() ?>index.php/basundhara/deedBasu?app=<?=$pen->application_no?>' class="btn btn-sm  btn-primary"><i class='fa fa-check-square-o'> </i> Action</a>
					<?php } ?>

					<?php if($pen->is_urban=='N'){ ?>
						<a href='<?php echo base_url() ?>index.php/lmmutation/fieldReport?app=<?=$pen->application_no?>' class='lmreportmut' > Report</a>
					<?php } else if($pen->is_urban=='Y'){ ?>
						<a href='<?php echo base_url() ?>index.php/lmmutation/officeReport?app=<?=$pen->application_no?>' class='lmreportmut'> Report</a>
				<?php } }else{ ?>
					<a href='<?php echo base_url() ?>index.php/basundhara/deedBasuCO?app=<?=$pen->application_no?>' class="btn btn-sm  btn-info"><i class='fa fa-check-square-o'></i> View</a>
				<?php } ?>
				
			<?php } else if($pen->service_code=='5'){ ?>
				<?php if(($pen->allow_reapply=='A') || ($pen->allow_reapply=='Y') || ($pen->allow_reapply=='N') 
				      || ($pen->allow_reapply=='E') && date($pen->modified_at) < date('2022-05-08')){ ?>
					<?php if($curr_date <= $co_date && $desg_code=='CO'){ ?>
						<a href='<?php echo base_url() ?>index.php/basundhara/allotmentBasu?app=<?=$pen->application_no?>' class="btn btn-sm  btn-primary"><i class='fa fa-check-square-o'> </i> Action</a>
					<?php } ?>
					<a href='<?php echo base_url() ?>index.php/allotment/viewproReject?app=<?=$pen->application_no?>' class='lmreportmut'> Report</a>
				<?php }else{ ?>
					<a href='<?php echo base_url() ?>index.php/basundhara/allotmentBasuCO?app=<?=$pen->application_no?>' class="btn btn-sm  btn-info"><i class='fa fa-check-square-o'></i> View</a>
				<?php } ?>
				
			<?php }else if($pen->service_code=='3'){ ?>
				<?php if(($pen->allow_reapply=='A') || ($pen->allow_reapply=='Y') || ($pen->allow_reapply=='N') 
				      || ($pen->allow_reapply=='E') && date($pen->modified_at) < date('2022-05-08')){ ?>
					<?php if($curr_date <= $co_date && $desg_code=='CO'){ ?>
						<a href='<?php echo base_url() ?>index.php/basundhara/partitionBasu?app=<?=$pen->application_no?>' class="btn btn-sm  btn-primary"><i class='fa fa-check-square-o'> </i> Action</a><br>
					<?php } ?>
					<?php if($pen->is_urban=='N'){ ?>
						<a href='<?php echo base_url() ?>index.php/lmmutation/fieldReport?app=<?=$pen->application_no?>' class='lmreportmut' > Report</a>
					<?php } else if($pen->is_urban=='Y'){ ?>
						<a href='<?php echo base_url() ?>index.php/lmmutation/officeReport?app=<?=$pen->application_no?>' class='lmreportmut'> Report</a>
					<?php } ?>
				<?php }else{ ?>
					<a href='<?php echo base_url() ?>index.php/basundhara/partitionBasuCO?app=<?=$pen->application_no?>' class="btn btn-sm  btn-info"><i class='fa fa-check-square-o'></i> View</a>
				<?php } ?>
				
			<?php }else if($pen->service_code=='9'){ ?>
				<?php if(($pen->allow_reapply=='A') || ($pen->allow_reapply=='Y') || ($pen->allow_reapply=='N') 
				      || ($pen->allow_reapply=='E') && date($pen->modified_at) < date('2022-05-08')){ ?>
					<?php if($curr_date <= $co_date && $desg_code=='CO'){ ?>
						<a href='<?php echo base_url() ?>index.php/basundhara/conversionBasu?app=<?=$pen->application_no?>' class="btn btn-sm  btn-primary"><i class='fa fa-check-square-o'> </i>Action</a>
					<?php } ?>
				<?php }else{ ?>
					<a href='<?php echo base_url() ?>index.php/basundhara/conversionBasuCO?app=<?=$pen->application_no?>' class="btn btn-sm  btn-info"><i class='fa fa-check-square-o'></i> View</a>
				<?php } ?>
				<a href='<?php echo base_url() ?>index.php/COconversionPartha/convBasuReport?app=<?=$pen->application_no?>' class='lmreportmut'> Report</a>
			<?php }else if($pen->service_code=='4'){  ?>
				<?php if(($pen->allow_reapply=='A') || ($pen->allow_reapply=='Y') || ($pen->allow_reapply=='N') 
				      || ($pen->allow_reapply=='E') && date($pen->modified_at) < date('2022-05-08')){ ?>
					<?php if($curr_date <= $co_date && $desg_code=='CO'){ ?>
						<a href='<?php echo base_url() ?>index.php/basundhara/reclassBasu?app=<?=$pen->application_no?>' class="btn btn-sm  btn-primary"><i class='fa fa-check-square-o'> </i> Action</a>
					<?php } ?>
				<?php }else{ ?>
					<a href='<?php echo base_url() ?>index.php/basundhara/inheritanceBasuCO?app=<?=$pen->application_no?>' class="btn btn-sm  btn-info"><i class='fa fa-check-square-o'></i> View</a>
				<?php } ?>
				<a href='<?php echo base_url() ?>index.php/lmmutation/reclassBasuReport?app=<?=$pen->application_no?>' class='lmreportmut'> Report</a>
			
			<?php }else if($pen->service_code=='7'){  ?>
				<?php if(($pen->allow_reapply=='A') || ($pen->allow_reapply=='Y') || ($pen->allow_reapply=='N') 
				      || ($pen->allow_reapply=='E') && date($pen->modified_at) < date('2022-05-08')){ ?>
					<?php if($curr_date <= $co_date && $desg_code=='CO'){ ?>
						<a href='<?php echo base_url() ?>index.php/basundhara/areaCorrectionbasu?app=<?=$pen->application_no?>' class="btn btn-sm  btn-primary"><i class='fa fa-check-square-o'> </i> Action</a>
					<?php } ?>
				<?php }else{ ?>
					<a href='<?php echo base_url() ?>index.php/basundhara/inheritanceBasuCO?app=<?=$pen->application_no?>' class="btn btn-sm  btn-info"><i class='fa fa-check-square-o'></i> View</a>
				<?php } ?>
				
			<?php }
			else if($pen->service_code=='6'){ ?>
				<?php if(($pen->allow_reapply=='A') || ($pen->allow_reapply=='Y') || ($pen->allow_reapply=='N') 
				      || ($pen->allow_reapply=='E') && date($pen->modified_at) < date('2022-05-08')){ ?>
					<?php if($curr_date <= $co_date && $desg_code=='CO'){ ?>
						<a href='<?php echo base_url() ?>index.php/basundhara/nameCorrectionbasu?app=<?=$pen->application_no?>' class="btn btn-sm  btn-primary"><i class='fa fa-check-square-o'></i> Action</a>
					<?php } ?>
					
				<?php }else{ ?>
					<a href='<?php echo base_url() ?>index.php/basundhara/inheritanceBasuCO?app=<?=$pen->application_no?>' class="btn btn-sm  btn-info"><i class='fa fa-check-square-o'></i> View</a>
				<?php } ?>
				
			<?php }
			else if($pen->service_code=='8'){ ?>
				<?php if(($pen->allow_reapply=='A') || ($pen->allow_reapply=='Y') || ($pen->allow_reapply=='N') 
				      || ($pen->allow_reapply=='E') && date($pen->modified_at) < date('2022-05-08')){ ?>
					<?php if($curr_date <= $co_date && $desg_code=='CO'){ ?>
						<a href='<?php echo base_url() ?>index.php/basundhara/nameCancelbasu?app=<?=$pen->application_no?>' class="btn btn-sm  btn-primary"><i class='fa fa-check-square-o'> </i>Action</a>
					<?php } ?>
				<?php }else{ ?>
					<a href='<?php echo base_url() ?>index.php/basundhara/inheritanceBasuCO?app=<?=$pen->application_no?>' class="btn btn-sm  btn-info"><i class='fa fa-check-square-o'></i> View</a>
				<?php } ?>
			
			
			<?php }else if($pen->service_code=='10'){ ?>
				<?php if(($pen->allow_reapply=='A') || ($pen->allow_reapply=='Y') || ($pen->allow_reapply=='N') 
				      || ($pen->allow_reapply=='E') && date($pen->modified_at) < date('2022-05-08')){ 
						if(($this->session->userdata('user_desig_code')=='CO')){
					    ?>
					    <?php if($curr_date <= $co_date && $desg_code=='CO'){ ?>
							<a href='<?php echo base_url() ?>index.php/basundhara/mobileUpdationBasu?app=<?=$pen->application_no?>'class="btn btn-sm  btn-primary"><i class='fa fa-check-square-o'> </i>Action</a>
						<?php } ?>
					   <?php }
					    else { ?>
						<a href='<?php echo base_url() ?>index.php/basundhara/mobileUpdationBasu?app=<?=$pen->application_no?>'class="btn btn-sm  btn-primary"><i class='fa fa-check-square-o'> </i>Action</a>
					<?php } 
					}
					else{ ?>
					<a href='<?php echo base_url() ?>index.php/basundhara/mobileUpdationBasuCO?app=<?=$pen->application_no?>' class="btn btn-sm  btn-info"><i class='fa fa-check-square-o'></i> View</a>
				<?php } } ?>
			</td>
	</tr>
<?php endforeach; ?>
</tbody>
</table>
<div id="myModal1" class="modal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
        </div>
    </div>
</div>
<!--  -->
<script>
	$(document).ready( function () {
    	$('#dataTable').DataTable();
} );
	$(function () {
        $('.lmreportmut').click(function (e) {
            e.preventDefault();
            $.ajax({
                url: $(this).attr('href'),
                success: function (data) {
                    $('.modal-content').html(data);
                    $('.modal').modal('show');
                    $('body').addClass('bodytest');
                }
            });
        });
    });
</script>