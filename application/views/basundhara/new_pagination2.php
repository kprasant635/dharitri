...
<?php foreach($pending as $pen): ?>
	<tr>
		...
		<td><?=$pen->service?></td>
    <td><?=$this->utilityclass->getCircleName($pen->dist_code,$pen->subdiv_code,$pen->cir_code)?></td>
...
$(document).ready(function() {
    var table = $('#dataTable1').DataTable({
        initComplete: function () {
            // For the country column (change 1 if country column is another index)
            this.api().columns(3).every(function () {
                var column = this;
                var select = $('<select><option value="">Filter by Circle</option></select>')
                    .appendTo( $(column.header()).empty() )  // Append to the column header
                    .on('change', function () {
