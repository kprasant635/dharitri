            ...
            <div class="modal-body" align="">
                <form action="">
                    <div class="row">
                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 form-group">
                            <label for="w3review" style="font-weight: bold">Enter Your Remarks</label>
                            <textarea class="form-control" name="w3review" id="approveRemarksDc" rows="4" required
                                      minlength="1"> </textarea>
                        </div>
                    </div>
                </form>
                ...
                <form action="">
                    <div class="row">
                        ...
                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 form-group">
                            ...
                            <input type="date" class="form-control" name="w3date" id="date" required
...
<script>
    ...
    $(document).on('click', '#revertFromDcToCoModalYes', function () {
        ...
        if (remarks != '') {
            ...
            $.ajax({
                url: BASE_URL + "/SettlementTenantUrbanDc/applicationRevertFromDCToCO",
                type: "post",
                dataType: "json",
    ...
    $(document).on('click', '#approveByDcModalYes', function () {
        ...
        if (remarks != '') {
            ...
            $('#approveByDcModal').modal('hide');
            $.ajax({
                url: BASE_URL + "/SettlementTenantUrbanDc/applicationApprovedByDcTenant",
                type: "post",
                dataType: "json",
                contentType: "application/json",
                success: function (data) {
                    if (data.responseType == 1) {
                        ...
                        showSuccessMessage(
                            "Application Forwarded To CO For Payment Generation  ");
                    } else if (data.responseType == 3) {
                        showErrorMessage("Data not found !");
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.responseType == 1) {
                        ...
                        $("#hearingDateShow2").html(data.hearing_date);
                        $("#hearingDateShow3").html(data.hearing_date);
                        ...
                        $.each(data.get_owners, function (i, valOwn) {
                            table +=
                                '<span>' + '  ' + valOwn['pdar_name'] + '  ,' +
                                '</span>';
                        });
                        $('#tableBuyer').html(table);
                        // $('#pattadar_name').html(table);

                        var table1 = '';
                        $.each(data.get_riotee, function (i, valRio) {
                            table1 +=
                                '<span>' + '  ' + valRio['pdar_name'] + '  ,' +
                                '</span>';
                        });
                        $('#tableOwner').html(table1);
                        $('#riot_name').html(table1);

                        // var table1 = '';
                        // $.each(data.applicantName, function (i, valRio)
                        // {
                        //     table1 +=
                        //         '<span>'+ '  '  + valRio['pdar_name']  + '  ,' +'</span>' ;
                        // });
                        $('#applicant_name').html(data.applicantName.pdar_name);
                        $('#tableOwner').html(data.applicantName.pdar_name);
