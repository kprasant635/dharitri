                             ...
                             <!--  <tr>
                              ...
                              <tr style="border-bottom:1px solid #227576">
                                    <td colspan="2">
                                        ...
                                        <!-- <button type="button" id="editarea<?=$all_deleted_dag->id?>" onclick="editAreaIns(<?=$all_deleted_dag->id?>,<?=$all_deleted_dag->dag_no?>);" class="btn btn-sm btn-warning" style="display:none">Edit Area</button> -->
                                        <?php } if(ENABLE_DAG_ELIGIBLE_BUTTON_INSTITUTE != 0){ ?>
                                        <button type="button" id="deldag<?=$all_deleted_dag->id?>" onclick="deleteDagIns(<?=$all_deleted_dag->id?>,<?=$all_deleted_dag->dag_no?>);" class="btn btn-sm btn-danger" style="display:none"><i class="fa fa-remove" style="color:white"></i> Dag Not Eligible</button>
                                        <?php if($basic['status'] == 'R') { ?>
                                        <button type="button" id="insdag<?=$all_deleted_dag->id?>" onclick="insertDagRevertIns(<?=$all_deleted_dag->id?>,<?=$all_deleted_dag->dag_no?>);" class="btn btn-sm btn-success" >Eligible</button>
                                        <?php } else { ?>
                                          <button type="button" id="insdag<?=$all_deleted_dag->id?>" onclick="insertDagIns(<?=$all_deleted_dag->id?>,<?=$all_deleted_dag->dag_no?>);" class="btn btn-sm btn-success" >Eligible</button>
                                        <?php }  }?>
                                        ...
                                        This dag was Removed by LRA!!!
                                        </div>
                                    </td>
                                    <td colspan="2" class="text-center">
                                        
                                    <!-- <a type="button" target="_blank" class="btn-sm  buttInfo" href="<?php echo base_url(); ?>index.php/SettlementCommonIns/apiDagWiseApplication?app=<?=$basic["applid"];?>&dag=<?=$all_deleted_dag->dag_no;?>">
