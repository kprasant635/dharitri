<div class="row p-2" >
  ...
  <div class="col-md-3 col-lg-3 col-sm-6 col-xs-12">
    <select name="is_landless" id="is_landless" class="form-control <?php if(form_error('is_landless')){echo 'lm_invalid';}?>" onchange="openSelectAddPropertyModal()">
      ...
      <option value="YES" <?= ($apLmNote == 'YES')?'selected':'' ?> class="completely_landless">Completely Landless</option>
      <option value="NO" <?= ($apLmNote == 'NO')?'selected':'' ?>>Landless as per land policy</option>
      <option value="OTHERS" <?= ($apLmNote == 'OTHERS')?'selected':'' ?>>Having Land</option>
    </select>
  </div>
  <div class="col-md-3 col-lg-3 col-sm-6 col-xs-12">
...
function confirmDelete(id){
  if(confirm("Are you sure you want to delete this Record?")){
    $.ajax({
      type: "POST",
      url: baseurl + "SettlementCommon/propertydel",
      async: false,
      data: {
        property_id: id
      },
...
$('#submitproperty').click(function(e){
  ...
  $.ajax({
    type: "POST",
    url: baseurl + "SettlementCommon/addProperty",
    async: false,
    data: {
      ref_no                    : ref_no,
