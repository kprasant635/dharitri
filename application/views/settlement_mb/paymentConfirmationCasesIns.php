<div class="col-lg-12 ">
    <div class="well well-sm mis_report">
        <h4 style="text-align: center;">
            Confirm Payment list of Non individual Juridical Entities
        </h4>
    </div>
    <!-- New Select Field -->
    <!-- //Select Field End -->
...
<h5 style="color:#9d0c0c;padding: 18px;">Note: Do not select the State Government Entity category while generating the Offer of Allotment/Settlement, as no payment is required for this category. Instead, directly update the Chitha and forward it to the DC for allotment certificate generation.</h5>
...
<?php
            if($_GET['service'] == '45' && NIJE_OFFER_OF_ALLOTMENT_SETTLEMENT==1 && NIJE_OFFER_OF_ALLOTMENT_SETTLEMENT_DATE <= date('Y-m-d'))
            {
            ?>
            <div class="row pt-2 urbanrural_show" >
                <div class="col-lg-12">
                    <div class="mt-4 row px-5">
                        ...
                        <div class="col-md-3">
                            <b style="color:#ff681d"><i class="fa fa-hand-o-right"></i> Click here for generate offer of allotment/settlement</b>
                        </div>
                        <div class="col-md-6">
                            <textarea placeholder="Remarks  ..." name="remark_co" id="remark_co_text" class="form-control p-2" cols="20" rows="10"></textarea>              
                        </div>
                        <div class="col-md-3">
                            <button type="submit" name="bulk_nr_revert" formtarget="bulk_payment_notice_generate_form" class=" text-white btn btn-primary btn-sm" onclick="return coForm()"><i class="fa fa-check-square-o"></i> Generate offer of allotment/settlement </button>
                        </div>
                    </div>
                </div>
...
<script>
    $(document).ready(function ()
    {
        ...
        function load_data(is_category = null, remark_cat = null, mouza_cat=null, lot_cat=null, nr_cat=null, payment_status=null, final_verification_report=null, co_approved = null,allotment_settlement = null)
        {
            ...
            //     var title = $(this).text();
            //     $(this).html(title+' <input type="text" class="input_search form-control form-control-sm" placeholder="Search ' + title + '" />');
            // });
            ...
            var table = $('#datatable').DataTable({
                ...
                'ajax':{
                    url: base_url+'index.php/SettlementInstitutionCo/paymentConfirmation',
                    type:'POST',
                    data: {
...
$('#bulk_payment_notice_generate_form').submit(function (e) {
    e.preventDefault();
    if(!confirm("Are you sure you want to generate the Offer of Allotment/Settlement for the selected cases? Please ensure that cases under the State Government entity are not selected !!!"))
    {
        return false;
    }
        // $("#overlay").fadeIn(300);
        ...
        $.ajax({
            url: baseurl + "SettlementInstitutionCo/coBulkPaymentNoticeGenerateAndSave",
            type: 'POST',
