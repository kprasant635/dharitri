                    ...
                    <th>
                        <select class="form-control" name="mouza_cat" id="mouza_cat">
                            ...
                            <?php if(isset($select_data)){ foreach($select_data as $select){?>
                                <option value="<?=$select->mouza_pargona_code?>">
                                    <?=$this->utilityclass->getMouzaName($select->dist_code, $select->subdiv_code, $select->cir_code, $select->mouza_pargona_code)?>
                                </option>
                            <?php }}?>
                        </select>
                    </th>
...
<script>
    $(document).ready(function ()
    {
        ...
        function load_data(is_category = null, remark_cat = null, mouza_cat=null, lot_cat=null, nr_cat=null, payment_status=null, final_verification_report=null)
        {
            ...
            var table = $('#datatable').DataTable({
                ...
                'ajax':{
                    url: base_url+'index.php/SettlementCommon/printPartialNoticeList',
                    type:'POST',
                    data: {
...
<!-- <script>
    $('#co_bulk_notice').submit(function (e) {
        e.preventDefault();
        if(!confirm("Are you sure you want to generate partial payment notice for this cases?"))
        {
            return false;
        }
        // $("#overlay").fadeIn(300);
        ...
        $.ajax({
            url: baseurl + "SettlementCommon/emiCasesNotice",
            type: 'POST',
