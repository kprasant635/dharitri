                ...
                <div class="panel panel-info panel-form">
                    <div class="panel-heading">
                        <h3 class="panel-title">
                            Reclassification <kbd>(<?=$case_no?>)</kbd>
                        </h3>
                    </div>
                    ...
                    ?>
                      ...
                      <table class="table">
                         ...
                         <tr class="bg-success">
                          <?php $i=1; foreach($applicant_info as $sp): ?>
                          <td> <?=$sp->name_ass ;?></td>
                      ...
                      <table class="table">
                         ...
                         <tr class="bg-success">
                          <td> <?=$this->utilityclass->getLandClassCode($app->present_land_class);?></td>
                          <td> <?=$this->utilityclass->getLandClassCode($app->proposed_land_class);?></td>
                          <td> <?=$app->new_landuse_year;?></td>
                         </tr>
                      </table>
                      ...
                      <table class="table">
                         ...
                         <tr class="bg-success">
                          ...
                          if(in_array($dist_code, json_decode(BARAK_VALLEY))){?>
                               <td colspan="4"><?=$app->dag_area_b;?>B-<?=$app->dag_area_k;?>K-<?=$app->dag_area_lc;?>C-<?=$app->dag_area_g;?>G </td>
                            <?php }

                          else{?>
                            <td colspan="2"><?=$app->dag_area_b;?>B-<?=$app->dag_area_k;?>K-<?=$app->dag_area_lc;?>L </td>
                          <?php }?>
                          <!-- <td><?=$app->area_b;?>B-<?=$app->area_k;?>K-<?=$app->area_l;?>L </td> -->
                          <td><i class="fa fa-rupee"></i> <?=number_format($app->present_land_revenue,2);?>+<?=number_format($app->present_land_localtax,2)?> = <?=number_format($app->present_total_revenue,2);?> </td>
                         </tr>
                      </table>
                    ...
                    <button class="btn query btn-sm pull-right btn-success"><i class='fa fa-hand-paper-o'></i>Query to Applicant(s)</button>
                      ...
                      <?php if($query){
                        ...
                        <input type="hidden" class="form-control" name='application_no' value="<?=$application_no?>">
                          ...
                          <input type="hidden" class="form-control" name='dag_revenue' value="<?=$app->present_land_revenue?>">
                          <input type="hidden" class="form-control" name='dag_local_tax' value="<?=$app->present_land_localtax?>">
                          <input type="hidden" class="form-control" name='sum' value="<?=$app->present_total_revenue?>">
                        ...
                        if(in_array($dist_code, json_decode(BARAK_VALLEY))){?>
                          ...
                          <div class="form-group">
                                <div class="col-sm-12">
                                    ...
                                    <textarea name="co_report_suffix" class="form-control" rows="5"><?php echo $lm->lm_name.", ";?><?php echo "লাট মণ্ডল, "; ?></textarea>
                          ...
                          <!-- /////////ESCALATION REMARK///////////// -->
                            <?php if (ESCALATION_ENABLE == 1 && ESCALATION_REMARK_ENABLE == 1 && isset($escRemarkData) && !empty($escRemarkData) && $escRemarkData->remark_status == null && $app->es_flag == 1 && $app->out_of_esc == 0) { ?>
                            <div class="col-lg-12">
                         ...
                         <ul class="list-group" style='margin-bottom: 10px'>
                          ...
                          <button type="submit" class="btn btn-sm btn-primary"><i class='fa fa-check-square-o'></i> Forward to CO</button>&nbsp;
                          <button class="btn reject hide btn-sm btn-danger"><i class='fa fa-arrows-alt'></i> Reject Application</button>&nbsp;
                          <button class="btn query hide btn-sm btn-success"><i class='fa fa-hand-paper-o'></i>Query to Applicant(s)</button>
                        </center>
                      </div>
                    </div>
                </div>
...
<script type="text/javascript">
  ...
  $('#formAjaxPost').on('submit', function(event){
    ...
    //console.log(formData);
        $.ajax({
            type        : 'POST', 
            url         : baseurl+'LandReclassification/reclassPost', 
            data        : formData, 
            dataType    : 'json', 
            encode      : true,
            beforeSend: function(){
