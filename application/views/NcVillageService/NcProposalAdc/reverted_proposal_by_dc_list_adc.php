...
<div class="row" style='padding: 40px 50px 40px 20px'>
    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 pull-left" style="padding-left: 25px">
        Process /
        Settlement MB /
        <a href="<?= base_url()?>index.php/NcCommonProposalAdc/revertMeetingListForAdc">Reverted Meeting </a> /
        Proposals

        <a class="btn btn-sm btn-danger pull-right" href="<?=base_url().'index.php/NcCommonProposalAdc/revertMeetingListForAdc'?>"><i class="fa fa-backward"></i>&nbsp; Go Back</a>
                    ...
                    <table class="table" width="100%">
                        ...
                        <?php $i=1; foreach ($proposals as $proposal): ?>
                            <tr>
                                ...
                                <td>
                                    <a class="rezaButt buttPrimary" target= "SDLACProposalNotice" href="<?php echo base_url(); ?>index.php/NcCommonSdoAdcDc/getProposalNotice/?case=<?php echo $proposal->id ?>">
                                        <i class="fa fa-print"></i> Print Notice
                                    </a>
                                    <a class="rezaButt buttCust"    target= "Download" href="<?php echo base_url(); ?>index.php/NcCommonSdoAdcDc/downloadCasesWithProposalId/?case=<?php echo $proposal->id ?>">
                                        <i class="fa fa-download"></i> Download
                                    </a>
                                    <a class="rezaButt buttSuccess"  target= "SDLACProposalNotice" href="<?php echo base_url(); ?>index.php/NcCommonProposalAdc/caseListUnderRevertedMeetingForAdc/?proposal=<?php echo $proposal->id ?>">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select class="form-control" id="select_nominee_<?=$row->user_code?>">
                                            ...
                                            <?php if($row->user_type == 'MLA' && in_array($this->session->userdata("dist_code").'_'.$row->user_code, SDLAC_MEM_MLA_NOMINEE_ALLOW_NC)) :?>
                                                <?php
                                                //list of members
                                                $nominee = $this->ncutility->getNomineeOfSdlacMember($row->user_code, $this->session->userdata('dist_code'));
                                                foreach($nominee as $nom)
                                                {
                                                    //for selected nominee
                                                    $checkedStatus = $this->ncutility->getSelectedNomineeOfSdlac($proposal_no, $nom->id, NC_KHAS_LAND_ID); ?>
                                                    <option <?=$checkedStatus?> value="<?=$nom->id?>"><?=$nom->nominee_name?></option>
                                                <?php } ?>

                                            <?php elseif(in_array($row->user_type,SDLAC_MEM_NOMINEE_ALLOW_NC)) : ?>
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="rezaText" style="padding-left: 10px; padding-right: 10px">
                                ...
                                & Chairman, SDLAC/CDLAC.
                                She/He welcomed all the members present in the meeting and apprised the house about the objectives of the meeting.
                                Initiating the discussion, the Chairman placed the settlement/allotment proposals of the individuals for each Revenue Circle
                                under  <span style="font-weight:bold;" class="subDivName"></span> Sub-Division in front of
                                the SDLAC/CDLAC for discussion and consideration.
...
<script>
    ...
    $(document).on('click','#uploadAdditionalFileModalYes',function ()
    {
        ...
        else
        {
            ...
            $.ajax({
                url: BASE_URL + "/NcCommonProposalAdc/postAdditionalFileUnderMeetingAdc",
                type: "post",
    ...
    $("#verifyCases").click(function()
    {
        ...
        $.ajax({
            url: BASE_URL + "/NcCommonProposalAdc/verifyCasesUnderRevertedMeetingByDc",
            type: "post",
            dataType: "json",
    ...
    function revertBulkToCO(revertBulkPull,revertBulkChitha)
    {
        ...
        else
        {
            ...
            $.ajax({
                url: BASE_URL + "/NcCommonSdoAdcDc/bulkRevertCasesInRevertedMeeting",
                type: "post",
                dataType: "json",
                contentType: "application/json",
                success: function (data) {
    ...
    $('#forwardToDcForFinalApproval').click(function()
    {
        ...
        $.ajax({
            url: BASE_URL + "/NcCommonProposalAdc/sendRevertedProposalsToDcMinute",
            type: "post",
            enctype: 'multipart/form-data',
            contentType: false,
            cache: false,
    ...
    $('#minutesGenerateModalYes').click(function(){
        ...
        else
        {
            ...
            $.ajax({
                url: BASE_URL + "/NcCommonProposalAdc/sendRevertedProposalsToDc",
                type: "post",
                enctype: 'multipart/form-data',
                contentType: false,
                cache: false,
    ...
    // print minutes Not Recommend Cases
    ...
    function deleteProposalWithZeroCases(proIds,message)
    {
        ...
        else
        {
            ...
            $.ajax({
                url: BASE_URL + "/NcCommonProposalAdc/deleteProposalWithZeroCasesByAdc",
                type: "post",
                dataType: "json",
                contentType: "application/json",
                success: function (data) {
