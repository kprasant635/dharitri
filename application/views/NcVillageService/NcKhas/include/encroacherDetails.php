<?php if($applicants_encroacher == true) { ?>
    ...
    <div class="tableCard">
        <table class="table table-bordered">
                    ...
                    <?php if(NC_ENABLE_BUTTON_CHANGE_ENCROACHER == 1){?>
                        ...
                        <td rowspan="2" style="vertical-align : middle;">
                            ...
                            if(isset($settlement_vlb_encroacher_check)):
                                                    ...
                                                    if(isset($settlement_land_bank_details))
                                                    {
                                                        foreach($settlement_land_bank_details as $bank)
                                                        {
                                                            if($bank != false)
                                                            {
                                                                ...
                                                                {
                                                                    ?>
                                                                    <button id="edit_encroacher_button" type="button" onclick="editEncroacher(<?=$riotee->dag_no?>,<?=$riotee->id?>, <?=$bank->encroacher_id?>,<?=$bank->land_bank_details_id?>);" class="rezaButtNew buttCust">
                                                                        <i class="fa fa-pencil-square-o"></i>&nbsp;Edit Occupier
                                                                    </button>
                                                                    <br>
                                                                    <?php
                                                                }
                                ...
                                { ?>
                                    <?php if(NC_ENABLE_BUTTON_ADD_ENCROACHER == 1): ?>
                                    <button type="button" onclick="addEncroacher('<?=$riotee->dag_no;?>',<?=$riotee->id?>);" class="rezaButtNew buttDanger add_encroacher_button">
                                        <i class="fa fa-plus-circle"></i>&nbsp; Add Occupier
                                    </button>
                                    <br>
                                <?php endif; ?>
                            ...
                            <?php if($riotee->nc_encroacher == 0): ?>
                                <button type="button" class="rezaButtNew buttPrimary" onclick="encroacherModal('<?=$riotee->dag_no;?>',<?=$riotee->id?>);" id="<?=$riotee->dag_no;?>">
                                    <i class="fa fa-check-square-o"></i> Select VLB List
                                </button>
                            <?php else: ?>
                                <button type="button" class="rezaButtNew buttInfo" onclick="encroacherModal('<?=$riotee->dag_no;?>',<?=$riotee->id?>);" id="<?=$riotee->dag_no;?>">
                                    <i class="fa fa-refresh"></i> Update Occupier
                                </button>
                            <?php endif; ?>
            ...
            include(APPPATH."views/SettlementView/include/encroacherNotEligibleView.php");
            ?>
        </table>
    </div>
    <?php
