...
<form
  action="<?php echo base_url()?>index.php/NcCommonController/chithaUpdate"
  method="post"
>
          ...
          <div class="col-12 text-center">
            ...
            <script>
                ...
                function manualPaymentDetailsSubmitHandle(){
                    ...
                    $.ajax({
                        url: baseurl + "NcCommonController/manualPaymentDetailsSubmitHandle",
                        type: 'POST',
    ...
    <div class="row">
      <div class="col-md-12 panel panel-body">
              <?php
                ...
                {
                  ?>
                    <h3 class="alert-danger">Final verification approval check is pending at LM!</h3>
                  <?php
                }
                elseif($alm->chitha_processing_details == 1)
                {
                  ...
                  <h3 class="pl-2 mt-2 text-danger text-shadow">#LM Verfication Report</h3>
                      ...
                      if($dagDetails)
                      {
                        foreach($dagDetails as $dagRow)
                        {
                          ...
                          <tr>
                            ...
                            <td><?=$this->utilityclass->getPattaName($dagRow->new_patta_type_code)?></td>
                          </tr>
                          ...
                          <tr>
                            <th>Proposed Landclass for Homestead </th>
                            <td><?php if($dagRow->new_landclass_home){echo $this->utilityclass->getLandClassCode($dagRow->new_landclass_home);}?></td>
                          </tr>

                          <tr>
                            <th>Homestead Land Revenue </th>
                            <td><?=$dagRow->newHomeRevenue?></td>
                          </tr>
                          <tr>
                            <th>Homestead Local Tax </th>
                ...
                elseif($alm->chitha_processing_details == 2)
                {
                  ...
                  <h3 class="pl-2 mt-2 text-shadow text-success">#LM Verfication Report(Approved)</h3>
                    ...
                    <?php
                      if($dagDetails)
                      {
                        foreach($dagDetails as $dagRow)
                        {
                          ...
                          <tr>
                            <th>Proposed Patta Type </th>
                            <td><?=$this->utilityclass->getPattaName($dagRow->new_patta_type)?></td>
                          </tr>
                          ...
                          <tr>
                            <th>Proposed Landclass for Homestead </th>
                            <td><?php if($dagRow->new_land_class_home){echo $this->utilityclass->getLandClassCode($dagRow->new_land_class_home);}?></td>
                          </tr>
                          ...
                          <tr>
                            <th>Proposed Landclass for Agriculture </th>
                            <td><?php if($dagRow->new_land_class_agri){echo $this->utilityclass->getLandClassCode($dagRow->new_land_class_agri);}?></td>
                          </tr>
              ...
              <?php
                ...
                ?>
                  <h4 class="text-center mt-2">
                    <button type="button" onclick="finalVerificationModal('<?=$alm->case_no?>');" class="btn btn-sm btn-warning"><b>EDIT</b></button>
                    <button type="button" onclick="finalApprove('<?=$alm->case_no?>')" class="btn btn-sm btn-success"><b>APPROVE</b></button>
                  </h4>
                <?php
                }
                else if($alm->chitha_processing_details == 2)
                {
                  ?>
                    ...
                    <h3 class="text-center">
                        <button type="button" onclick="finalChithaUpdate('<?=$alm->case_no?>')" class="btn btn-danger"><b>UPDATE CHITHA</b></button>
                    </h3>
...
<script>
    ...
    function finalVerificationModal(case_no)
    {
        ...
        $.ajax({
            url: baseurl+'NcCommonController/getFinalVerificationData',
            type: "POST",
            data: postData,
            success: function(data) 
            {
                ...
                for(i=0; i<arr.dagResult.length; i++)
                {
                    ...
                    tr += '<tr>'+
                           ...
                           if(arr.dagResult[i].new_home_land_revenue != false)
                            {
                                landClassRevHome = '<input type="number" id="revenue_home'+arr.dagResult[i].dag_no+'" name="revenue_home'+arr.dagResult[i].dag_no+'" value="'+arr.dagResult[i].new_home_land_revenue+'" placeholder="Enter Revenue" class="form-control" readonly>';
                            }
                            else
                            {
                                landClassRevHome = '<input type="number" id="revenue_home'+arr.dagResult[i].dag_no+'" name="revenue_home'+arr.dagResult[i].dag_no+'" placeholder="Enter Revenue" class="form-control" readonly>';
                            }
                            ...
                            {
                                landClassLocalTaxHome = '<input type="number" name="local_tax_home'+arr.dagResult[i].dag_no+'" id="local_tax_home'+arr.dagResult[i].dag_no+'" value="'+arr.dagResult[i].new_home_land_local_tax+'" placeholder="Enter Local Tax" class="form-control" readonly>'
                            }
                            else
                            {
                                landClassLocalTaxHome = '<input type="number" name="local_tax_home'+arr.dagResult[i].dag_no+'" id="local_tax_home'+arr.dagResult[i].dag_no+'" placeholder="Enter Local Tax" class="form-control" readonly>'
                            }
                           ...
                           tr += '<tr>'+
                                ...
                                '<th colspan="2">'+
                                    ...
                                    '<select name="land_class_code_homestead'+arr.dagResult[i].dag_no+'" class="form-control" onchange="getRevenueHome(\''+arr.dagResult[i].dag_no+'\', \''+arr.dagResult[i].case_no+'\')" id="land_class_code_homestead'+arr.dagResult[i].dag_no+'">'+
                            ...
                            '<tr>'+
                                '<th colspan="2">'+
                                    ...
                                    '<select name="land_class_code_agriculture'+arr.dagResult[i].dag_no+'" class="form-control" onchange="getRevenueAgri(\''+arr.dagResult[i].dag_no+'\', \''+arr.dagResult[i].case_no+'\')" id="land_class_code_agriculture'+arr.dagResult[i].dag_no+'">'+
                                        '<option value="">Select land class...</option>'+
                                          optionAgri+
                                    '</select>'+
                                '</th>'+
...
<script>
    ...
    function landmark_dist(side, dag_no) 
    {
        ...
        $.ajax({
            url: baseurl + "NcCommonController/getSubdiv/" + district,
            success: function(data) {
    ...
    function landmark_subdiv(side,dag_no) 
    {
        ...
        $.ajax({
            url: baseurl + "NcCommonController/getCircle/" + district+ "/"+ subdiv,
            success: function(data) {
    ...
    function landmark_cir(side, dag_no) 
    {
        ...
        $.ajax({
            url: baseurl + "NcCommonController/getMouza/" + district+ "/"+ subdiv+"/"+ cir,
            success: function(data) {
    ...
    function landmark_mouza(side,dag_no) 
    {
        ...
        $.ajax({
            url: baseurl + "NcCommonController/getLot/" + district+ "/"+ subdiv+"/"+ cir +"/"+ mouza,
            success: function(data) {
    ...
    function landmark_lot(side,dag_no) 
    {
        ...
        $.ajax({
            url: baseurl + "NcCommonController/getVillage/" + district+ "/"+ subdiv+"/"+ cir +"/"+ mouza+"/"+ lot,
            success: function(data) {
    ...
    function landmark_village(side,dag_no) 
    {
        ...
        $.ajax({
            url: baseurl + "NcCommonController/getAllDags/" + district+ "/"+ subdiv+"/"+ cir +"/"+ mouza+"/"+ lot+"/"+ village,
            success: function(data) {
...
<script>
    ...
    {   
        ...
        $.ajax({
            url: baseurl+'NcCommonController/getGuardianRelation',
            type: "POST",
            success: function(data) 
            {
                $.unblockUI();
    ...
    function addFamilyDetails(){
        ...
        $.ajax({
            url: baseurl+'NcCommonController/addFamilyDetails',
            type: "POST",
            data: postData,
            success: function(data) {
                arr = JSON.parse(data);
    ...
    function confirmDeleteFamily(id)
    {
        ...
        if(confirm("Are you sure you want to delete this Record?"))
        {
            $.ajax({
                type: "POST",
                url: baseurl+'NcCommonController/delFamilyDetailsExisted',
                async: false,
                // dataType: 'json',
    ...
    function confirmDeleteFamilyLmInserted(id)
    {
        ...
        if(confirm("Are you sure you want to delete this Record?"))
        {
            $.ajax({
                type: "POST",
                url: baseurl+'NcCommonController/delFamilyDetails',
                async: false,
                // dataType: 'json',
                data: { id: id, case_no:case_no },
                success: function (response) 
...
<script>
    ...
    $('#approvalForm').submit(function (e) {
        ...
        $.ajax({
            url: baseurl + "NcCommonController/chithaProcessingDetails",
            type: 'POST',
  ...
  function finalApprove(case_no)
  {
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: baseurl+'NcCommonController/coApproveLmReport',
                    type: "POST",
                    data: postData,
                    success: function(data) {
                        arr = JSON.parse(data);
...
<script>
    function getRevenueHome(dag_no, case_no)
    {
        ...
        $.ajax({
            url: baseurl + "NcCommonController/getRevenueDetails",
            type: 'POST',
            data: postData,
            dataType: 'json',
            success: function (data) {
...
<script>
    function finalChithaUpdate(case_no)
    {
        ...
        $.ajax({
            url: baseurl+'NcCommonController/chithaUpdateDetails',
            type: "POST",
            data: postData,
            success: function(data) 
            {
    ...
    $(document).on('click','#chithaUpdateModalYes',function ()
    {
        ...
        $.ajax({
            url: baseurl+'NcCommonController/checkIfBifurcateAreaExceed',
            type: "POST",
            // dataType: 'json',
