						...
						<div class='col-lg-10 col-lg-offset-1'>
							...
							<table class='table uni_text table_black'>
								...
								<tr>
									...
									<td>DEO Name : <?php $name=$this->utilityclass->getSelectedCOName($this->session->userdata('dist_code'),$this->session->userdata('subdiv_code'),$this->session->userdata('cir_code'),$this->session->userdata['basic_entry']->user_code);
									echo $name->username;
								?></td>
							...
							<?php if($firstparty=$this->session->userdata('popup_control')==1){?>
							<form class='form-horizontal' method='POST' action='<?php echo base_url() . 'index.php/chitha_basic_deo/coconfirm '?>' >
								<h2>Please Select Option</h2>
								<div class="form-group">
									<div class="form-check">
									  <input class="form-check-input col-lg-1" type="radio" name='status'  value="1" checked>
									  ...
									  <input type='hidden' name='mouza_pargona_code' value='<?=$this->session->userdata['basic_entry']->mouza_pargona_code ?>'>
									  <input type='hidden' name='lot_no' value='<?=$this->session->userdata['basic_entry']->lot_no ?>'>
									  <input type='hidden' name='vill_townprt_code' value='<?=$this->session->userdata['basic_entry']->vill_townprt_code ?>'>
									  <input type='hidden' name='dag_no' value='<?=$this->session->userdata['basic_entry']->dag_no ?>'>
									  <input type='hidden' name='patta_no' value='<?=$this->session->userdata['basic_entry']->patta_no ?>'>
									  <input type='hidden' name='patta_type_code' value='<?=$this->session->userdata['basic_entry']->patta_type_code ?>'>
									</div>
