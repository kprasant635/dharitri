...
<div id="treeViewModal" class="modal" tabindex="-1">
    <div class="modal-dialog" style="max-width:1312px;max-height: 450px;">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <h5 class="modal-title" style="color: #9f382f;border-left: 4px solid #b2412f;background: beige;padding: 9px;">Tree view structure of applicant</h5>
                <?php //echo "<pre>";var_dump($allApplicant); ?>
                <div class="familytree">
                   <?php if(isset($allApplicant['level']) && $allApplicant['level'] == 3){ ?>
                   

                    <ul><li><a href="#" style="background-color: #e87214;color: #fff;font-weight: bold;"><?=$allApplicant['GGP'][0]['name']?></a>
                    <ul>
                        <?php for($i = 0; $i < count($allApplicant['GP']);$i++){?>
                            <li>
                                <?php 
                                    if($allApplicant['GP'][$i]['rel'] == 'A_GP' && $i == $allApplicant['GP'][$i]['key_val']) {
                                echo "<a href='#'><b style='color:#2f96b2'>".$allApplicant['GP'][$i]['name']."</b></a>";
                                ?></a>
                                <ul>
                                    <?php for($j = 0; $j < count($allApplicant['P']);$j++){?>
                                            <?php 
                                                if($allApplicant['P'][$j]['rel'] == 'A_P' && $i == $allApplicant['P'][$j]['key_val']) {
                                                        echo "<li><a href='#'><b style='color:red'>".$allApplicant['P'][$j]['name']."</b></a>"; ?>
                                                        <ul>
                                                        <?php for($k = 0; $k < count($allApplicant['A']);$k++){?>
                                                            <?php if($i == $allApplicant['A'][$k]['key_val']){ ?>
                                                            <li><a href="#" style="color:#227b20;font-weight: bold;"><?=$allApplicant['A'][$k]['name']?></a></li>
                                                            <?php }?>
                                                        <?php }?>
                                                        </ul>
                                                    </li>
                                                    
                                            <?php  }else if($allApplicant['P'][$j]['rel'] != 'A_P' && $i == $allApplicant['P'][$j]['key_val']){
                                                ?>
                                            <?php echo "<li><a href='#'>".$allApplicant['P'][$j]['name']."</a></li>"; ?>
                                         <?php } ?>
                                        
                                    <?php } ?>
                                </ul>
                                  <?php  }else{
                                echo "<a href='#'>".$allApplicant['GP'][$i]['name']."</a>";
                                    }
                                ?>
                   ...
                   <?php }else{  ?>
                    <ul>
                        <li><a href="#" style="background-color: #e87214;color: #fff;font-weight: bold;"><?=$allApplicant['GP'][0]['name']?></a>
                        <ul>
                            <?php for($j = 0; $j < count($allApplicant['P']);$j++){?>
                                    <?php 
                                        if($allApplicant['P'][$j]['rel'] == 'A_P' && $j == $allApplicant['P'][$j]['key_val']) {
                                                echo "<li><a href='#' style='border: 1px solid #2f96b2;'><b style='color:#2f96b2'>".$allApplicant['P'][$j]['name']."</b></a>"; ?>
                                                <ul>
                                                <?php for($k = 0; $k < count($allApplicant['A']);$k++){?>
                                                    <?php if($j == $allApplicant['A'][$k]['key_val']){ ?>
                                                    <li><a href="#" style="border: 1px solid #227b20;color:#227b20;font-weight: 400;"><?=$allApplicant['A'][$k]['name']?></a></li>
                                                    <?php }?>
                                                <?php }?>
                                                </ul>
                                            </li>
                                            
                                    <?php  }else if($allApplicant['P'][$j]['rel'] != 'A_P' && $j == $allApplicant['P'][$j]['key_val']){
                                        ?>
                                    <?php echo "<li><a href='#' style='border: 1px solid #2f96b2;'>".$allApplicant['P'][$j]['name']."</a></li>"; ?>
                                 <?php } ?>
...
<script type="text/javascript">
  ...
  $('#formAjaxPost').on('submit', function(event){
    ...
    if(is_multigeneration == "S"){
        uriSegment = "inheritancePostOfficeSingleGeneration";
    }else if(is_multigeneration == "M"){
        uriSegment = "inheritancePostOfficeMultiGeneration";
    }
    // var formData = $(this).serialize();
    var formData = new FormData(document.getElementById('formAjaxPost'));
        $.ajax({
            type        : 'POST', 
