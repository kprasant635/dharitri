...
<?php if (ESCALATION_ENABLE == 1) { ?>
    $(document).ready(function(){
        $('#mouza_lot_second, #zone_status').change(function(){
            var mouza_code = null;
            var lot_no = null;
            ...
            load_data_second(mouza_code,lot_no, zone_status);
        });  
        load_data_second();
        function load_data_second(mouza_code,lot_no, zone_status){
            ...
            $(this).html(title+' <input type="text" class="form-control input_search form-control-sm" placeholder="Search ' + title + '" />');
              ...
              var dataTable = $('#user_data').DataTable({  
                   ...
                   // "order":[],  
                    "ordering": false,
                   "ajax":{  
                        url:"<?php echo base_url() . 'index.php/DataTableController/OmutCoSecondProceeding111'; ?>", 
                        type:"POST",
                        data: {
    ...
    $(document).ready( function () {
        ...
        function load_data(mouza_code,lot_no,vill_mouza_code,vill_lot_no,village_code,zone_status)
        {
            ...
            var table = $('#PendingOFMcases').DataTable({
                ...
                'ajax':{
                    url: base_url+'index.php/coofficemutation/getPendingMutationCasesCOEnd1111',
                    type:'POST',
                    data: {
                        mouza_code:mouza_code,
                        lot_no:lot_no,
            ...
            $(this).html(title+' <input type="text" class="form-control input_search form-control-sm" placeholder="Search ' + title + '" />');
              ...
              var dataTable = $('#user_data').DataTable({  
                   ...
                   // "order":[],  
                    "ordering": false,
                   "ajax":{  
                        url:"<?php echo base_url() . 'index.php/DataTableController/OmutCoSecondProceedingRevive'; ?>", 
                        type:"POST",
                        data: {
                            mouza_code:mouza_code,
                            lot_no:lot_no,
