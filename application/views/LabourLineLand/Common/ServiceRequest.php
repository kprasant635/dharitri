...
<center>
    ...
    <mark>
        <strong style="font-size: 20px;">
            Settlement for Labour Line Land
        </strong>
    </mark>
                ...
                <tr>
                    ...
                    <th>Urban/Rural
                        ...
                        <select class="form-control input_search" name="rural" id="rural" data-column-index="3">
                            <option value="">select</option>
                            <?php if (isset($selectList->urban_check)) {
                                foreach ($selectList->urban_check as $rural) {
                            ?>
                                    <option value="<?= $rural->is_urban ?>"><?php if ($rural->is_urban == 'Y') {
                                                                                echo 'Urban';
                                                                            } else {
                                                                                echo "Rural";
                                                                            } ?></option>
                    ...
                    <th>Name
                        <select class="form-control input_search" name="category" id="category" data-column-index="4">
                            <option value="">select</option>
                            <?php if (isset($selectList->vill_list)) {
                                foreach ($selectList->vill_list as $vill) {
                            ?>
    ...
    $(document).ready(function() {
        ...
        function load_data(category, rural, occupation) {
            ...
            var table = $('#datatable').DataTable({
                ...
                'ajax': {
                    url: base_url + 'index.php/LabourLineController/paginationAPI',
                    type: 'POST',
                    data: {
...
<script>
    ...
    function registrationInit(application_no) {
        ...
        $.ajax({
            url: baseurl + 'LabourLineController/registration',
            type: "POST",
            data: postData,
            success: function(data) {
                ...
                if (arr.responseType != 2) {
                    ...
                    window.location.href = baseurl + "LabourLineController/firstProceeding?an=" + application_no;
                }
