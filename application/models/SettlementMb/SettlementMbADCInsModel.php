<?php
class SettlementMbADCInsModel extends CI_Model
{
    ...
    public function getAllPendingSettlementKhas($dist_code)
    {
        ...
        $this->db->where('service_code', SLIJE_ID);
        $this->db->where('pending_officer', MB_ADD_DEPUTY_COMM);
    ...
    public function getAllAppInReportSendByDcToSdlacKhas($proposal_no)
    {
        ...
        settlement_basic.dist_code,settlement_basic.subdiv_code,settlement_basic.cir_code,settlement_institution_details.purpose_land_allot_co,
        settlement_institution_details.ins_cat_type_co,settlement_institution_details.under_venture_school,settlement_institution_details.under_ngo_trust_localbodies');
        ...
        $this->db->where('settlement_basic.service_code', SLIJE_ID);
        $this->db->where('settlement_proposal_cases.proposal_id', $proposal_no);
        $this->db->where("(settlement_basic.adc_code = '" . $this->session->userdata('user_code') . "' OR adc_code IS NULL)", null, false);
    ...
    public function rejectedCaseListIns($dist_code, $service_code, $rejected_by)
    {
        $this->db->select();
    ...
    public function saveProposalSDLACKhas($data)
    {
        ...
        $this->db->set('updated_at', 'NOW()', FALSE);
        $this->db->set('service_code',SLIJE_ID );
    ...
    public function getSdlacMemberReportDetailsKhas($proposal_no,$dist_code)
    {
        ...
        $this->db->where('settlement_sdlac_member_report.service_code', SLIJE_ID);
        $data = $this->db->get();
        return $data;
