<?php
class SettlementTeaAdcModel extends CI_Model
{
    ...
    public function getAllPendingSettlementKhas($dist_code)
    {
        $this->db->select('*');
        $this->db->from('settlement_basic');
        $this->db->where('service_code', SETTLEMENT_SPECIAL_CULTIVATORS_ID);
        $this->db->where('pending_officer', MB_ADD_DEPUTY_COMM);
    ...
    public function countMarkAsSDLACSettlementKhas($dist_code)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_SPECIAL_CULTIVATORS_ID)
            ->where('pending_officer', MB_ADD_DEPUTY_COMM)
    ...
    public function countAllPendingSettlementKhas($dist_code)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_SPECIAL_CULTIVATORS_ID)
            ->where('pending_officer', MB_ADD_DEPUTY_COMM)
    ...
    // get all pending settlement  cases KHAS
    public function getMarkAsSDLACSettlementKhas($dist_code)
    {
        $this->db->select('*');
        $this->db->from('settlement_basic');
        $this->db->where('service_code', SETTLEMENT_SPECIAL_CULTIVATORS_ID);
    ...
    // get all under consideration cases TEA
    public function getAllUnderConSettlementTea($dist_code)
    {
        $this->db->select('*');
        $this->db->from('settlement_basic');
        $this->db->where('service_code', SETTLEMENT_SPECIAL_CULTIVATORS_ID);
    ...
    // count all under consideration  cases TEA
    public function countAllUnderConsiderationAppTea($dist_code)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_SPECIAL_CULTIVATORS_ID)
            ->where('pending_officer', MB_ADD_DEPUTY_COMM)
    ...
    public function getAllProposalSendByDcToSdlacKhas($dist_code)
    {
        $this->db->select('*');
    ...
    public function countAllProposalSendByDcToSdlacKhas($dist_code)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_SPECIAL_CULTIVATORS_ID)
            ->where('status', 1)
    ...
    public function saveProposalSDLACKhas($data)
    {
        ...
        $this->db->set('updated_at', 'NOW()', FALSE);
        $this->db->set('service_code',SETTLEMENT_SPECIAL_CULTIVATORS_ID );
    ...
    // update proposal list
    public function updateProposalListByIdWithDistCode($proId,$dist_code,$data)
    {
        $this->db->where('id', $proId);
        $this->db->where('dist_code', $dist_code);
        $this->db->set('updated_at', 'NOW()', FALSE);
    ...
    // get all proposal list in send by DC to SDLAC KHAS
    public function getSdlacApprovalProposalListTea($dist_code)
    {
    ...
    public function getSdlacMemberReportDetailsKhas($proposal_no,$dist_code)
    {
        ...
        $this->db->where('settlement_sdlac_member_report.service_code', SETTLEMENT_SPECIAL_CULTIVATORS_ID);
        $data = $this->db->get();
        return $data;
    ...
    // get all pending settlement  cases TEA
    public function getMarkAsSDLACSettlementTea($dist_code)
    {
        $this->db->select('*');
        $this->db->from('settlement_basic');
        $this->db->where('service_code', SETTLEMENT_SPECIAL_CULTIVATORS_ID);
    ...
    // get all proposal list in send by DC to SDLAC KHAS
    public function getAllProposalSendByDcToSdlacTea($dist_code)
    {
        $this->db->select('*');
        $this->db->from('settlement_proposal_list');
        $this->db->where('service_code', SETTLEMENT_SPECIAL_CULTIVATORS_ID);
    ...
    // count  all pending settlement  cases TEA
    public function countAllPendingSettlementTea($dist_code)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_SPECIAL_CULTIVATORS_ID)
            ->where('pending_officer', MB_ADD_DEPUTY_COMM)
    ...
    // count all Mark list for SDLAC TEA
    public function countMarkAsSDLACSettlementTea($dist_code)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_SPECIAL_CULTIVATORS_ID)
            ->where('pending_officer', MB_ADD_DEPUTY_COMM)
    ...
    // count all proposal list in send by DC to SDLAC TEA
    public function countAllProposalSendByDcToSdlacTea($dist_code)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_SPECIAL_CULTIVATORS_ID)
            ->where('status', 1)
    ...
    // count all Re-Reverted By CO case for DC TEA
    public function countReRevertedByCoApplicationTea($dist_code)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_SPECIAL_CULTIVATORS_ID)
            ->where('from_office', MB_CIRCLE_OFFICER)
    ...
    // count all approve list TEA
    public function countAllApproveAppBySdlacTea($dist_code)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_SPECIAL_CULTIVATORS_ID)
            ->where('from_office', MB_ADD_DEPUTY_COMM)
    ...
    // count all rejected list by Dc TEA
    public function countAllRejectAppByDcTea($dist_code)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_SPECIAL_CULTIVATORS_ID)
            ->where('from_office', MB_ADD_DEPUTY_COMM)
    ...
    // count all Reverted By DEPARTMENT case for DC TEA
    public function countRevertedByDeptApplicationTea($dist_code)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_SPECIAL_CULTIVATORS_ID)
            ->where('from_office', MB_DEPARTMENT)
    ...
    // Count all chitha update application TEA
    public function countAllOrderChithaUpdateAppTea($dist_code)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_SPECIAL_CULTIVATORS_ID)
            ->where('from_office', MB_CIRCLE_OFFICER)
    ...
    // save proposal list for SDLAC
    public function saveProposalSDLACTea($data)
    {
        $this->db->set('created_at', 'NOW()', FALSE);
        $this->db->set('updated_at', 'NOW()', FALSE);
        $this->db->set('created_by', MB_ADD_DEPUTY_COMM);
        $this->db->set('service_code',SETTLEMENT_SPECIAL_CULTIVATORS_ID );
        $this->db->insert('settlement_proposal_list', $data);
        return $this->db->trans_status();
    ...
    // get all case under selected proposal in send by SDO to SDLAC Tea
    public function getAllAppInReportSendByDcToSdlacTea($proposal_no)
    {
        $this->db->select('*');
        $this->db->from('settlement_proposal_cases');
        $this->db->where('proposal_id', $proposal_no);
