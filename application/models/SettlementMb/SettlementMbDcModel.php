<?php
class SettlementMbDcModel extends CI_Model
{
    ...
    // count application id by case no by DC for SDLAC final verify
    public function countSettlementApplicationDetailsByCaseNoForSDLACFinalVerifyReCla($caseNo,$dist_code)
    {
        return $this->db->select()
            ->where('case_no', $caseNo)
            ->where('dist_code', $dist_code)
    ...
    // get all regular case under selected proposal in send by DC to SDLAC KHAS
    ...
    // get all rejected cases under seleted proposal sent to DC by SDLAC
    // public function getAllRejectedCasesSentBySdlac($proposal_no)
    // {
    //     $this->db->select('*');
    //     $this->db->from('settlement_proposal_cases');
    //     $this->db->where('proposal_id', $proposal_no);
    //     $this->db->where('rejected_flag', 1);
    //     $data = $this->db->get();
    //     return $data;
    // }
    ...
    // count all proposal list in send by DC to SDLAC KHAS
    ...
    // // Count all rejected cases by sdo/adc khas
    // public function rejectedCasesCount($dist_code)
    // {
    //     return $this->db->select()
    //         ->where('service_code', SETTLEMENT_KHAS_LAND_ID)
    //         ->where_in('from_office', [MB_ADD_DEPUTY_COMM, MB_SUB_DIV_COMM, MB_DEPUTY_COMM])
    //         ->where_in('pending_officer', [MB_DEPUTY_COMM])
    //         ->where('status', MB_REJECT_REASON_ENTERED)
    //         ->where('dist_code', $dist_code)
    //         ->get('settlement_basic')
    //         ->num_rows();
    // }

    // // get all rejected settlement  cases KHAS
    // public function getAllRejectedCasesBySdoAdc($dist_code)
    // {
    //     $this->db->select('*');
    //     $this->db->from('settlement_basic');
    //     $this->db->where('service_code', SETTLEMENT_KHAS_LAND_ID);
    //     $this->db->where_in('pending_officer', [MB_DEPUTY_COMM]);
    //     $this->db->where('status', MB_REJECT_REASON_ENTERED);
    //     $this->db->where('dist_code', $dist_code);
    //     $data = $this->db->get();
    //     return $data;
    // }
