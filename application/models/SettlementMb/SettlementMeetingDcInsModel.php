<?php
class SettlementMeetingDcInsModel extends CI_Model
{
    ...
    public function getProposalDetailAgainstMeetingIdMBTin($dist_code, $meetingId)
    {
        $this->db->select('service_code, proposal_name, id as proposal_id');
        $this->db->where('dist_code', $dist_code);
        $this->db->where('proposal_meeting_id', $meetingId);
    ...
    public function getProposalDetailAgainstMeetingIdSdoMBTin($dist_code, $meetingId)
    {
        $this->db->select('service_code, proposal_name, id as proposal_id');
        $this->db->where('dist_code', $dist_code);
    ...
    public function getPendingMeetingDetailByMeetingIDMbTin($meetingId){
        $this->db->select();
        $this->db->where('id', $meetingId);
        $this->db->where('mb_status', 3);
        $data = $this->db->get('proposal_meeting_list');
    ...
    // get all case under selected proposal in send by DC to SDLAC ins
    public function getSingleAppInReportSendByDcToSdlacIns($case_no)
    {
        $this->db->select('*');
        $this->db->where('case_no', $case_no);
        $this->db->where('service_code', '45');
        $data = $this->db->get('settlement_institution_details');
        return $data->row();
    ...
    public function DepartmentApprovedCaseList($service_code)
    {
        $this->db->select('*');
        $this->db->from('settlement_basic');
