<?php
class SettlementTenantUrbanAdcModel extends CI_Model
{
    ...
    public function updateSettlementBasicData($caseNo, $dist_code, $data)
    {
      ...
      public function updateSettlementBasicDataAdc($caseNo, $dist_code, $data)
    {
        $this->db->where('case_no', $caseNo);
        $this->db->where('dist_code', $dist_code);
    ...
    public function updateSettlementBasicDatav2($caseNo, $dist_code, $data)
    {
        $this->db->where('case_no', $caseNo);
        $this->db->where('dist_code', $dist_code);
        $this->db->where('pending_officer', MB_ADD_DEPUTY_COMM);
    ...
    public function getSettlementApplicationDetailsByCaseNoAdc($caseNo, $dist_code)
    {
        return $this->db->select()
            ->where('case_no', $caseNo)
            ->where('dist_code', $dist_code)
    ...
    public function hearingAdcCount($dist_code)
    {
        $user_code = $this->session->userdata('user_code');

        $this->db->select('count(*)');
        $this->db->where('dist_code', $dist_code);
        $this->db->where('status', 'JA');
        $this->db->where('service_code', SETTLEMENT_TENANT_URBAN_ID);
        $this->db->where('pending_officer', MB_ADD_DEPUTY_COMM);
        $this->db->where('(adc_code = \'' . $user_code . '\' or adc_code is null or adc_code = \'\')');
    ...
    public function countSettlementApplicationDetailsByCaseNov2($caseNo, $dist_code)
    {
        return $this->db->select()
            ->where('case_no', $caseNo)
            ->where('dist_code', $dist_code)
    ...
    {
        $this->db->select('exl_id, COUNT(exl_id) as count');
        $this->db->where('dist_code', $dist_code);
        $this->db->group_by('exl_id');
    ...
    public function getAllHearingPendingCasesForAdc($dist_code)
    {
        $user_code = $this->session->userdata('user_code');
