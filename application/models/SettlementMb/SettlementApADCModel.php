<?php
class SettlementApADCModel extends CI_Model
{
    ...
    public function getSdlacApprovalProposalListAp($dist_code)
    {
    ...
    public function getSdlacApprovalProposalIndividualAp($proposal_no,$dist_code)
    {
        $this->db->select('*');
        $this->db->from('settlement_proposal_list');
    ...
    public function countAllUnderConsiderationAppAp($dist_code)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_AP_TRANSFER_ID)
            ->where('pending_officer', MB_ADD_DEPUTY_COMM)
    ...
    // count all report in send by DC to SDLAC Ap
    public function countAllAppInReportSendByDcToSdlacAp($dist_code)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_AP_TRANSFER_ID)
            ->where('pending_officer', MB_ADD_DEPUTY_COMM)
    ...
    {
        $this->db->select('settlement_proposal_cases.*,settlement_basic.dist_code,settlement_basic.subdiv_code,settlement_basic.cir_code');
    ...
    // count all proposal list in send by ADC to SDLAC Ap
    ...
    // Count all final verify by dc application list Ap
    ...
    // get all final verify by dc application list Ap
