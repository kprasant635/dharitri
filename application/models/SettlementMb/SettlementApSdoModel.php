<?php
class SettlementApSdoModel extends CI_Model
{
    ...
    // get all case under selected proposal in send by SDO to SDLAC AP
    ...
    public function getAllPendingSettlementAp($dist_code, $suv_div)
    {
        $this->db->select('*');
    ...
    public function countAllPendingSettlementAp($dist_code, $suv_div)
    {
        return $this->db->select()
    ...
    public function countMarkAsSDLACSettlementAp($dist_code, $suv_div)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_AP_TRANSFER_ID)
            ->where('pending_officer', MB_SUB_DIV_COMM)
    ...
    // count all proposal list in send by SDO to SDLAC AP
    public function countAllProposalSendByDcToSdlacAp($dist_code, $suv_div)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_AP_TRANSFER_ID)
            ->where('status', 1)
    ...
    // count all Re-Reverted By CO case for SDO AP
    public function countReRevertedByCoApplicationAp($dist_code, $suv_div)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_AP_TRANSFER_ID)
            ->where('from_office', MB_CIRCLE_OFFICER)
    ...
    public function countAllApproveAppBySdlacAp($dist_code, $suv_div)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_AP_TRANSFER_ID)
    ...
    // count all rejected list by SDO AP
    public function countAllRejectAppByDcAp($dist_code, $suv_div)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_AP_TRANSFER_ID)
            ->where('from_office', MB_SUB_DIV_COMM)
    ...
    public function countAllUnderConsiderationAppAp($dist_code, $suv_div)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_AP_TRANSFER_ID)
            ->where('pending_officer', MB_SUB_DIV_COMM)
    ...
    // Count all final verify by SDO application list AP
    public function countAllCasesForFinalVerifyAppAp($dist_code, $suv_div)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_AP_TRANSFER_ID)
            ->where('from_office', MB_SUB_DIV_COMM)
    ...
    // count all Reverted By DEPARTMENT case for SDO AP
    public function countRevertedByDeptApplicationAp($dist_code, $suv_div)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_AP_TRANSFER_ID)
    ...
    public function countAllOrderChithaUpdateAppAp($dist_code, $suv_div)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_AP_TRANSFER_ID)
            ->where('from_office', MB_CIRCLE_OFFICER)
    ...
    // get all pending settlement  cases AP
    public function getMarkAsSDLACSettlementAp($dist_code, $suv_div)
    {
        $this->db->select('*');
        $this->db->from('settlement_basic');
        $this->db->where('service_code', SETTLEMENT_AP_TRANSFER_ID);
    ...
    // get all under consideration  cases AP
    public function getAllUnderConSettlementAp($dist_code, $suv_div)
    {
        $this->db->select('*');
        $this->db->from('settlement_basic');
        $this->db->where('service_code', SETTLEMENT_AP_TRANSFER_ID);
    ...
    // get all proposal list in send by SDO to SDLAC AP
    public function getAllProposalSendByDcToSdlacAp($dist_code, $suv_div)
    {
        $this->db->select('*');
    ...
    // get all Re-Reverted By CO case for SDO AP
    public function getReRevertedByCoApplicationAp($dist_code, $suv_div)
    {
        $this->db->select('*');
        $this->db->from('settlement_basic');
        $this->db->where('service_code', SETTLEMENT_AP_TRANSFER_ID);
    ...
    // Count all Sdlac Status List AP
    ...
    public function getSdlacApprovalProposalListAp($dist_code,$suv_div)
    {
        $this->db->select('*');
        $this->db->from('settlement_proposal_list');
        $this->db->where('dist_code', $dist_code);
    ...
    // get single proposal list in send by DC to SDLAC AP
    public function getSdlacApprovalProposalIndividualAp($proposal_no,$dist_code,$suv_div)
    {
        $this->db->select('*');
        $this->db->from('settlement_proposal_list');
    ...
    public function getSdlacMemberReportDetailsAp($proposal_no,$dist_code)
    {
        ...
        $this->db->where('settlement_sdlac_member_report.service_code', SETTLEMENT_AP_TRANSFER_ID);
        $data = $this->db->get();
        return $data;

    }

    // get SDLAC/CDLAC Member Status List AP
