<?php
class SettlementTenantUrbanDcModel extends CI_Model
{
    ...
    public function countSettlementApplicationDetailsByCaseNoAdc($caseNo, $dist_code)
    {
        return $this->db->select()
            ->where('case_no', $caseNo)
            ->where('dist_code', $dist_code)
        // ->where('general_notice_dc', 'y')
            ->where('status', MB_PENDING)
            ->where_in('pending_officer', [MB_ADD_DEPUTY_COMM])
            ->get('settlement_basic')
            ->num_rows();
    }
    ...
    public function paymentNoticeCases($dist_code)
    {
        ...
        $this->db->where('dept_approval', 'Y');
        // $this->db->where('(note_action_yn is not null or note_action_yn != \'y\')');
        $this->db->where('dist_code', $dist_code);
        $this->db->where('service_code', SETTLEMENT_TENANT_URBAN_ID);
        $this->db->where_in('status', [MB_PENDING]);
        $this->db->where_in('pending_officer', [MB_DEPUTY_COMM]);
        $data = $this->db->get();
        return $data;
    }

    public function paymentNoticeCasesBeforeList($dist_code)
    {
        $this->db->select('*');
        $this->db->from('settlement_basic');
        $this->db->where('general_notice_dc', 'y');
        $this->db->where('dept_approval is null');
        $this->db->where('(note_action_yn is not null or note_action_yn != \'y\')');
        $this->db->where('dist_code', $dist_code);
        $this->db->where('service_code', SETTLEMENT_TENANT_URBAN_ID);
        $this->db->where_in('status', [MB_PENDING]);
    ...
    public function exlReportGenCount($dist_code)
    {
        ...
        $this->db->where('settlement_basic.service_code', SETTLEMENT_TENANT_URBAN_ID);
    ...
    public function getCasesforwardedByAdcAfterHearing($dist_code)
    {
        $this->db->select('count(*)');
        $this->db->from('settlement_basic');
        $this->db->where('general_notice_dc', 'y');
    ...
    // count application id by case no for DC
    public function countSettlementApplicationDetailsByCaseNoDept($caseNo, $dist_code)
    {
        return $this->db->select()
            ->where('case_no', $caseNo)
            ->where('dist_code', $dist_code)
    ...
    // get application id by reverted by dept
    public function countCasesRevertedFromDept($dist_code)
    {
        return $this->db->select()
            ->where('dist_code', $dist_code)
            ->where('general_notice_dc', 'y')
            ...
            ->where('status', 'R')
            ->where('dept_revert', 1)
            ->where('from_office', 'DPT')
            ->get('settlement_basic')
            ->num_rows();
    }


    // count application id by reverted by dept
    public function getCasesRevertedFromDept($dist_code)
    {
        return $this->db->select()
            ->where('dist_code', $dist_code)
            ->where('general_notice_dc', 'y')
    ...
    // count application id by case no for DC
    public function countSettlementApplicationDetailsByRevertedCaseNo($caseNo, $dist_code)
    {
        return $this->db->select()
            ->where('case_no', $caseNo)
            ->where('dist_code', $dist_code)
    ...
    // get application id by case no
    public function getSettlementApplicationDetailsByRevertedCaseNo($caseNo, $dist_code)
    {
        return $this->db->select()
            ->where('case_no', $caseNo)
            ->where('dist_code', $dist_code)
