<?php
class basundhara3Model extends CI_Model {
    ...
    public function allCORequest($service){
        ...
        $url = API_LINK_MB3."serviceWiseRecordsCO/$service/$dist_code/$subdiv_code/$cir_code" ;
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
    ...
    public function allLmRequest($service){
        ...
        $lot_no = $this->session->userdata('lot_no');
        $ru=$this->session->userdata('user_desig_code');
        $url = API_LINK_MB3."serviceWiseRecords/$ru/$service/$dist_code/$subdiv_code/$cir_code/$mouza_pargona_code/$lot_no" ;
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
    ...
    function searchBasundharaLinkApp($case_no){
        ...
        if($linkAvail){
            $linkAvail=$linkAvail->basundhara;
            if($linkAvail){
                $url = API_LINK_MB3."serviceResponse?application_no=" . $linkAvail ;
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, $url);
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
    ...
    function adcRejectList(){
        $curl_handle = curl_init();
        //echo API_LINK;
        curl_setopt($curl_handle, CURLOPT_URL, API_LINK_MB3."rest/getrejectapp");
        curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($curl_handle, CURLOPT_SSL_VERIFYHOST,  2);
        curl_setopt($curl_handle, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($curl_handle, CURLOPT_CUSTOMREQUEST, "POST");
    ...
    function getRejectedApplications($conditions){
        ...
        curl_setopt($curl_handle, CURLOPT_URL, API_LINK_MB3."getRejectedApplications");
        curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($curl_handle, CURLOPT_SSL_VERIFYHOST,  2);
        curl_setopt($curl_handle, CURLOPT_SSL_VERIFYPEER, false);
    ...
    function geneartemb3PetitionNoReclass(){
        $petition_no = $this->db->query("select nextval('seq_max_reclasssuite') as count ")->row()->count;
        return $petition_no;
    }
