<?php
class SettlementApiModel extends CI_Model {
    ...
    public function allCORequest($service){
        ...
        $url = API_LINK_MB2."serviceWiseRecordsCO/$service/$dist_code/$subdiv_code/$cir_code" ;
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
    ...
    function QueryPost($case_no){
        ...
        curl_setopt($curl_handle, CURLOPT_URL, API_LINK_MB2."QueryReturn");
    ...
    public function allLmRequest($service){
        ...
        $lot_no = $this->session->userdata('lot_no');
        $ru=$this->session->userdata('user_desig_code');
        $url = API_LINK_MB2."serviceWiseRecords/$ru/$service/$dist_code/$subdiv_code/$cir_code/$mouza_pargona_code/$lot_no" ;
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
    ...
    function postApiBasundhara($application_no,$case,$rmk,$status,$task,$pen){
        ...
        $get_service_code_sql = $this->db->query('select * from settlement_basic where case_no = ?', array($case));

        if($get_service_code_sql->num_rows() <= 0){
            log_message('error', '#ERR169: Case not found!'. $case);
            return false;
        }
        $basic_row = $get_service_code_sql->row();

        if(!empty($basic_row->old_case_no) && $basic_row->review_flag == 1){
            
            $service_code = $basic_row->service_code;
    
            if(in_array($service_code, [SETTLEMENT_TENANT_ID, SETTLEMENT_AP_TRANSFER_ID, SETTLEMENT_TRIBAL_COMMUNITY_ID, SETTLEMENT_KHAS_LAND_ID, SETTLEMENT_PGR_VGR_LAND_ID, SETTLEMENT_SPECIAL_CULTIVATORS_ID])){
                //review_flag
                $post_array = array(
                    'application' => $application_no,
                    'dharitree' => $case,
        ...
        $curl_handle = curl_init();
        curl_setopt($curl_handle, CURLOPT_URL, API_LINK_MB2."applicationStatusUpdate");
        // curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, true);
        // curl_setopt($curl_handle, CURLOPT_SSL_VERIFYPEER, FALSE);
        curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($curl_handle, CURLOPT_SSL_VERIFYPEER, FALSE);
        ...
        if($httpcode != 200){
            log_message("error", " Curl-Error in api: ".API_LINK_MB2."applicationStatusUpdate with json_data: "
                .json_encode(array(
                    'application' => $application_no,
                    'dharitree' => $case,
                    'rmk' => $rmk,
    ...
    function searchBasundharaLinkApp($case_no){
        ...
        if($linkAvail){
            $linkAvail=$linkAvail->basundhara;
            if($linkAvail){
                $url = API_LINK_MB2."serviceResponse?application_no=" . $linkAvail ;
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, $url);
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
    ...
    function adcRejectList(){
        $curl_handle = curl_init();
        //echo API_LINK;
        curl_setopt($curl_handle, CURLOPT_URL, API_LINK_MB2."rest/getrejectapp");
        curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($curl_handle, CURLOPT_SSL_VERIFYHOST,  2);
        curl_setopt($curl_handle, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($curl_handle, CURLOPT_CUSTOMREQUEST, "POST");
    ...
    function postApiBasundharaMb3($application_no,$case,$rmk,$status,$task,$pen){
        // $caseRtpsBasu=$this->checkRtpsService($application_no);
        // $apilink=API_LINK_MB2;
        // return "y"; die;
    ...
    function insertNewDagIns($application_no, $dag_no, $encroacher_id, $patta_no, $patta_code,$possession_period,$encroacher_name,$encroacher_father_name,$tot_bigha,$tot_katha,$tot_lessa,$tot_ganda,$tot_kranti,$hbigha,$hkatha,$hlessa,$hganda,$hkranti){
        ...
        curl_setopt($curl_handle, CURLOPT_POSTFIELDS, http_build_query(array(
            ...
            'patta_code'=>$patta_code,
            'ip'=> $_SERVER['SERVER_ADDR'],
            'possession_period' => $possession_period,
            'encroacher_name' => $encroacher_name,
            'encroacher_father_name' => $encroacher_father_name,
            'tot_bigha' => $tot_bigha,
            'tot_katha' => $tot_katha,
            'tot_lessa' => $tot_lessa,
            'tot_ganda' => $tot_ganda,
    ...
    function applicationStatusUpdateBulkMb3($application_no,$case,$rmk,$status,$task,$pen){
        $apilink=API_LINK_MB3;
        $curl_handle = curl_init();
        curl_setopt($curl_handle, CURLOPT_URL, $apilink."applicationStatusUpdateBulk");
        // curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, true);
    ...
    function postApiBasundharaForRejectedCase2ndMb3($application_no,$case,$rmk,$status,$task,$pen,$rejectedCtgs){
        $caseRtpsBasu=$this->checkRtpsService($application_no);
