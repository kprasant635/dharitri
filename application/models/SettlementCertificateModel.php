...
<?php
    class SettlementCertificateModel extends CI_Model
    {
        ...
        public function getAllCasesAllocatedCertificateIssuedv2($start, $length, $order,  $selectService, $selectVillage, $selectCircle )
        {
            $searchByCol_0 = strtoupper($this->input->post('columns')[1]['search']['value']);
            if (! empty($searchByCol_0)) {
                $this->db->where("sb.case_no LIKE '%" . $searchByCol_0 . "%'");
            ...
            // Main query
            $this->db->select('sb.*, sid.ins_cat_type_co, acn.printing_status');
            $this->db->from('settlement_basic sb');
            $this->db->join('settlement_institution_details sid', 'sid.case_no = sb.case_no', 'left');
...
{
    // First query with old_dag_no
    $query = $this->db->query("
        select dag_revenue, dag_local_tax, patta_type_code, patta_no, old_patta_no 
        from chitha_basic 
        where dist_code=? and subdiv_code=? and cir_code=? and mouza_pargona_code=? 
          and lot_no=? and vill_townprt_code=? and old_dag_no=? and dag_no=?",
        [$dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no, $vill_townprt_code, $old_dag_no, $new_dag_no]
    );
    ...
    // Fallback query without old_dag_no
    $query2 = $this->db->query("
        select dag_revenue, dag_local_tax, patta_type_code, patta_no, old_patta_no 
        from chitha_basic 
        where dist_code=? and subdiv_code=? and cir_code=? and mouza_pargona_code=? 
          and lot_no=? and vill_townprt_code=? and dag_no=?",
        [$dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no, $vill_townprt_code, $new_dag_no]
    );
