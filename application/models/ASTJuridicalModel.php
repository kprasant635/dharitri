 ...
 */
class ASTJuridicalModel extends CI_Model{
    ...
    public function getPendingPremiumAST($user_code){
        ...
        $q = "select *,ba.basundhara from petition_basic fmb left join basundhar_application ba on fmb.case_no=ba.dharitree where not_fresh = 'Y' and status = 'P' and mut_type = '01' and co_order_conv_premium = 'Y'"
                ...
                . "and subdiv_code='$subdiv_code' and cir_code='$cir_code' and user_code like 'AS%' order by petition_no ASC";
        $cases = $this->db->query($q);
        return $cases;
    }
    ...
    public function getPendingReportBO($user_code){
        ...
        $year_no=year_no;
        $define_date=define_date;
        $q = "select *,ba.basundhara from petition_basic fmb left join basundhar_application ba on fmb.case_no=ba.dharitree where user_code = '$user_code' and bo_note_yn is null and "
    ...
    public function countPendingReportBO($user_code){
        ...
        $year_no=year_no;
        $define_date=define_date;
        return $this->db->query("select count(*) as c from   petition_basic where user_code = '$user_code' and bo_note_yn is null and "
    ...
    public function countPendingNoticeGeneratedASTMb3($user_code){
        ...
        $year_no=year_no;
        $define_date=define_date;
        return $this->db->query("select count(*) as c from     settlement_basic where ast_notice_hearing_date is not null and ast_notice_serve is null and status = 'W' and ast_notice_generated is null and dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$cir_code'")->row()->c;
    }
    ...
    public function getPendingNoticeGeneratedASTMb3($user_code){
        ...
        $year_no=year_no;
        $define_date=define_date;
        $q = "select settlement_basic.* from     settlement_basic where ast_notice_hearing_date is not null and ast_notice_serve is null and status = 'W' and ast_notice_generated is null and dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$cir_code'";
	...
	$db=  $this->session->userdata('db');
        ...
        return $this->db->query("select count(*) as c from settlement_basic where  status = 'W' and ast_notice_generated='Y' and dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$cir_code'")->row()->c;
    }
    ...
    public function getPendingActionTakenASTMb3($user_code){
        ...
        $year_no=year_no;
        $define_date=define_date;
        $q = "select * from settlement_basic where status = 'W' and ast_notice_generated='Y' and dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$cir_code' order by petition_no ASC";
        $cases = $this->db->query($q);
        // log_message('error',"sq1-------------".$this->db->last_query());
        return $cases;
    }
