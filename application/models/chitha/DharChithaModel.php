...
class DharChithaModel extends CI_Model
{
    ...
    public function getchithaDetailsALL($district_code, $subdivision_code, $circlecode, $mouzacode, $lot_code, $village_code, $dag_no_lower, $dag_no_upper)
    {
        ...
        $district = $this->db->query("
          ...
          cb.dag_local_tax,lcc.land_type,lcc.class_code_cat, cb.rs_data  from chitha_basic AS cb JOIN landclass_code
           AS lcc ON cb.land_class_code = lcc.class_code where dist_code='$district_code' and subdiv_code='$subdivision_code'
               and cir_code='$circlecode' 
           and mouza_pargona_code='$mouzacode' and lot_No='$lot_code' and Vill_townprt_code='$village_code' 
           and CAST(coalesce(cb.dag_no_int, '0') AS numeric)>=$dag_no_lower and 
        ...
        foreach ($outerdata as $chithadetails) {
            ...
            $data1[$dag_no] = array(
                ...
                'rs_data' => isset($chithadetails->rs_data) ? $chithadetails->rs_data : ''
            );
            ...
            $innerquery58 = "SELECT encro_evicted_yn,encro_name,encro_since,encro_land_b,encro_land_k,encro_land_lc,encro_land_g,encro_land_used_for,encro_evic_date,property_type_code FROM chitha_rmk_encro where  dist_code='$district_code' "
                . " and subdiv_code='$subdivision_code' and cir_code='$circlecode' and "
            ...
            foreach ($innerdata58 as $encro) {
                ...
                foreach ($innerdata_encroland as $encro_land) {
                    ...
                    $data1[$dag_no]['encro'][] = array(
                        ...
                        'land_used_by_encro' => $property_type_code ? $encro_land->name_eng : $encro_land->used_for
                    );
                }
            }
            ...
            $innerquery_sro = "SELECT  dag_area_b, dag_area_k, dag_area_lc,dag_area_g,reg_to_name,reg_from_name,name_of_sro,deed_no,date_of_deed, status FROM sro_note where  dist_code='$district_code' "
                . " and subdiv_code='$subdivision_code' and cir_code='$circlecode' and "
                . " mouza_pargona_code='$mouzacode' and  lot_No='$lot_code' and vill_townprt_code='$village_code' and dag_no ='$dag_no' "; //and rmk_type_hist_no='$rmk_type_hist_no' ";
            ...
            //alphaNumeric Dag Change
            // $alphaQuery = $this->db->query("SELECT * FROM deleted_alpha_dag_patta WHERE dist_code=? AND subdiv_code=? AND cir_code=? AND mouza_pargona_code=? AND lot_no=? AND vill_townprt_code=? AND dag_no=? AND (is_approved_dag=1 OR is_approved_patta=1)", [$district_code, $subdivision_code, $circlecode, $mouzacode, $lot_code, $village_code, $dag_no])->row();
            // if(!empty($alphaQuery)) {
            //     $montobyo = '';
            //     if($alphaQuery->change_dag == 1 && $alphaQuery->is_approved_dag == 1) {
            //         $montobyo .= 'AlphaNumeric Dag No changed from ' . $alphaQuery->dag_no_old . ' to ' . $alphaQuery->dag_no . '. ';
            //     }
            //     if($alphaQuery->change_patta == 1 && $alphaQuery->is_approved_patta == 1) {
            //         $montobyo .= 'AlphaNumeric Patta No changed from ' . $alphaQuery->patta_no_old . ' to ' . $alphaQuery->patta_no . '. ';
            //     }
            //     $data1[$dag_no]['alphaDag'] = $montobyo;
    ...
    public function getpattatype($patta_code)
    {
        ...
        $pattaTypeDetails = $this->db->query("Select patta_type from patta_code where type_code='$patta_code'");

        return $pattaTypeDetails->result();
    }
        ...
        foreach ($innerdata26 as $rmkGen) {
            ...
            if ($rmk_type_code == "01") {
                $innerquery27 = " select dag_no,ord_date,ord_no,case_no,ord_passby_desig,lm_code,co_code,ord_type_code,"
                    . " ord_ref_let_no,co_ord_date,new_dag_no,m_dag_area_b,m_dag_area_k,m_dag_area_lc,m_dag_area_g,user_code,operation, noc_no,noc_date "
                    ...
                    if ($ord_type_code == '01') {
                        ...
                        foreach ($innerdata32 as $rmk_conv) {
                            ...
                            $innerquery33 = "select land_area_b as land_area_b,land_area_k as land_area_k,land_area_lc as "
                                . "land_area_lc, land_area_g from chitha_rmk_convorder where dist_code='$district_code' "
                                . " and subdiv_code='$subdivision_code' and cir_code='$circlecode' and "
                                . " mouza_pargona_code='$mouzacode' and  lot_No='$lot_code' and vill_townprt_code='$village_code' "
