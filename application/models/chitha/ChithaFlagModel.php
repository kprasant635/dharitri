...
class ChithaFlagModel extends CI_Model {
	...
	public function getRejectedCountOfVillage($dist_code,$subdiv_code,$cir_code,$mouza_pargona_code, $lot_no,$vill_townprt_code,$status){
		...
		$district = $this->db->query($sql,array($dist_code,$subdiv_code,$cir_code,$mouza_pargona_code,$lot_no,$vill_townprt_code,$status));
		return $district->result();
	}
	...
	public function getAllListedMappingCategory(){
		$sql = "select * from dag_flag_master where flagid not in (1,2) and is_active = 'A'";
