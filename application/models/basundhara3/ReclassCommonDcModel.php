<?php
class ReclassCommonDcModel extends CI_Model
{
    ...
    public function getProposalNameByCaseNo($case)
    {
        ...
        $this->db->where('settlement_proposal_list.service_code', 40);
        $data = $this->db->get();
        return $data->result();
    }
...
//            ->where('patta_no',$appPatta)
    ...
    {
        //  $this->db->select('settlement_dag_details.*,reclass_suite_basic.status');
        //  $this->db->from('settlement_dag_details');
        //  $this->db->join('reclass_suite_basic','reclass_suite_basic.case_no = settlement_dag_details.case_no');
        //  $this->db->where('reclass_suite_basic.status !=','D');
        //  $this->db->where('reclass_suite_basic.status !=','F');
        //  $this->db->where('reclass_suite_basic.dc_proceeding',1);
        ...
        SELECT settlement_dag_details.*, reclass_suite_basic.status
	    ...
	    JOIN (select * from reclass_suite_basic where dist_code='$appDistrict' and subdiv_code='$appSubDiv' and cir_code='$appCircle' and 
	    mouza_pargona_code='$appMouza' and lot_no='$appLot' and vill_townprt_code='$appVillage'
	    and status not in('D','F') and dc_proceeding=1) reclass_suite_basic
	    ON reclass_suite_basic.case_no = settlement_dag_details.case_no
        ");
    ...
    public function getAllDagAreaDetailsByLocationNotSubmit($appDistrict,$appSubDiv,$appCircle,$appMouza,$appLot,$appVillage,$appDag,$appPattaType,$appPatta,$application_no)
    {
        ...
        //  $this->db->where('reclass_suite_basic.status !=','D');
        //  $this->db->where('reclass_suite_basic.status !=','F');
        //  $this->db->where('reclass_suite_basic.case_no !=',$application_no);
        //  $this->db->where('reclass_suite_basic.dc_proceeding',0);
        //  $this->db->where('settlement_dag_details.dist_code',$appDistrict);
        //  $this->db->where('settlement_dag_details.subdiv_code',$appSubDiv);
        //  $this->db->where('settlement_dag_details.cir_code',$appCircle);
        //  $this->db->where('settlement_dag_details.mouza_pargona_code',$appMouza);
	    ...
	    JOIN (select * from reclass_suite_basic where dist_code='$appDistrict' and subdiv_code='$appSubDiv' and cir_code='$appCircle' and 
	    mouza_pargona_code='$appMouza' and lot_no='$appLot' and vill_townprt_code='$appVillage'
	    and status not in('D','F') and dc_proceeding=0) reclass_suite_basic
	    ON reclass_suite_basic.case_no = settlement_dag_details.case_no
        ");
    ...
    public function fetch_sdlac_member_list($dist_code){
        ...
        $sql = $this->db->query("SELECT u.dlc_priority,u.user_code,u.username AS name,u.emailid as email,phone_no as phone,
                                    (SELECT user_desig FROM master_user_designation 
                                      WHERE user_desig_code=u.user_desig_code) AS designation
                                        FROM loginuser_table lg JOIN users u ON u.user_code=lg.user_code 
                                          WHERE lg.user_code LIKE 'SDLC%'
    ...
    public function updateFlag($dist_code,$memberId,$priority){
        ...
        if($sql->num_rows() > 0){
            ...
            $this->db->query("update users set dlc_priority = '$priority' where user_code = '$memberId' and dist_code = '$dist_code'");
    ...
    public function getUsersMpCopyTo($dist_code, $subdiv_code)
    {
        $this->db->select();
    ...
    // get all MP from minutes copy to table
    public function getUsersMpCopyToForSdo($dist_code,$subdiv_code)
    {
        $this->db->select();
        $this->db->where('user_desg', 'MP');
        $this->db->where('dist_code', $dist_code);
    ...
    public function getUsersMlaCopyTo($dist_code, $subdiv_code)
    {
        $this->db->select();
    ...
    // get all users from minutes copy to table
    public function getUsersMlaCopyToForSdo($dist_code,$subdiv_code)
    {
        $this->db->select();
        $this->db->where('user_desg', 'MLA');
        $this->db->where('dist_code', $dist_code);
    ...
    public function getUsersSdlacCopyTo($dist_code, $subdiv_code)
    {
        $this->db->select();
    ...
    // get all SDLAC users from minutes copy to table
    public function getUsersSdlacCopyToForSdo($dist_code,$subdiv_code)
    {
        $this->db->select();
        $this->db->where('user_desg', 'SDLC');
        $this->db->where('dist_code', $dist_code);
    ...
    // update settlement Basic table
    public function updateSettlementBasicDataHardCode($caseNo,$dist_code,$data)
    {
        $this->db->where('case_no',$caseNo);
        $this->db->where('dist_code', $dist_code);
        $this->db->set('date_update', 'NOW()', FALSE);
    ...
    public function getSettlementAppDetailsByCaseNoDistCode($caseNo,$dist_code)
    {
        ...
        $this->db->where('case_no', $caseNo);
        $this->db->where('dist_code', $dist_code);
        $data = $this->db->get('reclass_suite_basic');
        return $data;
    ...
    ($appDistrict,$appSubDiv,$appCircle,$appMouza,$appLot,$appVillage)
    {
        ...
        //     from settlement_dag_details sd INNER JOIN reclass_suite_basic sb
        ...
        $data = $this->db->query("
         ...
         AND mouza_pargona_code='$appMouza' AND lot_no='$appLot' AND vill_townprt_code='$appVillage')
         sd INNER JOIN (select * from reclass_suite_basic where 
         dist_code='$appDistrict' AND subdiv_code='$appSubDiv' AND cir_code='$appCircle'
    ...
    public function countCoRejectedCaseForADC($dist_code,$service_code)
    {
        ...
        $count = $this->db->query("select count(*) as c from reclass_suite_basic where  dist_code='$dist_code'
    ...
    public function getCoRejectedCaseForADC($service_code, $dist_code)
    {
        ...
        $sql = "SELECT vill_townprt_code, dist_code, subdiv_code, cir_code, mouza_pargona_code, lot_no 
                FROM reclass_suite_basic WHERE service_code = $service_code AND pending_officer = 'CO'
...
//        $this->db->where_in('sdlac_prceed_status', [2]);
    ...
    public function getForwardedMeetingListAdc($dist_code,$createdBy)
    {
        $this->db->select('*');
        $this->db->from('proposal_meeting_list');
        $this->db->where('adc_forward_to_dc_status', 1);
        $this->db->where('dist_code', $dist_code);
        $this->db->where('created_by', $createdBy);
        $this->db->where('meeting_type_ins', '40');
        $this->db->where('user_code',$this->session->userdata('user_code'));
    ...
    public function rejectedCaseList($dist_code, $service_code, $rejected_by)
    {
        ...
        return $this->db->count_all_results('reclass_suite_basic');
    }
    ...
    public function locationSelectCoRejectCasesDc($service_code)
    {
        ...
        $sql = "SELECT vill_townprt_code, dist_code, subdiv_code, cir_code, mouza_pargona_code, lot_no FROM 
                reclass_suite_basic WHERE service_code = $service_code AND from_office = 'DC' AND status = 'D' 
                                      ...
                                      WHERE user_desig_code=u.user_desig_code) AS designation
                                        FROM loginuser_table lg JOIN users u ON u.user_code=lg.user_code 
                                          WHERE lg.user_code IN ($allSelectedMember)
                                            AND (lg.dis_enb_option=? OR lg.special_status=?) AND lg.dist_code=? order by dlc_priority asc",
            array('E',1, $dist_code));
