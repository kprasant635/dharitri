<?php
class reclassSuiteADCModel extends CI_Model
{
    ...
    public function updateSettlementBasicData($caseNo,$dist_code,$data)
    {
        ...
        $this->db->update('reclass_suite_basic', $data);
        return $this->db->affected_rows();

    }

    public function getRecBasicDetails($caseNo)
    {
    ...
    public function getSettlementApplicationDetailsByCaseNo($caseNo,$dist_code)
    {
        ...
        $this->db->select('reclass_suite_basic.*,reclass_approval_authority.*');
        $this->db->from('reclass_suite_basic');
        $this->db->join('reclass_approval_authority','reclass_suite_basic.case_no = reclass_approval_authority.case_no');
        $this->db->where('reclass_suite_basic.case_no', $caseNo);
        ...
        $this->db->order_by('reclass_approval_authority.id', 'DESC');
        $data = $this->db->get();
        return $data->row();
    }

    // get application id by case no
    public function getSupportiveNoticeDoc($caseNo)
    {
        return $this->db->select()
            ->where('case_no', $caseNo)
            ->where('doc_flag', '1')
    ...
    public function getAllPendingReclassSuite($dist_code)
    {
        $this->db->select('*');
        $this->db->from('reclass_suite_basic');
    ...
    public function countAllPendingReclassSuite($dist_code)
    {
        return $this->db->select()
     ...
     public function countAllPendingReclassSuiteforNoticePart($dist_code)
    {
        return $this->db->select()
            ->where('service_code', RECLASS_ID)
            ->where('pending_officer', MB_ADD_DEPUTY_COMM)
            ->where('status', 'AN')
            ->where('dist_code', $dist_code)
            ->where('co_partition_enable', 'Y')
            ->where('notice_generated_yn',null)
            ->where('next_date_of_hearing',null)
            ->where('note_action_yn',null)
            ->where("(adc_code = '" . $this->session->userdata('user_code') . "' OR adc_code IS NULL)", null, false)
            ->get('reclass_suite_basic')
            ->num_rows();
    }

     public function countAllPendingReclassSuiteforNoteActionPart($dist_code)
    {
        return $this->db->select()
            ->where('service_code', RECLASS_ID)
            ->where('pending_officer', MB_ADD_DEPUTY_COMM)
            ...
            ->where('dist_code', $dist_code)
            ->where('co_partition_enable', 'Y')
            ->where('notice_generated_yn IS NOT NULL') // Correcting NULL check
            ->where('next_date_of_hearing IS NOT NULL') // Correcting NULL check
            ->where('note_action_yn IS NULL') // Correcting NULL check
            ->where("(adc_code = '" . $this->session->userdata('user_code') . "' OR adc_code IS NULL)", null, false)
            ->get('reclass_suite_basic')
            ->num_rows();
    }
    ...
    public function countMarkAsSDLACSettlementReclass($dist_code)
    {
        return $this->db->select()
            ->where('service_code', RECLASS_ID)
            ->where('pending_officer', MB_ADD_DEPUTY_COMM)
    ...
    // get all pending settlement  cases KHAS
    public function getMarkAsSDLACSettlementReclass($dist_code)
    {
        $this->db->select('*');
        $this->db->from('reclass_suite_basic');
        $this->db->where('service_code', RECLASS_ID);
    ...
    public function getAllUnderConSettlementKhas($dist_code)
    {
        $this->db->select('*');
    ...
    // count all report in send by DC to SDLAC KHAS
    public function countAllAppInReportSendByDcToSdlacKhas($dist_code)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_KHAS_LAND_ID)
            ->where('pending_officer', MB_ADD_DEPUTY_COMM)
    ...
    public function getAllAppInReportSendByDcToSdlacRecls($proposal_no)
    {
        $this->db->select('settlement_proposal_cases.*,
        reclass_suite_basic.dist_code,reclass_suite_basic.subdiv_code,reclass_suite_basic.cir_code');
    ...
    public function getAllProposalSendByDcToSdlacReclass($dist_code)
    {
        $this->db->select('*');
    ...
    public function countAllProposalSendByDcToSdlacReclasss($dist_code)
    {
        return $this->db->select()
            ->where('service_code', RECLASS_ID)
    ...
    public function saveProposalSDLACReclass($data)
    {
        $this->db->set('created_at', 'NOW()', FALSE);
        $this->db->set('updated_at', 'NOW()', FALSE);
        $this->db->set('service_code',RECLASS_ID );
    ...
    public function countAllApproveAppBySdlacKhas($dist_code)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_KHAS_LAND_ID)
    ...
    public function getAllApproveAppBySdlacKhas($dist_code)
    {
        $this->db->select('*');
        $this->db->from('settlement_basic');
        $this->db->where('service_code', SETTLEMENT_KHAS_LAND_ID);
    ...
    // count all rejected list by Dc KHAS
    public function countAllRejectAppByDcKhas($dist_code)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_KHAS_LAND_ID)
            ->where('from_office', MB_ADD_DEPUTY_COMM)
    ...
    public function getAllRejectAppByDcKhas($dist_code)
    {
        $this->db->select('*');
        $this->db->from('settlement_basic');
        $this->db->where('service_code', SETTLEMENT_KHAS_LAND_ID);
    ...
    public function countAllOrderChithaUpdateAppKhas($dist_code)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_KHAS_LAND_ID)
            ->where('from_office', MB_CIRCLE_OFFICER)
    ...
    public function getAllOrderChithaUpdateAppKhas($dist_code)
    {
        $this->db->select('*');
        $this->db->from('settlement_basic');
        $this->db->where('service_code', SETTLEMENT_KHAS_LAND_ID);
    ...
    // count all Reverted By DEPARTMENT case for DC KHAS
    public function countRevertedByDeptApplicationKhas($dist_code)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_KHAS_LAND_ID)
    ...
    // get all Reverted By DEPARTMENT case for DC KHAS
    public function getRevertedByDeptApplicationKhas($dist_code)
    {
        $this->db->select('*');
        $this->db->from('settlement_basic');
        $this->db->where('service_code', SETTLEMENT_KHAS_LAND_ID);
    ...
    // count all Re-Reverted By CO case for DC KHAS
    public function countReRevertedByCoApplicationKhas($dist_code)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_KHAS_LAND_ID)
            ->where('from_office', MB_CIRCLE_OFFICER)
    ...
    // get all Re-Reverted By CO case for DC KHAS
    public function getReRevertedByCoApplicationKhas($dist_code)
    {
        $this->db->select('*');
        $this->db->from('settlement_basic');
        $this->db->where('service_code', SETTLEMENT_KHAS_LAND_ID);
    ...
    // count application id by case no for DC
    public function caseForDcApprovalKhas($caseNo,$dist_code)
    {
        return $this->db->select()
            ->where('case_no', $caseNo)
            ->where('dist_code', $dist_code)
    ...
    public function countAllCasesForFinalVerifyAppKhas($dist_code)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_KHAS_LAND_ID)
            ->where('from_office', MB_ADD_DEPUTY_COMM)
    ...
    public function getAllCasesForFinalVerifyAppKhas($dist_code)
    {
        return $this->db->select()
            ->where('service_code', SETTLEMENT_KHAS_LAND_ID)
            ->where('from_office', MB_ADD_DEPUTY_COMM)
    ...
    // get all proposal list in send by DC to SDLAC KHAS
    public function getSdlacApprovalProposalListKhas($dist_code)
    {
    ...
    public function getSdlacApprovalProposalIndividualKhas($proposal_no,$dist_code)
    {
        $this->db->select('*');
        $this->db->from('settlement_proposal_list');
    ...
    public function getAllPendingReclassSuitePartition($dist_code)
    {
        ...
        $this->db->where('service_code', RECLASS_ID);
        $this->db->where('pending_officer', MB_ADD_DEPUTY_COMM);
        $this->db->where('status', 'AN');
        $this->db->where('co_partition_enable', 'Y');
        $this->db->where('notice_generated_yn',null);
        $this->db->where('next_date_of_hearing',null);
        $this->db->where('note_action_yn',null);
        $this->db->where('dist_code', $dist_code);
        $this->db->where("(adc_code = '" . $this->session->userdata('user_code') . "' OR adc_code IS NULL)", null, false);
        $data = $this->db->get();
        return $data;
    ...
    public function getAllPendingReclassSuitePartitionReNotice($dist_code)
    {
        $this->db->select('*');
        $this->db->from('reclass_suite_basic');
        $this->db->where('service_code', RECLASS_ID);
    ...
    public function getAllPendingReclassSuitePartitionNoteAction($dist_code)
    {
        $this->db->select('*');
        $this->db->from('reclass_suite_basic');
        $this->db->where('service_code', RECLASS_ID);
        ...
        $this->db->where('status', 'AN');
        $this->db->where('co_partition_enable', 'Y');
        $this->db->where('notice_generated_yn IS NOT NULL');
    ...
    // get dag details
    public function getDagDetailsReclass($case_no)
    {
        return $this->db->select()
    ...
    public function getBuyersReclass($case){
    ...
    public function getOwnersReclass($case){
    ...
    public function getOwnersReclassAll($case_no)
    {
        $sql = "SELECT * FROM  reclass_dag_details WHERE case_no='$case_no' and is_partition = 'Y'";
        $data['dag'] = $this->db->query($sql)->result();

        foreach($data['dag'] as $dag_row){

            $sql = "SELECT * FROM  Chitha_dag_pattadar WHERE dist_code = '$dag_row->dist_code' and subdiv_code = '$dag_row->subdiv_code' and cir_code = '$dag_row->cir_code' and mouza_pargona_code = '$dag_row->mouza_pargona_code'"
                . " and lot_no = '$dag_row->lot_no' and vill_townprt_code = '$dag_row->vill_townprt_code' and TRIM(patta_no) = trim('$dag_row->patta_no') and patta_type_code = '$dag_row->patta_type_code' and dag_no='$dag_row->dag_no' and (p_flag ='0' or p_flag is null)";
            $dagpattadar = $this->db->query($sql)->result();

            foreach ($dagpattadar as $p) {
                $sql = "SELECT * FROM  Chitha_pattadar WHERE dist_code = '$dag_row->dist_code' and subdiv_code = '$dag_row->subdiv_code' and cir_code = '$dag_row->cir_code' and mouza_pargona_code = '$dag_row->mouza_pargona_code'"
                        . " and lot_no = '$dag_row->lot_no' and vill_townprt_code = '$dag_row->vill_townprt_code' and TRIM(patta_no) = trim('$dag_row->patta_no') and patta_type_code = '$dag_row->patta_type_code' and pdar_id='$p->pdar_id' ";
                //echo $sql;
    ...
    public function countAllPendingReclassSuiteforFinalOrderDC($dist_code)
    {
        return $this->db->select()
            ->where('service_code', RECLASS_ID)
        ...
        // return $this->db->select()
        //     ->join('reclass_approval_authority', 'reclass_suite_basic.case_no = reclass_approval_authority.case_no')
        //     ->where('service_code', RECLASS_ID)
        //     ->where('pending_officer', MB_DEPUTY_COMM)
        //     ->where('status', 'W')
        //     ->where('dist_code', $dist_code)
        //     ->where('reclass_approval_authority.is_dlc', 'N')
        //     ->where('reclass_approval_authority.approval_by', 'DC')
        //     ->get('reclass_suite_basic')
        //     ->num_rows();
    }

    public function getAllPendingReclassSuiteFinalorderDC($dist_code)
    {
        // $user_code = $this->session->userdata('user_code');
        // $this->db->select('*');
        // $this->db->from('reclass_suite_basic');
        ...
        $this->db->from('reclass_suite_basic');
        $this->db->join('reclass_approval_authority', 'reclass_suite_basic.case_no = reclass_approval_authority.case_no');
        $this->db->where('reclass_suite_basic.service_code', RECLASS_ID);
        $this->db->where('reclass_suite_basic.pending_officer', MB_DEPUTY_COMM);
        $this->db->where('reclass_suite_basic.status', 'W');
        $this->db->where('reclass_suite_basic.dist_code', $dist_code);
        $this->db->where('reclass_approval_authority.is_dlc', 'N');
        $this->db->where('reclass_approval_authority.approval_by', 'DC');
        $this->db->order_by('reclass_approval_authority.id', 'DESC');
    ...
    public function getReclassSuiteAppDetails($caseNo,$dist_code)
    {
        return $this->db->select()
            ->where('case_no', $caseNo)
            ->where('dist_code', $dist_code)
    ...
    // count application id by case no for DC
    public function countSettlementApplicationDetailsByCaseNoDCFinal($caseNo,$dist_code)
    {
        return $this->db->select()
            ->where('case_no', $caseNo)
            ->where('dist_code', $dist_code)
    ...
    // get application id by case no
    public function getSettlementApplicationDetailsByCaseNoFinalDC($caseNo,$dist_code)
    {
     ...
     // update settlement Basic table
    public function updateSettlementBasicDataFromDC($caseNo,$dist_code,$data)
    {
        $this->db->where('case_no',$caseNo);
        $this->db->where('dist_code', $dist_code);
        $this->db->where('pending_officer', MB_DEPUTY_COMM);
