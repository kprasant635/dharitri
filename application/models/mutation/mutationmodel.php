...
class MutationModel extends CI_Model {
    ...
    public function getVillageCodeJSON($distCode, $subdivcode, $circode, $mouzacode, $lotno) {
        ...
        $district = $this->db->query("select distinct loc_name,vill_townprt_code from   location where "
                . "dist_code =?  and "
                . " subdiv_code=? and cir_code=? and mouza_pargona_code=? and "
                . " vill_townprt_code!='00000' and lot_no=? and (nc_btad is null or nc_btad='K')",array($distCode,$subdivcode,$circode,$mouzacode,$lotno));
...
///////////////////////////////// INSERT to Nok TEmp ///////////////////////////
    ...
    public function getSecondPartyPattadarList($dag, $pno, $ptype, $d, $s, $c, $m, $l, $v, $case){
        ...
        AND case_no=? )", array($dag, '1', $d, $s, $c, $m, $l, $v, $pno, $ptype, 
            $d, $s, $c, $m, $l, $v, $case));
        return $pattadar;
    }
    ...
    public function getSecondPartyPattadarListOM($dag, $pno, $ptype, $d, $s, $c, $m, $l, $v, $petitionNo){
        ...
        AND cir_code=? AND mouza_pargona_code=? AND lot_no=? AND vill_townprt_code=? 
        AND petition_no=? )", array($dag, '1', $d, $s, $c, $m, $l, $v, $pno, $ptype, 
    ...
    //added for escalation on 14 nov 2023
    public function getAllDistinctVillageListLotWise($append,$mouza_pargona_code,$lot_no){
        $query="select distinct dist_code,subdiv_code,cir_code,vill_townprt_code,mouza_pargona_code,lot_no from field_mut_basic fmb left join basundhar_application ba on fmb.case_no=ba.dharitree where "
           . " fmb.mut_type='01' and ".$append." and mouza_pargona_code = '$mouza_pargona_code' and lot_no = '$lot_no'";
        return $this->db->query($query)->result();
    }
    ...
    //get pending office partition cases in co end--------SEARCH-13022023
    public function getPendingFieldMutCaseLMend($dist_code,$subdiv_code,$cir_code,$start,$length,$order,$mouza_code,$lot_no,$vill_mouza_code,$vill_lot_no,$village_code,$searchByCol_0)
    {
          $col = 0;
          $dir = "asc";
          if(!empty($order)){
          ...
          $this->db->select('field_mut_basic.*,basundhar_application.basundhara');
          ...
          // $this->db->where('field_mut_basic.is_escalated', 0);
          $this->db->where('field_mut_basic.es_flag', '1');
          $this->db->where('field_mut_basic.order_passed', null);
          $this->db->where('field_mut_basic.date_of_order', null);
          // $this->db->where("(field_mut_basic.status != 'D' or field_mut_basic.status is null)");
          // $this->db->where('field_mut_basic.lm_note_yn', null);
          ...
          if($query->num_rows()>0){
              ...
              // $this->db->where('date(date_entry) >=', $define_date);
              //$this->db->limit($length, $start);
              $data['total_records']= $this->db->count_all_results('field_mut_basic');
              return $data;
          }
      }

          //used in search option in co end----------
    public function getAllDistinctVillageListLotWisePartition($append,$mouza_pargona_code,$lot_no){
        $query="select distinct dist_code,subdiv_code,cir_code,vill_townprt_code,mouza_pargona_code,lot_no from field_mut_basic fmb left join basundhar_application ba on fmb.case_no=ba.dharitree where fmb.is_dispose is null and "
           . " fmb.mut_type='02' and ".$append." and mouza_pargona_code = '$mouza_pargona_code' and lot_no = '$lot_no'";
        return $this->db->query($query)->result();
    }

    //get pending office partition cases in co end--------SEARCH-13022023
    public function getPendingFieldPartitionCaseLMendList($dist_code,$subdiv_code,$cir_code,$start,$length,$order,$mouza_code,$lot_no,$vill_mouza_code,$vill_lot_no,$village_code,$searchByCol_0)
    {
          $col = 0;
          $dir = "asc";
          if(!empty($order)){
      ...
      //used in search option in co end----------
    public function getAllDistinctVillageListLotWiseReclass($append,$mouza_pargona_code,$lot_no){
        $query="select distinct dist_code,subdiv_code,cir_code,vill_townprt_code,mouza_pargona_code,lot_no from t_reclassification recl left join basundhar_application ba on recl.case_no=ba.dharitree where "
           . " recl.lm_code is null and lm_yn is null and lm_date is null  and ".$append." and mouza_pargona_code = '$mouza_pargona_code' and lot_no = '$lot_no'";
        return $this->db->query($query)->result();
    }

    //get pending office partition cases in co end--------SEARCH-13022023
    public function getPendingReclassCaseLMend($dist_code,$subdiv_code,$cir_code,$start,$length,$order,$mouza_code,$lot_no,$vill_mouza_code,$vill_lot_no,$village_code,$searchByCol_0)
    {
          $col = 0;
          $dir = "asc";
          if(!empty($order)){
          ...
          $this->db->select('t_reclassification.*,basundhar_application.basundhara');
          $this->db->join('basundhar_application', 't_reclassification.case_no = basundhar_application.dharitree', 'left');
          $this->db->where('dist_code', $dist_code);
          $this->db->where('subdiv_code', $subdiv_code);
          $this->db->where('cir_code', $cir_code);
          $this->db->where('t_reclassification.lm_yn is null');
          $this->db->where('t_reclassification.lm_date is null');
          $this->db->limit($length, $start);
          ...
          if($query->num_rows()>0){
              ...
              $this->db->where('t_reclassification.lm_yn is null');
              $this->db->where('t_reclassification.lm_date is null');
              $data['total_records']= $this->db->count_all_results('t_reclassification');
              return $data;
          }
      }
    ...
    function pushSroNgdrsApi()
     {
        ...
        $sql = $this->db->query("select * from petition_basic where case_no=?",array($dharitree))->row();
        ...
        else
        {
            ...
            // var_dump(NGDRS_SRO_NOTE."co_query.php");exit;

            $curl_handle = curl_init();
            // curl_setopt($curl_handle, CURLOPT_URL, "http://129.154.247.103/ngdrs/ASMWebService/co_query");
    ...
    public function getVillageCodeJSONChithaFlag($distCode, $subdivcode, $circode, $mouzacode, $lotno) {
        $this->dbswitch();
        $district = $this->db->query("select distinct loc_name,vill_townprt_code from   location where "
                . "dist_code =?  and "
                . " subdiv_code=? and cir_code=? and mouza_pargona_code=? and "
    ...
    public function getPattaNo($dis,$subdiv,$cir,$mza,$lot,$vill) {
        $dtype = $this->db->query("Select dag_no,dag_no_int from   chitha_Basic where "
      . "dist_code='$dis' and subdiv_code='$subdiv' and  cir_code='$cir' and mouza_Pargona_code='$mza' and lot_No='$lot' "
      . "and vill_townprt_code='$vill' and" 
