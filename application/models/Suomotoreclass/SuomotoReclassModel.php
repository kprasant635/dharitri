...
class SuomotoReclassModel extends CI_Model {
//$db=  $this->session->userdata('db');
    ...
    public function getRejectedConversionSecondCases($dsg) {
        ...
        $q = "Select  *,ba.basundhara from petition_basic fmb left join basundhar_application ba on fmb.case_no=ba.dharitree where not_fresh = 'Y' and status = 'R' and mut_type='01' and co_user_code = '$dsg'  and date_entry >= '$define_date'"
                . " and dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$cir_code' order by petition_no ASC";
    ...
    public function getConvertionOrderPassedByDC($user_desig_code) {
        ...
        $year_no = year_no;
        $define_date = define_date;
        $q = "select *, ba.basundhara from petition_basic fmb left join basundhar_application ba on fmb.case_no=ba.dharitree where not_fresh = 'Y' and status = 'W' and mut_type='01' and add_off_desig = '$user_desig_code'  and date_entry >= '$define_date'"
                . " and dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$cir_code' order by petition_no ASC";
        // echo $q;
        $cases = $this->db->query($q);
        return $cases;
    ...
    public function getPendingLandReclassificationProposals() {
        ...
        $q = "select * from suomoto_reclass  where co_yn is null and dc_yn is null and dist_code='$dist_code' and "
    ...
    public function countPendingLandReclassificationDC() {
        ...
        $q = "select count(*) as count from   t_reclassification where co_yn = 'Y' and dc_yn is null and dc_approval is null and "
                . "dist_code='$dist_code' and (status = 'A')";
        return $this->db->query($q)->row()->count;
    }
    ...
    public function getRevertedPending() {
        ...
        $q = "select *,ba.basundhara from t_reclassification tr left join basundhar_application ba on tr.case_no=ba.dharitree  where co_yn = 'Y' and dc_yn is null and dc_approval is not null and dist_code='$dist_code' "
                . "and (status != 'R' OR status!='A' or status!='M' or status='C') ORDER BY proposal_no asc ";
        //echo $q;
        $cases = $this->db->query($q);
        return $cases;
    }
...
//            }
    ...
    $db=  $this->session->userdata('db');
        ...
        $year_no = year_no;
        $define_date = define_date;
        $q = "select *,ba.basundhara from petition_basic fmb left join basundhar_application ba on fmb.case_no=ba.dharitree where not_fresh = 'Y' and (status = 'P' or add_off_desig='DPT') and mut_type='01' and co_user_code = '$dsg'  and date_entry >= '$define_date'"
                . " and dist_code='$dist_code' and bo_note_yn is not null order by petition_no ASC";
        $cases = $this->db->query($q);
        return $cases;
    }
    ...
    public function getPendingConversionFirstCases_dc($dsg) {
        ...
        $q = "select *,ba.basundhara from petition_basic fmb left join basundhar_application ba on fmb.case_no=ba.dharitree  where bo_note_yn is null and bo_notice_gen is null and mut_type='01' and co_user_code = '$dsg' and add_off_desig = '$user_desig_code' and "
                . "date_entry >= '$define_date' and dist_code='$dist_code' and not status='D'  order by petition_no ASC";
    ...
    public function getPendingConversionFirstCases_adc($dsg) {
        ...
        $year_no = year_no;
        $define_date = define_date;
        $q = "select *,ba.basundhara from petition_basic fmb left join basundhar_application ba on fmb.case_no=ba.dharitree  where bo_note_yn is null and bo_notice_gen is null and mut_type='01' and co_user_code = '$dsg' and add_off_desig = '$user_desig_code' "
    ...
    public function countAppConversionSecondCases_adc_dpt($dsg) {
        ...
        $year_no = year_no;
        $define_date = define_date;
        return $this->db->query("select count(*) as c from  petition_basic where dept_note_yn='Y' and not_fresh = 'Y' and status = 'P' and mut_type='01' and co_user_code = '$dsg' and "
                        . "date_entry >= '$define_date' and dist_code='$dist_code' and bo_note_yn is not null")->row()->c;
    }
    ...
    public function getAppConversionSecondCases_adc_dpt($dsg) {
            ...
            // $q = "select *,ba.basundhara from petition_basic fmb left join basundhar_application ba on fmb.case_no=ba.dharitree where (dept_note_yn='Y' or add_off_desig='DPT') and not_fresh = 'Y' and (status = 'P' or status = 'W') and mut_type='01' and co_user_code = '$dsg'  and date_entry >= '$define_date'"
            //         . " and dist_code='$dist_code' and bo_note_yn is not null order by petition_no ASC";
            $q = "select *,ba.basundhara from petition_basic fmb left join basundhar_application ba on fmb.case_no=ba.dharitree where dept_note_yn='Y' and not_fresh = 'Y' and status = 'P' and mut_type='01' and co_user_code = '$dsg'  and date_entry >= '$define_date' and add_off_desig = '$user_desig_code'"
                    . " and dist_code='$dist_code' and bo_note_yn is not null order by petition_no ASC";
            $cases = $this->db->query($q);
            return $cases;
    }
    ...
    //suomoto reclass//
    public function getPendingSuomotoReclassificationADC() {
        $db=  $this->session->userdata('db');
        //$this->db->limit($limit, $start);
        $dist_code = $this->session->userdata('dist_code');
        $subdiv_code = $this->session->userdata('subdiv_code');
        ...
        $q = "select * from suomoto_reclass where co_yn = 'Y' and dc_yn is null  and dc_approval is null and dist_code='$dist_code' "
                . "and (status = 'A') ORDER BY proposal_no asc ";
        //echo $q;
        $cases = $this->db->query($q);
        return $cases;
    ...
    public function getPendingSuomotoReclassificationDC() {
        $db=  $this->session->userdata('db');
        //$this->db->limit($limit, $start);
        $dist_code = $this->session->userdata('dist_code');
        $subdiv_code = $this->session->userdata('subdiv_code');
        ...
        $q = "select * from suomoto_reclass where co_yn = 'Y' and dc_yn is null  and dc_approval is null and dist_code='$dist_code' "
                . "and (status = 'D') and notice_generated_yn is not null and not_fresh is not null ORDER BY proposal_no asc ";
        //echo $q;
        $cases = $this->db->query($q);
        return $cases;
    }
