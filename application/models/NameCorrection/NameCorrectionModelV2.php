...
class NameCorrectionModelV2 extends CI_Model {
    ...
    public function getMiscCases($user_code, $dist_code, $subdiv_code, $cir_code) {
        ...
        $sql = "select count(*) AS cnt from   misc_case_basic where status='1' and lm_note_yn is null and sk_note_yn is null and "
                ...
                . "dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$cir_code' and add_to_officer = '$user_code'";
...
//     }
    ...
    public function getMiscCases1() {
        ...
        $define_date = define_date;
        // status 1 is for circle officers first proceedin for name correction
        $sql = "select count(*) as c from   misc_case_basic where status='1' and lm_note_yn is not null and submission_date >= '$define_date' and fresh_yn='Y' and dist_code='$dist_code' and subdiv_code='$subdiv_code' "
                . "and cir_code='$cir_code' and add_to_officer = '$user_code' and es_flag='1'";
    ...
    public function getMiscCases2($start,$limit,$key=null) {
        ...
        if($key){
            $sql = "select distinct on (misc_case_no) *, ba.basundhara from misc_case_basic mcb left join basundhar_application ba on mcb.misc_case_no=ba.dharitree  where status='1' and lm_note_yn is not null "
                    . "and submission_date >= '$define_date' and fresh_yn='Y' and dist_code='$dist_code' and subdiv_code='$subdiv_code' "
                    . "and cir_code='$cir_code' and add_to_officer = '$user_code' and (misc_case_no like '%$key%' or ba.basundhara like '%$key%' and es_flag='1') limit 5 offset 0 ";
            $result = $this->db->query($sql)->result();
        }else{
            $sql = "select distinct on (misc_case_no) *, ba.basundhara from misc_case_basic mcb left join basundhar_application ba on mcb.misc_case_no=ba.dharitree  where status='1' and lm_note_yn is not null "
                    . "and submission_date >= '$define_date' and fresh_yn='Y' and dist_code='$dist_code' and subdiv_code='$subdiv_code' "
                    . "and cir_code='$cir_code' and add_to_officer = '$user_code' and es_flag='1' limit $start offset $limit ";
            $result = $this->db->query($sql)->result();
        }
        // log_message("error", "ABCD ".json_encode($this->db->last_query()));
        $this->Escalationmodel->getMiscellaneousEscaltionViewFormat($result);        
        return $result;
    }
    ...
    public function getNoticeReGenerateMiscCase1($user_code,$case_no) {
        ...
        $year_no = year_no;
        $define_date = define_date;
        $sql = "select mcb.misc_case_type,mcb.misc_case_no,mcb.submission_date,mcb.misc_case_petition_no,ba.basundhara from   misc_case_basic mcb left join basundhar_application ba on mcb.misc_case_no=ba.dharitree where status not in('l','10') and submission_date >= '$define_date' and "
    ...
    public function getMiscCaseLMEsc($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no) {
        $year_no = year_no;
		$db=  $this->session->userdata('db');
        $define_date = define_date;
        $sql = "select misc_case_type,misc_case_no,submission_date,misc_case_petition_no from   misc_case_basic where lm_note_yn is null "
                ...
                . "submission_date >= '$define_date' and es_flag='1' ORDER BY submission_date DESC";
        $result = $this->db->query($sql);
        return $result->result();
    }
    ...
    public function getMiscCaseLM1Esc($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no) {
        $year_no = year_no;
        $define_date = define_date;
		$db=  $this->session->userdata('db');

        $sql = "select mcb.misc_case_type,mcb.misc_case_no,mcb.submission_date,mcb.misc_case_petition_no,ba.basundhara,mcb.submission_date,mcb.es_flag,mcb.is_escalated,mcb.out_of_esc from   misc_case_basic mcb left join basundhar_application ba on mcb.misc_case_no=ba.dharitree where dist_code='$dist_code' and subdiv_code='$subdiv_code' "
                . "and cir_code='$cir_code' and mouza_pargona_code ='$mouza_pargona_code' and lot_no='$lot_no' and lm_note_yn is null and fresh_yn='Y' and status NOT IN ('F','L') and submission_date >= '$define_date' and es_flag='1' ORDER BY submission_date DESC ";
      ...
      public function getMiscCaseLMRe($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no) {
        ...
        $define_date = define_date;
        $db=  $this->session->userdata('db');
        $sql = "select mcb.misc_case_type,mcb.misc_case_no,mcb.submission_date,mcb.misc_case_petition_no,ba.basundhara from   misc_case_basic mcb left join basundhar_application ba on mcb.misc_case_no=ba.dharitree where dist_code='$dist_code' and subdiv_code='$subdiv_code' "
                . "and cir_code='$cir_code' and mouza_pargona_code ='$mouza_pargona_code' and lot_no='$lot_no' and lm_note_yn is null "
                . "and status='L' and mcb.misc_case_type='06' and fresh_yn='Y' and submission_date >= '$define_date' ORDER BY submission_date DESC ";
  ...
  //   }
    ...
    public function getFinalOrderMisc1($user_code) {
        ...
        $year_no = year_no;
        $define_date = define_date;
        $sql = "select mcb.dist_code,mcb.subdiv_code,mcb.cir_code,mcb.mouza_pargona_code,mcb.lot_no,mcb.vill_townprt_code,mcb.misc_case_type,mcb.misc_case_no,mcb.submission_date,mcb.misc_case_petition_no,ba.basundhara from   misc_case_basic mcb left join basundhar_application ba on mcb.misc_case_no=ba.dharitree  where status='02' and lm_note_yn='Y' and misc_case_type='06' and sk_note_yn='Y' and "
                . "submission_date >= '$define_date' and dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$cir_code' and "
                . "add_to_officer = '$user_code' ORDER BY misc_case_petition_no DESC";
        $result = $this->db->query($sql);
        return $result;
    ...
    public function getFinalOrderMiscDeletion($user_code) {
        ...
        $year_no = year_no;
        $define_date = define_date;
        $sql = "select mcb.dist_code,mcb.subdiv_code,mcb.cir_code,mcb.mouza_pargona_code,mcb.lot_no,mcb.vill_townprt_code,mcb.misc_case_type,mcb.misc_case_no,mcb.submission_date,mcb.misc_case_petition_no,ba.basundhara from   misc_case_basic mcb left join basundhar_application ba on mcb.misc_case_no=ba.dharitree  where status in('10','02')  and operation!='E' and misc_case_type='07' and lm_note_yn='Y' and sk_note_yn='Y' and "
                . "submission_date >= '$define_date' and dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$cir_code' and "
                . "add_to_officer = '$user_code' ORDER BY misc_case_petition_no DESC";
        $result = $this->db->query($sql);
        return $result;
    ...
    public function getCOReport($misc_case_no, $petition_no) {
        // s stands for sk
        $db=  $this->session->userdata('db');
        $sql = "select process_note from   misc_case_process_reports where misc_case_no='$misc_case_no' and misc_case_petition_no = '$petition_no' and operation='c' order by note_no desc";
        $result = $this->db->query($sql);
        return $result->row()->process_note;
    }
...
public function getNoticeGenerateMiscCasePagi($start,$limit,$key=null) {
    ...
    public function getMiscCaseLMforEscalation($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no) 
    {
        $year_no = year_no;
        $db=  $this->session->userdata('db');
        $define_date = define_date;
        $sql = "select misc_case_type,misc_case_no,submission_date,misc_case_petition_no from   misc_case_basic where lm_note_yn is  null  "
                . "and fresh_yn = ? and status NOT IN ('F','L') and dist_code=? and subdiv_code=? and cir_code=? and "
                . "mouza_pargona_code =? and lot_no=? and "
                . "submission_date >= ? and es_flag=? ORDER BY submission_date DESC";
        $result = $this->db->query($sql,array('Y',$dist_code, $subdiv_code, $cir_code, $mouza_pargona_code,$lot_no,$define_date,'1'));
        return $result->result();
    }

    public function getMiscCaseLMforEscalationCO($dist_code, $subdiv_code, $cir_code) 
    {
        $year_no = year_no;
        $db=  $this->session->userdata('db');
        $define_date = define_date;
        $sql = "select misc_case_type,misc_case_no,submission_date,misc_case_petition_no from   misc_case_basic where lm_note_yn is  not null  "
                . "and fresh_yn = ? and status IN ('1') and dist_code=? and subdiv_code=? and cir_code=? and "
                . "submission_date >= ? and es_flag=? ORDER BY submission_date DESC";
        $result = $this->db->query($sql,array('Y',$dist_code, $subdiv_code, $cir_code,$define_date,'1'));
        return $result->result();
    }



    public function getADCpendingcases() {
        $year_no = year_no;
        $db=  $this->session->userdata('db');
        $define_date = define_date;
    ...
    public function getAdcpendingcases1() {
        $year_no = year_no;
        $define_date = define_date;
        $db=  $this->session->userdata('db');
        $sql = "SELECT *,ba.basundhara FROM misc_case_basic AS t1 left join basundhar_application ba on t1.misc_case_no=ba.dharitree WHERE t1.submission_date >= '$define_date' and t1.status='A' and t1.lm_note_yn='Y'  and t1.es_flag='1'"
    ...
    public function getMiscCaseLMRevertEsc($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no) {
        $year_no = year_no;
        $db=  $this->session->userdata('db');
        $define_date = define_date;
        $sql = "select misc_case_type,misc_case_no,submission_date,misc_case_petition_no from   misc_case_basic where status='L' "
    ...
    public function getMiscCaseLMReEsc($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no) {
        $year_no = year_no;
        $define_date = define_date;
        $db=  $this->session->userdata('db');
        $sql = "select mcb.out_of_esc,mcb.es_flag,mcb.is_escalated,mcb.misc_case_type,mcb.misc_case_no,mcb.submission_date,mcb.misc_case_petition_no,ba.basundhara from   misc_case_basic mcb left join basundhar_application ba on mcb.misc_case_no=ba.dharitree where dist_code='$dist_code' and subdiv_code='$subdiv_code' "
                . "and cir_code='$cir_code' and mouza_pargona_code ='$mouza_pargona_code' and lot_no='$lot_no' and lm_note_yn is null "
                . "and status='L' and mcb.misc_case_type='06' and fresh_yn='Y' and submission_date >= '$define_date' and es_flag='1' ORDER BY submission_date DESC ";
    ...
    public function getMiscCaseCORevertEsc($dist_code, $subdiv_code, $cir_code) {
        $year_no = year_no;
        $db=  $this->session->userdata('db');
        $define_date = define_date;
        $sql = "select misc_case_type,misc_case_no,submission_date,misc_case_petition_no from   misc_case_basic where status='C' "
                . "and fresh_yn='Y' and lm_note_yn is not null and sk_note_yn is not null and misc_case_type='06' and dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$cir_code' and "
                . "submission_date >= '$define_date' and es_flag='1' ORDER BY submission_date DESC";
        $result = $this->db->query($sql);
        return $result->result();
    }

    public function getMiscCases2CORvt($start,$limit,$key=null) {
        $dist_code = $this->session->userdata('dist_code');
        $subdiv_code = $this->session->userdata('subdiv_code');
        $cir_code = $this->session->userdata('cir_code');
        $user_code=$this->session->userdata('user_code');
        $define_date = define_date;
        if($key){
            $sql = "select mcb.dist_code,mcb.subdiv_code,mcb.cir_code,mcb.mouza_pargona_code,mcb.lot_no,mcb.vill_townprt_code,mcb.misc_case_type,mcb.misc_case_no,mcb.submission_date,mcb.misc_case_petition_no,ba.basundhara from   misc_case_basic mcb left join basundhar_application ba on mcb.misc_case_no=ba.dharitree  where status='C' and lm_note_yn is not null and sk_note_yn is not null "
                    . "and submission_date >= '$define_date' and fresh_yn='Y' and dist_code='$dist_code' and subdiv_code='$subdiv_code' "
                    . "and cir_code='$cir_code' and add_to_officer = '$user_code' and (misc_case_no like '%$key%' or ba.basundhara like '%$key%' and es_flag='1') limit 5 offset 0 ";
            $result = $this->db->query($sql)->result();
        }else{
