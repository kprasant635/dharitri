...
class NameCorrectionModel extends CI_Model {
    ...
    public function getMiscCases1($user_code) {
        ...
        $define_date = define_date;
        // status 1 is for circle officers first proceedin for name correction
        $sql = "select misc_case_type,misc_case_no,submission_date,misc_case_petition_no from   misc_case_basic where status='1' and lm_note_yn is null and sk_note_yn is null and year_no='$year_no' "
    ...
    public function getNameCorrCaseInfo($misc_case_no, $petition_no) {
		$db=  $this->session->userdata('db');
        $sql = "select * from  misc_case_basic where misc_case_no='$misc_case_no' and misc_case_petition_no = '$petition_no'";
        $result = $this->db->query($sql);
        return $result->row();
    }
    ...
    public function getMiscCaseLM($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no) {
        ...
        $sql = "select misc_case_type,misc_case_no,submission_date,misc_case_petition_no from   misc_case_basic where lm_note_yn is  null and  next_date_of_hearing is not null "
                . "and fresh_yn='Y' and dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$cir_code' and "
    ...
    public function getMiscCaseLM1($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no) {
        ...
        $define_date = define_date;
		$db=  $this->session->userdata('db');
        $sql = "select misc_case_type,misc_case_no,submission_date,misc_case_petition_no from   misc_case_basic where dist_code='$dist_code' and subdiv_code='$subdiv_code' "
                . "and cir_code='$cir_code' and mouza_pargona_code ='$mouza_pargona_code' and lot_no='$lot_no' and lm_note_yn is null "
                . "and next_date_of_hearing is not null and fresh_yn='Y' and submission_date >= '$define_date' ORDER BY submission_date DESC ";
        $result = $this->db->query($sql);
        return $result;
    }
    ...
    public function getMiscCaseSK($user_code) {
        ...
        $sql = "select misc_case_type,misc_case_no,submission_date,misc_case_petition_no from   misc_case_basic where status='02' and lm_note_yn='Y' and sk_note_yn is null and "
    ...
    public function getLMReport($misc_case_no, $petition_no) {
        // l stands from   lm
		$db=  $this->session->userdata('db');
        $sql = "select process_note from   misc_case_process_reports where misc_case_no='$misc_case_no' and operation='l' and misc_case_petition_no = '$petition_no'";
        $result = $this->db->query($sql);
    ...
    public function getSKReport($misc_case_no, $petition_no) {
        // s stands for sk
		$db=  $this->session->userdata('db');
        $sql = "select process_note from   misc_case_process_reports where misc_case_no='$misc_case_no' and misc_case_petition_no = '$petition_no' and operation='s'";
        $result = $this->db->query($sql);
        return $result->row()->process_note;
    }
