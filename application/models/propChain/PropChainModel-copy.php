...
class PropChainModel extends CI_Model
{
    ...
    public function getPropCreateBtn($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no, $vill_townprt_code, $dag_no, $patta_no, $patta_type_code, $bigha, $katha, $lessa, $ganda, $revenue, $local_tax, $ulpin, $old_ulpin)
    {
        ...
        $property_id = $this->utilityclass->generatePropertyId(LOC_TYPE_RURAL, $vill_townprt_code, $patta_no, $dag_no, $ulpin);
        $landclass_code = $this->getLandClassCode($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no, $vill_townprt_code, $patta_no, $dag_no);
        ...
        $property_data = $this->utilityclass->getCreatePropArray($getPattadars, $location_id, $patta_no, $dag_no, $landclass_code, $patta_type_code, $bigha, $katha, $lessa, $ulpin, $old_ulpin, $revenue, $local_tax, $ganda);
        // echo "<pre>";
    ...
    //     $locationId = $this->utilityclass->generateLocId($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no, $vill_townprt_code);

    //     $pattadars = $this->getPattadars($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no, $vill_townprt_code, $patta_no, $dag_no);

    //     $gisCode = $this->utilityclass->generateGisCode($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no, $vill_townprt_code);
    ...
    //         $checkUlpin2 = $this->utilityclass->checkUlpin($gisCode, ASSAM_STATE_CODE, $dag_no);
    ...
    //     $checkPropAndChitha = $this->utilityclass->checkPropAndChitha($user_code, $office_code, $locationId, $patta_no, $dag_no, $pattadars, $dag_area_b, $dag_area_k, $dag_area_lc, $dag_area_g, $data['ulpin']);
    ...
    public function chainChithaUlpinCheckProcess($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no, $vill_townprt_code, $patta_no, $dag_no, $dag_area_b, $dag_area_k, $dag_area_lc, $dag_area_g, $patta_type_code)
    {
        ...
        $get_bhunaksha_area = json_decode($this->utilityclass->getGeoJsonAPI(ASSAM_STATE_CODE, $gisCode, $dag_no));
        // echo "<pre>";
        ...
        $checkPropAndChitha = $this->utilityclass->checkPropAndChitha($user_code, $office_code, $locationId, $patta_no, $dag_no, $pattadars, $dag_area_b, $dag_area_k, $dag_area_lc, $dag_area_g, $data['ulpin'], $bhunaksha_area);
        // echo "<pre>";
    ...
    public function checkGetUlpin($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no, $vill_townprt_code, $patta_no, $dag_no)
    {
        ...
        if ($checkInDb->ulpin == null || $checkInDb->ulpin == '') {
            ...
            $checkApi = $this->utilityclass->checkUlpin($gisCode, ASSAM_STATE_CODE, $dag_no);
    ...
    public function createAssetAndUptMap($pattadar_details, $create_prop_data, $gis_code, $dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no, $vill_townprt_code, $property_id, $location_id, $user_code, $office_code, $property_signature, $property_signer_key, $old_dag_no, $old_patta_no, $old_patta_type_code)
    {
        ...
        $getUlPin = $this->utilityclass->getGeoJsonAPI($state, $gis_code, $old_dag_no);
        ...
        if (!empty($ulpinDetails->features) && $ulpinDetails->features[0]->properties->pniu != null) {
            ...
            if ($insertUlpinDetails) {
                ...
                $result = $this->utilityclass->propChainCreateApi($create_prop_data, $user_code, $office_code, $property_id, $property_signature, $property_signer_key);
                ...
                if ($result->success == 1) {
                    ...
                    $ulpinChainUpdData = $this->utilityclass->getMapUpdateArray($pattadar_details, $create_prop_data['location'], $property_id, $ulpin, $create_prop_data['dagno'], $create_prop_data['pattano'], $create_prop_data['pattatype'], $create_prop_data['landclass'], $create_prop_data['bigha'], $create_prop_data['katha'], $create_prop_data['lessa'], $create_prop_data['ganda'], CERTMNEMONIC_MAP, $property_signature, $property_signer_key, $office_code, $user_code, $ulpin, $old_ulpin, $create_prop_data['revenue'], $create_prop_data['localtax'], $geomType, $coordinates);


                    $ulpinUpdate = $this->utilityclass->propertyChainUpdateApi($ulpinChainUpdData);
    ...
    //     // $property_id_update = $this->utilityclass->generatePropertyId($type, $vill_townprt_code, $old_patta, $old_dag, $ulpin);

    //     // if ($certmnemonic == 'PRT' || $certmnemonic == 'BLP') {
    //     //     $chain_update_data = $this->utilityclass->getUpdateChainArray($pattadar_details, $location_id, $property_id_update, $reference_id, $old_dag, $old_patta, $patta_type_code, $land_class_code, $remaining_b, $remaining_k,  $remaining_lc, $remaining_g, $ce0p-;rtmnemonic, $property_signature, $property_signer_key, $office_code, $user_code, $ulpin, $old_ulpin, $dag_revenue, $dag_local_tax, $new_patta_no, $old_dag, $old_dag_revenue, $old_dag_local_tax, $old_land_class_code, $new_bigha, $new_katha, $new_lessa, $new_ganda);

    //     //     $create_prop_data = $this->utilityclass->getCreatePropArray($pattadar_details, $location_id, $new_patta_no, $old_dag, $land_class_code, $patta_type_code, $new_bigha, $new_katha, $new_lessa, $ulpin, $old_ulpin, $dag_revenue, $dag_local_tax, $new_ganda);
    //     // } elseif ($certmnemonic == 'CONV' || $certmnemonic == 'BLC'  || $certmnemonic == 'APC' || $certmnemonic == 'SETT' || $certmnemonic == 'ALLT') {
    //     //     $chain_update_data = $this->utilityclass->getConvChainArray($pattadar_details, $location_id, $property_id_update, $reference_id, $old_dag, $old_patta, $patta_type_code, $land_class_code, $remaining_b, $remaining_k,  $remaining_lc, $remaining_g, $certmnemonic, $property_signature, $property_signer_key, $office_code, $user_code, $ulpin, $old_ulpin, $dag_revenue, $dag_local_tax, $new_patta_no, $old_dag, $old_dag_revenue, $old_dag_local_tax, $new_patta_type_code);

    //     //     $create_prop_data = $this->utilityclass->getCreatePropArray($pattadar_details, $location_id, $new_patta_no, $old_dag, $land_class_code, $new_patta_type_code, $new_bigha, $new_katha, $new_lessa, $ulpin, $old_ulpin, $dag_revenue, $dag_local_tax, $new_ganda);
    //     // }
    ...
    //         if ($certmnemonic == 'PRT' || $certmnemonic == 'BLP') {
    //             $chain_update_data = $this->utilityclass->getUpdateChainArray($pattadar_details, $location_id, $property_id_update, $reference_id, $old_dag, $old_patta, $patta_type_code, $land_class_code, $remaining_b, $remaining_k, $remaining_lc, $remaining_g, $certmnemonic, $property_signature, $property_signer_key, $office_code, $user_code, $ulpin, $old_ulpin, $dag_revenue, $dag_local_tax, $new_patta_no, $old_dag, $old_dag_revenue, $old_dag_local_tax, $old_land_class_code, $new_bigha, $new_katha, $new_lessa, $new_ganda);
    //         } elseif ($certmnemonic == 'CONV' || $certmnemonic == 'BLC' || $certmnemonic == 'APC' || $certmnemonic == 'SETT' || $certmnemonic == 'ALLT') {
    //             $chain_update_data = $this->utilityclass->getConvChainArray($pattadar_details, $location_id, $property_id_update, $reference_id, $old_dag, $old_patta, $patta_type_code, $land_class_code, $remaining_b, $remaining_k, $remaining_lc, $remaining_g, $certmnemonic, $property_signature, $property_signer_key, $office_code, $user_code, $ulpin, $old_ulpin, $dag_revenue, $dag_local_tax, $new_patta_no, $old_dag, $old_dag_revenue, $old_dag_local_tax, $new_patta_type_code);
    //         }

    //         $update_chain_api = $this->utilityclass->propertyChainUpdateApi($chain_update_data);
    ...
    //             $property_id_create = $this->utilityclass->generatePropertyId($type, $vill_townprt_code, $new_patta_no, $old_dag, $ulpin);
    ...
    public function chainFullDagProcess($chain_data)
    {
        ...
        $property_id_update = $this->utilityclass->generatePropertyId($type, $chain_data->vill_townprt_code, $chain_data->old_patta, $chain_data->old_dag, $chain_data->ulpin);
        ...
        if ($chain_data->certmnemonic == 'PRT' || $chain_data->certmnemonic == 'BLP') {
            ...
            $chain_update_data = $this->utilityclass->getConvChainArrayN((object)$update_params);
        }
    ...
    public function chainPartialDagProcess($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no, $vill_townprt_code, $old_patta, $old_dag, $patta_type_code, $reference_id, $land_class_code, $remaining_b, $remaining_k, $remaining_lc, $remaining_g, $certmnemonic, $property_signature, $property_signer_key, $office_code, $user_code, $ulpin, $old_ulpin, $dag_revenue, $dag_local_tax, $new_patta_no, $new_dag_no, $old_dag_revenue, $old_dag_local_tax, $old_land_class_code, $bigha_new, $katha_new, $lessa_new, $ganda_new, $new_patta_type_code)
    {
        ...
        // if ($certmnemonic == 'PRT' || $certmnemonic == 'BLP')
        //     $chain_update_data = $this->utilityclass->getUpdateChainArray($pattadar_update_details, $location_id, $property_id_update, $reference_id, $old_dag, $old_patta, $patta_type_code, $land_class_code, $remaining_b, $remaining_k,  $remaining_lc, $remaining_g, $certmnemonic, $property_signature, $property_signer_key, $office_code, $user_code, $ulpin, $old_ulpin, $dag_revenue, $dag_local_tax, $new_patta_no, $new_dag_no, $old_dag_revenue, $old_dag_local_tax, $old_land_class_code, $bigha_new, $katha_new, $lessa_new, $ganda_new);
        // elseif ($certmnemonic == 'CONV' || $certmnemonic == 'BLC' ||  $certmnemonic == 'SETT' || $certmnemonic == 'ALLT')
        //     $chain_update_data = $this->utilityclass->getConvChainArray($pattadar_update_details, $location_id, $property_id_update, $reference_id, $old_dag, $old_patta, $patta_type_code, $land_class_code, $remaining_b, $remaining_k,  $remaining_lc, $remaining_g, $certmnemonic, $property_signature, $property_signer_key, $office_code, $user_code, $ulpin, $old_ulpin, $dag_revenue, $dag_local_tax, $new_patta_no, $new_dag_no, $old_dag_revenue, $old_dag_local_tax, $new_patta_type_code);
        // echo "<pre>";
    ...
    public function getFieldMutPartTrans($office_code, $user_code, $propertyId, $prop_data, $certmnemonic, $referenceId)
    {
        ...
        $result = $this->utilityclass->getPropTransData($office_code, $user_code, $propertyId, $prop_data, $certmnemonic, $referenceId);
    ...
    //     // check if ulpin exists and get GeoJson Data
    //     $getUlPin = $this->utilityclass->getGeoJsonAPI($state, $gisCode,  $dag);
    ...
    //         //     //////////////////////////////////////////////////////////////////////////// dsc test /////////////////////////////////////////////////////////
    //         //     $property_id = $this->utilityclass->generatePropertyId(LOC_TYPE_RURAL, $vill_code, $patta, $dag, $ulpin);

    //         //     $create_prop_data = $this->utilityclass->getCreatePropArray($property_data->pid, $property_data->location, $property_data->pattano, $property_data->dagno, $property_data->landclass, $property_data->pattatype, $property_data->bigha, $property_data->katha, $property_data->lessa, $ulpin, $old_ulpin, $property_data->revenue, $property_data->localtax, $property_data->ganda);
    //         //     $result = $this->utilityclass->propChainCreateApi($create_prop_data, $user_code, $office_code, $property_id, $prop_sign, $prop_sign_key, $digital_certificate);
    //         // }
    //         // ////////////////////////////////////////////////////////////////////////////dsc test end///////////////////////////////////////////////////////
    ...
    //                     $ulpinChainUpdData = $this->utilityclass->getMapUpdateArray($property_data->pid, $property_data->location, $property_id, $ulpin, $property_data->dagno, $property_data->pattano, $property_data->pattatype, $property_data->landclass, $property_data->bigha, $property_data->katha,  $property_data->lessa, $property_data->ganda, CERTMNEMONIC_MAP, $prop_sign, $prop_sign_key, $office_code, $user_code, $ulpin, $old_ulpin, $property_data->revenue, $property_data->localtax, $geomType, $coordinates);
            ...
            if ($updateChithaBasic > 0) {
                ...
                if ($insertUlpinDetails) {
                    ...
                    if ($result->success == 1) {
                        ...
                        $ulpinChainUpdData = $this->utilityclass->getMapUpdateArrayN((object)$update_map_params);
        ...
        if (!empty($ulpinDetails->features) && $ulpinDetails->features[0]->properties->pniu != null) {
            ...
            if ($updateChithaBasic > 0) {
                ...
                if ($insertUlpinDetails) {
                    ...
                    // call create asset api
                    // $result = $this->utilityclass->propChainCreateApi($create_prop_data, $user_code, $office_code, $property_id, $prop_sign, $prop_sign_key, $digital_certificate);
                    $create_data = $this->utilityclass->getCreateChainArray($property_data, $property_id, $prop_sign, $prop_sign_key, $digital_certificate);
    ...
    /*public function updateMapData()
    {
        ...
        $ulpinUpdate = $this->utilityclass->propertyChainUpdateApi($map_data);
        // var_dump($ulpinUpdate);
    ...
    public function save_chain_data($json_chain_data, $case_no)
    {
        ...
        if ($json_chain_data == null) {
            ...
            $validate_json = $this->utilityclass->validatePropJson($json_chain_data);
            if ($validate_json) {
                ...
                if ($check_create_or_update[0] == CERTMNEMONIC_ROR) {
                    ...
                    if ($check_create_or_update[0] == CERTMNEMONIC_MAP) {
                        ...
                        $get_prop_chain = $this->utilityclass->fetchPropChainData($dist_code, $subdiv_code, $circle_code, $mouza_code, $lot_no, $vill_code, $prop_data['property_data']['dagno'], $prop_data['property_data']['pattano'], $user_code, $circle_code, $prop_data['property_data']['ulpin']);
    ...
    public function upd_chain_send_data($case_no, $reference_no)
    {
        $this->dbswitch();
        $db_data = array(
            'sending_status' => 'Y',
            'prop_chain_update_time' => date('Y-m-d H:i:s')
        );
    ...
    public function getPropCreateBtnNew($data)
    {
        ...
        $property_id = $this->utilityclass->generatePropertyId(LOC_TYPE_RURAL, $data->vill_townprt_code, $data->patta_no, $data->dag_no, $data->ulpin);

        $property_data = $this->utilityclass->getCreatePropArray($getPattadars, $location_id, $data->patta_no, $data->dag_no, $data->landclass_code, $data->patta_type_code, $data->bigha, $data->katha, $data->lessa, $data->ulpin, $data->old_ulpin, $data->revenue, $data->local_tax, $data->ganda);
    ...
    public function ror_bulk_push()
    {
        ...
        $create_chain_trans = $this->utilityclass->propChainCreateApiBulk($property_records, $user_code, $office_code);
    ...
    public function map_bulk_push()
    {
        ...
        foreach ($update_records as $key => $record) {
            $update_chain_trans = $this->utilityclass->propertyChainUpdateApi($record);
            if ($update_chain_trans->success == 1) {
                ...
                $update = $this->update_prop_chain_sent_data($key, 'F', json_encode($update_chain_trans));
                if ($update <= 0) {
                    echo 'Flag not update in prop_chain_sent_data for reference id: ' . $key . '. Error-code: MAPBULKERROR0002';
                    log_message('error', 'Flag not update in prop_chain_sent_data for reference id: ' . $key . '. Error-code: MAPBULKERROR0002');
                }
