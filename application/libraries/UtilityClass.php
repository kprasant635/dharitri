...
class UtilityClass {
    ...
    public function getUuid($dist_code, $subdiv_code, $circle_code, $mouza_code, $lot_no, $vill_code) {
        $CI = & get_instance();
        $this->dbswitch($dist_code);
        return $this->db->query("SELECT uuid FROM location WHERE dist_code='$dist_code' AND subdiv_code='$subdiv_code' AND cir_code='$circle_code' AND mouza_pargona_code='$mouza_code' AND lot_no='$lot_no' AND vill_townprt_code='$vill_code'")->row()->uuid;
    }
...
//function created for displaying the mouza name
    ...
    public function getAllCircleNameWithDistCode($dist_code) {
        $CI = & get_instance();
        $this->dbswitch($dist_code);
        $cir_code = $this->db->query("select * from location where dist_code =?  and "
            . " subdiv_code!='00' and cir_code!='00' and mouza_pargona_code='00' and "
            . " vill_townprt_code='00000' and lot_no='00'",array($dist_code));
        return $cir_code->result();
    }

    public function getAllCircleNameWithDistCodeSubDiv($dist_code,$subdiv_code) {
        $CI = & get_instance();
        $this->dbswitch($dist_code);
        $cir_code = $this->db->query("select * from location where dist_code =?  and "
            . " subdiv_code=? and cir_code!='00' and mouza_pargona_code='00' and "
            . " vill_townprt_code='00000' and lot_no='00'",array($dist_code,$subdiv_code));
        return $cir_code->result();
    }
...
//explode by - sign
    ...
    public function getSelectedCOName($d, $s, $c, $user = null) {
        $CI = & get_instance();
        //$ds=$CI->session->userdata['db'];
        if($user != null) {
            $query = "select u.username, u.user_code from users u where u.dist_code='$d' and u.subdiv_code='$s' and u.cir_code='$c' and"
            . " u.user_code='$user'";
        }
        else {
            $query = "select u.username, u.user_code from users u, loginuser_table lut where u.dist_code=lut.dist_code and u.subdiv_code=lut.subdiv_code and u.cir_code=lut.cir_code and u.user_code=lut.user_code and lut.dis_enb_option='E' and u.dist_code='$d' and u.subdiv_code='$s' and u.cir_code='$c' and"
            . " u.user_desig_code='CO' order by lut.date_of_creation DESC";
        }
    ...
    function maxpatta($d, $s, $c, $m, $l, $v, $pp) {
        ...
        $q = "select max(distinct(patta_no::bigint)) as patta_no from chitha_basic where  patta_no ~'^[0-9]+$' and dist_code ='$d' and subdiv_code ='$s' and cir_code ='$c' and mouza_pargona_code = '$m' and lot_no ='$l' and vill_townprt_code = '$v' and patta_type_code ='$pp'";
    ...
    function getPattaTypeNo($d, $s, $c, $m, $l, $v, $dag) {
        $CI = & get_instance();
        $this->dbswitch($d);
        $query = "select sum(dag_revenue+dag_local_tax) as sum,dag_revenue,dag_local_tax,dag_area_b,dag_area_k,dag_area_lc,dag_area_g,dag_area_kr,patta_no,patta_type_code,land_class_code from chitha_basic where dist_code= ? and subdiv_code=? and cir_code=? and lot_no=? and mouza_pargona_code=? and "
            . " vill_townprt_code=? and  trim(dag_no)=trim(?) group by dag_revenue,dag_local_tax,dag_area_b,dag_area_k,dag_area_lc,dag_area_g,dag_area_kr,patta_no,patta_type_code,land_class_code  ";
        $sql = $this->db->query($query, array($d, $s, $c, $l, $m, $v, $dag))->row();
        return $sql;
    }
    ...
    function getEkhajanarelationByID($id){
        $CI = & get_instance();
        if($id == null)
        {
            return "NA";
    ...
    public function getCaseNoByApplId($dist_code,$applicationNo)
    {
        ...
        $countCaseNo = $this->db->query("select case_no from settlement_basic where applid ='$applicationNo' and review_flag = 0");
    ...
    public function lmAuthBasic($case_no)
    {
        ...
        if($dataFound->c <=0){
            ...
            $CI->session->set_flashdata('message', "#ERRC300299: Case Already forwarded from LM. case no : ".$case_no);
            redirect(base_url() . "index.php/home");
            return false;
        }
    }
    ...
    public function lmAuthFirstProceeding($case_no)
    {
        ...
        $sqlCheckExist="Select count(*) as c from  settlement_basic where case_no='$case_no' and status in ('Z', 'RA')";
    ...
    public function lmAuthFirstProceedingAp($case_no)
    {
        ...
        $sqlCheckExist="Select count(*) as c from  settlement_basic where case_no='$case_no' and pending_officer ='LM' and status in ('V','RA')";
    ...
    public function countSdlacApproveCasesCircleWise($mId,$distCode,$subCode,$cirCode,$caseStatus)
    {
     ...
     public function checkIp($ip) {
        ...
        // Modified by Abhijit --2024-05-14
        $sorted_ip = '';
        if(count($ip) > 0){
            $sorted_ip = $ip[0];
            if(isset($ip[1])){
        ...
        if(in_array($sorted_ip, json_decode(RESTRICT_IP))){
            return true;
        }
    ...
    public function getSelectedADCName($d, $user)
    {
        $CI = &get_instance();
        //$ds=$CI->session->userdata['db'];
        $query = "select username,user_code from users where dist_code='$d' and subdiv_code='00' and"
            . " user_code='$user'";

        return $CI->db->query($query)->row();
    }

    public function csrf__token(){
        return $this->getCSRFKeys()['csrf_value'];
    }

    public function csrf__key(){
        return $this->getCSRFKeys()['csrf_key'];
    }
    ...
    function mb2ServiceName($service_code){
        ...
        }else if($service_code =='17'){
            $name='Regularization of settlement of PGR VGR LAND';
        }else if($service_code =='45'){
            $name='Digitalized settlement of land to non-individual juridical entities';
        }
        return $name;
    }
    ...
    public function getVillages($d, $s, $c, $m, $l) {
        $CI = & get_instance();
        $this->dbswitch($d);
        return $this->db->query("SELECT vill_townprt_code, loc_name, locname_eng, uuid FROM location WHERE dist_code=? AND subdiv_code=? AND cir_code=? AND mouza_pargona_code=? AND lot_no=? AND vill_townprt_code != '00000'", [$d, $s, $c, $m, $l])->result();
    }

    public function getVillageDags($d, $s, $c, $m, $l, $v) {
        $this->dbswitch($d);

        return $this->db->query("SELECT cb.dag_no FROM chitha_basic cb, patta_code pc WHERE cb.patta_type_code=pc.type_code AND pc.conversion='y' AND cb.dist_code=? AND cb.subdiv_code=? AND cb.cir_code=? AND cb.mouza_pargona_code=? AND cb.lot_no=? AND cb.vill_townprt_code=?", [$d, $s, $c, $m, $l, $v])->result();
    }

    public function getDagDetails($d, $s, $c, $m, $l, $v, $dag) {
        $this->dbswitch($d);

        $data['dag_details'] = $this->db->query("SELECT dag_area_b, dag_area_k, dag_area_lc, dag_area_g, dag_area_kr, patta_no, patta_type_code FROM chitha_basic WHERE dist_code=? AND subdiv_code=? AND cir_code=? AND mouza_pargona_code=? AND lot_no=? AND vill_townprt_code=? AND dag_no=?", [$d, $s, $c, $m, $l, $v, $dag])->row();

        $data['pattadar_details'] = $this->db->query("SELECT cdp.pdar_id, cp.pdar_name, cp.pdar_father FROM chitha_dag_pattadar cdp, chitha_pattadar cp WHERE cdp.dist_code=cp.dist_code AND cdp.subdiv_code=cp.subdiv_code AND cdp.cir_code=cp.cir_code AND cdp.mouza_pargona_code=cp.mouza_pargona_code AND cdp.lot_no=cp.lot_no AND cdp.vill_townprt_code=cp.vill_townprt_code AND cdp.patta_no=cp.patta_no AND cdp.patta_type_code=cp.patta_type_code AND cdp.pdar_id=cp.pdar_id AND cdp.dist_code=? AND cdp.subdiv_code=? AND cdp.cir_code=? AND cdp.mouza_pargona_code=? AND cdp.lot_no=? AND cdp.vill_townprt_code=? AND cdp.dag_no=?", [$d, $s, $c, $m, $l, $v, $dag])->result();

        return $data;
    }

    public function getPattadarDetails($d, $s, $c, $m, $l, $v, $dag, $patta_no, $patta_type_code, $pdar_id) {
        $this->dbswitch($d);

        return $this->db->query("SELECT cdp.pdar_id, cp.pdar_name, cp.pdar_father, cp.pdar_guard_reln, cp.pdar_gender, cp.pdar_mobile, cp.pdar_add1, cp.pdar_add2, cp.pdar_add3, cp.pdar_pan_no FROM chitha_dag_pattadar cdp, chitha_pattadar cp WHERE cdp.dist_code=cp.dist_code AND cdp.subdiv_code=cp.subdiv_code AND cdp.cir_code=cp.cir_code AND cdp.mouza_pargona_code=cp.mouza_pargona_code AND cdp.lot_no=cp.lot_no AND cdp.vill_townprt_code=cp.vill_townprt_code AND cdp.patta_no=cp.patta_no AND cdp.patta_type_code=cp.patta_type_code AND cdp.pdar_id=cp.pdar_id AND cdp.dist_code=? AND cdp.subdiv_code=? AND cdp.cir_code=? AND cdp.mouza_pargona_code=? AND cdp.lot_no=? AND cdp.vill_townprt_code=? AND cdp.dag_no=? AND cdp.patta_no=? AND cdp.patta_type_code=? AND cdp.pdar_id=?", [$d, $s, $c, $m, $l, $v, $dag, $patta_no, $patta_type_code, $pdar_id])->row();
    }

    public function getAstCode($d, $s, $c) {
        $this->dbswitch($d);

        return $this->db->query("SELECT lut.user_code FROM loginuser_table lut, users u WHERE lut.user_code=u.user_code AND lut.dist_code=u.dist_code AND lut.subdiv_code=u.subdiv_code AND lut.cir_code=u.cir_code AND u.user_desig_code='AST' AND lut.dis_enb_option='E' AND lut.priv='mut' AND lut.dist_code=? AND lut.subdiv_code=? AND lut.cir_code=? ORDER BY lut.date_of_creation DESC LIMIT 1", [$d, $s, $c])->row();
    }

    // Added By Abhijit START -- 2024-02-27
    public function getMaritalStatusName($code){
        if($code == 1){
            $maritalStatName = 'Married';
        }elseif($code == 2){
            $maritalStatName = 'Unmarried';
        }elseif($code == 3){
            $maritalStatName = 'Divorced'; 
        }elseif($code == 4){
            $maritalStatName = 'Widow'; 
        }elseif($code == 5){
            $maritalStatName = 'Widower'; 
        }else{
            $maritalStatName = '-';
        }
    
        return $maritalStatName;
    }

    public function getCasteCategoryName($code){
        if($code == 1){
            $catName = 'ST';
        }elseif($code == 2){
            $catName = 'SC';
    ...
    public function getTribeCategoryName($code){
        if($code == 1){
            $tribalCatName = 'Plains Tribals';
        }elseif($code == 2){
            $tribalCatName = 'Hills Tribals';
        }elseif($code == 3){
            $tribalCatName = 'Tea Garden Tribals'; 
        }elseif($code == 4){
            $tribalCatName = 'Santhals'; 
        }elseif($code == 5){
            $tribalCatName = 'Nepali Cultivator graziers'; 
        }elseif($code == 6){
            $tribalCatName = 'Scheduled Class'; 
        }elseif($code == 7){
            $tribalCatName = 'Koch Rajbongshis'; 
        }elseif($code == 8){
            $tribalCatName = 'Indigenous Nath (yogi)'; 
        }elseif($code == 9){
            $tribalCatName = 'Tai Ahom'; 
        }elseif($code == 10){
            $tribalCatName = 'Chutia'; 
        }elseif($code == 11){
            $tribalCatName = 'Gorkha'; 
        }elseif($code == 12){
            $tribalCatName = 'Moran'; 
        }elseif($code == 13){
            $tribalCatName = 'Matak'; 
        }elseif($code == 14){
            $tribalCatName = 'Not Available'; 
        }else{
            $tribalCatName = '-';
        }
        return $tribalCatName;
    }
    public function getlmNameFromCode($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no,$lm_code){
        $CI = &get_instance();
        $query = "select lm_name from lm_code 
        where lm_code='$lm_code' 
        ...
        return $sql->row()->lm_name;
    }

    public function getBasundharaFromCaseNo($case_no) {
    ...
    public function getDisposedDataFromAppID($applicationNo){
            $url = RTPS_API_LINK . "serviceResponse?application_no=" . $applicationNo;
            // Requesting RTPS CURL here
            $output = sendCurlRequest($url);
            $applicationData = json_decode($output);
            return $applicationData->application->date_submission;
    }


    public function getEkhajanaCoPendingCases($dist_code, $subdiv_code, $cir_code){    
        $date = date("Y-m-d", strtotime("yesterday"));     
        $this->dbswitch($dist_code);
		$pending_count_query = $this->db->query("select count(*) c from ekhajana_basic where status=? and dist_code=?
		and subdiv_code=? and cir_code=? and date(modified_at)<=?", array('COM_F', $dist_code, $subdiv_code, 
        $cir_code,$date));
        if($pending_count_query->num_rows() == 0){
            $pending_count = 0;
        }else{
            $pending_count = $pending_count_query->row()->c;
        }
		return $pending_count;
	}


    public function getEkhajanaLmPendingCases($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code,$lot_no){    
        $date = date("Y-m-d", strtotime("yesterday"));     
        $this->dbswitch($dist_code);
		$pending_count_query = $this->db->query("select count(*) c from ekhajana_basic where status=? and dist_code=?
		and subdiv_code=? and cir_code=? and mouza_pargona_code=? and lot_no=? and date(created_at)<=?", array('MOU_F', $dist_code, $subdiv_code, 
        $cir_code,$mouza_pargona_code,$lot_no,$date));
        if($pending_count_query->num_rows() == 0){
            $pending_count = 0;
        }else{
            $pending_count = $pending_count_query->row()->c;
    ...
    // Added by Abhijit -- 2024-09-09
    public function isTheCasePassable($dharitree_case_no, $service_code, $original_inhabitants=null){
        /**
         * $service_code will be FMUT, OMUT, FPART etc
        */
        ...
        $restricted_locations = json_decode(LOCATIONS_TO_PREVENT_THE_CASE_PASS);
        $restricted_districts = json_decode(DISTRICTS_TO_PREVENT_THE_CASE_PASS);
        $location = '';
        $dist = '';
        $should_check = true;
        switch($service_code){
            case 'FMUT':
                $case = $this->db->where('case_no', $dharitree_case_no)->get('field_mut_basic')->row();
                if($case){
                    $location = $case->dist_code .'_' . $case->subdiv_code . '_' . $case->cir_code . '_' . $case->mouza_pargona_code . '_' . $case->lot_no . '_' . $case->vill_townprt_code;
                    $dist = $case->dist_code;

                    $basundhar_app = $this->db->where('dharitree', $dharitree_case_no)->get('basundhar_application')->row();
                    if($basundhar_app){
                        if (strpos($basundhar_app->basundhara, 'MUTD') !== false && $original_inhabitants!=1)
                        {
                            $should_check = true;
                        }else{
                            $should_check = false;
                    ...
                    $response['message'] = 'Case is not found';
                }
            break;
            case 'OMUT':
                $case = $this->db->where('case_no', $dharitree_case_no)->get('petition_basic')->row();
                if($case){
                    $location = $case->dist_code .'_' . $case->subdiv_code . '_' . $case->cir_code . '_' . $case->mouza_pargona_code . '_' . $case->lot_no . '_' . $case->vill_townprt_code;
                    $dist = $case->dist_code;
        ...
        if($should_check){
            if(in_array($dist, $restricted_districts) || in_array($location, $restricted_locations)){
                $response['success'] = false;
                $response['message'] = 'Case ' . $dharitree_case_no . ' cannot be passed as per the notice (ECF. 544102/2024/256)  on dated 23-06-2025 issued by Govt. Of Assam';
            }
        }
        return $response;
    }

    public function getCaseNoByApplIdReclass($dist_code,$applicationNo)
    {

        $CI = & get_instance();
        $this->dbswitch($dist_code);

        $countCaseNo = $this->db->query("select case_no from reclass_suite_basic where applid ='$applicationNo'");
    ...
    public function errIfCaseExist($case_no){
        $CI = & get_instance();

        $sql = $CI->db->query('select * from settlement_basic where case_no = ?', array($case_no));
        if($sql->num_rows() > 0){
            return [
                'responseType' => 1,
                'message' => '#ERR2776: Case number already exists!'
            ];
        }

    }

    public function getCaseNoByApplIdMb3($dist_code,$applicationNo)
    {
    ...
    //newly added for Mb3 Hridayjit (05-11-2024)
    function getPattaTypeNoMB3Api($d, $s, $c, $m, $l, $v, $dag) {
        $CI = & get_instance();
        $this->dbswitch($d);
        $query = "select sum(dag_revenue+dag_local_tax) as sum,dag_revenue,dag_local_tax,dag_area_b,dag_area_k,dag_area_lc,dag_area_g,dag_area_kr,patta_no,patta_type_code,land_class_code from chitha_basic where dist_code= ? and subdiv_code=? and cir_code=? and lot_no=? and mouza_pargona_code=? and "
            . " vill_townprt_code=? and  dag_no_int=? group by dag_revenue,dag_local_tax,dag_area_b,dag_area_k,dag_area_lc,dag_area_g,dag_area_kr,patta_no,patta_type_code,land_class_code  ";
        $sql = $this->db->query($query, array($d, $s, $c, $l, $m, $v, $dag))->row();
        return $sql;
    }

    public function get_relation_from_id($relation) {
        $CI = & get_instance();
        $ds=$CI->session->userdata['dist_code'];
        $this->dbswitch($ds);
        $relation = strtoLower($relation);
    ...
    public function lmAuthFirstProceedingReclass($case_no)
    {
        $CI = & get_instance();

        $sqlCheckExist="Select count(*) as c from  reclass_suite_basic where case_no='$case_no' and status in ('Z', 'RA')";
     ...
     public function getApplidFromCaseNoReclass($case_no) {
        ...
        // $d=$CI->session->userdata['dist_code'];
        // $this->dbswitch($d);
        $applid = $CI->db->query("select applid from reclass_suite_basic where case_no ='$case_no'");
        return $applid->row()->applid;
    }

     public function authCheckCoSkReclass($case_no, $user_desig_code)
    {
        $CI = & get_instance();

        if($user_desig_code == 'CO')
        {
            $query = $CI->db->query('select * from reclass_suite_basic where case_no = ? and (pending_officer = ? OR pending_officer = ?)', array($case_no, $user_desig_code, 'SK'));
        }
        else
        {
            $query = $CI->db->query('select * from reclass_suite_basic sb left join settlement_ap_lmnote sal on sb.case_no=sal.case_no where sal.lm_note =? and sb.status in(\'W\',\'X\') and sb.from_office=? and sb.case_no = ? and (sb.pending_officer = ? OR sb.pending_officer = ?)', array('1', 'LM', $case_no, $user_desig_code,'CO'));
    ...
    public function checkUserAuthForCaseForCoReclass($case_no){
        $CI = & get_instance();
        $CI->load->library('session');
        $d=$CI->session->userdata['dist_code'];
        $this->dbswitch($d);
        ...
        // $sql = "SELECT * FROM settlement_basic WHERE case_no = '$case_no' AND dist_code = '$session_dist_code' AND subdiv_code = '$session_subdiv_code' AND cir_code = '$session_cir_code'";
        // $sql = "SELECT * FROM settlement_basic WHERE case_no = '$case_no' AND dist_code = '$session_dist_code' AND subdiv_code = '$session_subdiv_code' AND cir_code = '$session_cir_code' AND (co_code = '$session_user_code' or co_code is null)";
        $sql = "SELECT * FROM reclass_suite_basic WHERE case_no = '$case_no' AND dist_code = '$session_dist_code' AND subdiv_code = '$session_subdiv_code' AND cir_code = '$session_cir_code'";
    ...
    public function lmAuthBasicReclass($case_no)
    {
        $CI = & get_instance();

        $sqlCheckExist="Select count(*) as c from  reclass_suite_basic where case_no='$case_no' and pending_officer ='LM' and status = 'R'";
    ...
    public function checkIfAlreadyUpdatedByLmReclass($case_no){
        $CI = & get_instance();
        $sql = $CI->db->query("SELECT pending_officer FROM reclass_suite_basic WHERE case_no = '$case_no'");
        if ($sql->num_rows() > 0) {
            if($sql->row()->pending_officer == 'LM'){
                return 'y';
            }
    ...
    // check Auth user ADC
    public function checkUserAuthForCaseForAdcReclass($case_no)
    {
        $CI = & get_instance();
        $CI->load->library('session');
        $ses_dist = $CI->session->userdata['dist_code'];
        ...
        $sql = "SELECT * FROM reclass_suite_basic WHERE case_no = '$case_no' AND dist_code = '$ses_dist'";
        $result = $this->db->query($sql);
     ...
     public function checkUserAuthForCaseForAdcWithRollbackReclass($case_no)
    {
    ...
    public function getCaseNoByApplIdBhoodan($dist_code,$applicationNo)
    {
      $CI = & get_instance();
      $this->dbswitch($dist_code);
      $countCaseNo = $this->db->query("select case_no from settlement_basic where applid =?", array($applicationNo));
    ...
    public function lmAuthFirstProceedingIns($case_no)
    {
        $CI = & get_instance();

        $sqlCheckExist="Select count(*) as c from  settlement_basic where case_no='$case_no' and status in ('W')";
    ...
    public function ins_cat_name($dist_code, $ins_cat) {
        $CI = & get_instance();
        $this->dbswitch($dist_code);
        $category_name = $this->db->query("select abbr AS c from ins_master_category where id = ? ",array($ins_cat));
        return $category_name->row()->c;
    }

    public function ins_cat_name_full($dist_code, $ins_cat) {
        $CI = & get_instance();
        $this->dbswitch($dist_code);
        $category_name = $this->db->query("select category_name AS c from ins_master_category where id = ? ",array($ins_cat));
        return $category_name->row()->c;
    }
    ...
    public function getLmNameGeoTag($d, $s, $c, $m, $l)
    {
        $CI = &get_instance();

        $relation = $CI->db->query("select  STRING_AGG(lm.lm_name,',') as name from lm_code as lm join loginuser_table lg
            on
            lm.dist_code=lg.dist_code and lm.subdiv_code=lg.subdiv_code and lm.cir_code=lg.cir_code
            and lm.mouza_pargona_code=lg.mouza_pargona_code and lm.lot_no=lg.lot_no and lm.lm_code=lg.user_code
           where lm.dist_code='$d' and lm.subdiv_code='$s' and lm.cir_code='$c' 
            and lm.mouza_pargona_code='$m' and lm.lot_no='$l' and lg.dis_enb_option='E' ");
        $name = $relation->row()->name;
        // log_message('error','NAME###$##'.$name);
        return $name;
    }

    public function getlandRevenue25years($dist_code,$subdiv_code,$cir_code,$mouza_pargona_code,$lot_no,$vill_townprt_code, $dag_no) 
    {
        ...
        $sql = $CI->db->query($query, array($dist_code,$subdiv_code,$cir_code,$mouza_pargona_code,$lot_no,$vill_townprt_code, $dag_no));

        $revenueData=$sql->num_rows();
        log_message('error','LAND_REVENUE_UERY========'.$CI->db->last_query());
        if($revenueData > 0)
        {
            $revenueData=$sql->row()->sum;
            // 25 years revenue////////////
            $revenueData = ($revenueData * 25);
            return $revenueData;
        }
        else
        {
    ...
    public function getCaseNoByApplIdAptoPp($dist_code,$applicationNo)
    {
      $CI = & get_instance();
      $this->dbswitch($dist_code);
      $countCaseNo = $this->db->query("select dharitree from basundhar_application where basundhara =?", array($applicationNo));

      if($countCaseNo->num_rows() > 0)
      {
        $getCaseNo = $countCaseNo->row()->dharitree;
      }
      else
      {
        $getCaseNo = '';
    ...
    public function getChithaAreaName($dist_code,$id)
    {
        $CI = & get_instance();
      $this->dbswitch($dist_code);
      $areaDetails = $this->db->query("select * from settlement_premium_area where paid =?", array($id));

      if($areaDetails->num_rows() > 0)
      {
        $area = $areaDetails->row()->area;
      }
      else
      {
        $area = 'No flag found, please flag the dag before proceed';
      }
      return $area ;
    }

     public function getLandClassCodeNew($d) {
        $CI = & get_instance();
        //$ds=$CI->session->userdata['db'];
        $query = "Select name from land_class_groups where id=$d";
        return $CI->db->query($query)->row()->name;
    }


    public function getClassInfo($class_code)
    {
        $CI = & get_instance();
        $sql =  $CI->db->query("SELECT * FROM landclass_code  WHERE class_code = ? and class_code_cat = ?", array($class_code,'01'));
    ...
    public function getPattaNameUpdated($class_code)
    {
        $CI = & get_instance();
        $sql =  $CI->db->query("SELECT name FROM patta_code_groups  WHERE id = ?", array($class_code));
        if ($sql->num_rows()<=0) {
           return  null ;
        } else {
            return $sql->row()->name;
     ...
     public function EnabledMondalNameAll($d, $s, $c, $m, $l) {
        $CI = & get_instance();
        //$ds=$CI->session->userdata['db'];
        $relation = $CI->db->query("SELECT * FROM lm_code as c JOIN loginuser_table as t ON c.dist_code=t.dist_code and "
            . "c.subdiv_code = t.subdiv_code and c.cir_code = t.cir_code and c.mouza_pargona_code=t.mouza_pargona_code and c.lot_no=t.lot_no "
            . "and c.lm_code = t.user_code and t.dist_code='$d' and "
            . "t.subdiv_code='$s' and t.cir_code='$c' and t.mouza_pargona_code = '$m' and t.lot_no = '$l' and t.dis_enb_option='E'");
        return $relation->result();
    }

    public function getM2PendingCount($dist_code, $subdiv_code, $cir_code) {
        $CI = & get_instance();
        $CI->db->select('*');
        $CI->db->from('settlement_basic a');
        $CI->db->where('a.status', 'W');
        $CI->db->where_in('a.pending_officer', ['LM', 'CO']);
        $CI->db->where('a.service_code', 16);
        $CI->db->where('a.dist_code', $dist_code);
        $CI->db->where('a.subdiv_code', $subdiv_code);
        $CI->db->where('a.cir_code', $cir_code);

        $query = $CI->db->get();
        return $query->num_rows(); // Return pending case count
    }
    

    public function getCoBlockFlagForLandClassAndRateUpdate($dist_code, $subdiv_code, $cir_code)
    {
        log_message('error', '[LCU-CHECK-START] DIST=' . $dist_code . ', SUBDIV=' . $subdiv_code . ', CIRCLE=' . $cir_code);

        // Step 1: total villages in chitha_basic
        $sql_cb = "
            ...
            FROM (
                ...
                AND subdiv_code = ?
                AND cir_code = ?
                GROUP BY dist_code, subdiv_code, cir_code, mouza_pargona_code, lot_no, vill_townprt_code
            ) AS v
        ";
        $cb_row = $this->db->query($sql_cb, array($dist_code, $subdiv_code, $cir_code))->row();
        $cb_count = !empty($cb_row) ? (int)$cb_row->total_villages : 0;
        log_message('error', '[LCU-CB-COUNT] DIST=' . $dist_code . ', SUBDIV=' . $subdiv_code . ', CIRCLE=' . $cir_code . ', CB_COUNT=' . $cb_count);

        // Step 2: total villages in land_class_updates
        $sql_lcu = "
            SELECT COUNT(*) AS total_villages
            FROM (
                SELECT 1
                FROM land_class_updates
                WHERE dist_code = ?
                AND subdiv_code = ?
                AND cir_code = ?
                GROUP BY dist_code, subdiv_code, cir_code, mouza_pargona_code, lot_no, vill_townprt_code
        ...
        ";
        $lcu_row = $this->db->query($sql_lcu, array($dist_code, $subdiv_code, $cir_code))->row();
        $lcu_count = !empty($lcu_row) ? (int)$lcu_row->total_villages : 0;
        log_message('error', '[LCU-LCU-COUNT] DIST=' . $dist_code . ', SUBDIV=' . $subdiv_code . ', CIRCLE=' . $cir_code . ', LCU_COUNT=' . $lcu_count);

        // Step 3: compare counts
        $status_flag = ($cb_count > 0 && $cb_count === $lcu_count) ? 'updated' : 'not_updated';
        log_message('error', '[LCU-STATUS] DIST=' . $dist_code . ', SUBDIV=' . $subdiv_code . ', CIRCLE=' . $cir_code . ', STATUS=' . $status_flag);

        log_message('error', '[LCU-CHECK-END] DIST=' . $dist_code . ', SUBDIV=' . $subdiv_code . ', CIRCLE=' . $cir_code);

        return $status_flag;
    }
    function getDisableUserDate($unm,$code,$d, $s, $c, $m, $l){
        $CI = & get_instance();
        $sql="Select update_date_time_timestamp as date from updates.loginuser_table_updates where use_name=? and user_code=? and dist_code=? and subdiv_code=? and cir_code=? and mouza_pargona_code=? and lot_no=? order by date(update_date_time_timestamp) desc limit 1" ;
        $data=$CI->db->query($sql,[$unm,$code,$d, $s, $c, $m, $l]);
        if($data->num_rows() > 0){
            // return $data->row()->date;
            $disable_date = $data->row()->date;
            return !empty($disable_date) ? date('Y-m-d', strtotime($disable_date)) : 'NA';
        }else{
            return 'NA';
        }
    }

     public function lmNameforJB($case_no)
    {
        $sql = $this->db->query("select * from misc_case_process_reports where misc_case_no=? and user_code like 'M%'",array($case_no));
        $data2 = $sql->row();

        $sql = $this->db->query("select * from misc_case_basic where misc_case_no=?",array($case_no));
        $data = $sql->row();

        $q = "Select lm_name,lm_code from   lm_code where dist_code=? and subdiv_code=? and cir_code=? and mouza_pargona_code=? and  lot_no=? and lm_code=?";
        $lm_code = $this->db->query($q,array($data->dist_code,$data->subdiv_code,$data->cir_code,$data->mouza_pargona_code,$data->lot_no,$data2->user_code))->row()->lm_name;
