...
class SettlementApADC extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('SettlementMb/SettlementApADCModel');
...
//********************** COMMON **********************************
    ...
    function randomFileName()
    {
        ...
        if($this->SettlementApADCModel->checkDuplicateFileNameInProposal($new_case_no) != 0)
        {
            $this->randomFileName();
        }
        else
    ...
    public function applicationRevertFromDCToCO()
    {
        ...
        else
        {
            ...
            $caseCount = $this->SettlementApADCModel->countSettlementApplicationDetailsByCaseNo($case_no,$dist_code);
            ...
            else
            {
                ...
                $this->db->trans_begin();
                if($this->SettlementApADCModel->updateSettlementBasicData($case_no,$dist_code,$updateData)== 0)
                {
                    $this->db->trans_rollback();
                    echo json_encode(array(
                        'responseType' => 1,
    ...
    public function applicationRejectedBySdlac()
    {
        ...
        else
        {
            ...
            $caseCount = $this->SettlementApADCModel->countSettlementApplicationDetailsByCaseNoForSDLAC($case_no,$dist_code);
            if($caseCount == 0)
            {
                echo json_encode(array(
                    'responseType' => 3,
            ...
            else
            {
                ...
                else
                {
                    ...
                    $mmnn = $this->SettlementApADCModel->updateSettlementAppDetailsByCaseNoUnderProposal($case_no,$updatePro);
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    $applicants_encroacher = $this->SettlementApModel->getAllApplicantEncroacher($case_no);
                    foreach($applicants_encroacher as $applicant_enc){
                        ...
                        if($enc_check->num_rows() > 0){
                            ...
                            if($sql_land_bank->num_rows() > 0){
                                ...
                                $insertVLBquery = $this->SettlementApADCModel->lbdetailsApproveSettlementCases($lb_details_id,$elb_enc_id,$uuid,$dag_no,$application_no,$lb_approval_rmk);
    ...
    public function applicationApprovedBySdlac()
    {
        ...
        else
        {
            ...
            $caseCount      = $this->SettlementApADCModel->countSettlementApplicationDetailsByCaseNoForSDLAC($case_no,$dist_code);
            $caseCountInPro = $this->SettlementApADCModel->countSettlementAppDetailsByCaseNoUnderProposal($case_no);
            $dag = $this->SettlementCommonDcModel->getSettlementDagCommon($case_no);
            $urbanByLm = $this->SettlementCommonDcModel->getLandFallsUnderUrban($case_no);
            ...
            else
            {
                ...
                else
                {
                    ...
                    $this->SettlementApADCModel->updateSettlementAppDetailsByCaseNoUnderProposal($case_no,$updatePro);
    ...
    public function viewPaymentReceivedAppDetailsByDc()
    {
        ...
        $dist_code = $this->session->userdata('dist_code');
        $this->utilityclass->checkUserAuthForCaseForAdcWithRollback($case_no);
        $caseCount = $this->SettlementApADCModel->countSettlementPaymentReceivedAppDetailsByCaseNo($case_no,$dist_code);
        ...
        {
            $caseDetails = $this->SettlementApADCModel->getSettlementPaymentReceivedAppDetailsByCaseNo($case_no,$dist_code);
            $proceedings = $this->SettlementApADCModel->getSettlementProceeding($case_no);
            $data['caseCount']   = $caseCount;
            $data['caseDetails'] = $caseDetails;
            $data['proceedings'] = $proceedings;

            $data['_view'] = 'settlementView/Adc/Ap/payment_received_app_details_ap';
    ...
    public function updateProposalHearingDateAp()
    {
        ...
        else
        {
            ...
            $allCases      = $this->SettlementApADCModel->getAllAppInReportSendByDcToSdlacAp($proposalNo);
    ...
    public function updateHearingDateGenerateNoticeAp()
    {
        ...
        else
        {
            ...
            $proposalDetails = $this->SettlementApADCModel->getProposalDetailsById($proposalNo,$dist_code);
            if($proposalDetails == '')
            {
                echo json_encode(array(
                    'responseType' => 3,
            ...
            else
            {
                ...
                else
                {
                    ...
                    $this->db->trans_begin();
                    if($this->SettlementApADCModel->updateProposalListById($proposalNo,$updateProposalData)== 0)
                    {
                        $this->db->trans_rollback();
                        echo json_encode(array(
                            'responseType' => 1,
...
//********************** COMMON **********************************
    ...
    // 1st landing page ap ADC
    public function SettlementApLandDc()
    {
        $this->checkingLoginUserAccessSingleUser();
        $dist_code = $this->session->userdata('dist_code');
        $user_code = $this->session->userdata('user_code');
        ...
        // todo
        $firstProceedingCount = $this->SettlementApADCModel->countAllPendingSettlementAp($dist_code);
        $SDLACCommitteeCount  = $this->SettlementCommonDcModel->countAllSDLACCommitteeMember($dist_code,$user_code,$user_desig_code);
        $SDLACNoticeCount     = $this->SettlementApADCModel->countMarkAsSDLACSettlementAp($dist_code);
        $SDLACReportCount     = $this->SettlementApADCModel->countAllProposalSendByDcToSdlacAp($dist_code);
        $caseStatusCount      = 0;
        $sdlacMemberApproval  = $this->SettlementApADCModel->countSdlacStatusList($dist_code);
        $SDLACConsideration   = $this->SettlementApADCModel->countAllUnderConsiderationAppAp($dist_code);

        // $reReportByCOCount    = $this->SettlementApADCModel->countReRevertedByCoApplicationAp($dist_code);
        // $approvedListCount    = $this->SettlementApADCModel->countAllApproveAppBySdlacAp($dist_code);
        // $rejectedListCount    = $this->SettlementApADCModel->countAllRejectAppByDcAp($dist_code);
        // $finalVerifyCaseCount = $this->SettlementApADCModel->countAllCasesForFinalVerifyAppAp($dist_code);
        // $revertedByDepartmentCount = $this->SettlementApADCModel->countRevertedByDeptApplicationAp($dist_code);
        // $chithaUpdateOrderCount    = $this->SettlementApADCModel->countAllOrderChithaUpdateAppAp($dist_code);

        $coRejectedListCount = $this->SettlementCommonDcModel->countCoRejectedCaseForADC($dist_code,SETTLEMENT_AP_TRANSFER_ID);
        $coModificationListCount = $this->SettlementPullModel->countCoModificationRequestCaseForADC($dist_code,SETTLEMENT_AP_TRANSFER_ID);

        $rejctedListCount = $this->SettlementCommonDcModel->rejectedCaseList($dist_code,SETTLEMENT_AP_TRANSFER_ID, MB_ADD_DEPUTY_COMM);
        $revivalListCount = $this->SettlementCommonDcModel->revivalListCount($dist_code,SETTLEMENT_AP_TRANSFER_ID, MB_ADD_DEPUTY_COMM);
        ...
        $data['_view'] = 'settlementView/Adc/Ap/first_landing_page_dc_ap';
        $this->load->view('layouts/main', $data);

    }


    // view all first Proceeding case list ap ADC
    public function viewAllApFirstProceedingDCCaseList()
    {
        $this->checkingLoginUserAccessSingleUser();
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementApADCModel->getAllPendingSettlementAp($dist_code);
        ...
        $data['_view'] = 'settlementView/Adc/Ap/first_proceeding_case_dc_ap';
        $this->load->view('layouts/main', $data);
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows) {
                ...
                $json[] = array(
                    ...
                    '<a class="btn btn-success" href="'.base_url().'index.php/SettlementApADC/getSettlementApApplicationDetails/?case='.$rows->case_no.'">
    ...
    // settlement application details ap ADC
    public function getSettlementApApplicationDetails()
    {
        ...
        $dags = $this->SettlementApModel->getSettlementDag($application_no);
        $lmnotes = $this->SettlementApModel->getSettlementTenantLmNote($application_no);
        ...
        $nominee = $this->SettlementApModel->getAllNomineeDetail($application_no);
        ...
        else
        {
            ...
            $data['newDag']        = $checkAreaDetails['newDag'];


            $caseDetails = $this->SettlementApADCModel->getSettlementApplicationDetailsByCaseNo($case_no,$dist_code);
            $proceedings = $this->SettlementApADCModel->getSettlementProceeding($case_no);
            $data['caseCount']   = $caseCount;
            $data['caseDetails'] = $caseDetails;
            $data['proceedings'] = $proceedings;
            ...
            $data['additional_property'] = $this->SettlementApModel->getAdditionalProperty($application_no);
            ...
            $data['_view'] = 'settlementView/Adc/Ap/settlement_app_details_ap';
            $this->load->view('layouts/main', $data);
        }
    }


    // view all mark as SDLAC ap ADC
    public function viewAllMarkAsSDLACListForDCAp()
    {
        $this->checkingLoginUserAccessSingleUser();
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementApADCModel->getMarkAsSDLACSettlementAp($dist_code);
        ...
        $data['_view'] = 'settlementView/Adc/Ap/mark_as_sdlac_case_dc_ap';
        $this->load->view('layouts/main', $data);
    ...
    public function getAllProposalListSdlacAp()
    {
        $this->checkingLoginUserAccessSingleUser();
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementApADCModel->getAllProposalSendByDcToSdlacAp($dist_code);
        ...
        $data['_view'] = 'settlementView/Adc/Ap/proposal_list_send_to_sdlac_ap';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getAllUnderConSdlacAp()
    {
        $this->checkingLoginUserAccessSingleUser();
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementApADCModel->getAllUnderConSettlementAp($dist_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'settlementView/Adc/Ap/under_consideration_case_dc_ap';
        $this->load->view('layouts/main', $data);
    }


    // get all application send by adc to sdlac for report ap
    public function getAllApplicationInReportSendByDcToSdlacAp()
    {
        $this->checkingLoginUserAccessSingleUser();
        $proposal_no = $this->input->get('case');
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementApADCModel->getAllAppInReportSendByDcToSdlacAp($proposal_no);
        $proposalDetails = $this->SettlementApADCModel->getProposalDetailsById($proposal_no,$dist_code);
        $data['dist_code']        = $dist_code;
        ...
        $data['_view'] = 'settlementView/Adc/Ap/send_to_sdlac_case_dc_ap';
        $this->load->view('layouts/main', $data);

    }


    // generate proposal notice ap ADC
    public function generateNoticeSendAllMarkAppToSDLACByDcAp()
    {
        ...
        else
        {
            ...
            if(!empty($allSelectedList))
            {
                ...
                $this->db->trans_begin();
                if($this->SettlementApADCModel->saveProposalSDLACAp($dataProSave) == 0)
                {
                    $this->db->trans_rollback();
                    echo json_encode(array(
                        'responseType' => 1,
                ...
                else
                {
                    ...
                    foreach ($allSelectedList as $row)
                    {
                        ...
                        if($this->SettlementApADCModel->saveProposalCaseListSDLACAp($saveCaseList) == 0)
                        {
                            //$this->SettlementApADCModel->deleteProposalSDLAC($proposalId);
    ...
    public function getAllApprovedBySDLACListAp()
    {
        $this->checkingLoginUserAccessSingleUser();
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementApADCModel->getAllApproveAppBySdlacAp($dist_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'settlementView/Adc/Ap/approve_list_by_sdlac_ap';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function viewApprovedAppDetailsAp()
    {
        ...
        $caseCount = $this->SettlementApADCModel->countSettlementAppDetailsByCaseNoOnlyView($case_no,$dist_code);
        if($caseCount == 0)
        {
            $this->SettlementApLandDc();
        }
        else
        {
            ...
            $caseDetails = $this->SettlementApADCModel->getSettlementAppDetailsByCaseNoOnlyView($case_no,$dist_code);
            $proceedings = $this->SettlementApADCModel->getSettlementProceeding($case_no);
            $data['caseCount']   = $caseCount;
            $data['caseDetails'] = $caseDetails;
            $data['proceedings'] = $proceedings;
            ...
            $data['_view'] = 'settlementView/Adc/Ap/settlement_app_details_only_view_ap';
            $this->load->view('layouts/main', $data);
        }
    }
    ...
    public function getAllRejectByDcListAp()
    {
        $this->checkingLoginUserAccessSingleUser();
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementApADCModel->getAllRejectAppByDcAp($dist_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'settlementView/Adc/Ap/rejected_list_by_dc_ap';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function viewRejectedAppDetailsAp()
    {
        ...
        else
        {
            ...
            $data['_view'] = 'settlementView/Adc/Ap/settlement_app_details_rejected_only_view_ap';
            $this->load->view('layouts/main', $data);
        }
    }
    ...
    public function getAllOrderChithaUpdateForDcAppAp()
    {
        $this->checkingLoginUserAccessSingleUser();
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementApADCModel->getAllOrderChithaUpdateAppAp($dist_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'settlementView/Adc/Ap/order_chitha_update_list_by_dc_ap';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getAllReReportAppByCOForDcAppAp()
    {
        $this->checkingLoginUserAccessSingleUser();
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementApADCModel->getReRevertedByCoApplicationAp($dist_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'settlementView/Adc/Ap/re_revert_by_co_list_ap';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getAllRevertedAppByDeptForDcAppAp()
    {
        $this->checkingLoginUserAccessSingleUser();
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementApADCModel->getRevertedByDeptApplicationAp($dist_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'settlementView/Adc/Ap/revert_by_dept_list_ap';
        $this->load->view('layouts/main', $data);
    }


    // Application Order for payment generate Dc
    public function applicationPaymentGenerateDcAp()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
        $this->load->library('form_validation');
        $this->form_validation->set_rules('caseNo', 'Case Number', 'trim|required');
        ...
        else
        {
            ...
            $caseCount = $this->SettlementApADCModel->caseForDcApprovalAp($case_no,$dist_code);
            if($caseCount == 0)
            {
                echo json_encode(array(
                    'responseType' => 3,
    ...
    public function getAllCasesForFinalVerifyByDcAp()
    {
        $this->checkingLoginUserAccessSingleUser();
        $dist_code = $this->session->userdata('dist_code');
        $allCase = $this->SettlementApADCModel->getAllCasesForFinalVerifyAppAp($dist_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $allCase->result();
        $data['pendingCaseCount'] = $allCase->num_rows();

        $data['_view'] = 'settlementView/Adc/Ap/final_verify_list_by_dc_ap';
        $this->load->view('layouts/main', $data);
    }
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows)
            {
                ...
                $json[] = array(
                    ...
                    '<a class="btn btn-success" href="'.base_url().'index.php/SettlementApADC/getSettlementApApplicationDetails/?case='.$rows->case_no.'">View Application</a>'
                );
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows) {
                ...
                $json[] = array (
                    ...
                    <a class="btn btn-sm btn-success" href="'.base_url().'index.php/SettlementApADC/getAllApplicationInReportSendByDcToSdlacAp/?case='.$rows->id.'">
                      '.$this->lang->line('process').'</a>'
    ...
    public function checkForSdlacStatus()
    {
        ...
        $processStatus = $this->db->query("SELECT * FROM settlement_proposal_list
                                    WHERE sdlac_prceed_status ".PROPOSAL_SEND_TO_SDLAC." AND dist_code = ? 
    ...
    public function getAllSdlacMemberApprovalProposalListAp()
    {
        $this->checkingLoginUserAccessSingleUser();
        $dist_code   = $this->session->userdata('dist_code');
        $subdiv_code = $this->session->userdata('subdiv_code');
        $pendingCase = $this->SettlementApADCModel->getSdlacApprovalProposalListAp($dist_code);
        ...
        $data['_view'] = 'settlementView/Adc/Ap/sdlac_approval_proposal_list_ap';
        $this->load->view('layouts/main', $data);
    ...
    public function getAllSdlacMemberApprovalProposalListDataAp()
    {
        $this->checkingLoginUserAccessSingleUser();
        $service     = $this->input->post('service');
        $by_case_no  = $this->input->post('case_no');
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows)
            {
                ...
                <div class="text-green" id="list_of_cases_'.$rows->id.'"></div>',
                    ...
                    <a class="rezaButt btn-success" href="'.base_url().'index.php/SettlementApADC/getSdlacMemberApproveProposalViewIndividualAp/?case='.$rows->id.'">
                    '.'Process'.'</a>'
    ...
    public function getSdlacMemberApproveProposalViewIndividualAp()
    {
        $this->checkingLoginUserAccessSingleUser();
        $dist_code   = $this->session->userdata('dist_code');
        $proposal_no = $this->input->get('case');
        $proposalDetails  = $this->SettlementApADCModel->getSdlacApprovalProposalIndividualAp($proposal_no,$dist_code);
        $reportDetails    = $this->SettlementApADCModel->getSdlacMemberReportDetailsAp($proposal_no,$dist_code);
        $getMembersStatus = $this->SettlementApADCModel->getSdlacMemberStatus($dist_code, $proposal_no);
        ...
        $data['_view'] = 'settlementView/Adc/Ap/sdlac_committee_report_details_ap';
        $this->load->view('layouts/main', $data);
    }


    // proposal Forward To Dc For Final Verify By ADC
    public function proposalForwardToDcForFinalVerifyAp()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
        $this->load->library('form_validation');
        ...
        else
        {
            ...
            foreach ($cases as $case)
            {
                ...
                if($case->case_status == 1 || $case->case_status == 0)
                {
                    ...
                    if($this->SettlementApADCModel->updateSettlementAppDetailsByCaseNoUnderProposal($caseNo,$updateCase)== 0)
                    {
                        $this->db->trans_rollback();
                        echo json_encode(array(
                            'responseType' => 1,
                    ...
                    if($this->SettlementApADCModel->updateSettlementBasicData($caseNo,$dist_code,$updateBasicData)== 0)
                    {
                        $this->db->trans_rollback();
                        echo json_encode(array(
                            'responseType' => 1,
                ...
                if($case->case_status == 2)
                {
                    ...
                    else
                    {
                        ...
                        $mmnn = $this->SettlementApADCModel->updateSettlementAppDetailsByCaseNoUnderProposal($caseNo,$updatePro);
            ...
            if($this->SettlementApADCModel->updateProposalListById($proposal_no,$dataUpdate)== 0)
            {
                $this->db->trans_rollback();
                echo json_encode(array(
                    'responseType' => 1,
