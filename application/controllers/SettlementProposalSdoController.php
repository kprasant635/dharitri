...
class SettlementProposalSdoController extends CI_Controller
{
    ...
    public function commonProposalListView()
    {
        ...
        $pendingCase   = $this->SettlementCommonDcModel->getPendingProposalsOfSdo($dist_code,$subdiv_code,$createdBy);
        $data['cases'] = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();
        $data['countPendingCase'] = $this->SettlementCommonDcModel->getPendingProposalsReadyForMeeting($dist_code,MB_SUB_DIV_COMM);
        ...
        $data['_view'] = 'SettlementView/Sdo/common_proposal_list_forward_to_dc';
    ...
    public function listOfProposalsAllServices()
    {
        ...
        $dist_code   = $this->session->userdata('dist_code');
        $subdiv_code = $this->session->userdata('subdiv_code');
        $userCode    = $this->session->userdata('user_desig_code');
        ...
        if(!empty($by_case_no)) { //join table settlement_proposal_cases
            ...
            $this->db->where('settlement_proposal_list.mb_status', 0);
            $this->db->where_in('settlement_proposal_list.service_code',MB_2_SERVICE_CODE_ALLOW_FOR_PROPOSAL);
        ...
        $this->db->where('settlement_proposal_list.meeting_create_status', 1);
        $this->db->where('settlement_proposal_list.sdlac_prceed_status', 2);
        $this->db->where_in('settlement_proposal_list.created_by', MB_SUB_DIV_COMM);
        $this->db->where('settlement_proposal_list.mb_status', 0);
        $this->db->where_in('settlement_proposal_list.service_code',MB_2_SERVICE_CODE_ALLOW_FOR_PROPOSAL);
        ...
        if($query->num_rows() > 0) {
            ...
            $this->db->where('meeting_create_status', 1);
            $this->db->where('sdlac_prceed_status', 2);
            $this->db->where_in('created_by', MB_SUB_DIV_COMM);
            $this->db->where('mb_status', 0);
            $this->db->where_in('service_code',MB_2_SERVICE_CODE_ALLOW_FOR_PROPOSAL);
            ...
            foreach($result as $rows) {
                ...
                <div class="text-green" id="list_of_cases_'.$rows->id.'"></div>',
                    ...
                    <a class="btn btn-xs btn-dark" style=" padding: 7px!important; margin-bottom: 5px!important;" target= "SDLACProposalNotice" href="'.base_url().'index.php/SettlementCommonDc/downloadCasesWithProposalId/?case='.$rows->id.'">
                        <i class="fa fa-download" aria-hidden="true"></i> Excel
                    </a>
                    
                    <a class="btn btn-xs btn-success" style=" padding: 7px!important; margin-bottom: 5px!important;" target= "SDLACProposalNotice" href="'.base_url().'index.php/SettlementProposalSdoController/proposalEditOnSdlacMinutesSdo/?case='.$rows->id.'">
    ...
    public function sendProposalsToDcMinute()
    {
        ...
        {
            if(date('Y-m-d H:i:s',strtotime(MEETING_PROPOSAL_SDLAC_NOTICE_DATE)) < date('Y-m-d H:i:s',strtotime($meeting_date)))
            {
                echo json_encode(array(
                    'responseType' => 1,
                    'response'     => 1,
                    'message'      => 'Maximum Date of meeting '.MEETING_PROPOSAL_SDLAC_NOTICE_DATE_SHOW
                ));
                return;
            }
        }
        ...
        $meetingName = $distEngName.'/SDLAC/'.date("Y").'/'.$generate_meeting_id;

        $proposalDetails = $this->SettlementCommonDcModel->getAllProposalDetailsByProId($proposals);
        ...
        $subDivCode = $this->session->userdata('subdiv_code');
    ...
    public function sendProposalsToDc()
    {
        ...
        if(in_array(SETTLEMENT_PGR_VGR_LAND_ID, array_column($checkVal, 'service_code')))
        {
            ...
            if(count($allSelectedCluster) == 0)
            {
                echo json_encode(array(
                    'response' => 1,
                    'message'  => "#MRCLU001144: Cluster not found ! Kindly contact system administrator",
                ));
                return false;
            }
            ...
            foreach ($allSelectedCluster as $singleCluster)
            {
                ...
                if($this->db->affected_rows() == 0)
                {
                    ...
                    echo json_encode(array(
                        'responseType' => 1,
                        'message'      => '#MRCLU001164: There is some problem, Please try again'
                    ));
                    return;
                }
            }
        ...
        if(count($nomineeCheck) == 0)
        {
            $json = [
                'response' => 1,
                'message'  => '#ERMR00717: There is no SDLAC/CDLAC Member, Case can not be forwarded. Kindly contact 
    ...
    // All recommended / not recommended cases By MRIDU SIR New
    public function generateProposalCases($proposals)
    {
        ...
        $serviceNames[16] = 'Settlement of Khas & Ceiling Surplus Land';
    ...
    // online meeting list pending on SDO end
    public function pendingProposalList()
    {
        ...
        $data['_view'] = 'SettlementView/Sdo/pending_proposal_list_holds_for_48_hrs';
        $this->load->view('layouts/main', $data);
    ...
    public function getPendingMeetingDetailsBySdoAdc()
    {
        ...
        if($pendingMeetingCount == 0)
        {
            ...
            redirect(base_url() . "index.php/SettlementProposalSdoController/pendingProposalList");
        ...
        else
        {
            ...
            $data['_view'] = 'SettlementView/Sdo/pending_meeting_details_holds_for_48_hrs';
            $this->load->view('layouts/main', $data);
        }
    }
    ...
    public function revertMeetingListForSdo()
    {
        $dist_code = $this->session->userdata('dist_code');
        $createdBy = $this->session->userdata('user_desig_code');
        $subdiv_code = $this->session->userdata('subdiv_code');
        ...
        $pendingCase              = $this->SettlementCommonDcModel->getRevertedMeetingListSDO($dist_code,$subdiv_code,$createdBy);
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'SettlementView/Sdo/reverted_meeting_list_sdo';
        $this->load->view('layouts/main', $data);
    }


    // view proposal under reverted meeting
    public function getProposalUnderRevertedMeetingForSdo()
    {
        $meetingId   = $this->input->get('meetingId');
        $dist_code   = $this->session->userdata('dist_code');
        $subdiv_code = $this->session->userdata('subdiv_code');

        $pendingMeeting      = $this->SettlementCommonDcModel->checkRevertedMeetingExistOrNotWithMeetingId($meetingId,$dist_code,$subdiv_code);
        $meetingDetails      = $pendingMeeting->row();
        $pendingMeetingCount = $pendingMeeting->num_rows();
        ...
        if($pendingMeetingCount == 0)
        {
            ...
            redirect(base_url() . "index.php/SettlementProposalSdoController/revertMeetingListForSdo");
            $this->pendingProposalList();
        }
        else
        {
            //list of reverted proposals against meeting id
            $proposalDetail = $this->SettlementCommonDcModel->getRevertedProposalDetailAgainstMeetingId($dist_code,$meetingId);
            $commMembers    = $this->SettlementMbDcModel->getMembersFromUsersWithUserType($dist_code);
            ...
            $data['_view'] = 'SettlementView/Sdo/reverted_proposal_by_dc_list';
            $this->load->view('layouts/main', $data);
        }
    }
    ...
    public function caseListUnderRevertedMeetingForSdo()
    {
        ...
        $proposalDetails = $this->SettlementCommonDcModel->getRevertedProposalDetailsById($proposal_no,$dist_code,$subDiv_code);
        ...
        $data['_view'] = 'settlementView/Sdo/reverted_application_by_proposal';
        $this->load->view('layouts/main', $data);
    ...
    public function sendRevertedProposalsToDcMinute()
    {
        ...
        $proposalDetails = $this->SettlementCommonDcModel->getRevertedProposalDetailAgainstMeetingId($dist_code, $meetingId)->result();
        $proposals = [];
    ...
    public function sendRevertedProposalsToDc()
    {
        ...
        $proposalQ = $this->SettlementCommonDcModel->getRevertedProposalDetailAgainstMeetingId($dist_code, $meetingId);
            ...
            else
            {
                ...
                foreach ($getProposalsList as $prop)
                {
                    ...
                    if($caseCount == 0)
                    {
                        ...
                        log_message('error', '#MR0003134: Cases not found for proposal id '.$proposal_no);
                        echo json_encode(array(
                            'responseType' => 1,
                            'message'      => '#MR0003134: Unable to process for final approval. 
                                      Kindly contact system administrator !!!!',
                        ));
                        return;
                    }
    ...
    /// ***************  Forwarded Meeting by SDO ********************

    // get Forwarded meeting list
    public function forwardedMeetingListForSdo()
    {
        $dist_code = $this->session->userdata('dist_code');
        $createdBy = $this->session->userdata('user_desig_code');
        $subdiv_code = $this->session->userdata('subdiv_code');
        ...
        $pendingCase              = $this->SettlementCommonDcModel->getForwardedMeetingListSDO($dist_code,$subdiv_code,$createdBy);
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'SettlementView/Sdo/forwarded_meeting_list_sdo';
        $this->load->view('layouts/main', $data);
    }


    // get Meeting details by SDO
    public function getForwardedMeetingDetailsByIdSdo()
    {
        ...
        $meeting = $this->SettlementCommonDcModel->getForwardedMeetingDetailByMeetingID(
            $meetingId,$dist_code,$subdiv_code,$createdBy)->row();

        $meetingName = trim($meeting->meeting_name);
        if($meetingName == '')
        {
            $this->session->set_flashdata('error', "Meeting Not Found !");
            redirect(base_url() . "index.php/SettlementProposalSdoController/forwardedMeetingListForSdo");
        }

        //list of proposals against meeting id
        $proposalDetail = $this->SettlementCommonDcModel->getProposalDetailAgainstMeetingIdForSdo($meetingId,
            $dist_code,$subdiv_code,$createdBy)->result();
        ...
        $data['_view'] = 'SettlementView/Sdo/forwarded_proposals_against_meeting_id';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function postAdditionalFileUnderMeetingSdo()
    {
        $this->load->library('form_validation');
    ...
    public function proposalEditOnSdlacMinutesSdo()
    {
        ...
        if($proposalDetails->meeting_create_status != 1)
        {
            ...
            redirect(base_url() . "index.php/SettlementProposalSdoController/commonProposalListView");
        }
        ...
        $data['_view'] = 'settlementView/Sdo/edit_proposal_from_sdlac_minutes_sdo';
        $this->load->view('layouts/main', $data);
    ...
    //// VGR/PGR REVERT CASE FROM SDO END  ///****************

    // get VGR/PGR reverted case page
    public function getVgrPgrRevertedCaseListSdo()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $createdBy   = $this->session->userdata('user_desig_code');
        $subDiv_code = $this->session->userdata('subdiv_code');

        $allCases = $this->SettlementCommonDcModel->getAllVgrPgrRevertedCaseForSdo($dist_code,$subDiv_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $allCases->result();
        $data['pendingCaseCount'] = $allCases->num_rows();

        $data['_view'] = 'settlementView/Sdo/vgr_pgr_reverted_sdo';
        $this->load->view('layouts/main', $data);
    ...
    public function getVgrPgrRevertedCaseDetailsSdo()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $createdBy   = $this->session->userdata('user_desig_code');
        $subDiv_code = $this->session->userdata('subdiv_code');
        ...
        if($caseId == '' || $caseId == NULL)
        {
            ...
            redirect(base_url() . "index.php/SettlementProposalSdoController/getVgrPgrRevertedCaseListSdo");
        }
        ...
        $data['_view'] = 'settlementView/Sdo/vgr_pgr_reverted_details_sdo';
        $this->load->view('layouts/main', $data);
    ...
    public function revertedVgrCaseRevertToCoSdo()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
        $this->load->library('form_validation');
    ...
    public function revertedVgrCaseForwardToDcSdo()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
        $this->load->library('form_validation');
        $this->form_validation->set_rules('remarks', 'Remarks', 'trim|required|min_length[2]|max_length[3000]');
