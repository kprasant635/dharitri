...
class ReclassSuiteCommonDc extends CI_Controller
{
    ...
    public function getSdlacCommitteeCommon()
    {
        ...
        $memberDetails = $this->ReclassCommonDcModel->fetch_sdlac_member_list($dist_code);
        $data['committeeCount'] = $memberDetails->num_rows();
        $data['committeeList']  = $memberDetails->result();
        $data['_view'] = 'reclass_suite/Adc/sdlac_committee_list';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getMinutesApprovedRejectedBySdlac()
    {
        ...
        else
        {
            ...
            if($actionType == 1)
            {
                $minute = 'The case '. $case_no. ' has been approved by SDLAC on '.
            ...
            if($actionType == 2)
            {
                $minute = 'The case '. $case_no. ' has been rejected by SDLAC on '.
                    date("F j, Y") .' for Proposal number ' .$proposalId->proposal_id .
                    ' in favour of '.$applicantsArray .'.';
    ...
    public function downloadCasesWithProposalId()
    {
        ...
        $file_name  = time()."_proposal_reclass.xlsx";

         $data = $this->db->query("SELECT 
                (SELECT locname_eng FROM location WHERE dist_code = t1.dist_code AND subdiv_code = '00') AS district,
                (SELECT locname_eng FROM location WHERE dist_code = t1.dist_code AND subdiv_code = t1.subdiv_code  AND cir_code = t1.cir_code AND mouza_pargona_code = '00') AS circle,
                (SELECT locname_eng FROM location WHERE dist_code = t1.dist_code AND subdiv_code = t1.subdiv_code AND cir_code = t1.cir_code AND mouza_pargona_code = t1.mouza_pargona_code AND lot_no = '00') AS mouza,
                (SELECT locname_eng FROM location WHERE dist_code = t1.dist_code AND subdiv_code = t1.subdiv_code AND cir_code = t1.cir_code AND mouza_pargona_code = t1.mouza_pargona_code AND lot_no = t1.lot_no AND vill_townprt_code = '00000') AS lot,
                (SELECT locname_eng FROM location WHERE dist_code = t1.dist_code AND subdiv_code = t1.subdiv_code AND cir_code = t1.cir_code AND mouza_pargona_code = t1.mouza_pargona_code AND lot_no = t1.lot_no AND vill_townprt_code = t1.vill_townprt_code) AS village,
                t1.uuid,t1.applid AS application_no,t1.case_no,
                t2.applicant_name,t2.guardian_name,t2.Address,t3.dag_no,t3.applied_area,
                t1.reclass_type,t1.occupation_applicant AS Occupation
                FROM (SELECT DISTINCT (case_no), proposal_id  FROM settlement_proposal_cases  WHERE proposal_id = $ProposalNo) t11
                LEFT JOIN (SELECT applid,case_no,dist_code,subdiv_code,cir_code,mouza_pargona_code,lot_no,vill_townprt_code,uuid,occupation_applicant,
                CASE WHEN co_partition_enable = 'Y' THEN 'Partial Reclass'
                ELSE 'Full Reclass' END AS reclass_type
                FROM reclass_suite_basic GROUP BY co_partition_enable,applid,case_no,dist_code,subdiv_code,cir_code,mouza_pargona_code,lot_no,vill_townprt_code,uuid,occupation_applicant) t1 ON t11.case_no = t1.case_no
                LEFT JOIN (SELECT case_no, sa.pdar_name AS applicant_name, sa.pdar_guardian AS guardian_name, sa.pdar_add1 AS Address 
                FROM reclass_applicant sa WHERE is_applicant = '1' GROUP BY case_no, sa.pdar_name,sa.pdar_guardian,sa.pdar_add1) t2 ON t11.case_no = t2.case_no

                LEFT JOIN (SELECT case_no,string_agg(DISTINCT dag_no, ', ') AS dag_no,string_agg(
                DISTINCT dag_no || '-area(' || dag_area_b || 'B-' || dag_area_k || 'K-' || dag_area_lc || 'L)',',') AS applied_area
                FROM reclass_dag_details GROUP BY case_no) t3 ON t11.case_no = t3.case_no")->result_array();
    ...
    public function updateMemberPriority()
    {
        ...
        if(isset($priority) && !empty($priority)){
            for ($i=0; $i < count($sdlc_code); $i++) {
                $updated = $this->ReclassCommonDcModel->updateFlag($dist_code,$sdlc_code[$i],$priority[$i]);
                if($updated == 3){
                    echo json_encode(array(
                        'responseType' => 3,
    ...
    public function addEditCopyTo()
    {
        ...
        $inserted_data = $this->db->query("SELECT * FROM minute_meeting_copy_to WHERE dist_code = ? and subdiv_code = ? and created_by = ?", array($dist_code, $subdiv_code, $user_desig_code));
...
//        $inserted_data = $this->db->query("SELECT * FROM minute_meeting_copy_to WHERE dist_code = ? and subdiv_code = ?", array($dist_code, $subdiv_code));
