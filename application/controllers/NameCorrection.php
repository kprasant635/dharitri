...
class NameCorrection extends CI_Controller {
    ...
    public function ASTStep1() {
        ...
        foreach ($c as $x) {
            ...
            //echo "Select user_code as user_c from    loginuser_table where user_code='".$x->user_code."' and dis_enb_option = 'E' and dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$cir_code'";
            if (count($select) == '1') {
                $convertions[] = array(
                    'co_name' => $x->username,
        ...
        // $this->load->view('../views/NameCorrection/ASTStep1', $data);
        // $this->load->view('../views/footer');


        $data['_view'] = 'NameCorrection/ASTStep1';
...
//                             . "and year_no='$year_no' and misc_case_type='06'")->row()->petition_no;
//          if($petition_no_case==null){
//              $petition_no_case = 1;
//          }
            
//          $financialyeardate = (date('m') < '07') ? date('Y', strtotime('-1 year')) . "-" . date('y') : date('Y') . "-" . date('y', strtotime('+1 year'));
//             $case_no = $cir_dist_name . "/" . $financialyeardate . "/" . $petition_no_case . "/MiNC";
//            echo $case_no;
//            echo $petition_no;
//            exit();
            ...
            $case_no=$case_name.$petition_no."/MiNC";
            ...
            if ($land == null || $land == '') {
                ...
                redirect('NameCorrection/ASTStep1', 'refresh');
            } else {
                $userdata = array(
                    'dist_code' => $dist_code,
                ...
                redirect(base_url() . "index.php/NameCorrection/ASTStep2");
            }
        }
    }
  ...
  //       $this->load->view('../views/header');
        ...
        //var_dump($data);
        // $this->load->view('../views/NameCorrection/ASTStep2', $data);
        // $this->load->view('../views/footer');



        $data['_view'] = 'NameCorrection/ASTStep2';
        $this->load->view('layouts/main',$data);
        ...
        if (isset($_POST['ASTSTEP2Submit'])) {
            ...
            $petition_pdar_name_new = $this->input->post('petition_pdar_name_new');
            $userdata = array(
                'dist_code' => $dist_code,
                'subdiv_code' => $subdiv_code,
                'cir_code' => $cir_code,
                ...
                'petition_pdar_name_new' => $petition_pdar_name_new,
            ...
            $this->session->set_flashdata('message', 'Case No ' . $misc_case_no . ' Successfully Registered for Name Correction!!');
            redirect(base_url() . "index.php/home/index");
        }
    }
  ...
  // //       $this->load->view('../views/header');
  //       $user_code = $this->session->userdata('user_code');
  //       $config['base_url'] = base_url() . '/index.php/NameCorrection/COStep1/';
  //       //original link
  //       $data['Misc'] = $this->NameCorrectionModel->getMiscCases1($user_code);
  //       $cases['TotMisc'] = $this->NameCorrectionModel->getMiscCases2($user_code)->result();

  //       $case_array = array();
  //       foreach ($cases['TotMisc'] as $c) {
  //           $q = $this->db->query("select misc_case_type,misc_case_no,submission_date from    misc_case_basic where status='1' and lm_note_yn is null and sk_note_yn is null "
  //                           . "and notice_generated_yn is null and fresh_yn='Y'")->row();
  //           array_push($case_array, $c);
  //       }
  //       //var_dump($case_array);
  //       $data['MisCases'] = $case_array;
  //       // $this->load->view('../views/NameCorrection/COStep1', $data);
  //       // $this->load->view('../views/footer');

  //       $data['_view'] = 'NameCorrection/COStep1';
  //       $this->load->view('layouts/main',$data);
  //   }
...
public function COStep1() {
    ...
    $allowed = ['CO'];
        ...
        $config['base_url'] = base_url().'index.php/NameCorrection/COStep1';        
        $config['total_rows'] = $this->NameCorrectionModel->getMiscCases1();        
        $config['per_page'] = 10;        
        ...
        $cases['MisCases'] = $this->NameCorrectionModel->getMiscCases2($config["per_page"], $page,$searchKeyword); 

        if(ESCALATION_ENABLE == 1){
            foreach($cases['MisCases'] as $rows) {
               if($rows->es_flag == '1'){
        ...
        $cases['_view'] = 'NameCorrection/COStep1';
  ...
  //       $this->load->view('../views/header');
        ...
        // $this->load->view('../views/NameCorrection/COStep2', $data);
        // $this->load->view('../views/footer');
        ...
        $data['_view'] = 'NameCorrection/COStep2';
        $this->load->view('layouts/main',$data);
    }
    ...
    public function COStep2_save() {
        ...
        if(!isset($_POST['application_no']) || !isset($_POST['misc_case_no']) || !isset($_POST['misc_case_petition_no']) || !isset($_POST['next_date_of_hearing']) || !isset($_POST['next_date_time']) || !isset($_POST['p1']) || !isset($_POST['p2']) || $_POST['application_no']=='' || $_POST['misc_case_no']=='' || $_POST['misc_case_petition_no']=='' || $_POST['next_date_of_hearing']=='' || $_POST['next_date_time']=='' || $_POST['p1']=='' || $_POST['p2']==''){
            //ERRNMECORRCO0001
            log_message('error', 'Improper Input. Error: ERRNMECORRCO0001');
            $this->session->set_flashdata('message', "Improper Input. Error: ERRNMECORRCO0001");
            redirect(base_url('index.php/NameCorrection/COStep1'));
        }
        ...
        if($validquery['status']=='n') {
            //ERRNMECORRCO0009
            log_message('error', $validquery['messages'] .'Error: ERRNMECORRCO0009');
            $this->session->set_flashdata('message', 'Input Parameter has malicious characters. Error: ERRNMECORRCO0009');
        ...
        if(!empty($validAppNo)) {
            //ERRNMECORRCO0002
            log_message('error', 'Application No. cant have special characters. Error: ERRNMECORRCO0002');
            $this->session->set_flashdata('message', "Application No. cant have special characters. Error: ERRNMECORRCO0002");
            redirect(base_url('index.php/NameCorrection/COStep1'));
        }
        // var_dump($validCaseNo, $_POST['misc_case_no']);
        // die();
        if(!empty($validCaseNo)) {
            //ERRNMECORRCO0003
            log_message('error', 'Case No. cant have special characters. Error: ERRNMECORRCO0003');
            $this->session->set_flashdata('message', "Case No. cant have special characters. Error: ERRNMECORRCO0003");
            redirect(base_url('index.php/NameCorrection/COStep1'));
        }
       
        if(!preg_match('/^[0-9]*$/', $_POST['misc_case_petition_no'])) {
            //ERRNMECORRCO0004
            log_message('error', 'Case Petition No. must be numerical. Error: ERRNMECORRCO0004');
            $this->session->set_flashdata('message', "Case Petition No. must be numerical. Error: ERRNMECORRCO0004");
            redirect(base_url('index.php/NameCorrection/COStep1'));
        }
        if(!preg_match('/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/', $_POST['next_date_of_hearing']) && !preg_match('/^[0-9]{2}-[0-9]{2}-[0-9]{4}$/', $_POST['next_date_of_hearing'])) {
            //ERRNMECORRCO0005
            log_message('error', 'Input not in date format. Error: ERRNMECORRCO0005');
            $this->session->set_flashdata('message', "Input not in date format. Error: ERRNMECORRCO0005");
            redirect(base_url('index.php/NameCorrection/COStep2?misc_case_no='.$_POST['misc_case_no'].'&petition_no='. $_POST['misc_case_petition_no']));
        }
        if(!preg_match('/^[0-9]{2}:[0-9]{2}$/', $_POST['next_date_time']) && !preg_match('/^[0-9]{2}:[0-9]{2}:[0-9]{2}$/', $_POST['next_date_time'])) {
            //ERRNMECORRCO0006
            log_message('error', 'Input not in time format. Error: ERRNMECORRCO0006');
            $this->session->set_flashdata('message', "Input not in time format. Error: ERRNMECORRCO0006");
            redirect(base_url('index.php/NameCorrection/COStep2?misc_case_no='.$_POST['misc_case_no'].'&petition_no='. $_POST['misc_case_petition_no']));
        }
        ...
        if($response['status']=='n') {
            //ERRNMECORRCO0007
            log_message('error', $response['messages'] .' Error: ERRNMECORRCO0007');
            $this->session->set_flashdata('message', $response['messages'] ." Error: ERRNMECORRCO0007");
            redirect(base_url('index.php/home'));
        }
        ...
        //     //ERRNMECORRCO0007
        //     log_message('error', 'User not logged in. Error: ERRNMECORRCO0007');
        //     $this->session->set_flashdata('message', "User not logged in. Error: ERRNMECORRCO0007");
        //     redirect(base_url('index.php/home'));
        // }
        ...
        //     //ERRNMECORRCO0008
        //     log_message('error', 'User not authorized. Error: ERRNMECORRCO0008');
        //     $this->session->set_flashdata('message', "User not authorized. Error: ERRNMECORRCO0008");
        //     redirect(base_url('index.php/home'));
        // }
       ...
       if($proInsert==false || $proInsert===false)
        ...
        $this->db->query($updateSqlFirstParty);
         ...
         ///////////////////////////////////
        $penUser='LM';
        $rmrk="CO submitted his report successfully";
        ...
        if($application_no)
        {
            ...
            $this->session->set_flashdata('message', "Name Correction Order Has Passed  !!  Now LM and SK Verify Applicant Details with case no $misc_case_no ");
            //////////////////////////////////
            // $data=array(
  ...
  //       $this->load->view('../views/header');
        ...
        $misc_case_no = $this->input->get('misc_case_no');
        $data['miscCaseInfo'] = $this->NameCorrectionModel->getNameCorrCaseInfo($misc_case_no);
        ...
        $sql1 = "select patta_no from misc_case_basic where misc_case_no='$misc_case_no'";
        $result1 = $this->db->query($sql1);
        $patta_no = $result1->row()->patta_no;

        $data['SupportDoc'] = $this->NameCorrectionModel->getSupportedDoc($supported_doc_code);
        $data['pattaType'] = $this->APCancellationModel->getPattaName($patta_type_code);
        $data['Petitioner'] = $this->NameCorrectionModel->getPetitionerInfo($misc_case_no, $patta_no);
        //var_dump($data['Petitioner']);
        //load the MisModel
        $this->load->model('misreport/MisModel');
        $districtdata = $this->MisModel->getDistrictName($dist_code);
        ...
        //merge all the data
        $data['namedata'] = array_merge($districtdata, $subdivdata, $circledata, $mouzadata, $lotnodata, $villagedata);
        // $this->load->view('../views/NameCorrection/ViewNCorrPetition', $data);
        // $this->load->view('../views/footer');

        $data['_view'] = 'NameCorrection/ViewNCorrPetition';
        $this->load->view('layouts/main',$data);
    }
  ...
  // //       $this->load->view('../views/header');
  //       $user_code = $this->session->userdata('user_code');
  //       $config['base_url'] = base_url() . '/index.php/NameCorrection/ASTNoticeGenerate/';
  //       $data['countMiscCase'] = $this->NameCorrectionModel->getNoticeGenerateMiscCase($user_code);
  //       $cases['TotMisc'] = $this->NameCorrectionModel->getNoticeGenerateMiscCase1($user_code)->result();
  //       $case_array = array();
  //       foreach ($cases['TotMisc'] as $c) {
  //           $q = $this->db->query("select misc_case_type,misc_case_no,submission_date from    misc_case_basic where lm_note_yn is null and sk_note_yn is null and "
  //                           . " notice_generated_yn is null")->row();
  //           array_push($case_array, $c);
  //       }
  //       $data['MisCases'] = $case_array;
  //       //var_dump($data);
  //       //$counts['NoticeGenerate'] = $this->NameCancellationModel->getNoticeGenerateMiscCase();
  //       // $this->load->view('../views/NameCorrection/ASTNoticeGenerate', $data);
  //       // $this->load->view('../views/footer');
  //       $data['_view'] = 'NameCorrection/ASTNoticeGenerate';
  //       $this->load->view('layouts/main',$data);
  //   }
...
////pagination////
      ...
      public function ASTNoticeGenerate() {
        ...
        $cases['searchKeyword'] = $this->session->userdata('searchKeyword');
        $config['base_url'] = base_url() . 'index.php/NameCorrection/ASTNoticeGenerate/';
        $config['total_rows'] = $this->NameCorrectionModel->count_getNoticeGenerateMiscCase();        
        $config['per_page'] = 50;        
        $config['uri_segment'] = 3;        
        $config['full_tag_open'] = '<ul class="pagination">';        
        $config['full_tag_close'] = '</ul>';        
        ...
        $this->pagination->initialize($config);        
        $cases['links'] = $this->pagination->create_links();        
        $cases['MisCases'] = $this->NameCorrectionModel->getNoticeGenerateMiscCasePagi($config["per_page"], $page,$searchKeyword); 

        if(ESCALATION_ENABLE == 1){
            foreach($cases['MisCases'] as $rows) {
                if($rows->es_flag == 1){
                    $escRow = $this->Escalationmodel->getEscalatedRowDetailsCaseNo($rows->misc_case_no);
                    log_message('error', '#645: From escalation_detail_table : '.json_encode($escRow));
                    if(!empty($escRow) && $escRow != null) {
                        ...
                        log_message('error', '#649: Escalation details : '.json_encode($escData));
                        $rows->escalation_date = $escData->escalation_date;
                        $rows->escalation_zone = $escData->escalation_zone;
                        $rows->assigned_date   = $escData->assigned_date;
                    }
        ...
        $cases['_view'] = 'NameCorrection/ASTNoticeGenerate';
        $this->load->view('layouts/main',$cases);
    }
  ...
  //       $this->load->view('../views/header');
        ...
        $data['secondparty'] = $this->NameCorrectionModel->get2ndPartyInfo($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no, $vill_code, $patta_no, $dag_no, $patta_type_code, $misc_case_no);
        //var_dump($data['secondparty']);
        $data['coOrderdate'] = $this->NameCorrectionModel->getCOorderDate($misc_case_no);
        $districtdata = $this->MisModel->getDistrictName($dist_code);
        $subdivdata = $this->MisModel->getSubDivName($dist_code, $subdiv_code);
        $circledata = $this->MisModel->getCircleName($dist_code, $subdiv_code, $cir_code);
        $mouzadata = $this->MisModel->getMouzaName($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code);
        ...
        // $this->load->view('../views/NameCorrection/ASTNoticeGenerate1', $data);
        // $this->load->view('../views/footer');
        ...
        $data['_view'] = 'NameCorrection/ASTNoticeGenerate1';
        $this->load->view('layouts/main',$data);
        $user_code = $this->session->userdata('user_code');
    }


    public function ASTNoticeReGenerate() {
        ...
        $config['base_url'] = base_url() . '/index.php/NameCorrection/ASTNoticeGenerate/';
        $data['countMiscCase'] = $this->NameCorrectionModel->getNoticeGenerateMiscCase($user_code);
        //$cases['TotMisc'] = $this->NameCorrectionModel->getNoticeReGenerateMiscCase1($user_code)->result();
        $case_array = array();
        
        $data['MisCases'] = $case_array;

        $case_no = $this->input->POST('case_no');
        $data['_view'] = 'NameCorrection/ASTNoticeReGenerate';
        $this->load->view('layouts/main',$data);
    }

   public function ASTNoticeReGenerate1() {
        // $db=  $this->session->userdata('db');
  //       $this->load->helper('html');
  //       $this->load->view('../views/header');
        ...
        $case_no = $this->input->POST('case_no');
        $data['Misccaseno']=$this->NameCorrectionModel->getNoticeReGenerateMiscCase1($user_code,$case_no);

        if($data['Misccaseno'])
        {
            $data['_view'] = 'NameCorrection/ASTNoticeReGenerate1';
            $this->load->view('layouts/main',$data);
        }
        else{
    ...
    public function notice_generation_save() {
        $db=  $this->session->userdata('db');
        $user_code = $this->session->userdata('user_code');
        $misc_case_no = $this->input->get('case_no');
        $petition_no = $this->input->get('petition_no');
        $date = date('Y-m-d');
        ...
        //var_dump($date_entry);
        $this->db->query("UPDATE  misc_case_basic SET notice_generated_yn='Y', notice_generated_date='$date',operation='a', date_of_operation='$date' "
                . "WHERE misc_case_no = '$misc_case_no' and misc_case_petition_no = '$petition_no'");
        ...
        $this->session->set_flashdata('message', "Notice Generated for Miscellaneous Cases no # $misc_case_no");
        redirect(base_url() . "index.php/home");
    }
  ...
  //       $this->load->view('../views/header');
        ...
        $config['base_url'] = base_url() . '/index.php/NameCorrection/LMStep1/';
        $data['countMiscCase'] = $this->NameCorrectionModel->getMiscCaseLM($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no);

        $cases['TotMisc'] = $this->NameCorrectionModel->getMiscCaseLM1($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no)->result();
        $case_array = array();
        //$var_dump($case_array);
        foreach ($cases['TotMisc'] as $c) {
        ...
        // $this->load->view('../views/footer');
        $data['_view'] = 'NameCorrection/LMStep1';
        $this->load->view('layouts/main',$data);
    }
  ...
  //       $this->load->view('../views/header');
        ...
        $cases['TotMisc'] = $this->NameCorrectionModel->getMiscCaseLMRe($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no)->result();
        $case_array = array();
        //$var_dump($case_array);
        foreach ($cases['TotMisc'] as $c) {
            $q = $this->db->query("select misc_case_type,misc_case_no,submission_date from    misc_case_basic where lm_note_yn is  null and next_date_of_hearing is not null and fresh_yn='Y'")->row();
            array_push($case_array, $c);
        }
        $data['MisCases'] = $case_array;
        // $this->load->view('../views/NameCorrection/LMStep1', $data);
  ...
  //       $this->load->view('../views/header');
        ...
        $data['_view'] = 'NameCorrection/LMStep2';
        $this->load->view('layouts/main',$data);
    }

    public function LMStep2_save_old_25_3_22() {
        //$db=  $this->session->userdata('db');
        $misc_case_no = $this->input->post('misc_case_no');
        $petition_no = $this->input->post('misc_case_petition_no');
    ...
    public function SKStep1() {
        // $db=  $this->session->userdata('db');
  //       $this->load->helper('html');
  //       $this->load->view('../views/header');
        $user_code = $this->session->userdata('user_code');
        $config['base_url'] = base_url() . '/index.php/NameCorrection/SKStep1/';
        $data['countMiscCase'] = $this->NameCorrectionModel->getMiscCaseSK($user_code);
        $cases['TotMisc'] = $this->NameCorrectionModel->getMiscCaseSK1($user_code)->result();
        $case_array = array();
        foreach ($cases['TotMisc'] as $c) {
            $q = $this->db->query("select misc_case_type,misc_case_no,submission_date from    misc_case_basic where status='02' and lm_note_yn='Y' and sk_note_yn is null")->row();
            array_push($case_array, $c);
        }
        $data['MisCases'] = $case_array;
        // $this->load->view('../views/NameCorrection/SKStep1', $data);
        // $this->load->view('../views/footer');

        $data['_view'] = 'NameCorrection/SKStep1';
        $this->load->view('layouts/main',$data);
    }
  ...
  //       $this->load->view('../views/header');
        ...
        // $this->load->view('../views/NameCorrection/SKStep2', $data);
        // $this->load->view('../views/footer');
        ...
        $data['_view'] = 'NameCorrection/SKStep2';
        $this->load->view('layouts/main',$data);
    }

    public function SKStep2_save_old_25_3_22() {
        //$db=  $this->session->userdata('db');
        $misc_case_no = $this->input->post('misc_case_no');
        $petition_no = $this->input->post('misc_case_petition_no');
        ...
        $this->db->insert("misc_case_process_reports", $userdata);
        $updateSqlBasic = "update  misc_case_basic set sk_note_yn='Y', operation='$operation', date_of_operation='$note_date',  "
                . " status='$status' where misc_case_no='$misc_case_no' and misc_case_petition_no = '$petition_no' ";
        $this->db->query($updateSqlBasic);
    ...
    public function LMStep2_save() {
       ...
       if($proInsert==false || $proInsert===false)
        ...
        if($data['miscCaseInfo']->es_flag == 1 && ESCALATION_ENABLE == 1){
            ...
            $serviceChoose = explode('/',$data['miscCaseInfo']->misc_case_no);
            $next_date_of_hearing = $data['miscCaseInfo']->next_date_of_hearing.date(' H:i:s');

            //same method calling for LM End============
            if($serviceChoose[4] == 'MiNC')
            {
                $escalationUpdateStatus = $this->Escalationmodel->escalationLMReportNCOR($executionDate,$dist_code,$subdiv_code,$cir_code,$data['miscCaseInfo']->misc_case_no,$user_code,$next_date_of_hearing);
            }
            else
            {
                $escalationUpdateStatus = $this->Escalationmodel->escalationLMReportNCAN($executionDate,$dist_code,$subdiv_code,$cir_code,$data['miscCaseInfo']->misc_case_no,$user_code,$next_date_of_hearing);
            }
            log_message("error", "#ESC3337, transaction-error-STATUS======".json_encode($escalationUpdateStatus));

            if($escalationUpdateStatus['responseType'] == 0){
                $this->db->trans_rollback();
                log_message("error", "#ESC3337, transaction-error in method 'NameCancellation/LMStep2' with case-no :". $case_no);
                $this->session->set_flashdata('message', "Something went wrong. NCOR- Error Code(#ESC3337)");
                redirect(base_url() . "index.php/home");
            }
            ///////////////END ESCALATION//////////////
        }
...
////////////////////////
       public function lmNameCorrectionReport()
    {
        ...
        $json = [
            'success' => 'true',
            'details' => $this->NameCorrectionModel->getLMReport($misc_case_no, $petition_no)
        ];
        echo json_encode($json);
        return;
    }
    ...
    public function skNameCorrectionReport()
    {
        ...
        $json = [
            'success' => 'true',
            'details' => $this->NameCorrectionModel->getSKReport($misc_case_no, $petition_no)
        ];
        echo json_encode($json);
        return;
    }
    ...
    public function finalOrderCONameCorrection()
    {
        ...
        $data['_view'] = 'NameCorrection/finalOrderPassCONameCorrect';
        $this->load->view('layouts/main',$data);    
    }

        public function finalOrderCONameCorrection_save()
        {
            $allowed = ['CO'];
            $user_desig_code = $this->session->userdata('user_desig_code');
            ...
            $this->form_validation->set_rules('ord_type_code', 'Order Type Code', 'required');
            $this->form_validation->set_rules('ord_passby_desig', 'Order Passby Designation', 'required');
            $this->form_validation->set_rules('dag_no', 'Dag No.', 'required');
            $this->form_validation->set_rules('lm_code', 'LM Code', 'required');
            $this->form_validation->set_rules('lm_sign_date', 'LM Sign Date', 'required');
            // $this->form_validation->set_rules('sk_code', 'SK Code', 'required');
            // $this->form_validation->set_rules('sk_sign_date', 'SK Sign Date', 'required');
            // $this->form_validation->set_rules('co_report', 'CO Report', 'required');
            $this->form_validation->set_rules('misc_case_no', 'Misc case No', 'required');
            $this->form_validation->set_rules('misc_case_petition_no', 'Misc Case Petition No', 'required');
            $this->form_validation->set_rules('ord_passby_sign_yn', 'Order Passby Sign Yn', 'required');
            $this->form_validation->set_rules('lm_sign', 'LM Sign', 'required');
            $this->form_validation->set_rules('sk_sign', 'SK Sign', 'required');
            $this->form_validation->set_rules('co_sign', 'CO Sign', 'required');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                log_message('error', $errors .'. Error: ERRNMECORRFOCO0001');
                $this->session->set_flashdata('message', $errors . ". Error: ERRNMECORRFOCO0001");
                redirect(base_url('index.php/NameCorrection/COFinalOrderMiscCase1/06'));
            }
            else
            {
                // if(!isset($_POST['application_no']) || !isset($_POST['ord_type_code']) || !isset($_POST['ord_passby_desig']) || !isset($_POST['dag_no']) || !isset($_POST['lm_code']) || !isset($_POST['lm_sign_date']) || !isset($_POST['sk_code']) || !isset($_POST['sk_sign_date']) || !isset($_POST['co_report']) || !isset($_POST['misc_case_no']) || !isset($_POST['misc_case_petition_no']) || !isset($_POST['ord_passby_sign_yn']) || !isset($_POST['lm_sign']) || !isset($_POST['sk_sign']) || !isset($_POST['co_sign']) || !isset($_POST['ord_date']) || $_POST['application_no']=='' || $_POST['ord_type_code']=='' || $_POST['ord_passby_desig']=='' || $_POST['dag_no']=='' || $_POST['lm_code']=='' || $_POST['lm_sign_date']=='' || $_POST['sk_code']=='' || $_POST['sk_sign_date']=='' || $_POST['co_report']=='' || $_POST['misc_case_no']=='' || $_POST['misc_case_petition_no']=='' || $_POST['ord_date']=='') {
                //     //ERRNMECORRFOCO0001
                ...
                $validreport = specialCharacterCheckingInInput($_POST['co_report'], ['.', ',', '|', '-',':','।','\'','/', '(', ')' ,"’", '০', 'ত্‍', 'ৎ', ' ', '&']);
                if($validreport['status']=='n') {
                    //ERRNMECORRFOCO0005
                ...
                $validguard = specialCharacterCheckingInInput(preg_replace($pattern, '', preg_replace('/\s+/', ' ', strip_tags($_POST['infavor_of_guardian'], ['script']))), ['.', ',', '|', '-',':','।','\'','/', '(', ')' ,"’", '০', 'ত্‍', 'ৎ', ' ', '&']);
                ...
                if(isset($_POST['sk_sign_date']) && $_POST['sk_sign_date'] != '') {
               ...
               if($proInsert==false || $proInsert===false)
                ...
                $tchithaOrdBasic = [
                    ...
                    'ord_ref_let_no' => isset($letter_no) ? $letter_no : null,
                    'lm_code' => $val['lm_code'],
                    'lm_sign_yn' => $val['lm_sign'],
                    'lm_sign_date' => date('Y-m-d', strtotime($val['lm_sign_date'])),
                    'sk_code' => (isset($val['sk_code']) && $val['sk_code'] != '') ? $val['sk_code'] : null,
                    'sk_sign_yn' => $val['sk_sign'],
                    'sk_sign_date' => (isset($val['sk_sign_date']) && $val['sk_sign_date'] != '') ? date('Y-m-d', strtotime($val['sk_sign_date'])) : null,
                    'co_code' => $user_code,
                    'co_sign_yn' => $val['co_sign'],
                    'co_ord_date' => date('Y-m-d G:i:s'),
                    'ord_date' => $ord_date,
                    'wrt_order1' => (isset($val['wrt1']) && $val['wrt1'] != '') ? $val['wrt1'] : null,
                    'wrt_order2' => (isset($val['wrt2']) && $val['wrt2'] != '') ? $val['wrt2'] : null,
                    'wrt_order3' => (isset($val['wrt3']) && $val['wrt3'] != '') ? $val['wrt3'] : null,
                    'wrt_order4' => (isset($val['wrt4']) && $val['wrt4'] != '') ? $val['wrt4'] : null    
                ];
    ...
    public function updateChithaNameCorrection($case_no, $misc_case_petition_no) {
        ...
        $date=date('Y-m-d');
        $redirectUrl = base_url().'index.php/NameCorrection/finalOrderCONameCorrection?misc_case_no='.$case_no.'&petition_no='.$misc_case_petition_no;
        ...
        foreach ($data as $d) {
            ...
            $where = [
                'patta_no = ' => trim($d->patta_no), // requires a manual query if using TRIM
                'pdar_id'           => $d->pdar_id,
                'dist_code'         => $d->dist_code,
                'subdiv_code'       => $d->subdiv_code,
                'cir_code'          => $d->cir_code,
