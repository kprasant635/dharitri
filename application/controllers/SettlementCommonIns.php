<?php
class SettlementCommonIns extends CI_Controller
{
...
//        die();
    ...
    public function getDagList()
    {
        ...
        if ($service == SLIJE_ID && IS_PRODUCTION == 1) {
            $dag = $this->db->query(" SELECT distinct on(dag_no_int) trim(cb.dag_no) as dag_no,dag_no_int,
                dag_area_b AS bigha,
                dag_area_k AS katha,
                dag_area_lc AS lessa,
                dag_area_g AS ganda,
                dag_area_kr AS kranti FROM c_land_bank_details cl
                join c_land_bank_encroacher_details cn 
                ON  cn.c_land_bank_details_id=cl.id
                join chitha_basic cb on 
                ...
                where cl.nature_of_reservation in (1,2,6,7,8,9,10,11,12) and cb.subdiv_code=? and cb.cir_code=? 
                and cb.mouza_pargona_code=?
                and cb.lot_No=? and cb.vill_townprt_code=? and whether_encroached='Y' 
                AND cn.type_of_land_use in (7) order by dag_no_int",
        ...
        if ($service == SLIJE_ID && IS_PRODUCTION == 0) {
            $dag = $this->db->query(" SELECT distinct on(dag_no_int) trim(cb.dag_no) as dag_no,dag_no_int,
                dag_area_b AS bigha,
                dag_area_k AS katha,
                dag_area_lc AS lessa,
                ...
                and cl.mouza_pargona_code=cb.mouza_pargona_code and cl.lot_no=cb.lot_no
                and cl.vill_townprt_code=cb.vill_townprt_code and trim(cl.dag_no)=trim(cb.dag_no) 
                where cl.nature_of_reservation in (1,2,7,8,10,11,12) and cb.subdiv_code=? and cb.cir_code=? 
                and cb.mouza_pargona_code=?
                and cb.lot_No=? and cb.vill_townprt_code=? and whether_encroached='Y' 
                and cn.type_of_land_use in (7)
                order by dag_no_int",
                array($subdiv, $circle, $mouza, $lot, $village));
    ...
    public function saveDagDetail()
    {
        ...
        if ($nature_of_land == HOMESTEAD_INS) {
    ...
    settlement_dag_details WHERE case_no=? AND dag_no=?",
        ...
        if ($service_code == SLIJE_ID) {
            ...
            if ($nature_of_land == HOMESTEAD_INS) { //should not be more than 1 bigha
                ...
                if ($in_bigha > KHAS_MAX_HOMESTEAD_INSTITUTE) {
                    $this->db->trans_rollback();
                    log_message('error', 'Maximum limit to apply for this service is ' . KHAS_MAX_HOMESTEAD_INSTITUTE . ' Bigha for case no ' . $case_no);
                    $json = [
                        'responseType' => 3,
                        'message' => 'Maximum limit to apply for this service is ' . KHAS_MAX_HOMESTEAD_INSTITUTE . ' Bigha. Kindly contact System Administrator',
                    ];
                    echo json_encode($json);
                    return;
                }
        ...
        if ($sameEncrocher->num_rows() > 0) {
            $this->db->trans_rollback();
            log_message('error', '#ERROR0007553: Already available in settlement applicant for case no ' . $case_no);
            $json = [
                'responseType' => 3,
                'message' => '#ERROR0007553: Same occupier has already been added. Kindly contact System Administrator',
            ];
            echo json_encode($json);
            return;
        }
        ...
        if ($sameDag->num_rows() > 0) {
            $this->db->trans_rollback();
            log_message('error', '#ERROR0007565: Already available in settlement Dag for case no ' . $case_no);
            $json = [
                'responseType' => 3,
                'message' => '#ERROR0007565: Same Dag has already been added. Kindly contact System Administrator',
            ];
            echo json_encode($json);
            return;
        }
        ...
        if ($insertEncrocher != 1) {
            $this->db->trans_rollback();
            log_message('error', '#ERROR0002: Insertion failed in settlement_applicant for case no ' . $case_no);
            $json = [
                'responseType' => 3,
                'message' => '#ERROR0002: Failed to add occupier detail. Kindly contact System Administrator',
            ];
            echo json_encode($json);
            return;
        }
        ...
        $status = $this->SettlementApiModel->insertNewDagIns($application_no, $getChithaDag, $encroacher_id, $pattaDetail->patta_no, $pattaDetail->patta_type_code,$possession_period,$encroacher_name,$encroacher_father_name,$tot_bigha,$tot_katha,$tot_lessa,$tot_ganda,$tot_kranti,$hbigha,$hkatha,$hlessa,$hganda,$hkranti);

        if (trim($status) != 'y') {
            $this->db->trans_rollback();
            log_message('error', '#DAGINS0007920: Unable to insert new dag, RTPS Case No '.$application_no);
            $data = array(
                'error'=>"#DAGINS0007920: Unable to insert new dag, for case no : ".$application_no
            );
            echo json_encode($data);
            return false;
            exit;
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $rows) {
                ...
                $json[] = array(
                    ...
                    $s_code == SLIJE_ID ? $khas_ins_link : "NA"
                );
            }
    ...
    public function bulkReviveCases()
    {
        ...
        if($caseCount > 0)
        {
            ...
            $rtps_status=$this->SettlementApiModel->applicationStatusUpdateBulkMb3($allAPICasesIds,'NA',$rmk,$status,$task,$pen);
            if($rtps_status!="y")
            {
                $this->db->trans_rollback();
    ...
    {
        $basics = $this->db->query('select * from settlement_basic where case_no = ?', array($case_no));
        ...
        foreach($dagRes as $dagRow){
            ...
            $this->db->trans_begin();
            $getPremSql = $this->db->query('select * from settlement_premium where case_no = ? and dag_no = ?', array($case_no, $dagRow->dag_no));
            ...
            if(!empty($land_class_code_agri)){
                $landSqlagri = $this->db->query('select * from revenue_land_class_wise where class_code = ? and ruralurban = ? order by date_entry desc limit 1', array($land_class_code_agri, $isUrban));
            ...
            if(!empty($land_class_code_home)){
                $landSqlahome = $this->db->query('select * from revenue_land_class_wise where class_code = ? and ruralurban = ? order by date_entry desc limit 1', array($land_class_code_home, $isUrban));
            ...
            ///****Update in chitha */
            $csql = $this->db->query('select * from settlement_basic where case_no = ? and order_passed is not null', array($case_no));
            if($csql->num_rows() > 0){
                $alsql = $this->db->query('select * from chitha_settlement_allottee where ord_no = ? and old_dag = ? limit 1', array($case_no, $dagRow->dag_no));
    ...
    public function RejectedCasesByHeadView(){
        ...
        $data['selectList'] = $this->db->query('select * from location where dist_code = ? and subdiv_code = ? and cir_code = ? and mouza_pargona_code != ? and lot_no != ? and vill_townprt_code != ?', array($dist_code, $subdiv_code, $cir_code, '00', '00', '00000'))->result();
    ...
    public function applicationRemoveFromProposal()
    {
        ...
        else
        {
            ...
            if($this->SettlementCommonDcModel->countSettlementProposalListMbTin($proposalId) != 1)
            {
                echo json_encode(array(
                    'responseType' => 1,
            ...
            {
                echo 'Case Number '. $case_no . 'is not belongs to MB3';
                die();
            }
    ...
    public function applicationRemoveFromRevertedProposal()
    {
        ...
        else
        {
            ...
            if($this->SettlementCommonDcModel->countSettlementProposalListMbTin($proposalId) != 1)
            {
                echo json_encode(array(
                    'responseType' => 1,
                    'message'  => '#ERMR002443: Proposal not found in Meeting ! Kindly contact system administrator',
                ));
                return;
            }
            if($this->SettlementCommonDcModel->countSettlementProposalPendingCaseByCaseNo($case_no) != 1)
    ...
    // get all rejected case list
    public function rejectedListIns()
    {
        $service_code = $this->input->get('service');
        $status = $this->input->get('s');
        $data['select_data'] = $this->SettlementCommonModel->locationSelectCoRejectCases($service_code, $status);
        $data['_view'] = 'SettlementView/rejected_case_list_ins';

        $this->load->view('layouts/main', $data);
    }

    public function rejectedListPaginationIns()
    {
        $s_code      = $this->input->post('service');
        $search_term = $this->input->post('search_term');
        $remark_cat  = $this->input->post('remark_cat');
        ...
        $this->db->where('a.dist_code', $this->session->userdata('dist_code'));
        $this->db->where('a.adc_code', $this->session->userdata('user_code'));
    ...
    public function downloadCasesWithProposalId()
    {
		...
		t3.applied_area,t9.reservation_area, t3.proposed_area as proposed_area_with_reservation_area, t5.encroacher_name, t6.joint_applicants,t1.occupation_applicant as Occupation,
		t7.institution_name,t7.purpose_land,t17.period_possession,
		CASE WHEN t7.ins_cat_type_co='8' THEN 'State Government'
		WHEN t7.ins_cat_type_co='9'  THEN 'State Government (Under Taking)'
		WHEN t7.ins_cat_type_co='10' THEN 'Central Government'
		WHEN t7.ins_cat_type_co='11' THEN 'Central Government (Under Taking)'
		WHEN t7.ins_cat_type_co='12' THEN 'Non Government'
		else 'NA'
		end as entity,	
		...
		END, ',' )
		from settlement_premium where case_no=t11.case_no and is_final=1 and  area_name is not null and t11.case_no is not null) as area_type
        ...
        left join ( select case_no,string_agg(distinct(dag_no),'-') as dag_no,string_agg(distinct(dag_no || '-area( home: ' || home_b || ' B-'||home_k||' K-'||home_lc ||'L, agri: '||agri_b||'B-'||agri_k||'K-'||agri_lc||'L)'),',') as applied_area, string_agg(distinct(dag_no || '-area( Total_Proposed_area: ' || s_dag_area_b || ' B-'||s_dag_area_k||' K-'||s_dag_area_lc ||'L)'),',') as proposed_area from settlement_dag_details sdd group by case_no) t3 on t11.case_no=t3.case_no
        left join ( select case_no, string_agg(distinct(dag_no || '-area(reservation_area: ' || bigha || ' B-'||katha||' K-'||lessa ||'L)'),',') as reservation_area from settlement_reservation rdd where is_deleted = 0 group by case_no) t9 on t11.case_no=t9.case_no
        ...
        left join ( select case_no,string_agg(distinct(pdar_name),'-') as joint_applicants from settlement_applicant sa where is_applicant!='1' and pdar_type='B' group by case_no) t6 on t11.case_no=t6.case_no
		  left join (Select case_no,ins_name_co as institution_name, purpose_land_allot_co as purpose_land,ins_cat_type_co from settlement_institution_details ) t7 on t7.case_no=t1.case_no
		  left join (Select case_no,period_possession from settlement_ap_lmnote ) t17 ON t17.case_no=t1.case_no")->result_array();
    ...
    // download case CO end
    public function downloadCasesWithPendingOfficerAdcForCoOnly()
    {
        $file_name  = time()."_case_ins_list_co.xlsx";
        ...
        (select locname_eng from location where dist_code=t1.dist_code and subdiv_code=t1.subdiv_code and cir_code=t1.cir_code and mouza_pargona_code=t1.mouza_pargona_code and lot_no=t1.lot_no and vill_townprt_code='00000') lot,
        (select locname_eng from location where dist_code=t1.dist_code and subdiv_code=t1.subdiv_code and cir_code=t1.cir_code and mouza_pargona_code=t1.mouza_pargona_code and lot_no=t1.lot_no and vill_townprt_code=t1.vill_townprt_code) village,
        t1.uuid,t1.applid as application_no,t1.case_no,t1.pending_officer, t2.applicant_name, t2.guardian_name, t2.Address, t3.dag_no, 
		t3.applied_area, t3.proposed_area  as proposed_area_with_reservation_area, t5.encroacher_name, t6.joint_applicants,t1.occupation_applicant as Occupation,
		t7.institution_name,t7.purpose_land,
		CASE WHEN t7.ins_cat_type_co='8' THEN 'State Government'
		WHEN t7.ins_cat_type_co='9'  THEN 'State Government (Under Taking)'
		WHEN t7.ins_cat_type_co='10' THEN 'Central Government'
		WHEN t7.ins_cat_type_co='11' THEN 'Central Government (Under Taking)'
        ...
        from (select case_no from settlement_basic spc where service_code='45' and dist_code = '$dist_code' and subdiv_code = '$subdiv_code' and cir_code = '$cir_code') t11
        left join (select applid,case_no,dist_code,subdiv_code, cir_code, mouza_pargona_code, lot_no, vill_townprt_code,uuid,occupation_applicant,pending_officer from settlement_basic a WHERE dist_code = '$dist_code' and subdiv_code = '$subdiv_code' and cir_code = '$cir_code' and service_code='45' ) t1 on t11.case_no=t1.case_no
        left join ( select case_no,sa.pdar_name as applicant_name,sa.pdar_guardian as guardian_name,sa.pdar_add1 as Address from settlement_applicant sa where is_applicant='1') t2 on t11.case_no=t2.case_no
        left join ( select case_no,string_agg(distinct(dag_no),'-') as dag_no,string_agg(distinct(dag_no || '-area( home: ' || home_b || ' B-'||home_k||' K-'||home_lc ||'L, agri: '||agri_b||'B-'||agri_k||'K-'||agri_lc||'L)'),',') as applied_area, string_agg(distinct(dag_no || '-area( Total_Proposed_area: ' || s_dag_area_b || ' B-'||s_dag_area_k||' K-'||s_dag_area_lc ||'L)'),',') as proposed_area from settlement_dag_details sdd group by case_no) t3 on t11.case_no=t3.case_no
        left join ( select case_no,array_agg(distinct(pdar_name)) as encroacher_name from settlement_applicant sap where pdar_type='EN' group by case_no) t5 on t11.case_no=t5.case_no
        left join ( select case_no,string_agg(distinct(pdar_name),'-') as joint_applicants from settlement_applicant sa where is_applicant!='1' and pdar_type='B' group by case_no) t6 on t11.case_no=t6.case_no
		left join (Select case_no,ins_name_co as institution_name, purpose_land_allot_co as purpose_land,ins_cat_type_co from settlement_institution_details ) t7 on t7.case_no=t1.case_no")->result_array();


            $this->UtilsModel->downloadExcelReportForCoOnly($file_name,$data);
    ...
    // download case ADC end
    public function downloadCasesWithPendingOfficerAdcForAdcOnly()
    {
        $file_name  = time()."_case_ins_list.xlsx";
        ...
        from (select case_no from settlement_basic spc where service_code='45' and dist_code = '$dist_code') t11
        left join (select applid,case_no,dist_code,subdiv_code, cir_code, mouza_pargona_code, lot_no, vill_townprt_code,uuid,occupation_applicant,pending_officer from settlement_basic a WHERE dist_code = '$dist_code' and service_code='45' ) t1 on t11.case_no=t1.case_no
        left join ( select case_no,sa.pdar_name as applicant_name,sa.pdar_guardian as guardian_name,sa.pdar_add1 as Address from settlement_applicant sa where is_applicant='1') t2 on t11.case_no=t2.case_no
        left join ( select case_no,string_agg(distinct(dag_no),'-') as dag_no,string_agg(distinct(dag_no || '-area( home: ' || home_b || ' B-'||home_k||' K-'||home_lc ||'L, agri: '||agri_b||'B-'||agri_k||'K-'||agri_lc||'L)'),',') as applied_area, string_agg(distinct(dag_no || '-area( Total_Proposed_area: ' || s_dag_area_b || ' B-'||s_dag_area_k||' K-'||s_dag_area_lc ||'L)'),',') as proposed_area from settlement_dag_details sdd group by case_no) t3 on t11.case_no=t3.case_no
        left join ( select case_no,array_agg(distinct(pdar_name)) as encroacher_name from settlement_applicant sap where pdar_type='EN' group by case_no) t5 on t11.case_no=t5.case_no
        left join ( select case_no,string_agg(distinct(pdar_name),'-') as joint_applicants from settlement_applicant sa where is_applicant!='1' and pdar_type='B' group by case_no) t6 on t11.case_no=t6.case_no
