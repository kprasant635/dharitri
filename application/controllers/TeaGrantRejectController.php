...
class TeaGrantRejectController extends CI_Controller
{
    ...
    public function postRejectedReason()
    {
        ...
        $params = $this->rejectTableServiceWise($case_no, $user_code);
        if ($params[1] != 1) {
            $this->db->trans_rollback();
        ...
        foreach ($reject_code as $rej_code)
        {
            foreach($sub_rejected_remark as $sub_key => $sub_val)
            {
                if($sub_key == $rej_code)
                {
                    ...
                    $sql = $this->db->query("SELECT * FROM reject_master WHERE reject_code = ?", array($rej_code));
    ...
    public function rejectTableServiceWise($case_no, $user_code)
    {
        $case_no = trim($case_no);
        $table = array();
        if ($this->session->userdata('user_desig_code') == MB_DEPUTY_COMM || $this->session->userdata('user_desig_code') == MB_ADD_DEPUTY_COMM || $this->session->userdata('user_desig_code') == MB_SUB_DIV_COMM)
        ...
        $array = [
            ...
            // 'dc_code'         => $user_code,
            // // 'status'          => MB_DISMISS,
            // 'status'          => MB_REJECT_REASON_ENTERED,
    ...
    public function postRejectedDirectReason()
    {
        ...
        $rtps_status = $this->SettlementApiModel->postApiBasundharaMb3($application_no, $case_no, $rmk, $status, $task, $pen, $rejectCodeArray);
        if ($rtps_status != "y")
        {
            $this->db->trans_rollback();
