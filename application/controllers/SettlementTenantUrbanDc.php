...
class SettlementTenantUrbanDc extends CI_Controller
{
    ...
    public function randomFileNameGeneral()
    {
        ...
        if ($this->SettlementTenantUrbanDcModel->checkDuplicateFileNameInGeneral($new_case_no) != 0) {
            $this->randomFileName();
        } else {
            return $new_case_no;
        }
    ...
    public function applicationRevertFromDCToCO()
    {
        ...
        if ($this->form_validation->run() == false)
        {
            ...
            $caseCount = $this->SettlementTenantUrbanDcModel->countSettlementApplicationDetailsByCaseNo($case_no, $dist_code);
            if ($caseCount == 0) {
                echo json_encode([
                    'responseType' => 3,
                ]);
                ...
                $this->db->trans_begin();
                if ($this->SettlementTenantUrbanDcModel->updateSettlementBasicData($case_no, $dist_code, $updateData) == 0) {
    ...
    public function applicationRejectedBySdlac()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $user_code = $this->session->userdata('user_code');
            $this->utilityclass->checkUserAuthForCaseForDc($case_no);
            $caseCount = $this->SettlementTenantUrbanDcModel->countSettlementApplicationDetailsByCaseNoForSDLAC($case_no, $dist_code);
            if ($caseCount == 0) {
                echo json_encode([
                    'responseType' => 3,
                ]);
                ...
                if ($this->SettlementTenantUrbanDcModel->updateSettlementBasicData($case_no, $dist_code, $updateData) == 0) {
                    ...
                    $this->SettlementTenantUrbanDcModel->updateSettlementAppDetailsByCaseNoUnderProposal($case_no, $updatePro);
    ...
    public function applicationApprovedBySdlac()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $caseCount      = $this->SettlementTenantUrbanDcModel->countSettlementApplicationDetailsByCaseNoForSDLAC($case_no, $dist_code);
            $caseCountInPro = $this->SettlementTenantUrbanDcModel->countSettlementAppDetailsByCaseNoUnderProposal($case_no);
    ...
    public function viewPaymentReceivedAppDetailsByDc()
    {
        ...
        $dist_code = $this->session->userdata('dist_code');
        $this->utilityclass->checkUserAuthForCaseForDc($case_no);
        $caseCount = $this->SettlementTenantUrbanDcModel->countSettlementPaymentReceivedAppDetailsByCaseNo($case_no, $dist_code);

        if ($caseCount == 0) {
            $this->SettlementApFirstLandDc();
        } else {
            $caseDetails         = $this->SettlementTenantUrbanDcModel->getSettlementPaymentReceivedAppDetailsByCaseNo($case_no, $dist_code);
            ...
            $data['_view'] = 'SettlementView/Dc/TenantUrban/payment_received_app_details_tenant';
    ...
    public function updateProposalHearingDateTenant()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $allCases      = $this->SettlementTenantUrbanDcModel->getAllAppInReportSendByDcToSdlacTenant($proposalNo);
    ...
    public function updateHearingDateGenerateNoticeTenant()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $proposalDetails = $this->SettlementTenantUrbanDcModel->getProposalDetailsById($proposalNo, $dist_code);
            if ($proposalDetails == '') {
                echo json_encode([
                    'responseType' => 3,
                ]);
                ...
                if ($allCasesCount == 0) {
                    ...
                    $this->db->trans_begin();
                    if ($this->SettlementTenantUrbanDcModel->updateProposalListById($proposalNo, $updateProposalData) == 0) {
                        $this->db->trans_rollback();
                        echo json_encode([
                            'responseType' => 1,
                        ]);
    ...
    public function SettlementApFirstLandDc()
    {
        ...
        // todo
        $firstProceedingCount = $this->SettlementTenantUrbanDcModel->countAllPendingSettlementTenant($dist_code);
        $paymentConfirmCount  = $this->SettlementTenantUrbanDcModel->paymentConfirmNoticeCount($dist_code);
        $paymentReceivedCount = $this->SettlementTenantUrbanDcModel->paymentReceivedCount($dist_code);

        $rejctedListCount = $this->SettlementCommonDcModel->rejectedCaseList($dist_code, SETTLEMENT_TENANT_URBAN_ID, MB_DEPUTY_COMM);

        $revivalListCount = $this->SettlementCommonDcModel->revivalListCount($dist_code, SETTLEMENT_TENANT_URBAN_ID, MB_DEPUTY_COMM);

        $noticeGeneratedAdcHearingCount = $this->SettlementTenantUrbanDcModel->noticeGeneratedAdcHearingCasesCount($dist_code);

        $generatePaymentNoticeCount         = $this->SettlementTenantUrbanDcModel->generatePaymentNoticeCount($dist_code);
        $getCasesforwardedByAdcAfterHearing = $this->SettlementTenantUrbanDcModel->getCasesforwardedByAdcAfterHearing($dist_code);

        $deptRevertedCases = $this->SettlementTenantUrbanDcModel->countCasesRevertedFromDept($dist_code);

        // $reReportByCOCount = $this->SettlementTenantUrbanDcModel->countReRevertedByCoApplicationTenant($dist_code);
        // $approvedListCount = $this->SettlementTenantUrbanDcModel->countAllApproveAppBySdlacTenant($dist_code);
        // $rejectedListCount = $this->SettlementTenantUrbanDcModel->countAllRejectAppByDcTenant($dist_code);
        // $revertedByDepartmentCount = $this->SettlementTenantUrbanDcModel->countRevertedByDeptApplicationTenant($dist_code);
        ...
        $data['lotCount']                  = $this->SettlementTenantUrbanDcModel->lotCount($dist_code);
        $data['dist_code']                 = $dist_code;
        $data['beneficiaryCount']          = $beneficiaryCount;
        $data['genExlReportCount']         = $this->SettlementTenantUrbanDcModel->exlReportGenCount($dist_code)->count;
        ...
        $data['deptRevertedCases']         = $deptRevertedCases;
        ...
        $data['getCasesforwardedByAdcAfterHearing'] = $getCasesforwardedByAdcAfterHearing;

        //$data['SDLACCommitteeCount']  = $SDLACCommitteeCount;

        $data['_view'] = 'SettlementView/Dc/TenantUrban/first_landing_page_dc_tenant';
        $this->load->view('layouts/main', $data);
    ...
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementTenantUrbanDcModel->getAllPendingSettlementTenant($dist_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'SettlementView/Dc/TenantUrban/first_proceeding_case_dc_tenant';
        $this->load->view('layouts/main', $data);
    ...
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementTenantUrbanDcModel->getAllNoticeGeneratedCases($dist_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['select_data'] = $this->db->query('select * from settlement_basic where dist_code = ? and service_code = ? and status =? and pending_officer = ?', [$dist_code, SETTLEMENT_TENANT_URBAN_ID, 'W', 'DC'])->result();

        $data['adcList'] = $this->db->query('SELECT u.username, l.user_code FROM loginuser_table l JOIN users u ON l.dist_code = u.dist_code AND l.subdiv_code = u.subdiv_code AND l.user_code = u.user_code WHERE l.dist_code = ? AND l.subdiv_code = ? AND u.user_desig_code = ? AND l.dis_enb_option = ?', [$dist_code, '00', 'ADC', 'E'])->result();

        $data['_view'] = 'SettlementView/Dc/TenantUrban/notice_generated_adc_hearing';
        $this->load->view('layouts/main', $data);
    ...
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementTenantUrbanDcModel->paymentNoticeCases($dist_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'SettlementView/Dc/TenantUrban/paymenet_notice_forwarded_by_adc';
        $this->load->view('layouts/main', $data);

    }


    public function paymentNoticeCasesBeforeList()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementTenantUrbanDcModel->paymentNoticeCasesBeforeList($dist_code);
    ...
    public function generatePaymentNoticeCaseView()
    {
        ...
        if ($caseCount == 0) {
            ...
            $caseDetails = $this->SettlementTenantUrbanDcModel->getSettlementApplicationDetailsByCaseNo($case_no, $dist_code);
            $proceedings = $this->SettlementTenantUrbanDcModel->getSettlementProceeding($case_no);
            // $data['caseCount']   = $caseCount;
            $data['caseDetails']         = $caseDetails;
            $data['proceedings']         = $proceedings;
            ...
            foreach ($lmnotes as $r_remark) {
                ...
                if ($rejected_list_json) {
                    foreach ($rejected_list_json as $re_list) {
                        ...
                        $sql = $this->db->query("select remark_head from reject_master where reject_code = ?", [$r_code]);
            ...
            $data['_view'] = 'SettlementView/Dc/TenantUrban/payment_notice_case_view';
            $this->load->view('layouts/main', $data);
        }
    }
    ...
    public function hearingNoticeAdcPagination()
    {
        ...
        $this->db->where('service_code', SETTLEMENT_TENANT_URBAN_ID);
    ...
    public function getSettlementApApplicationDetails()
    {
        ...
        if ($caseCount == 0) {
            ...
            $data['_view'] = 'SettlementView/Dc/TenantUrban/settlement_app_details_tenant';
            $this->load->view('layouts/main', $data);
        }
    }
    ...
    public function generatePaymentNotice()
    {
        ...
        if (isset($_POST['generate_notice'])) {
            ...
            $this->load->view('SettlementView/Dc/TenantUrban/paymentNotice', $data);
        } else {
            $case_no = $this->input->get('case');
            ...
            if ($data['validation_bypass'] != 1) {
                $premSql = $this->db->query('select * from settlement_premium where case_no = ?', [$case_no]);

                if ($premSql->num_rows() <= 0) {
                    $district['dags'] = $this->SettlementKhasModel->getSettlementDag($case_no);

                    $revenue             = $this->db->query("SELECT dag_revenue, dag_area_b, dag_area_k, dag_area_lc, dag_area_g,dag_area_kr FROM chitha_basic WHERE dist_code=? AND subdiv_code=? AND cir_code=? AND mouza_pargona_code=? AND lot_no=? AND vill_townprt_code=? AND dag_no=?", [$district['dags'][0]->dist_code, $district['dags'][0]->subdiv_code, $district['dags'][0]->cir_code, $district['dags'][0]->mouza_pargona_code, $district['dags'][0]->lot_no, $district['dags'][0]->vill_townprt_code, $district['dags'][0]->dag_no])->row();
            ...
            $data['_view']        = 'SettlementView/DC/TenantUrban/generateNoticeView';
            $this->load->view('layouts/main', $data);
    ...
    public function savePaymentNotice()
    {
        ...
        if ($applicants_riotee_nok == true) {
            ...
            $getEndata = $this->db->query("SELECT * FROM settlement_applicant WHERE case_no = ? AND pdar_type = ?", [$case_no, 'EN']);
            ...
            if ($getEndata->num_rows() > 0) {
                ...
                foreach ($applicants_riotee_nok as $nok) {
                    if ($firstCheck === true) {
                        ...
                        $tenant_id = $this->db->query($sql, [$nok->subdiv_code, $nok->cir_code, $nok->mouza_pargona_code, $nok->lot_no, $nok->vill_townprt_code, $tenant_khatian_no, $tenant_dag_no]);
        ...
        $notice_no                  = "MB3/PN/" . date('Y') . "/" . SETTLEMENT_TENANT_URBAN . "/" . $service_details->petition_no;
        ...
        if ($this->db->trans_status() == false) {
            ...
            $sp             = $this->SettlementApModel->getSettlementPremiumCo($case_no);
            $due_amount     = $sp->due_amount;
            $govt_share     = $sp->govt_share;
            $landlord_share = $sp->landlord_share;
            ...
            $curl_handle = curl_init();
            curl_setopt($curl_handle, CURLOPT_URL, API_LINK_MB3 . "uploadNoticeMultipleHead");
            curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($curl_handle, CURLOPT_SSL_VERIFYPEER, false);
            curl_setopt($curl_handle, CURLOPT_CUSTOMREQUEST, "POST");
            curl_setopt($curl_handle, CURLOPT_POSTFIELDS, http_build_query([
            ...
            if (trim($result) != 'y') {
                ...
                redirect(base_url() . 'index.php/SettlementTenantUrbanDc/SettlementApFirstLandDc');
            }
        }
    }
    ...
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementTenantUrbanDcModel->getAllApproveAppBySdlacTenant($dist_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'SettlementView/Dc/TenantUrban/approve_list_by_sdlac_tenant';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getAllRejectByDcListTenant()
    {
        ...
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementTenantUrbanDcModel->getAllRejectAppByDcTenant($dist_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'SettlementView/Dc/TenantUrban/rejected_list_by_dc_tenant';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function viewRejectedAppDetailsTenant()
    {
        ...
        if ($caseCount == 0) {
            ...
            $data['_view'] = 'SettlementView/Dc/TenantUrban/settlement_app_details_rejected_only_view_tenant';
            $this->load->view('layouts/main', $data);
        }
    ...
    public function excelGeneratedCases()
    {
        ...
        $data['_view'] = 'SettlementView/Dc/TenantUrban/excel_generated_cases_view';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getAllOrderChithaUpdateForDcAppTenant()
    {
        ...
        $pendingCase  = $this->SettlementTenantUrbanDcModel->getPaymentConfirmationDc($service_code, $dist_code);

        $data['dist_code']        = $dist_code;
        $data['pendingCaseCount'] = $pendingCase->num_rows();
        $data['cases']            = $pendingCase->result();

        $data['_view'] = 'SettlementView/Dc/TenantUrban/order_chitha_update_list_by_dc_tenant';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function confirmPaymentChithaUpdate()
    {
        ...
        $data['_view'] = 'SettlementView/Dc/TenantUrban/cofirm_payment_owner_end';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function confirmPaymentApplicantDc()
    {
        ...
        if (strtoupper($pay_status) == 'Y') {
            $sqlCheck = $this->db->query('select * from settlement_premium where case_no = ? and is_final = ? and grn_no is null limit 1', [$case_no, 1]);
        ...
        //     'payment_date' => $payment_status_check->payment_date,
        //     'case_no_rtps' => $case_no_rtps,
        //     '_view' => 'SettlementView/Dc/TenantUrban/confirmPaymentApplicantView'
        // ];
    ...
    public function confirmPaymentOwnerDc()
    {
        ...
        $data = [
            ...
            'payment_date'   => $payment_status_check->payment_date,
            'case_no_rtps'   => $case_no_rtps,
            '_view'          => 'SettlementView/Dc/TenantUrban/confirmPaymentOwnerView',
        ];
        $this->load->view('layouts/main', $data);
    ...
    public function AllApplicantPaymentConfirmedCases()
    {
        $service_code = SETTLEMENT_TENANT_URBAN_ID;
        $dist_code    = $this->session->userdata('dist_code');
        $getDistrict  = $this->SettlementTenantUrbanDcModel->getLocationName($dist_code);
        ...
        $data['dist_code']                = $dist_code;
        $data['getPaymentConfirmationCo'] = $this->SettlementTenantUrbanDcModel->getPaymentReceivedApplicant($service_code);

        $data['_view'] = 'SettlementView/Dc/TenantUrban/all_applicant_confirmed_payment_cases';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function AllApplicantPaymentConfirmedCasesForNotice()
    {
        ...
        $data['_view'] = 'SettlementView/Dc/TenantUrban/tenant_notice_list';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function AllApplicantPaymentConfirmedCasesForNoticeCases($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no, $vill_townprt_code)
    {
        ...
        $data['_view'] = 'SettlementView/Dc/TenantUrban/tenant_notice_list_cases';
        $this->load->view('layouts/main', $data);
    }
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($result as $rows) {
                $json[] = [
                    ...
                    '<a class="btn btn-success" href="' . base_url() . 'index.php/SettlementTenantUrbanDc/getSettlementApApplicationDetails/?case=' . $rows->c_no . '">View Application</a>',
                ];
    ...
    public function getPaymentReceivedCasesNoticeByVillage()
    {
        ...
        $this->db->where_in('sb.status', [MB_PAYMENT_RECEIVED, 'VN']);
        $this->db->where_in('sb.pending_officer', [MB_ADD_DEPUTY_COMM, MB_SUB_DIV_COMM, MB_DEPUTY_COMM, MB_CIRCLE_OFFICER]);
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($result as $rows) {
                $gen_notice = 1;
                $sql        = $this->db->query('select * from settlement_village_wise_notice where dist_code = ? and subdiv_code = ? and cir_code = ? and mouza_pargona_code = ? and lot_no = ? and vill_townprt_code = ? and service_code = ?', [$rows->dist_code, $rows->subdiv_code, $rows->cir_code, $rows->mouza_pargona_code, $rows->lot_no, $rows->vill_townprt_code, $service]);
                if ($sql->num_rows() > 0) {
                    $gen_notice = 0;
                }
                ...
                $json[] = [
                    ...
                    $gen_notice == 1 ? '<button class="btn btn-danger btn-sm" onclick="generateVillageNotice(\'' . $rows->dist_code . '\',\'' . $rows->subdiv_code . '\',\'' . $rows->cir_code . '\',\'' . $rows->mouza_pargona_code . '\',\'' . $rows->lot_no . '\',\'' . $rows->vill_townprt_code . '\',\'' . $service . '\')">Generate Notice</button>' : '<a href="' . base_url() . 'index.php/SettlementTenantUrbanDc/printNotice?loc=' . $rows->dist_code . '_' . $rows->subdiv_code . '_' . $rows->cir_code . '_' . $rows->mouza_pargona_code . '_' . $rows->lot_no . '_' . $rows->vill_townprt_code . '_' . $service . '" type="button" class="btn btn-success btn-sm" target="printVillageNotice">Print Notice</a>',
    ...
    public function excelDataInsertion()
    {
        ...
        if ($sql_id->num_rows() > 0) {
            ...
            $excel_id     = SETTLEMENT_TENANT_URBAN . $dist_code . 'XLS' . $excel_id_tem;
        } else {
            $excel_id_tem = 1;
            $excel_id     = SETTLEMENT_TENANT_URBAN . $dist_code . 'XLS' . $excel_id_tem;
        }
    ...
    public function individualCaseConfirmFinalPayment($excel_id)
    {
        ...
        $data['_view'] = 'SettlementView/Dc/TenantUrban/individual_case_payment_confirm';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function confirmPaymentDc()
    {
        ...
        $data                 = [
            ...
            'payment_date'   => $payment_status_check->payment_date,
            'case_no_rtps'   => $case_no_rtps,
            '_view'          => 'SettlementView/Dc/TenantUrban/confirmPaymentView',
        ];
    ...
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementTenantUrbanDcModel->getReRevertedByCoApplicationTenant($dist_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'SettlementView/Dc/TenantUrban/re_revert_by_co_list_tenant';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getAllRevertedAppByDeptForDcAppTenant()
    {
        ...
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementTenantUrbanDcModel->getRevertedByDeptApplicationTenant($dist_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'SettlementView/Dc/TenantUrban/revert_by_dept_list_tenant';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function applicationApprovedByDcTenant()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $user_code = $this->session->userdata('user_code');
            $this->utilityclass->checkUserAuthForCaseForDc($case_no);
            $caseCount = $this->SettlementTenantUrbanDcModel->caseForDcApprovalTenant($case_no, $dist_code);
            if ($caseCount == 0) {
                echo json_encode([
                    'responseType' => 3,
                ]);
    ...
    public function generateGeneralNotice()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($caseCount == 0) {
                ...
                $get_dag_details = $this->SettlementTenantUrbanDcModel->getDagDetailsTenant($case_no);
                ...
                $getKhatian   = $this->SettlementTenantUrbanDcModel->getKhatianDetailsByCaseNo($case_no);
                $get_owners   = $this->SettlementApModel->getOwners($case_no);
    ...
    {
        $case_no       = $this->input->get('case');
        $noticeDetails = $this->SettlementTenantUrbanDcModel->getGeneralNoticeDetails($case_no);
    ...
    public function getAllBeneficiaryForDcAppTenant()
    {
        ...
        $beneficiary = $this->SettlementTenantUrbanDcModel->getAllBeneficiaryListTenant($dist_code, $serviceCode);

        $data['beneficiary']      = $beneficiary->result();
        $data['beneficiaryCount'] = $beneficiary->num_rows();

        $data['_view'] = 'SettlementView/Dc/TenantUrban/beneficiary_list_tea';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getAllBeneficiaryListByCaseNo()
    {
        ...
        $beneficiary = $this->SettlementTenantUrbanDcModel->getAllBeneficiaryIndividualByCaseNo($case_no, $dist_code);

        $data['case_no']          = $case_no;
        $data['beneficiary']      = $beneficiary->result();
        $data['beneficiaryCount'] = $beneficiary->num_rows();

        $data['_view'] = 'SettlementView/Dc/TenantUrban/beneficiary_list_case_wise_tea';
        $this->load->view('layouts/main', $data);
    ...
    public function updateBeneficiaryPaymentStatus()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($this->SettlementTenantUrbanDcModel->countBeneficiaryByCaseNo($case_no, $dist_code) == 0) {
                echo json_encode([
                    'responseType' => 3,
                ]);
                return;
            ...
            if ($payStatus == 0 or $payStatus == 1) {
                ...
                $this->db->trans_begin();
                if ($this->SettlementTenantUrbanDcModel->updateBeneficiaryPaymentStatus($case_no, $dist_code, $updateData) == 0) {
                    $this->db->trans_rollback();
                    echo json_encode([
                        'responseType' => 1,
                    ]);
    ...
    public function downloadBeneficiaryList()
    {
        ...
        $allBeneficiary = $this->SettlementTenantUrbanDcModel->getAllBeneficiaryIndividualByCaseNo($case_no, $dist_code);
    ...
    public function downloadAllBeneficiaryWiseDist()
    {
        ...
        $allBeneficiary = $this->SettlementTenantUrbanDcModel->getAllBeneficiaryForDownload($dist_code, $serviceCode);
        $allBeneficiary = $allBeneficiary->result();
    ...
    public function confirmPaymentForwardToCo()
    {
        ...
        redirect(base_url() . 'index.php/SettlementTenantUrbanDc/excelGeneratedCases');
    ...
    public function generateVillNotice()
    {
        ...
        $sql = $this->db->query('select * from settlement_basic where dist_code =? and subdiv_code =? and cir_code =? and mouza_pargona_code =? and lot_no = ? and vill_townprt_code =? and status =? and service_code = ?', [$dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no, $vill_townprt_code, 'P', $service]);
        ...
        foreach ($basicResult as $row) {
            $getOwner = $this->db->query('select * from settlement_applicant where case_no = ? and pdar_type = ?', [$row->case_no, 'O']);
            ...
            $getApplicant = $this->db->query('select * from settlement_applicant where case_no = ? and pdar_type = ? and is_applicant = ?', [$row->case_no, 'B', 1]);
            ...
            $get_missing_rel = $this->db->query('select * from settlement_applicant where case_no = ? and pdar_type in(\'P\',\'GP\',\'GGP\')', [$row->case_no]);
            ...
            $getRioteeDetails = $this->db->query('select * from settlement_applicant where case_no = ? and pdar_type = ?', [$row->case_no, 'EN']);
            ...
            $getDagDetails = $this->db->query('select * from settlement_dag_details where case_no = ?', [$row->case_no]);
            ...
            $getPrem = $this->db->query('select * from settlement_premium where case_no = ? and is_final = ?', [$row->case_no, 1]);
    ...
    public function genVIllNotice()
    {
        ...
        $base_64_file_path    = TENANT_VILLAGE_NOTICE . '/' . SETTLEMENT_TENANT_URBAN . $dist_code . $subdiv_code . $cir_code . $mouza_pargona_code . $lot_no . $vill_townprt_code . $service . ".json";
        $file_to_write_base64 = fopen($base_64_file_path, "w") or die("Unable to open file!");
        ...
        $checkifalreadyInsert = $this->db->query('select * from settlement_village_wise_notice where dist_code = ? and subdiv_code = ? and cir_code = ? and mouza_pargona_code = ? and lot_no = ? and vill_townprt_code =? and service_code =?', [$dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no, $vill_townprt_code, $service]);
        ...
        foreach ($case_array as $case) {
            ...
            $checkPaymentDoneSql = $this->db->query('select * from settlement_premium where case_no = ? and is_final = ? and due_amount > paid_amount', [$case, 1]);
            ...
            $sql = $this->db->query('select * from settlement_notice where case_no = ? and notice_type =?', [$case, 'VN']);
            if ($sql->num_rows() > 0) {
                echo json_encode([
                    'responseType' => 0,
            ...
            $notice_no = "MB3/VN/" . date('Y') . "/" . SETTLEMENT_TENANT_URBAN_ID . "/" . $timestamp;
    ...
    public function printNotice()
    {
        ...
        $sql = $this->db->query('select * from settlement_village_wise_notice where dist_code = ? and subdiv_code = ? and cir_code = ? and mouza_pargona_code = ? and lot_no = ? and vill_townprt_code = ? and service_code = ?', [$dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no, $vill_townprt_code, $service]);
    ...
    public function pullBackCasesFromDepartmentForDCTenant()
    {
        $dist_code         = $this->session->userdata('dist_code');
        $data['dist_code'] = $dist_code;
        ...
        $data['_view'] = 'SettlementView/Dc/pull_back_cases_from_dept_tenant';
        $this->load->view('layouts/main', $data);
    }


    // Ajax for pull back case list
    public function pullBackCasesWithDeptPaginationAPITenant()
    {
        $service       = SETTLEMENT_TENANT_URBAN_ID;
    ...
    public function getPullBackCaseDetailsTenant()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
        $this->load->library('form_validation');
        ...
        else
        {
            ...
            $checkCase = $this->db->select()
                ...
                ->where('service_code', SETTLEMENT_TENANT_URBAN_ID)
                ->where('status', 'W')
    ...
    public function finalPullBackRevertToCoSubmitTenant()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
        $this->load->library('form_validation');
    ...
    public function departmentRevertedCaseToDC()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementTenantUrbanDcModel->getCasesRevertedFromDept($dist_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'SettlementView/Dc/TenantUrban/department_reverted_cases_dc';
        $this->load->view('layouts/main', $data);

    }


    // get dept reverted case details
    public function deptRevertedCaseViewDC()
    {
        $case_no   = $this->input->get('case');
        $dist_code = $this->session->userdata('dist_code');
        $this->utilityclass->checkUserAuthForCaseForDc($case_no);
        ...
        $caseCount = $this->SettlementTenantUrbanDcModel->countSettlementApplicationDetailsByRevertedCaseNo($case_no, $dist_code);
        ...
        else
        {
            ...
            $caseDetails                 = $this->SettlementTenantUrbanDcModel->getSettlementApplicationDetailsByRevertedCaseNo($case_no, $dist_code);
            $proceedings                 = $this->SettlementTenantUrbanDcModel->getSettlementProceeding($case_no);
            $data['caseCount']           = $caseCount;
            $data['caseDetails']         = $caseDetails;
            $data['proceedings']         = $proceedings;
            ...
            $data['_view'] = 'SettlementView/Dc/TenantUrban/settlement_app_details_tenant_dept_revert';
            $this->load->view('layouts/main', $data);
        }
    }
    ...
    // OLD ONE IT SHOULD BE DELETED
    public function forwardToDept()
    {
        $case_no = $this->input->get('case');
        $endDate = HOLD_CASES_FORWARD_TO_DEPT_BY_DC_TENANT_URBAN;
        $today   = date('Y-m-d H:i:s');
        if(strtotime($endDate) < strtotime($today))
        {
            $this->session->set_flashdata('message', "#MR00313: Cases cannot be sent to the department, as forwarding has been stopped...#" . $case_no);
            redirect(base_url() . 'index.php/SettlementTenantUrbanDc/paymentNoticeCasesBeforeList');
        }
        ...
        $this->db->where('case_no', $case_no);
        $this->db->update('settlement_basic', $basicStat);

        if ($this->db->affected_rows() != 1) {
            $this->db->trans_rollback();
            $this->session->set_flashdata('message', "#ERR1459: Unable to process! Something went wrong...#" . $case_no);
            redirect(base_url() . 'index.php/home');
        }
        ...
        $insertArr = [
            ...
            'date_of_hearing'      => date('Y-m-d h:i:s'),
            'next_date_of_hearing' => date('Y-m-d h:i:s'),
            'note_on_order'        => 'Case fowarded to Department',
            'status'               => 'W',
            'user_code'            => $this->session->userdata('user_code'),
            'date_entry'           => date('Y-m-d h:i:s'),
            'operation'            => 'E',
            ...
            'office_from'          => 'DC',
            'office_to'            => 'DPT',
            'task'                 => 'Case fowarded to Department',
        ];
        $insertProc = $this->db->insert('settlement_proceeding', $insertArr);
        if ($insertProc != 1) {
            $this->db->trans_rollback();
            $this->session->set_flashdata('message', "#ERR1486: Unable to process! Something went wrong...#" . $case_no);
            redirect(base_url() . 'index.php/home');
        }

        $this->db->trans_commit();
        $this->session->set_flashdata('message', "Case no " . $case_no . " successfully forwarded to department...");
        redirect(base_url() . 'index.php/SettlementTenantUrbanDc/SettlementApFirstLandDc');

    }


    // CASE FORWARD TO DEPARTMENT BY DC
    public function applicationForwardToDept()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
        $this->load->library('form_validation');
        $this->form_validation->set_rules('caseNo', 'Case Number', 'trim|required');
        ...
        if ($this->form_validation->run() == FALSE)
        {
            ...
            echo json_encode(array(
                'responseType' => 1,
                'message'  => '#ERMR002029: Validation error ! ' .$errors,
            ));
            return;
        }
        else
        {
            ...
            if(strtotime($endDate) < strtotime($today))
            {
                echo json_encode([
                    'responseType' => 1,
                    'message'  => "#MR003130: Cases cannot be sent to the department, as forwarding has been stopped... #$case_no",
                ]);
                return;
            }

            $caseCount = $this->SettlementTenantUrbanDcModel->countSettlementApplicationDetailsByCaseNoDept($case_no, $dist_code);
            if ($caseCount == 0)
            {
                echo json_encode([
                    'responseType' => 3,
                    'message'  => '#ERMR002025: Application not found !',
            ...
            $basicStat = [
                ...
                'dc_code'                  => $user_code,
                'date_update'              => date('Y-m-d H:i:s'),
                'assign_ast_code'          => NULL,
                'dept_js_approve'          => NULL,
                'js_approve_date'          => NULL,
                'ast_verification'         => NULL,
                'ast_remarks'              => NULL,
                'adlr_verification'        => NULL,
                'adlr_asign_code'          => NULL,
                'adlr_code'                => NULL,
                'adlr_remark'              => NULL,
                'aso_recommendation'       => NULL,
                'a_officer_recommendation' => NULL,
            ];
            $this->db->where('case_no', $case_no);
            $this->db->update('settlement_basic', $basicStat);
            ...
            if ($this->db->affected_rows() != 1)
            {
                ...
                echo json_encode([
                    'responseType' => 1,
                    'message'  => '#ERMR002030: There is some problem! Please contact the administration',
                ]);
                return;
            }
            ...
            $note = 'DC verification done & Recommended';
            if (!empty($remarks))
            {
                $note .= ' (' . $remarks . ')';
            }
            ...
            $insertArr = [
                ...
                'office_from'          => 'DC',
                'office_to'            => 'DPT',
                'task'                 => 'DC verification done & Recommended',
            ];
            $insertProc = $this->db->insert('settlement_proceeding', $insertArr);
            if ($insertProc != 1)
            {
                $this->db->trans_rollback();
                log_message('error', '#ERMR002031: Insertion failed in settlement_proceeding for case no :' . $case_no);
                echo json_encode([
                    'responseType' => 1,
                    'message'  => '#ERMR002031: There is some problem! Please contact the administration',
            ...
            echo json_encode([
                'responseType' => 2,
                'message'  => "Case no " . $case_no . " successfully forwarded to department...",
                'redirect' => base_url('index.php/SettlementTenantUrbanDc/SettlementApFirstLandDc'),
    ...
    // Revert from dc to co (dept reverted case)
    public function deptRevertedApplicationRevertFromDCToCO()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
        $this->load->library('form_validation');
