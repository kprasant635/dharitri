<?php
class SettlementKhasLand extends CI_Controller
{
    ...
    public function applicationKhaslandRegistration($review_flag = false) {
        ...
        if($_SERVER['REQUEST_METHOD'] != 'POST'){
            $lmdata['_view'] = 'SettlementView/SettlementKhasLandView';
            // $this->load->view('layouts/main',$district);
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            if($_POST['lm_note'] == '2')
            {
                if(isset($_POST['rejected_reasons']))
                {
                    ...
                    $validation_bypass_array = $this->getValidationBypass(SETTLEMENT_KHAS_LAND_ID);
                        ...
                        for($i = 0; $i < $fileCount; $i++)
                        {
                            ...
                            if ($this->upload->do_upload('file'))
                            {
                                $document= array(
                                    ...
                                    'mut_type'   => SETTLEMENT_KHAS_LAND_ID,
                                );
    ...
    public function secondProceeding()
    {
        ...
        if($_SERVER['REQUEST_METHOD'] != 'POST'){
            $lmdata['_view'] = 'SettlementView/Lm/Khas/SettlementKhasView';
            $this->load->view('layouts/main', $lmdata);
        }
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){     
            ...
            if($distCode == NULL)
            {
                redirect(base_url(). 'index.php/home/SettlementKhasLandLm?service='.SETTLEMENT_KHAS_LAND_ID);
            }
            if($case_no == NULL)
            {
                redirect(base_url(). 'index.php/home/SettlementKhasLandLm?service='.SETTLEMENT_KHAS_LAND_ID);
            ...
            if($validation_bypass == 0)
            {
                ...
                $sql_geo = $this->db->query('select distinct dag_no from supportive_document where applid in (?, ?) and file_name = ?', array($case_no, $application_no_geo, 'Geo Tag Photo'));
