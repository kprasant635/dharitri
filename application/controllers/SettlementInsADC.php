...
class SettlementInsADC extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('SettlementMb/SettlementMbADCInsModel');
...
//********************** COMMON **********************************
    ...
    function randomFileName()
    {
        ...
        if($this->SettlementMbADCInsModel->checkDuplicateFileNameInProposal($new_case_no) != 0)
        {
            $this->randomFileName();
        }
        else
    ...
    public function applicationRevertFromDCToCO()
    {
        ...
        else
        {
            ...
            $caseCount = $this->SettlementMbADCInsModel->countSettlementApplicationDetailsByCaseNo($case_no,$dist_code);
            ...
            else
            {
                ...
                $this->db->trans_begin();
                if($this->SettlementMbADCInsModel->updateSettlementBasicData($case_no,$dist_code,$updateData)== 0)
                {
                    $this->db->trans_rollback();
                    echo json_encode(array(
                        'responseType' => 1,
    ...
    public function applicationRejectedBySdlac()
    {
        ...
        else
        {
            ...
            $caseCount = $this->SettlementMbADCInsModel->countSettlementApplicationDetailsByCaseNoForSDLAC($case_no,$dist_code);
            if($caseCount == 0)
            {
                echo json_encode(array(
                    'responseType' => 3,
            ...
            else
            {
                ...
                else
                {
                    ...
                    $mmnn = $this->SettlementMbADCInsModel->updateSettlementAppDetailsByCaseNoUnderProposal($case_no,$updatePro);
    ...
    public function sendAllMarkAppToSDLACByDc()
    {
        ...
        else
        {
            ...
            {
                if(date('Y-m-d H:i:s',strtotime(MEETING_PROPOSAL_SDLAC_NOTICE_DATE_INSTITUTE)) < date('Y-m-d H:i:s',strtotime($hearingDate)))
                {
                    echo json_encode(array(
                        'responseType' => 4,
                        'response'     => 4,
                        'message'      => 'Maximum Date of processing '.MEETING_PROPOSAL_SDLAC_NOTICE_DATE_SHOW_INSTITUTE
                    ));
                    return;
                }
            }
            ...
            if(SELECTED_CASES_APPROVED_BY_DEPT_DC == 1)
            {
                ...
                foreach ($allSelectedList as $case_no)
                {
                    ...
                    if(trim($basic['approve_by'])  == 'GOVT')
                    {
                        $mm = 1;
                    }
                    elseif(trim($basic['approve_by'])  == 'DC')
                    {
                        $nn = 1;
                    }
                    else
                    {
                        $approvedByArray[] = $case_no;
                    }
                }
                ...
                if(count($approvedByArray) > 0)
                {
                    $case_str_app = '';
                    foreach ($approvedByArray as $err_app)
                    {
                        $case_str_app = $case_str_app.$err_app.', ';
                    }

                    echo json_encode(array(
                        'responseType' => 4,
                        'message'      => '#MREZA002: Cases .'. $case_str_app .' Remove from SDLAC/CDLAC List & Re-mark as SDLAC/CDLAC again '
                    ));
                    return;
                }
    ...
    public function markApplicationForSDLAC()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                $chithaDagFlagCheck = $this->SettlementInsModel->checkChithaFlagUpdatePremiumAreaExceptSocioCultureEduWithGovtDag($case_no);
                if($chithaDagFlagCheck['response'] != 2)
                {
                    echo json_encode(array(
                        'responseType' => 101,
                        'response'     => 101,
                        'message'      => $chithaDagFlagCheck['msg'],
                    ));
                    return false;
                }
    ...
    public function applicationApprovedBySdlac()
    {
        ...
        else
        {
            ...
            $caseCount      = $this->SettlementMbADCInsModel->countSettlementApplicationDetailsByCaseNoForSDLAC($case_no,$dist_code);
            $caseCountInPro = $this->SettlementMbADCInsModel->countSettlementAppDetailsByCaseNoUnderProposal($case_no);
            ...
            else
            {
                ...
                else
                {
                    ...
                    $this->SettlementMbADCInsModel->updateSettlementAppDetailsByCaseNoUnderProposal($case_no,$updatePro);
    ...
    public function viewPaymentReceivedAppDetailsByDc()
    {
        ...
        $dist_code = $this->session->userdata('dist_code');
        $this->utilityclass->checkUserAuthForCaseForAdc($case_no);
        $caseCount = $this->SettlementMbADCInsModel->countSettlementPaymentReceivedAppDetailsByCaseNo($case_no,$dist_code);
        ...
        {
            $caseDetails = $this->SettlementMbADCInsModel->getSettlementPaymentReceivedAppDetailsByCaseNo($case_no,$dist_code);
            $proceedings = $this->SettlementMbADCInsModel->getSettlementProceeding($case_no);
    ...
    public function updateProposalHearingDateKhas()
    {
        ...
        else
        {
            ...
            $allCases      = $this->SettlementMbADCInsModel->getAllAppInReportSendByDcToSdlacKhas($proposalNo);
    ...
    public function updateHearingDateGenerateNoticeKhas()
    {
        ...
        else
        {
            ...
            $proposalDetails = $this->SettlementMbADCInsModel->getProposalDetailsById($proposalNo,$dist_code);
            if($proposalDetails == '')
            {
                echo json_encode(array(
                    'responseType' => 3,
            ...
            else
            {
                ...
                else
                {
                    ...
                    $this->db->trans_begin();
                    if($this->SettlementMbADCInsModel->updateProposalListById($proposalNo,$updateProposalData)== 0)
                    {
                        $this->db->trans_rollback();
                        echo json_encode(array(
                            'responseType' => 1,
...
//********************** COMMON **********************************
    ...
    public function SettlementInsLandDc()
    {
        $dist_code = $this->session->userdata('dist_code');
        $user_code = $this->session->userdata('user_code');
        $user_desig_code = $this->session->userdata('user_desig_code');
        $firstProceedingCount = $this->SettlementMbADCInsModel->countAllPendingSettlementKhas($dist_code);
        $SDLACCommitteeCount  = $this->SettlementCommonDcModel->countAllSDLACCommitteeMember($dist_code,$user_code,$user_desig_code);
        $SDLACNoticeCount     = $this->SettlementMbADCInsModel->countMarkAsSDLACSettlementKhas($dist_code);
        $SDLACReportCount     = $this->SettlementMbADCInsModel->countAllProposalSendByDcToSdlacKhas($dist_code);
        $caseStatusCount      = 0;
        $sdlacMemberApproval  = $this->SettlementMbADCInsModel->countSdlacStatusList($dist_code);
        $coRejectedListCount  = $this->SettlementCommonDcModel->countCoRejectedCaseForADC($dist_code,SLIJE_ID);
        $rejctedListCount     = $this->SettlementMbADCInsModel->rejectedCaseListIns($dist_code,SLIJE_ID, MB_ADD_DEPUTY_COMM);
        $SDLACConsideration   = $this->SettlementMbADCInsModel->countAllUnderConsiderationAppKhas($dist_code);

        //        $coModificationListCount = $this->SettlementPullModel->countCoModificationRequestCaseForADC($dist_code,SLIJE_ID);
//        $revivalListCount        = $this->SettlementCommonDcModel->revivalListCount($dist_code,SLIJE_ID, MB_ADD_DEPUTY_COMM);
        ...
        $data['_view'] = 'settlementView/Adc/Ins/first_landing_page_dc_khas_ins';
        $this->load->view('layouts/main', $data);
    ...
    public function viewAllInsFirstProceedingDCCaseList()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementMbADCInsModel->getAllPendingSettlementKhas($dist_code);
        ...
        $data['_view'] = 'settlementView/Adc/Ins/first_proceeding_case_dc_khas_ins';
        $this->load->view('layouts/main', $data);
    ...
    public function getSettlementInsApplicationDetails()
    {
        $case_no = $this->input->get('case');
        $dist_code = $this->session->userdata('dist_code');
        $this->utilityclass->checkUserAuthForCaseForAdc($case_no);
        ...
        $premiumData = $this->db->query("Select * from settlement_premium where case_no='$case_no' and is_final=1")->result();
        ...
        else
        {
            ...
            $caseDetails = $this->SettlementMbADCInsModel->getSettlementApplicationDetailsByCaseNo($case_no,$dist_code);
            $proceedings = $this->SettlementMbADCInsModel->getSettlementProceeding($case_no);
            $data['caseCount']   = $caseCount;
            $data['caseDetails'] = $caseDetails;
            $data['proceedings'] = $proceedings;
            ...
            $data['additional_property'] = $this->SettlementInsModel->getAdditionalProperty($application_no);
            ...
            if (in_array($dist_code, json_decode(BARAK_VALLEY)))
            {
                ...
                if((MAX_APPLIED_ADDITIONAL_AREA_INS) * 6400 < $totalLandArea)
                {
                    $checkArea = 1;
                }
            }
            else
            {
                ...
                $totalLandArea = $totalDagAreaLessaValidation + $totalAdditionalProToLessa;
                if((MAX_APPLIED_ADDITIONAL_AREA_INS) * 100 < $totalLandArea)
                {
                    $checkArea = 1;
                }
            }
            ...
            $data['_view'] = 'settlementView/Adc/Ins/settlement_app_details_khas_ins';
            $this->load->view('layouts/main', $data);
        }
    }
    ...
    public function viewAllMarkAsSDLACListForDCKhas()
    {
        ...
        $data['location'] = $circleList;
        $data['_view']    = 'settlementView/Adc/Ins/mark_as_sdlac_case_dc_khas_ins';
        $this->load->view('layouts/main', $data);
    ...
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementMbADCInsModel->getAllProposalSendByDcToSdlacKhas($dist_code);
        ...
        $data['_view'] = 'settlementView/Adc/Ins/proposal_list_send_to_sdlac_khas_ins';
        $this->load->view('layouts/main', $data);
    }
    ...
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementMbADCInsModel->getAllUnderConSettlementKhas($dist_code);
        ...
        $data['_view'] = 'settlementView/Adc/Ins/under_consideration_case_dc_khas';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getAllApplicationInReportSendByDcToSdlacKhas()
    {
        ...
        $dist_code       = $this->session->userdata('dist_code');
        $pendingCase     = $this->SettlementMbADCInsModel->getAllAppInReportSendByDcToSdlacKhas($proposal_no);
        $proposalDetails = $this->SettlementMbADCInsModel->getProposalDetailsById($proposal_no,$dist_code);
        ...
        $data['_view'] = 'settlementView/Adc/Ins/send_to_sdlac_case_dc_khas_ins';
        $this->load->view('layouts/main', $data);
    ...
    public function generateNoticeSendAllMarkAppToSDLACByDcKhas()
    {
        ...
        else
        {
            ...
            if(!empty($allSelectedList))
            {
                ...
                // checking Chitha dag flag
                $arrayOfError = array();
                $errorStatus  = 0;
                $errorMessage = '';
                foreach ($allSelectedList as $rowNew)
                {
                    $case_no    = $rowNew;
                    ...
                    foreach ($dagDetails as $key => $value)
                    {
                        ...
                        {
                            $errorStatus    = 1;
                            $arrayOfError[] = $case_no;
                            $errorMessage   = "Chitha dag flag not found, please flag the dag using chitha dag flag module before proceedings ";
                        }
                    }
                }
                if($errorStatus == 1)
                {
                    if(count($arrayOfError) > 0)
                    {
                        $case_str = '';
                        foreach ($arrayOfError as $err)
                        {
                            $case_str = $case_str.$err.',';
                        }
                    }
                    echo json_encode(array(
                        'responseType' => 101,
                        'response'     => 101,
                        'message'      => $errorMessage . ' for Case No. '. $case_str,
                    ));
                    return false;
                }
                ...
                // checking Govt dag
                $arrayOfErrorGovt = array();
                $errorStatusGovt  = 0;
                $errorMessageGovt = '';
                foreach ($allSelectedList as $rowNewNew)
                {
                    $case_no    = $rowNewNew;
                    $dagDetailsGovt = $this->db->query("select * from settlement_dag_details where case_no = ?",array($case_no))->result();
                    foreach ($dagDetailsGovt as $key => $value)
                    {
                        ...
                        {
                            $errorStatusGovt    = 1;
                            $arrayOfErrorGovt[] = $case_no;
                            $errorMessageGovt   = "DAG is not classified under GOVT-LAND therefore, the application has been placed under reservation.";
                        }
                    }
                }
                if($errorStatusGovt == 1)
                {
                    if(count($arrayOfErrorGovt) > 0)
                    {
                        $case_str_govt = '';
                        foreach ($arrayOfErrorGovt as $err)
                        {
                            $case_str_govt = $case_str_govt.$err.',';
                        }
                    }
                    echo json_encode(array(
                        'responseType' => 101,
                        'response'     => 101,
                        'message'      => $errorMessageGovt . ' for Case No. '. $case_str_govt,
                    ));
                    return false;
                }
                ...
                $this->db->trans_begin();
                if($this->SettlementMbADCInsModel->saveProposalSDLACKhas($dataProSave) == 0)
                {
                    $this->db->trans_rollback();
                ...
                else
                {
                    ...
                    foreach ($allSelectedList as $row)
                    {
                        ...
                        // check chitha dag flag
                        $chithaDagFlagCheck = $this->SettlementInsModel->checkChithaFlagUpdatePremiumAreaExceptSocioCultureEduWithoutTran($case_no);
                        if($chithaDagFlagCheck['response'] != 2)
                        {
                            echo json_encode(array(
                                'responseType' => 101,
                                'response'     => 101,
                                'message'      => $chithaDagFlagCheck['msg'] . ' for Case No. '. $case_no,
                            ));
                            return false;
                        }
                        ...
                        if($this->SettlementMbADCInsModel->saveProposalCaseListSDLACKhas($saveCaseList) == 0)
                        {
//                            $this->SettlementMbADCInsModel->deleteProposalSDLAC($proposalId);
    ...
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementMbADCInsModel->getAllApproveAppBySdlacKhas($dist_code);
    ...
    public function viewApprovedAppDetailsKhas()
    {
        ...
        $caseCount = $this->SettlementMbADCInsModel->countSettlementAppDetailsByCaseNoOnlyView($case_no,$dist_code);
        if($caseCount == 0)
        {
            $this->SettlementInsLandDc();
        }
        else
        {
            ...
            $caseDetails = $this->SettlementMbADCInsModel->getSettlementAppDetailsByCaseNoOnlyView($case_no,$dist_code);
            $proceedings = $this->SettlementMbADCInsModel->getSettlementProceeding($case_no);
            $data['caseCount']   = $caseCount;
            $data['caseDetails'] = $caseDetails;
            $data['proceedings'] = $proceedings;
    ...
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementMbADCInsModel->getAllRejectAppByDcKhas($dist_code);
    ...
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementMbADCInsModel->getAllOrderChithaUpdateAppKhas($dist_code);
    ...
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementMbADCInsModel->getReRevertedByCoApplicationKhas($dist_code);
    ...
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementMbADCInsModel->getRevertedByDeptApplicationKhas($dist_code);
    ...
    public function applicationPaymentGenerateDcKhas()
    {
        ...
        else
        {
            ...
            $caseCount = $this->SettlementMbADCInsModel->caseForDcApprovalKhas($case_no,$dist_code);
            if($caseCount == 0)
            {
                echo json_encode(array(
                    'responseType' => 3,
    ...
    {
        $dist_code   = $this->session->userdata('dist_code');
        $allCase = $this->SettlementMbADCInsModel->getAllCasesForFinalVerifyAppKhas($dist_code);
        ...
        else
        {
            ...
            foreach ($cases as $case)
            {
                ...
                if($case->case_status == 1)
                {
                    ...
                    if($this->SettlementMbADCInsModel->updateSettlementAppDetailsByCaseNoUnderProposal($caseNo,$updateCase)== 0)
                    {
                        $this->db->trans_rollback();
                    ...
                    if($this->SettlementMbADCInsModel->updateSettlementBasicData($caseNo,$dist_code,$updateBasicData)== 0)
                    {
                        $this->db->trans_rollback();
                ...
                if($case->case_status == 2)
                {
                    ...
                    else
                    {
                        ...
                        $mmnn = $this->SettlementMbADCInsModel->updateSettlementAppDetailsByCaseNoUnderProposal($caseNo,$updatePro);
            ...
            if($this->SettlementMbADCInsModel->updateProposalListById($proposal_no,$dataUpdate)== 0)
            {
                $this->db->trans_rollback();
    ...
    public function InsfirstProceedingPaginationAPI()
    {
        $service     = SLIJE_ID;
        ...
        if(!empty($ins_cat)){  //settlement_ap_lmnote, lm_note
            $this->db->where('settlement_institution_details.ins_cat_type_co', $ins_cat);
        }
        if(!empty($already_alloted)){  //settlement_ap_lmnote, lm_note
            $this->db->where('settlement_ap_lmnote.already_alloted', $already_alloted);
        }

        $this->db->select('*');
        $this->db->from('settlement_basic');
        $this->db->join('settlement_ap_lmnote', 'settlement_basic.case_no = settlement_ap_lmnote.case_no');
        $this->db->join('settlement_institution_details', 'settlement_basic.case_no = settlement_institution_details.case_no');
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows) {
                ...
                {
                    $approved_by = "<span style='color:gray'>NA</span>";
                }
                ...
                $json[] = array(
                    ...
                    $this->utilityclass->ins_cat_name($rows->dist_code,$rows->ins_cat_type_co),

                    $typeCat,

                    $approved_by,

                    '<a class="btn btn-success" href="'.base_url().'index.php/SettlementInsADC/getSettlementInsApplicationDetails/?case='.$rows->case_no.'">
    ...
    public function secondProceedingSdlacRecommendedMarked()
    {
        ...
        if(!empty($approvedBy))
        {
            ...
            {
                $this->db->where("(trim(settlement_basic.approve_by)='GOVT')" );
            }
            if($approvedBy == 2) // DC
            {
                $this->db->where("(trim(settlement_basic.approve_by)='DC')" );
            }
        }
        $this->db->limit($length, $start);
        $query = $this->db->get();
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows)
            {
                ...
                {
                    $approved_by = "<span style='color:green'>Department</span>";
                }
                else if($rows->approve_by == 'DC' || $approvedBy == 2)
                {
                    $approved_by = "<span style='color:blue'>DC</span>";
                }
                else
                {
                    $approved_by = "<span style='color:red'>Please Remove From SDLAC/CDLAC Marked </span>";
                }


                $this->utilityclass->ins_cat_name($rows->dist_code,$rows->ins_cat_type_co);
                $typeCat = 'Allotment';
                if($rows->ins_cat_type_co == 10 || $rows->ins_cat_type_co == 11)
                {
                    $typeCat = 'Land Transfer';

                }
                else if(($rows->ins_cat_type_co == 8 || $rows->ins_cat_type_co == 9 || $rows->ins_cat_type_co == 12 ) && $rows->already_alloted == 'Y')
                {
                    $typeCat = 'Settlement';
                }
                $json[] = array(
                    ...
                    '<a class="btn btn-success btn-sm" href="'.base_url().'index.php/SettlementInsADC/getSettlementInsApplicationDetails/?case='.$rows->case_no.'">View Application</a>'
                );
    ...
    //pagination of third proceeding SDLAC/CDLAC Report
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows) {
                ...
                $json[] = array (
                    ...
                    <a class="btn btn-sm btn-dark" target= "SDLACProposalNotice" href="'.base_url().'index.php/SettlementCommonIns/downloadCasesWithProposalId/?case='.$rows->id.'">Download</a>
       
                    <a class="btn btn-sm btn-success" href="'.base_url().'index.php/SettlementInsADC/getAllApplicationInReportSendByDcToSdlacKhas/?case='.$rows->id.'">
                      '.$this->lang->line('process').'</a>'
    ...
    public function checkForSdlacStatus()
    {
        ...
        $insCategory = $this->input->post('allInsData');
        $dist_code   = $this->session->userdata('dist_code');
        ...
        foreach ($insCategory as $case)
        {
            // check chitha dag flag
            $chithaDagFlagCheck = $this->SettlementInsModel->checkChithaFlagUpdatePremiumAreaExceptSocioCultureEduWithGovtDag($case['case_no']);
            if($chithaDagFlagCheck['response'] != 2)
            {
                echo json_encode(array(
                    'response'     => 1,
                    'message'      => $chithaDagFlagCheck['msg'].' for Case No '. $case['case_no'],
                ));
                return false;
            }
            if($case['ins_cat_type_co_id'] == 12)
            {
                if(isset($case['institution_falls_ngo']))
                {
                    if($case['institution_falls_ngo'] == '' || $case['institution_falls_ngo'] == NULL)
                    {
                        $json = [
                            'response' => 1,
                            'message'  => 'Institution falls under category of NGOs, Trusts, Local Bodies,  Associations, Societies ?
                                       Not Set for Case No '.$case['case_no'],
                        ];
                        echo json_encode($json);
                        return false;
                    }
                ...
                if($case['purpose_land_allot_co_id'] == 'Educational')
                {
                    if (isset($case['institution_venture_school']))
                    {
                        if ($case['institution_venture_school'] == '' || $case['institution_venture_school'] == NULL) {
                            $json = [
                                'response' => 1,
                                'message' => 'Is the Educational Institution a Non Provincialised Venture School  ?
                                       Not Set for Case No ' . $case['case_no'],
                            ];
                            echo json_encode($json);
                            return false;
    ...
    public function sdlacReportOnlineApprove()
    {
        ...
        if(count($detail) == 0)
        {
            echo json_encode(array(
                'response' => 1,
                'message'  => '#ERMR2496: There is no case found. Kindly contact system administrator',
        ...
        foreach($detail as $row)
        {
            ...
            if(trim($row['ins_cat_type_co']) == 12)
            {
                if(trim($row['institution_falls_ngo']) == '' || trim($row['institution_falls_ngo']) == NULL)
                {
                    $this->db->trans_rollback();
                    $json = [
                        'response' => 1,
                        'message'  => 'Institution falls under category of NGOs, Trusts, Local Bodies, Associations, Societies Field Empty',
                    ];
                    echo json_encode($json);
                    return;
                }
                ...
                $updateInsDetails = [
                    'under_ngo_trust_localbodies' =>  trim($row['institution_falls_ngo']),
                ];

                if(trim($row['purpose_land_allot_co']) == 'Educational')
                {
                    if(trim($row['institution_venture_school']) == '' || trim($row['institution_venture_school']) == NULL)
                    {
                        $this->db->trans_rollback();
                        $json = [
                            'response' => 1,
                            'message'  => 'Is the Educational Institution a Non Provincialised Venture School Field Empty',
                        ];
                        echo json_encode($json);
                        return;
                    }
                    ...
                    $updateInsDetails = [
                        'under_ngo_trust_localbodies' =>  trim($row['institution_falls_ngo']),
                        'under_venture_school'        =>  trim($row['institution_venture_school']),
                    ];
                }

                $this->db->where('case_no', trim($row['case_no']));
                $this->db->update('settlement_institution_details', $updateInsDetails);
                if($this->db->affected_rows() <= 0 )
                {
                    $this->db->trans_rollback();
                    log_message('error', 'Updation failed in settlement_institution_details '.$this->db->last_query());
                    $json = [
                        'response' => 1,
                        'message'  => 'Case can not be forwarded to SDLAC. Kindly contact system administrator',
                    ];
        ...
        else
        {
            ...
            $json = [
                'response' => 2,
                'message'  => 'This Proposal would be available under process (MB3) SDLAC/CDLAC Minutes',
            ];
            echo json_encode($json);
        }
    }
    ...
    public function getAllSdlacMemberApprovalProposalListKhas()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $subdiv_code = $this->session->userdata('subdiv_code');
        $pendingCase = $this->SettlementMbADCInsModel->getSdlacApprovalProposalListKhas($dist_code);
    ...
    public function getSdlacMemberApproveProposalViewIndividualKhas()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $proposal_no = $this->input->get('case');
        $proposalDetails  = $this->SettlementMbADCInsModel->getSdlacApprovalProposalIndividualKhas($proposal_no,$dist_code);
        $reportDetails    = $this->SettlementMbADCInsModel->getSdlacMemberReportDetailsKhas($proposal_no,$dist_code);
        $getMembersStatus = $this->SettlementMbADCInsModel->getSdlacMemberStatus($dist_code, $proposal_no);
