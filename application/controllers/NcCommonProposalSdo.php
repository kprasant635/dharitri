...
class NcCommonProposalSdo extends CI_Controller
{
    ...
    // NC code by Masud Reza (15/02/2024)
    ...
    public function commonProposalListViewSdo()
    {
        ...
        // $pendingCase = $this->NcCommonSdoAdcDcModel->getPendingProposalsOfCommon($dist_code,$subdiv_code,$createdBy);
        // $data['cases'] = $pendingCase->result();
        // $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['countPendingCase'] = $this->NcCommonSdoAdcDcModel->getPendingProposalsReadyForMeeting($dist_code,MB_SUB_DIV_COMM);
        ...
        $data['_view'] = 'NcVillageService/NcProposalSdo/common_proposal_list_forward_to_sdo';
        $this->load->view('layouts/main', $data);
    }


    // list of common proposals for all services
    public function listOfProposalsAllServicesSdo()
    {
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows) {
                ...
                <div class="text-green" id="list_of_cases_'.$rows->id.'"></div>',
                    ...
                    <a class="btn btn-xs btn-success" style=" padding: 7px!important; margin-bottom: 5px!important;" target= "SDLACProposalNotice" href="'.base_url().'index.php/NcCommonProposalSdo/proposalEditOnSdlacMinutesSdo/?case='.$rows->id.'">
    ...
    public function proposalEditOnSdlacMinutesSdo()
    {
        ...
        $proposalDetails = $this->NcCommonSdoAdcDcModel->getRevertedProposalDetailsById($proposal_no,$dist_code,$subDiv_code);

        if($proposalDetails->meeting_create_status != 1)
        {
            $this->session->set_flashdata('error', "Proposal already add in Meeting !");
            redirect(base_url() . "index.php/NcCommonProposalSdo/listOfProposalsAllServicesSdo");
        }
        ...
        $data['_view'] = 'NcVillageService/NcProposalSdo/edit_proposal_from_sdlac_minutes_sdo';
        $this->load->view('layouts/main', $data);
    ...
    public function applicationRevertToCoFromProposalUnderMinutes()
    {
        ...
        else
        {
            ...
            if($this->NcCommonSdoAdcDcModel->updateNcBasicDataSdo($case_no,$dist_code,$subDiv_code,$caseDetails->service_code,$updateData)== 0)
            {
                $this->db->trans_rollback();
                echo json_encode(array(
                    'responseType' => 1,
    ...
    public function saveCcDataNc()
    {
        ...
        if(count($municipal_board) == 0)
        {
            ...
            redirect(base_url() . "index.php/NcCommonProposalSdo/addEditCopyToNc");
            return false;
        }
    ...
    public function sendProposalsToDcMinute()
    {
        ...
        $userMp = $this->NcCommonSdoAdcDcModel->getUsersMpCopyToForSdo($dist_code, $subdiv_code, $proposalCreatedBy);
        $userMpCount = $userMp->num_rows();
        $userMp = $userMp->result();

        $userMla = $this->NcCommonSdoAdcDcModel->getUsersMlaCopyToForSdo($dist_code, $subdiv_code, $proposalCreatedBy);
        $userMlaCount = $userMla->num_rows();
        $userMla = $userMla->result();

        $userSdlac = $this->NcCommonSdoAdcDcModel->getUsersSdlacCopyToForSdo($dist_code, $subdiv_code, $proposalCreatedBy);
    ...
    public function forwardedMeetingListForSdo()
    {
        ...
        $pendingCase              = $this->NcCommonSdoModel->getForwardedMeetingListSdo($dist_code,$subdiv_code,$createdBy);
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'NcVillageService/NcProposalSdo/forwarded_meeting_list_sdo';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getForwardedMeetingDetailsByIdSdo()
    {
        ...
        $meeting = $this->NcCommonSdoModel->getForwardedMeetingDetailByMeetingID(
        ...
        if($meetingName == '')
        {
            ...
            redirect(base_url() . "index.php/NcCommonProposalSdo/forwardedMeetingListForSdo");
        }

        //list of proposals against meeting id
        $proposalDetail = $this->NcCommonSdoModel->getProposalDetailAgainstMeetingIdForSdo($meetingId,
            $dist_code,$subdiv_code,$createdBy)->result();

        //list of SDLAC/CDLAC Member report
        $sdlacReport = $this->NcCommonSdoModel->sdlacMemberReportDetail($dist_code,
            $meetingId)->result();

        $additionalDoc = $this->NcCommonSdoModel->getMeetingAdditionalDocumentDetail($meetingName)->result();
        ...
        $data['_view'] = 'NcVillageService/NcProposalSdo/forwarded_proposals_against_meeting_id';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function revertMeetingListForSdo()
    {
        ...
        $pendingCase              = $this->NcCommonSdoAdcDcModel->getRevertedMeetingListSDO($dist_code,$subdiv_code,$createdBy);
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();
        $data['dist_code']        = $dist_code;

        $data['_view'] = 'NcVillageService/NcProposalSdo/reverted_meeting_list_sdo';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getProposalUnderRevertedMeetingForSdo()
    {
        ...
        $pendingMeeting      = $this->NcCommonSdoAdcDcModel->checkRevertedMeetingExistOrNotWithMeetingId($meetingId,$dist_code,$subdiv_code);
        ...
        if($pendingMeetingCount == 0)
        {
            ...
            redirect(base_url() . "index.php/NcCommonProposalSdo/revertMeetingListForSdo");
        ...
        else
        {
            ...
            $data['_view'] = 'NcVillageService/NcProposalSdo/reverted_proposal_by_dc_list_sdo';
            $this->load->view('layouts/main', $data);
        }
    }
    ...
    public function caseListUnderRevertedMeetingForSdo()
    {
        ...
        $data['_view'] = 'NcVillageService/NcProposalSdo/reverted_application_by_proposal_sdo';
        $this->load->view('layouts/main', $data);
    ...
    public function verifyCasesUnderRevertedMeetingByDc()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                $getProposalsList = $this->NcCommonSdoModel->getProposalDetailAgainstMeetingIdForSdo($meetingId,$dist_code,$subdiv_code,$createdBy)->result();
...
//                                     Cases not found for proposal id '.$proposal_no,
//                        ));
//                        return;
//                    }
//                    else
...
//                    }
    ...
    public function deleteProposalWithZeroCasesBySdo()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
