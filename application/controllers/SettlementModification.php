<?php
class SettlementModification extends CI_Controller
{
    ...
    // checking login user access all 3
    ...
    // checking login user access only DC
    public function checkingLoginUserAccessDcUser()
    {
        $allowed = ['DC'];
        $user_desig_code = $this->session->userdata('user_desig_code');
        if(! in_array($user_desig_code, $allowed))
    ...
    // checking login user access only ADC
    public function checkingLoginUserAccessAdcUser()
    {
        $allowed = ['DC','ADC'];
        $user_desig_code = $this->session->userdata('user_desig_code');
        if(! in_array($user_desig_code, $allowed))
    ...
    // checking login user access only CO
    public function checkingLoginUserAccessCoUser()
    {
        $allowed = ['CO'];
        $user_desig_code = $this->session->userdata('user_desig_code');
        if(! in_array($user_desig_code, $allowed))
    ...
    public function caseListForPullRequest()
    {
        ...
        if ($this->session->userdata('user_desig_code') == 'CO')
        {
            ...
            $status = 'Z';
            $data['select_data'] = $this->SettlementPullModel->locationSelectPullRequest($service_code, $status);

            $data['_view'] = 'SettlementView/PullRequest/settlement_mb_pull_request_co';
    ...
    {
        $this->checkingLoginUserAccessCoUser();
        ...
        $this->db->where_in('a.service_code',MODIFICATION_REQUEST_SERVICE_CODE);
        $this->db->where_in('a.case_no',MODIFICATION_REQUEST_ENABLE_FOR_CASES);
        ...
        if ($query->num_rows() > 0)
        {
            foreach ($query->result() as $rows)
            {
                if($this->session->userdata('user_desig_code') == 'CO')
                {
                    $tenant_link = '<a  type="button" href="#" onclick="openModalForFlag(\''.$rows->case_no.'\',\''.$rows->applid.'\')" class="btn-sm btn btn-primary">
                        <i class="fa fa-arrow-right" aria-hidden="true"></i> Modification Request</a>                          
                        <a type="button" href="' . base_url() . 'index.php/SettlementCommonDc/viewApplicationDetailsOnly/?case=' . $rows->case_no . '" class=" btn-sm btn btn-success">
                    ...
                    $ap_link = '<a  type="button" href="#" onclick="openModalForFlag(\''.$rows->case_no.'\',\''.$rows->applid.'\')" class="btn-sm btn btn-primary">
                        <i class="fa fa-arrow-right" aria-hidden="true"></i> Modification Request</a>
                        <a type="button" href="' . base_url() . 'index.php/SettlementCommonDc/viewApplicationDetailsOnly/?case=' . $rows->case_no . '" class=" btn-sm btn btn-success">
                        <i class="fa fa-eye" aria-hidden="true"></i> View</a>';
                    ...
                    $vgr_link = '<a  type="button" href="#" onclick="openModalForFlag(\''.$rows->case_no.'\',\''.$rows->applid.'\')" class="btn-sm btn btn-primary">
                        <i class="fa fa-arrow-right" aria-hidden="true"></i> Modification Request</a>
                        <a type="button" href="' . base_url() . 'index.php/SettlementCommonDc/viewApplicationDetailsOnly/?case=' . $rows->case_no . '" class=" btn-sm btn btn-success">
                        <i class="fa fa-eye" aria-hidden="true"></i> View</a>';
                    ...
                    $tea_link = '<a  type="button" href="#" onclick="openModalForFlag(\''.$rows->case_no.'\',\''.$rows->applid.'\')" class="btn-sm btn btn-primary">
                        <i class="fa fa-arrow-right" aria-hidden="true"></i> Modification Request</a>
                        <a type="button" href="' . base_url() . 'index.php/SettlementCommonDc/viewApplicationDetailsOnly/?case=' . $rows->case_no . '" class=" btn-sm btn btn-primary">
                        <i class="fa fa-eye" aria-hidden="true"></i> View</a>';
    ...
    public function checkWhetherPullRequestByCO()
    {
        ...
        if($case_no == null || $applid == null || $co_remarks == null){
            echo json_encode([
                'responseType' => 1,
                'message' => '#MRPULL0055: Modification request cancelled...!case no missing/No Remarks ',
            ]);
            return false;
        }

        if(!in_array($case_no, MODIFICATION_REQUEST_ENABLE_FOR_CASES))
        {
            echo json_encode([
                'responseType' => 1,
                'message' => '#MRPULL0056: This application is not permitted for modification requests. !',
            ]);
            return false;
        }


        if(!in_array($this->session->userdata('dist_code'), MODIFICATION_REQUEST_ENABLE_FOR_DIST_CODE) ||
            !in_array($this->session->userdata('cir_code'), MODIFICATION_REQUEST_ENABLE_FOR_CIR_CODE))
        {
            echo json_encode([
                'responseType' => 1,
                'message' => '#MRPULL0057: You are not authorized to perform this process.. !',
            ]);
            return false;
        }
        ...
        $pullReqCheck = $this->SettlementPullModel->checkModificationRequestAlreadyExist($case_no,$dist_code);
...
//        }
    ...
    public function caseListForModificationRequestedByCo()
    {
        ...
        if ($this->session->userdata('user_desig_code') == 'CO')
        {
            ...
            $data['_view'] = 'SettlementView/PullRequest/all_pull_requested_cases_by_co';
            $this->load->view('layouts/main', $data);
        }
        else
        {
    ...
    public function paginationForModificationRequestedByCo()
    {
        ...
        if ($query->num_rows() > 0)
        {
            foreach ($query->result() as $rows)
            {
                if($this->session->userdata('user_desig_code') == 'CO')
                {
                    $tenant_link = '<a style="margin-top: 5px" type="button" href="' . base_url() . 'index.php/SettlementCommonDc/viewApplicationDetailsOnly/?case=' . $rows->case_no . '" class=" btn-sm btn btn-success">
                        <i class="fa fa-eye" aria-hidden="true"></i> View Case</a>';

                    $tribal_link = '<a style="margin-top: 5px" type="button" href="' . base_url() . 'index.php/SettlementCommonDc/viewApplicationDetailsOnly/?case=' . $rows->case_no . '" class=" btn-sm btn btn-success">
                        <i class="fa fa-eye" aria-hidden="true"></i> View Case</a>';

                    $ap_link = '<a style="margin-top: 5px" type="button" href="' . base_url() . 'index.php/SettlementCommonDc/viewApplicationDetailsOnly/?case=' . $rows->case_no . '" class=" btn-sm btn btn-success">
                        <i class="fa fa-eye" aria-hidden="true"></i> View Case</a>';

                    $khas_link = '<a style="margin-top: 5px" type="button" href="' . base_url() . 'index.php/SettlementCommonDc/viewApplicationDetailsOnly/?case=' . $rows->case_no . '" class=" btn-sm btn btn-success">
                        <i class="fa fa-eye" aria-hidden="true"></i> View Case</a>';

                    $vgr_link = '<a style="margin-top: 5px" type="button" href="' . base_url() . 'index.php/SettlementCommonDc/viewApplicationDetailsOnly/?case=' . $rows->case_no . '" class=" btn-sm btn btn-success">
                        <i class="fa fa-eye" aria-hidden="true"></i> View Case</a>';

                    $tea_link = '<a style="margin-top: 5px" type="button" href="' . base_url() . 'index.php/SettlementCommonDc/viewApplicationDetailsOnly/?case=' . $rows->case_no . '" class=" btn-sm btn btn-primary">
                        <i class="fa fa-eye" aria-hidden="true"></i> View Case</a>';
    ...
    public function modificationRequestReject()
    {
        ...
        $requested = $this->SettlementPullModel->getModificationRequestCaseDetailsForCommon
    ...
    public function modificationRequestForwardToDC()
    {
        ...
        if(!in_array($case_no, MODIFICATION_REQUEST_ENABLE_FOR_CASES))
        {
            $this->session->set_flashdata('message', " This application is not permitted for modification requests ! ");
            redirect(base_url() . 'index.php/Home/index');
        }
    ...
    // modification request accept by ADC only
        ...
        elseif(in_array($user_desig_code,$userAccess))
        {
            ...
            if($caseIdSdlacProposal != 0)
            {
                if(trim($caseDetails->pending_officer) == $user_desig_code)
                {
                    ...
                    $proposalDetails = $this->SettlementPullModel->getProposalDetailsByProIdPull(trim($getProposalID->proposal_id));
                    if(trim($proposalDetails->proposal_meeting_id) != '')
                    {
                        $caseInMeeting = $this->SettlementPullModel->countMeetingDetailByMeetingIDPull(trim($proposalDetails->proposal_meeting_id));
                        if($caseInMeeting != 0)
                        {
                            $acceptButtAccess = 0;
                        }
            ...
            {
                $clustSql = $this->db->query('select * from settlement_circle_cluster_cases where case_no = ?', array($case_no));
                if($clustSql->num_rows() > 0)
                {
                    $cluster_cases = $clustSql->result();
                    foreach ($cluster_cases as $cluster_case)
                    {
                        $deleteClusterCase = $this->SettlementPullModel->deleteClusterCaseDetailsByClusterId($cluster_case->id);
                        if($deleteClusterCase != 1)
                        {
                            $this->db->trans_rollback();
                            log_message('error', '#MRVGR001: Deletion failed in settlement_circle_cluster_cases for case no :'. $case_no);
                            echo json_encode(array(
                                'responseType' => 1,
                                'message'  => '#MRVGR001: Accept request cancelled ! Kindly contact system administrator',
                            ));
                            return false;
                        }
                    }
                ...
                $checkIfRevertedCSql = $this->db->query('select * from settlement_vgr_pgr_revert_cases where case_no = ?', array($case_no));
                if($checkIfRevertedCSql->num_rows > 0)
                {
                    $this->db->query('delete from settlement_vgr_pgr_revert_cases where case_no = ?', array($case_no));
                    if($this->db->affected_rows() != 1)
                    {
                        $this->db->trans_rollback();
                        log_message('error', '#MRVGR002: Deletion failed in settlement_vgr_pgr_revert_cases for case no :'. $case_no);
                        echo json_encode(array(
                            'responseType' => 1,
                            'message'  => '#MRVGR002: Accept request cancelled ! Kindly contact system administrator',
                        ));
                        return false;
                    }
                }
    ...
    public function getAllModificationRequestApplicationByCoForAdc()
    {
        ...
        $this->checkingLoginUserAccessAdcUser();
        ...
        {
            $data['select_data'] = $this->SettlementPullModel->locationSelectPullRequestWithOutStatusADC($service_code);
            $data['application'] = 1;
        ...
        $data['_view'] = 'SettlementView/PullRequest/modification_requested_by_co_list_for_adc';
        $this->load->view('layouts/main', $data);
        ...
        if ($query->num_rows() > 0)
        {
            foreach ($query->result() as $rows)
            {
                $tenant_link = '<a type="button" target="_blank" href="' . base_url() . 'index.php/SettlementCommonDc/viewApplicationDetailsOnly/?case=' . $rows->case_no . '" class="lmreportmut btn btn-primary">
                        ...
                        <a type="button" target="_blank" href="' . base_url() . 'index.php/SettlementModification/getModificationRequestApplicationDetailsForAdc/?case=' . $rows->case_no . '" class="lmreportmut btn btn-success">
    ...
    public function getModificationRequestApplicationDetailsForAdc()
    {
        ...
        else
        {
            ...
            $requested = $this->SettlementPullModel->getModificationRequestCaseDetailsForAdc($dist_code,$service_code,$case_no);
            if($requested->num_rows() == 0)
            {
                $this->session->set_flashdata('message', " Case not found ! ");
                redirect(base_url() . 'index.php/Home/index');
            ...
            if($caseIdSdlacProposal != 0)
            {
                ...
                if(trim($proposalDetails->proposal_meeting_id) != '')
                {
                    ...
                    {
                        $caseInMeeting = 1;
                        $meetingDetails  = $this->SettlementPullModel->getMeetingDetailByMeetingIDPull(trim($proposalDetails->proposal_meeting_id));
                    }
                }
            }
            ...
            $forwardButtAccess = 0;
            $acceptButtAccess  = 0;
            $userAccess = [MB_DEPUTY_COMM,MB_ADD_DEPUTY_COMM];
            if(in_array($user_desig_code,$userAccess))
            {
                $rejectButtAccess = 1;
            ...
            $data['_view'] = 'SettlementView/PullRequest/modification_requested_case_details_for_adc';
            $this->load->view('layouts/main', $data);
    ...
    public function getAllModificationRequestApplicationByCoForDc()
    {
        ...
        $this->checkingLoginUserAccessDcUser();
        $data['select_data'] = $this->SettlementPullModel->locationSelectPullRequestWithOutStatusDC();
        $data['application'] = 1;

        $data['_view'] = 'SettlementView/PullRequest/modification_requested_by_co_list_for_dc';
        $this->load->view('layouts/main', $data);
    ...
    public function paginationForModificationRequestApplicationByCoForDc()
    {
        ...
        if ($query->num_rows() > 0)
        {
            foreach ($query->result() as $rows)
            {
                $link = '<a type="button" target="_blank" href="' . base_url() . 'index.php/SettlementCommonDc/viewApplicationDetailsOnly/?case=' . $rows->case_no . '" class="lmreportmut btn btn-primary">
                        <i class="fa fa-eye"></i> View Case</a>
                        
                        <a type="button" target="_blank" href="' . base_url() . 'index.php/SettlementModification/getModificationRequestApplicationDetailsForDc/?case=' . $rows->case_no . '" class="lmreportmut btn btn-success">
                        <i class="fa fa-step-forward"></i> Process</a>
               ';
    ...
    public function getModificationRequestApplicationDetailsForDc()
    {
        ...
        else
        {
            ...
            {
                $this->session->set_userdata('message', "Modification request already processed ! ");
                redirect(base_url().'index.php/SettlementModification/getAllModificationRequestApplicationByCoForDc?service='.$service_code);
            }

            $requested = $this->SettlementPullModel->getModificationRequestCaseDetailsForDc($dist_code,$service_code,$case_no);
            if($requested->num_rows() == 0)
            {
                $this->session->set_flashdata('message', " Case not found ! ");
                redirect(base_url() . 'index.php/Home/index');
            ...
            $data['_view'] = 'SettlementView/PullRequest/modification_requested_case_details_for_dc';
            $this->load->view('layouts/main', $data);
        }
    }
        ...
        if($service_code == SETTLEMENT_PGR_VGR_LAND_ID)
        {
            ...
            if($clustSql->num_rows() > 0)
            {
                ...
                foreach ($cluster_cases as $cluster_case)
                {
                    ...
                    {
                        $this->db->trans_rollback();
                        log_message('error', '#MRVGR003: Deletion failed in settlement_circle_cluster_cases for case no :'. $case_no);
                        echo json_encode(array(
                            'responseType' => 1,
                            'message'  => '#MRVGR003: Accept request cancelled ! Kindly contact system administrator',
                        ));
                        return false;
                    }
                }
            ...
            if($checkIfRevertedCSql->num_rows > 0)
            {
                ...
                {
                    $this->db->trans_rollback();
                    log_message('error', '#MRVGR004: Deletion failed in settlement_vgr_pgr_revert_cases for case no :'. $case_no);
                    echo json_encode(array(
                        'responseType' => 1,
                        'message'  => '#MRVGR004: Accept request cancelled ! Kindly contact system administrator',
                    ));
                    return false;
                }
            }
    ...
    public function getAllModificationRequestApplicationByCoForSdo()
    {
        ...
        {
            $data['select_data'] = $this->SettlementPullModel->locationSelectPullRequestWithOutStatusSDO($service_code);
            $data['application'] = 1;
            if($service_code == SETTLEMENT_TENANT_ID)
            {
                $data['service_name'] = 'Settlement Occupancy Tenant';
        ...
        $data['_view'] = 'SettlementView/PullRequest/modification_requested_by_co_list_for_sdo';
        $this->load->view('layouts/main', $data);
        ...
        if ($query->num_rows() > 0)
        {
            foreach ($query->result() as $rows)
            {
                $tenant_link = '<a type="button" target="_blank" href="' . base_url() . 'index.php/SettlementCommonDc/viewApplicationDetailsOnly/?case=' . $rows->case_no . '" class="lmreportmut btn btn-primary">
                        ...
                        <a type="button" target="_blank" href="' . base_url() . 'index.php/SettlementModification/getModificationRequestApplicationDetailsForSdo/?case=' . $rows->case_no . '" class="lmreportmut btn btn-success">
                        <i class="fa fa-step-forward"></i> Process</a>
                        ';
                $tribal_link = '<a type="button" target="_blank" href="' . base_url() . 'index.php/SettlementCommonDc/viewApplicationDetailsOnly/?case=' . $rows->case_no . '" class="lmreportmut btn btn-primary">
                        <i class="fa fa-eye"></i> View Case</a>
    ...
    public function getModificationRequestApplicationDetailsForSdo()
    {
        ...
        else
        {
            ...
            $requested = $this->SettlementPullModel->getModificationRequestCaseDetailsForSdo($dist_code,$subdiv_code,$service_code,$case_no);
            if($requested->num_rows() == 0)
            {
                $this->session->set_flashdata('message', " Case not found ! ");
                redirect(base_url() . 'index.php/Home/index');
            ...
            $data['_view'] = 'SettlementView/PullRequest/modification_requested_case_details_for_sdo';
            $this->load->view('layouts/main', $data);
