...
class SettlementProposalControllerReclass extends CI_Controller
{
    ...
    public function commonProposalListView()
    {
        ...
        $pendingCase              = $this->SettlementCommonDcModel->getPendingProposalsReCla($dist_code,$createdBy);
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();
        $data['countPendingCase'] = $this->ReclassCommonDcModel->getPendingProposalsReadyForMeeting($dist_code,MB_ADD_DEPUTY_COMM);
        ...
        $data['_view'] = 'reclass_suite/Adc/common_proposal_list_forward_to_dc';
    ...
    public function listOfProposalsAllServices()
    {
        ...
        if(!empty($by_case_no))
        {
            ...
            $this->db->where('settlement_proposal_list.service_code', RECLASS_ID);
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows) {
                ...
                <div class="text-green" id="list_of_cases_'.$rows->id.'"></div>',
                    ...
                    <a class="btn btn-xs btn-dark" style=" padding: 7px!important; margin-bottom: 5px!important" target= "SDLACProposalNotice" href="'.base_url().'index.php/ReclassSuiteCommonDc/downloadCasesWithProposalId/?case='.$rows->id.'">
                        <i class="fa fa-download" aria-hidden="true"></i> Excel
                    </a>
                    
                    <a class="btn btn-xs btn-success" style=" padding: 7px!important; margin-bottom: 5px!important" target= "SDLACProposalNotice" href="'.base_url().'index.php/SettlementProposalControllerReclass/proposalEditOnSdlacMinutesAdc/?case='.$rows->id.'">
    ...
    public function checkAllSdlacPresentMember()
    {
        ...
        else
        {
            ...
            $proSql = $this->db->query("SELECT service_code FROM settlement_proposal_list
                                          WHERE id IN ($allProposals) AND dist_code=?",
                array($dist_code));
    ...
    public function sendProposalsToDcMinute()
    {
        ...
        {
            if(date('Y-m-d H:i:s',strtotime(MEETING_PROPOSAL_DLC_NOTICE_DATE)) < date('Y-m-d H:i:s',strtotime($meeting_date)))
            {
                echo json_encode(array(
                    'responseType' => 1,
                    'response'     => 1,
                    'message'      => 'Maximum Date of meeting '.MEETING_PROPOSAL_DLC_NOTICE_DATE_SHOW
                ));
                return;
            }
        }
        ...
        $memoName    = $distEngName.'/MEMO/'.date("Y").'/'.$generate_meeting_id;
        $meetingName = $distEngName.'/DLC-REC/'.date("Y").'/'.$generate_meeting_id;

        $proposalDetails = $this->ReclassCommonDcModel->getAllProposalDetailsByProId($proposals);
    ...
    public function sendProposalsToDc()
    {
        ...
        if($this->SettlementCommonDcModel->checkProposalAlreadyExistInMeeting($proposals) != 0)
        {
            $json = [
                'response' => 1,
                'message'  => 'Some of the Proposals Already listed in Meeting ! kindly check in DLC Minutes list',
            ];
            echo json_encode($json);
            return false;
        }
        ...
        // check for PGR/VGR not allowed in other proposals
        $proSql = $this->db->query("SELECT service_code FROM settlement_proposal_list
                                          WHERE id IN ($allProposalId) AND dist_code=?",
            array($dist_code));
        ...
        if(count($nomineeCheck) == 0)
        {
            $json = [
                'response' => 1,
                'message'  => '#ERMR00708: There is no DLC Member, Case can not be forwarded. Kindly contact 
        ...
        if($adc_forward_to_dc == 1)
        {
            ...
            if (isset($allAPICases_reco) && count($allAPICases_reco)>0) {
                ...
                {
                    $this->db->trans_rollback();
                    log_message('error', '#ERRCODE2650: Updation failed in reclass_suite_basic for recommended '.$this->db->last_query());
                    $json = [
                        'response' => 1,
            ...
            if (isset($allAPICases_nrec) && count($allAPICases_nrec)>0) {
                ...
                {
                    $this->db->trans_rollback();
                    log_message('error', '#ERRCODE2669: Updation failed in reclass_suite_basic for not recommended '.$this->db->last_query());
                    $json = [
                        'response' => 1,
        ...
        else
        {
            ...
            $json = [
                ...
                'message'  => 'Proposal(s) will be hold for 48 hrs for online cases. 
                        Pending for DLC Member`s action',
            ];
            echo json_encode($json);
        }
...
//            $sql = " select string_agg((sd.land_type),',') ladtype from reclass_dag_details sdwhere case_no='$r->case_no'";
    ...
    public function pendingProposalList()
    {
        ...
        $pendingCase   = $this->SettlementCommonDcModel->getPendingProposals($dist_code,$createdBy);
        $data['cases'] = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'SettlementView/Adc/pending_proposal_list_holds_for_48_hrs';
        $this->load->view('layouts/main', $data);
    ...
    public function getPendingMeetingDetailsBySdoAdc()
    {
        ...
        if($pendingMeetingCount == 0)
        {
            ...
            redirect(base_url() . "index.php/SettlementProposalController/pendingProposalList");
        ...
        else
        {
            ...
            $data['_view'] = 'SettlementView/Adc/pending_meeting_details_holds_for_48_hrs';
            $this->load->view('layouts/main', $data);
        }
    }
    ...
    public function viewCasesAgainstProposalNo()
    {
        ...
        //list of cases
        $cases = $this->ReclassCommonDcModel->getCaseDetailsByProposalNos($proposalNo);
        ...
        $json = [
            ...
            'proposal_name' => $this->ReclassCommonDcModel->getProposalNameByProposalNo($proposalNo),
        ];
        echo json_encode($json);
        return;
    }
    ...
    public function forwardOnlineMeetingToDcForFinalVerification()
    {
        ...
        if (isset($allAPICases_reco) && count($allAPICases_reco)>0)
        {
            ...
            {
                $this->db->trans_rollback();
                log_message('error', '#ERMR01746: Updation failed in reclass_suite_basic for recommended '.$this->db->last_query());
                $json = [
                    'response' => 1,
        ...
        if (isset($allAPICases_nrec) && count($allAPICases_nrec)>0)
        {
            ...
            {
                $this->db->trans_rollback();
                log_message('error', '#ERMR01764: Updation failed in reclass_suite_basic for not recommended '.$this->db->last_query());
                $json = [
                    'response' => 1,
    ...
    public function revertMeetingListForAdc()
    {
        ...
        $pendingCase              = $this->ReclassCommonDcModel->getRevertedMeetingList($dist_code,$createdBy);
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'reclass_suite/Adc/reverted_meeting_list_adc';
    ...
    public function getProposalUnderRevertedMeetingForAdc()
    {
        ...
        $pendingMeeting      = $this->ReclassCommonDcModel->checkRevertedMeetingExistOrNotWithMeetingId($meetingId,$dist_code,$subdiv_code);
        $pendingMeetingCount = $pendingMeeting->num_rows();

        if($pendingMeetingCount == 0)
        {
            $this->session->set_flashdata('error', "Reverted Meeting by DC Not Found !");
            redirect(base_url() . "index.php/SettlementProposalControllerReclass/revertMeetingListForAdc");
            $this->pendingProposalList();
        }
        else
        {
            ...
            $proposalDetail = $this->ReclassCommonDcModel->getRevertedProposalDetailAgainstMeetingId($dist_code,$meetingId);
            $commMembers    = $this->SettlementMbDcModel->getMembersFromUsersWithUserType($dist_code);
            ...
            $data['_view'] = 'reclass_suite/Adc/reverted_proposal_by_dc_list';
    ...
    public function caseListUnderRevertedMeetingForAdc()
    {
        ...
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->ReclassCommonDcModel->getAllCaseInProposalUnderRevertedMeeting($proposal_no);
        $proposalDetails = $this->ReclassCommonDcModel->getRevertedProposalDetailsByIdAdc($proposal_no,$dist_code);
        ...
        $data['_view'] = 'reclass_suite/Adc/reverted_application_by_proposal';
        $this->load->view('layouts/main', $data);
    ...
    public function sendRevertedProposalsToDcMinuteAdc()
    {
        ...
        $proposalDetails = $this->ReclassCommonDcModel->getRevertedProposalDetailAgainstMeetingId($dist_code,$meetingId)->result();
        $proposals = [];
        ...
        $pendingMeeting      = $this->ReclassCommonDcModel->checkRevertedMeetingExistOrNotWithMeetingIdAdc($meetingId,$dist_code);
        $meetingDetails      = $pendingMeeting->row();
        $pendingMeetingCount = $pendingMeeting->num_rows();
    ...
    public function sendRevertedProposalsToDcAdc()
    {
        ...
        $proposalQ       = $this->ReclassCommonDcModel->getRevertedProposalDetailAgainstMeetingId($dist_code,$meetingId);
        ...
        if(count($nomineeCheck) == 0)
        {
            $json = [
                'response' => 1,
                'message'  => '#ERMR002480: There is no DLC Member, Case can not be forwarded. Kindly contact 
                        system administrator',
            ];
            echo json_encode($json);
            return false;
        ...
        if($adc_forward_to_dc == 1)
        {
            ...
            if (isset($allAPICases_reco) && count($allAPICases_reco)>0) {
                ...
                {
                    $this->db->trans_rollback();
                    log_message('error', '#ERRCODE2710: Updation failed in reclass_suite_basic for recommended '.$this->db->last_query());
                    $json = [
                        'response' => 1,
            ...
            if (isset($allAPICases_nrec) && count($allAPICases_nrec)>0) {
                ...
                {
                    $this->db->trans_rollback();
                    log_message('error', '#ERMR002725: Updation failed in reclass_suite_basic for not recommended '.$this->db->last_query());
                    $json = [
                        'response' => 1,
        ...
        try
        {
            ...
            else
            {
                ...
                foreach ($getProposalsList as $prop)
                {
                    ...
                    $pendingCase = $this->ReclassCommonDcModel->getCaseDetailsByProposalNos($proposal_no);
                    ...
                    if($caseCount == 0)
                    {
                        ...
                        echo json_encode(array(
                            'responseType' => 1,
                            'message'      => '#MR0003155: Unable to process for final approval. 
                                          Kindly contact system administrator !!!!',
                        ));
                        return;
                    }
    ...
    public function forwardedMeetingListForAdc()
    {
        ...
        $pendingCase              = $this->ReclassCommonDcModel->getForwardedMeetingListAdc($dist_code,$createdBy);
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'reclass_suite/Adc/forwarded_meeting_list_adc';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getForwardedMeetingDetailsByIdAdc()
    {
        ...
        $meeting = $this->ReclassCommonDcModel->getForwardedMeetingDetailByMeetingIDAdc(
        ...
        {
            $this->session->set_flashdata('error', "Meeting Not Found !");
            redirect(base_url() . "index.php/SettlementProposalControllerReclass/forwardedMeetingListForAdc");
        }

        //list of proposals against meeting id
        $proposalDetail = $this->ReclassCommonDcModel->getProposalDetailAgainstMeetingIdForAdc($meetingId,
            $dist_code,$createdBy)->result();

        //list of SDLAC/CDLAC Member report
        $sdlacReport = $this->ReclassCommonDcModel->sdlacMemberReportDetail($dist_code,
            $meetingId)->result();

        $additionalDoc = $this->ReclassCommonDcModel->getMeetingAdditionalDocumentDetail($meetingName)->result();
        ...
        $data['_view'] = 'reclass_suite/Adc/forwarded_proposals_against_meeting_id_adc';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function proposalEditOnSdlacMinutesAdc()
    {
        ...
        $pendingCase = $this->ReclassSuiteMeetingDcModel->getAllCaseInProposalUnderRevertedMeeting($proposal_no);
        $proposalDetails = $this->ReclassSuiteMeetingDcModel->getRevertedProposalDetailsByIdAdc($proposal_no,$dist_code);

        if($proposalDetails->meeting_create_status != 1)
        {
            $this->session->set_flashdata('error', "Proposal already add in Meeting !");
            redirect(base_url() . "index.php/SettlementProposalControllerReclass/commonProposalListView");
        }
        ...
        $data['_view'] = 'reclass_suite/Adc/edit_proposal_from_sdlac_minutes_adc';
    ...
    public function applicationRemoveFromRevertedProposal()
    {
        ...
        else
        {
            ...
            if($this->ReclassCommonDcModel->countSettlementProposalList($proposalId) != 1)
            {
                echo json_encode(array(
                    'responseType' => 1,
            ...
            if($this->ReclassCommonDcModel->countSettlementProposalPendingCaseByCaseNo($case_no) != 1)
            {
                echo json_encode(array(
                    'responseType' => 1,
            ...
            if($this->ReclassCommonDcModel->countSettlementAppDetailsByCaseNo($case_no) != 1)
            {
                echo json_encode(array(
                    'responseType' => 1,
            ...
            $proposalDetails = $this->ReclassCommonDcModel->getProposalDetailsByProId($proposalId);
            ...
            $deleteCase = $this->ReclassCommonDcModel->getSettlementRevertedProposalCaseDetailsByCaseNo($case_no);
            ...
            $deleteProCase = $this->ReclassCommonDcModel->deleteSettlementProposalCaseDetailsById($deleteCase->id);
            if($deleteProCase != 1)
            {
                $this->db->trans_rollback();
    ...
    public function applicationRevertToCoFromProposalUnderMinutes()
    {
        ...
        else
        {
            ...
            {
                $this->db->trans_rollback();
                log_message('error', '#ERMR003669: Updating failed in reclass_suite_basic for case no :'. $case_no. 'Last Q' .$this->db->last_query());
                echo json_encode(array(
                    'responseType' => 1,
    ...
    public function getVgrPgrRevertedCaseDetails()
    {
        ...
        if($caseId == '' || $caseId == NULL)
        {
            ...
            redirect(base_url() . "index.php/SettlementProposalController/getVgrPgrRevertedCaseList");
        }
