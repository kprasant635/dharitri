...
class AsistantJuridicalMb extends CI_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->model('mutation/mutationmodel');
        $this->load->model('ASTJuridicalModel');
    ...
    public function GoToAST() {
        ...
        if ($process == '1') {
            $config['total_rows'] = $this->ASTJuridicalModel->countPendingNoticeGeneratedASTMb3($user_code);
            $cases['cases'] = $this->ASTJuridicalModel->getPendingNoticeGeneratedASTMb3($user_code)->result();
        } 
        elseif ($process == '2') {
            $config['total_rows'] = $this->ASTJuridicalModel->countPendingActionTakenASTMb3($user_code);
            $cases['cases'] = $this->ASTJuridicalModel->getPendingActionTakenASTMb3($user_code)->result();
        } 
        // elseif ($process == '3') {
        //     $config['total_rows'] = $this->ASTJuridicalModel->countPendingPremiumAST($user_code);
        //     $cases['cases'] = $this->ASTJuridicalModel->getPendingPremiumAST($user_code)->result();
        // } elseif ($process == '4') {
        //     $config['total_rows'] = $this->ASTJuridicalModel->countPendingPaymentAST($user_code);
        //     $cases['cases'] = $this->ASTJuridicalModel->getPendingPaymentAST($user_code)->result();
        // }
        ...
        //$this->load->view('../views/footer');
        $cases['_view'] = 'ast_juridical_cases_mb';
...
//echo "select max(petition_no) as count from petition_basic where dist_code = '$dist_code' and subdiv_code = '$subdiv_code' and cir_code = '$cir_code' ";
    ...
    public function notice_generation() {
        ...
        $settlement_basic = $this->db->query("select * from    settlement_basic where case_no='$case_no'")->row();

        $location = $this->db->query("select dist_code,subdiv_code,cir_code,lot_no,vill_townprt_code,mouza_pargona_code,date_entry,pending_officer,ast_notice_hearing_date"
                        . " from  settlement_basic where case_no='$case_no' "
        ...
        $data['location'] = array(
            ...
            'add_to' => $location['pending_officer'],
            'case_no' => $case_no,
            'next_date_of_hearing' => $location['ast_notice_hearing_date']
        );

        $landdetails = $this->db->query("select dag_no,s_dag_area_b,s_dag_area_k,s_dag_area_lc,patta_no,patta_type_code from    settlement_dag_details where dist_code='$settlement_basic->dist_code' and subdiv_code='$settlement_basic->subdiv_code' and cir_code='$settlement_basic->cir_code' and lot_no='$settlement_basic->lot_no' and vill_townprt_code='$settlement_basic->vill_townprt_code' and mouza_pargona_code='$settlement_basic->mouza_pargona_code' and case_no='$settlement_basic->case_no'")->row_array();
        $data['land_details'] = array(
            'dag' => $landdetails['dag_no'],
            'm_dag_area_b' => $landdetails['s_dag_area_b'],
            'm_dag_area_k' => $landdetails['s_dag_area_k'],
            'm_dag_area_lc' => $landdetails['s_dag_area_lc'],
        ...
        $pattadardetails = $this->db->query("select * from settlement_applicant where dist_code='$settlement_basic->dist_code' and subdiv_code='$settlement_basic->subdiv_code' and cir_code='$settlement_basic->cir_code' and lot_no='$settlement_basic->lot_no' and vill_townprt_code='$settlement_basic->vill_townprt_code' and mouza_pargona_code='$settlement_basic->mouza_pargona_code' and case_no='$settlement_basic->case_no' and dag_no='$landdetails[dag_no]' and TRIM(patta_no)=trim('$landdetails[patta_no]') and patta_type_code= '$landdetails[patta_type_code]'")->result();
        ...
        $data['_view'] = 'notice_generation_mb_ins';
        $this->load->view('layouts/main',$data);
    ...
    public function notice_generation_save() {
        ...
        $petitionBasicUpdate = $this->db->query("UPDATE settlement_basic SET ast_notice_generated = 'Y' WHERE case_no = '$case_no'");
    
        if($this->db->affected_rows() != 1) {
            $this->db->trans_rollback();
            log_message('error', 'Error in updation in settlement_basic table. Error: ERRINSNOTICE');
            $this->session->set_flashdata('message', "Notice generated failed for Case no # $case_no");
        ...
        $insertArr = [
            ...
            'office_from' => 'AST',
            'office_to' => 'CO',
            'task' => 'Notice generated by AST',
        ];
        $insertProc = $this->db->insert('settlement_proceeding', $insertArr);
        if ($insertProc != 1) 
        {
            $this->db->trans_rollback();
            log_message('error', '#ERRNOTICE25: Insertion failed in settlement_proceeding');
            $json = [
                'responseType' => 3,
                'message' => '#ERRNOTICE25: Failed to generate notice. Kindly contact System Administrator',
            ];
            echo json_encode($json);
            return false;
        }
        ...
        $this->db->trans_commit();
        $this->session->set_flashdata('message', "Notice generated for Case no # $case_no");
        redirect(base_url() . "index.php/home");
    }
    ...
    public function notice_action_taken() {
        ...
        $this->session->set_userdata(array('vill_townprt_code' => $vill_townprt_code1));
        $case_no = $this->input->get('case_no');
        $location = $this->db->query("select * from  settlement_basic where case_no='$case_no'")->row_array();
        $petition_no = $location['petition_no'];
        $landdetails = $this->db->query("select * from  settlement_dag_details where case_no='$case_no'")->row_array();
        $locationData = array(
            'dist_code' => $location['dist_code'],
            'subdiv_code' => $location['subdiv_code'],
            'cir_code' => $location['cir_code'],
        ...
        $data['location'] = array(
            ...
            'add_to' => $location['pending_officer'],
            'case_no' => $case_no,
            'date_of_hearing' => $location['ast_notice_hearing_date'],
            'dag' => $landdetails['dag_no'],
            'm_dag_area_b' => $landdetails['s_dag_area_b'],
            'm_dag_area_k' => $landdetails['s_dag_area_k'],
            'm_dag_area_lc' => $m_dag_area_lc,
        ...
        $query = "select * from    settlement_proceeding where case_no = '$case_no' order by proceeding_id";
        $data['cases'] = $this->db->query($query)->result();
        $petition_basic = $this->db->query("select * from    settlement_basic where case_no='$case_no'")->row();
        $landdetails = $this->db->query("select * from    settlement_dag_details where  case_no='$case_no'")->row_array();
        $data['land_details'] = array(
            'dag' => $landdetails['dag_no'],
            'm_dag_area_b' => $landdetails['s_dag_area_b'],
        ...
        $pattadardetails = "select * from    settlement_institution_details where case_no='$case_no'";
        $data['p_in_order'] = $this->db->query($pattadardetails)->result();

        $data['post_url'] = 'index.php/AsistantJuridicalMb/action_taken_conversion_save';
        $data['_view'] = 'action_taken_ins_mb';
        $this->load->view('layouts/main',$data);
    }
    ...
    public function action_taken_conversion_save() 
    {
        ...
        if($formValidation['status'] == 'n') {
            ...
            redirect(base_url('index.php/AsistantJuridicalMb/GoToAST?pro=2'));
        }
        ...
        $this->db->trans_begin();
        $updateBasic = $this->db->query("UPDATE settlement_basic SET ast_notice_serve = 'Y' WHERE case_no = '$case_no'");
        if($this->db->affected_rows() != 1) {
            log_message("error","Error - #ERRSLIJE021. Action Taken Report update Failed in settlement_basic.");
            $this->db->trans_rollback();
            $this->session->set_flashdata('message', "#ERRSLIJE021: Action Taken Report Failed for case no # $case_no");
            redirect(base_url() . "index.php/home");
            exit();

        }

        $path = UPLOAD_BASE_SLIJEDOCS.UPLOAD_SEPARATOR;
        if(!file_exists($path)) {
         ...
         $sl = $count+1;
         $file = $settlement_basic->petition_no . date('Y') . '_' . $sl;
         ...
         if ($this->upload->do_upload('up_notice_conv')) 
         {
             ...
             if(!$insertNocStatus || $this->db->affected_rows() < 1){
                 ...
                 $this->db->trans_rollback();
                 log_message("error","Error - #ERRSLIJE1617. Notice upload data insertion failed in supportive_document for dist: " . $dist_code.", case no: ". $case_no);
         ...
         {
                 $this->db->trans_rollback();
                 log_message("error","Error - #ERRSLIJE1618. Notice upload data insertion failed in supportive_document for dist: " . $dist_code.", case no: ". $case_no);
                 $this->session->set_flashdata('message', "Unable to upload file Conversion case no # $case_no");
                 redirect(base_url() . "index.php/home");
                 exit();
        ...
        ////////////////////////////////////////
        $this->session->set_flashdata('message', "Action Taken Report Passed for Juridical Entity case no # $case_no");
        redirect(base_url() . "index.php/home");
    }
