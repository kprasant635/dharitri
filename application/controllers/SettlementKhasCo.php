<?php
class SettlementKhasCo extends CI_Controller
{
    ...
    public function settlementKhasCo()
    {
        ...
        $lmdata['_view'] = 'SettlementView/Co/Khas/SettlementKhasCoView';
    ...
    public function generateNoticeCo()
    {
        ...
        if (isset($_POST['forward_to_dc'])) {
            ...
            // wetland above 10 bigha check start
            $get_basic = $this->SettlementApModel->getSettlementBasicCo($case_no);
            
            $premiumData = $this->SettlementCommonModel->getPremiumData($case_no);
            $premiumData = $premiumData->result();
            // var_dump($premiumData); die;
            ...
            foreach ($premiumData as $row) {
                ...
                $is_rural_check =$row->area_name;
                if($is_rural_check == 10)
                {
                    $is_rural_flag=true;
                }

            
                // add lessa for each row
                $totalLessa += (float)$row->total_lessa;

                // Only query for wetland until we find one = 6
                if (!$wetlandFound) {
                    $wetlands = $this->SettlementCommonModel->getWetland($get_basic, $row->dag_no);
                    foreach ($wetlands as $w) {
                        if ((int)$w->wet_land === 6) {
                            $wetlandFound = true;
                            break; // break ONLY inner loop, keep summing remaining dags
                        }
                    }
                }
            }

            // rural closed if urban case still pending
            $this->db->select('*');
            $this->db->from('settlement_basic a');
            $this->db->join('settlement_premium b', 'a.case_no = b.case_no');
            $this->db->where('a.status', 'W');
            $this->db->where('a.pending_officer', 'CO');
            $this->db->where('a.service_code', 16);
            $this->db->where('b.is_final', 1);
            $this->db->where('a.dist_code', $get_basic->dist_code);
            $this->db->where('a.subdiv_code', $get_basic->subdiv_code);
            $this->db->where('a.cir_code', $get_basic->cir_code);
            $this->db->where('a.co_code', $this->session->userdata('user_code'));
            $this->db->where_not_in('b.area_name', ['10']);
            ...
            if ($query->num_rows() > 0) {
                ...
                if ($is_rural_flag == true) {
                    $this->db->trans_rollback();

                    log_message('error', '#ERRCO00038343: Failed to forward to DC as urban case are pending');

                    $this->session->set_flashdata(
                        'message',
                        "Case no # $case_no  Failed to forward to DC as urban case are pending"
                    );
            ...
            //         $this->db->trans_rollback();
            //         log_message('error', '#ERRCO00038343: Failed to forward to DC as urban case are pending');
            //         $this->session->set_flashdata('message', "Case no # $case_no  Failed to forward to DC as urban case are pending");
            //         redirect(base_url() . "index.php/home");
            //         return false;
            // }
            // var_dump($totalLessa); die;

            // Final condition for wetland check with more than 10 bigha not allowed
            if (($get_basic->is_wed_land == 1 && $totalLessa > 1000) || ($wetlandFound && $totalLessa > 1000)) {
                log_message('error', "Application $case_no STOPPED (wet_land=1, wetland=6 found, total lessa=$totalLessa)");
            ...
            // wetland above 10 bigha check end
    ...
    public function FirstProceeding()
    {
        ...
        $data['select_data'] = $this->SettlementCommonModel->locationSelectAll($service_code, $status);
    ...
    public function generatePaymentNoticeCo()
    {
        if(isset($_GET['case'])){
            ...
            $data['_view'] = 'SettlementView/Co/Khas/generateNoticeViewNew';
            $this->load->view('layouts/main', $data);
        }
    ...
    public function generatePaymentNoticeCoSave()
    {
        ...
        if($caseUrban =='Y')/////consider as urbam case
        {
            ...
            $this->session->set_flashdata('message', "ERRPAYNOTURB007: something went wrong !!!");
            redirect(base_url().'index.php/home');
        }
    }
    ...
    public function generatePaymentNoticeCoNew()
    {
        if(isset($_GET['case']))
        {
            $case_no = $this->input->get('case');
            ...
            $premium_data = $this->db->query("SELECT sp.*, spa.area, spl.land_type, spr.house_type FROM settlement_premium sp inner join settlement_premium_area spa on spa.paid=sp.area_name inner join settlement_premium_land_type spl 
                on spl.plid=sp.land_type inner join settlement_premium_rate spr on spr.prid=sp.rate_type 
                where case_no=? and is_final=?",[$case_no, 1]);

            $data['premium_data'] = $premium_data->result();
    ...
    public function generatePaymentNoticeCoSaveV2()
    {
        $case_no = $this->input->post('case_no');
        $remark = $this->input->post('remark_co');
        $applicant_buyer = $this->SettlementKhasModel->getAllApplicantBuyers($case_no);
        ...
        $premium_data = $this->db->query("SELECT sp.*,spa.area,spl.land_type,spr.house_type, spr.mb_land, spr.max_land, spr.prid FROM settlement_premium sp inner join settlement_premium_area spa on spa.paid=sp.area_name inner join settlement_premium_land_type spl on spl.plid=sp.land_type inner join settlement_premium_rate spr on spr.prid=sp.rate_type where case_no=? and is_final=?", array($case_no, 1));

        $house_type = $this->getHouseType($premium_data->row()->prid);
        ...
        if($premium_data->num_rows() > 0)
        {
            ...
            if($caseUrban =='Y') /////consider as urban case
            {
                ...
                $total_house_amount_1 = 0;
                $total_house_amount_2 = 0;
                ...
                foreach($premium_data_arr as $premium)
                {
                    ...
                    $data['first_area'] = implode ( "<br> ", $area_all );
                    $data['first_dag_no'] = implode ( "<br> ", $dag_arr );
                    ...
                    if($mbAreaLimit != null && $total_lessa > $mbAreaLimit)
                    {
                        $this->session->set_flashdata('message', "#ERR3671: Please contact admin #".$case_no);
                        redirect(base_url().'index.php/home');
                    }

                    if($maxLand != null && $total_lessa > $maxLand)
                    {
                        $this->session->set_flashdata('message', "#ERR3670: Area limit exceeeds #".$case_no);
                        redirect(base_url().'index.php/home');
                    }
                    ...
                    if($house_type == 'RCC_HOUSE')
                    {
                        if($total_lessa  > $mbAreaLimit)
                        {
                            //*****For 50% premium */
                            $limitPremium   = ($mbAreaLimit * ($zonalValue * 50/100));                            
                            $exceedPremium  = ($total_lessa - $mbAreaLimit) * ($zonalValue * 150/100); // 150%

                            // Calculate amount for this specific dag
                            $amount_for_this_dag_1 = ($limitPremium * 10) / 100; // conditional patta
                            $amount_for_this_dag_2 = ($limitPremium * 50) / 100; // full patta
                            
                            // Add to the total RCC amount
                            $total_house_amount_1 += $amount_for_this_dag_1;
                            $total_house_amount_2 += $amount_for_this_dag_2;
                            ...
                            $trArr .= '<tr>
                                ...
                                <td>
                                    ...
                                    * '.$area.': : লৈকে - অসম চৰকাৰৰ ৰাজহ আৰু দুৰ্যোগ ব্যবস্থাপনা বিভাগৰ অধিসূচনা No. RDM-12011(17)/15/2022/LR-REV-R & DM/14 dtd 21-Aug-2023 আৰু No E-540451/I/874570/2024 dtd 27-Dec-2024 মৰ্মে  অনুমোদিত ভূমিৰ প্ৰিমিয়াম হিচাপে মাণ্ডলিক মূল্যৰ ১০% 
                                    </p>
                                </td>
                                <td>'.$premium_per_bigha.'</td>
                                <td>'.$dag_no.'</td>
                                <td style="white-space: nowrap;">'.$area.'</td>
                                <td style="white-space: nowrap;" class="text-right pr-2">+₹ '.$amount_for_this_dag_1.'</td>
                            </tr>';

                            $trArr2 .= '<tr>
                                    <td rowspan="'.$loloCounter.'" class="text-center">'.$sl_counter1++.'</td>
                                    <td>
                                        <b><u>আৰ.চি.চি. ঘৰ</u></b>
                                        <br> 
                                        <p style="line-height: 1.6!important;">

                                        * '.$area.': : লৈকে - অসম চৰকাৰৰ ৰাজহ আৰু দুৰ্যোগ ব্যবস্থাপনা বিভাগৰ অধিসূচনা No. RDM-12011(17)/15/2022/LR-REV-R & DM/14 dtd 21-Aug-2023 আৰু No E-540451/I/874570/2024 dtd 27-Dec-2024 মৰ্মে  অনুমোদিত ভূমিৰ প্ৰিমিয়াম হিচাপে মাণ্ডলিক মূল্যৰ ৫০%
                                        </p>
                                    </td>
                                    <td>'.$premium_per_bigha.'</td>
                                    <td>'.$dag_no.'</td>
                                    <td style="white-space: nowrap;">'.$area.'</td>
                                    <td style="white-space: nowrap;" class="text-right pr-2">+₹ '.$amount_for_this_dag_2.'</td>
                                </tr>';//.$exceed_pre.$consc.'';
                            ...
                            if($exceedPremium != false)
                            {
                                $trArr .= '<tr>
                                    <td rowspan="'.$loloCounter.'" class="text-center">'.$sl_counter++.'</td>
                                    <td>
                                        <b><u>অতিৰিক্ত ভূমি</u></b>
                                        <br> 
                                        <p style="line-height: 1.6!important;">

                                        * '.$exceed_area.': 0লে- অসম চৰকাৰৰ ৰাজহ আৰু দুৰ্যোগ ব্যবস্থাপনা বিভাগৰ অধিসূচনা No.RDM-12011(17)/15/2022-LR-REV-R&DM/14  dtd 21-Aug-2023  মৰ্মে  অনুমোদিত অতিৰিক্ত ভূমিৰ প্ৰিমিয়াম হিচাপে মাণ্ডলিক মূল্যৰ ১৫০%  
                                        </p>
                                    </td>
                                    <td>'.$exceed_premium_per_bigha.'</td>
                                    <td>'.$dag_no.'</td>
                        ...
                        {
                            // Calculate amount for this specific dag
                            $amount_for_this_dag_1 = ($total_amount * 10) / 100; // conditional patta
                            $amount_for_this_dag_2 = ($total_amount * 50) / 100; // full patta
                    ...
                    if($house_type == 'ONYA_PROKAR_GHAR')
                    {
                        if($total_lessa  > $mbAreaLimit)
                        {
                            //*****For 50% premium */
                            ...
                            // Calculate amount for this specific dag
                            $amount_for_this_dag_1 = ($limitPremium * 3) / 100; // conditional patta
                            $amount_for_this_dag_2 = ($limitPremium * 30) / 100; // full patta
                            ...
                            $trArr .= '<tr>
                                    <td rowspan="'.$loloCounter.'" class="text-center">'.$sl_counter++.'</td>
                                    <td>
                                        <b><u>অন্য প্ৰকাৰৰ ঘৰ</u></b>
                                        <br> 
                                        <p style="line-height: 1.6!important;">

                                        * '.$area.': : লৈকে - অসম চৰকাৰৰ ৰাজহ আৰু দুৰ্যোগ ব্যবস্থাপনা বিভাগৰ অধিসূচনা No. RDM-12011(17)/15/2022/LR-REV-R & DM/14 dtd 21-Aug-2023 আৰু No E-540451/I/874570/2024 dtd 27-Dec-2024 মৰ্মে  অনুমোদিত ভূমিৰ প্ৰিমিয়াম হিচাপে মাণ্ডলিক মূল্যৰ ৩%
                                        </p>
                                    </td>
                                    <td>'.$premium_per_bigha.'</td>
                                    <td>'.$dag_no.'</td>
                            ...
                            $trArr2 .= '<tr>
                                    ...
                                    <td>
                                        ...
                                        * '.$area.': : লৈকে - অসম চৰকাৰৰ ৰাজহ আৰু দুৰ্যোগ ব্যবস্থাপনা বিভাগৰ অধিসূচনা No. RDM-12011(17)/15/2022/LR-REV-R & DM/14 dtd 21-Aug-2023 আৰু No E-540451/I/874570/2024 dtd 27-Dec-2024 মৰ্মে  অনুমোদিত ভূমিৰ প্ৰিমিয়াম হিচাপে মাণ্ডলিক মূল্যৰ ৩০%
                                        </p>
                                    </td>
                                    <td>'.$premium_per_bigha.'</td>
                                    <td>'.$dag_no.'</td>
                        ...
                        {
                            // Calculate amount for this specific dag
                            // $amount_for_this_dag_1 = ($total_amount * 3) / 100; // conditional patta
                            // $amount_for_this_dag_2 = ($total_amount * 30) / 100; // full patta

                            $percent =30;
                            $total = $total_amount / ($percent / 100); // calculate original amount
                            $amount_for_this_dag_1 = $total * (3 / 100); // conditional patta
                            $amount_for_this_dag_2 = ($total * 30) / 100; // full patta
                ...
                if(trim($premium->concession) == 'YES'){
                    $concession_discount_1 = $total_house_amount_1 * 0.25;
                    $concession_discount_2 = $total_house_amount_2 * 0.25;
                }
                else
                {
                    $concession_discount_1 = 0;
                    $concession_discount_2 = 0;
                }

                $trArr .= '
                        <tr>
                            <td rowspan="'.$loloCounter.'" class="text-center">'.$sl_counter++.'</td>
                            <td>বিশেষ শ্ৰেণীৰ বাবে ৰেহাই - প্ৰিমিয়ামৰ ২৫% ৰেহাই ।</td>
                            <td>ST/SC/Widow/Person with disabilities</td>
                            <td></td>
                            <td style="white-space: nowrap;"></td>
                            <td style="white-space: nowrap;" class="text-right pr-2">- ₹ '.$concession_discount_1.'</td>
                        </tr>
                        <tr>
                            <td colspan="5" class="text-center"><b>প্ৰকৃত / চূড়ান্ত দিবলগীয়া প্ৰিমিয়াম (চৰ্তযুক্ত বন্দৱস্তী পাবলৈ)</b></td>
                            <td class="text-right pr-2"><b>₹ '.$total_house_amount_1.'</b></td>
                        </tr>';
                ...
                $trArr2 .= '
                        <tr>
                            ...
                            <td></td>
                            <td style="white-space: nowrap;"></td>
                            <td style="white-space: nowrap;" class="text-right pr-2">- ₹ '.$concession_discount_2.'</td>
                        </tr>
                        <tr>
                            <td colspan="5" class="text-center"><b>প্ৰকৃত / চূড়ান্ত দিবলগীয়া প্ৰিমিয়াম (সম্পূৰ্ণ বন্দৱস্তী পাবলৈ)</b></td>
                            <td class="text-right pr-2"><b>₹ '.$total_house_amount_2.'</b></td>
                        </tr>';

                if($exceedPremium != false)
                {
                    $total_house_amount_1 = $total_house_amount_1 + $exceedPremium;
                    $total_house_amount_2 = $total_house_amount_2 + $exceedPremium;
                }

                $data['net_premium_payable_1'] = $total_house_amount_1 - $concession_discount_1;
                $data['net_premium_payable_2'] = $total_house_amount_2 - $concession_discount_2;
        ...
        if($caseUrban =='Y')/////consider as urbam case
        {
            $this->load->view('SettlementView/include/urban_notice_v2', $data);
        }
    }
     

    protected function getHouseType($type)
    {
        $house_type = '';
        if (in_array($type, RCC_HOUSE)) {
            $house_type = 'RCC_HOUSE';
        } elseif (in_array($type, ONYA_PROKAR_GHAR)) {
            $house_type = 'ONYA_PROKAR_GHAR';
        }
        return $house_type;
    }

    public function generatePaymentNoticeCoNewRural()
    {
        if(isset($_GET['case']))
        {
            $case_no = $this->input->get('case');
            ...
            $this->db->trans_begin();
            $settlement_premium_insertion = $this->SettlementCommonModel->premiumReCalculationRuralMb2Unpaid($case_no);
            ...
            $data['_view'] = 'SettlementView/Co/Khas/generateNoticeViewNewRural';
            $this->load->view('layouts/main', $data);
        }
       
    }

    public function generatePaymentNoticeCoSaveV2Rural()
    {
        $case_no = $this->input->post('case_no');
        $remark = $this->input->post('remark_co');
        $applicant_buyer = $this->SettlementKhasModel->getAllApplicantBuyers($case_no);
        ...
        //  1. Get the latest record where notice_type = 'PN'
        $data['payment_notice_date'] = null;

        $this->db->where('notice_type', 'PN');
        $this->db->where('case_no', $case_no);
        $this->db->order_by('id', 'DESC');
        $this->db->limit(1);
        $query = $this->db->get('settlement_notice');
        $notice = $query->row();
        if ($notice && $notice->payment_version === NULL) {

            if (!empty($notice->payment_notice_date)) {
                $data['payment_notice_date'] = $notice->payment_notice_date;
            }

        } else {
            log_message('error', '#ERROR151220231156: Unable to process — premium notice already generated '.$case_no);
            $this->session->set_flashdata('message', "#ERROR151220231156: Unable to process ".$case_no);
            redirect(base_url('index.php/home/index'));
            exit;
        }

        // Final validation: Stop if still null
        if ($data['payment_notice_date'] === null) {
            log_message('error', '#ERROR151220231158: payment_notice_date not assigned for '.$case_no);
            $this->session->set_flashdata('message', "#ERROR151220231158: payment_notice_date not assigned for ".$case_no);
            redirect(base_url('index.php/home/index'));
            exit;
        }
        ...
        if($caseUrban =='N')/////consider as rural case
        {
            $this->load->view('SettlementView/include/rural_notice_v2', $data);
        }
    }
