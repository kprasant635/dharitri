...
class SettlementMbSdo extends CI_Controller
{
...
//********************** COMMON **********************************
    ...
    function randomFileName()
    {
        ...
        if($this->SettlementMbSdoModel->checkDuplicateFileNameInProposal($new_case_no) != 0)
        {
            $this->randomFileName();
        }
        else
    ...
    public function applicationRevertFromDCToCO()
    {
        ...
        else
        {
            ...
            $caseCount = $this->SettlementMbSdoModel->countSettlementApplicationDetailsByCaseNo($case_no,$dist_code,$subDiv_code);
            ...
            else
            {
                ...
                $this->db->trans_begin();
                if($this->SettlementMbSdoModel->updateSettlementBasicData($case_no,$dist_code,$subDiv_code,$updateData)== 0)
                {
                    $this->db->trans_rollback();
                    log_message('error', '#MR000438: Insertion failed in settlement_basic for case no :'. $case_no);
                    echo json_encode(array(
                        'responseType' => 1,
                    ));
                    return;
    ...
    public function applicationRejectedBySdlac()
    {
        ...
        else
        {
            ...
            $caseCount = $this->SettlementMbSdoModel->countSettlementApplicationDetailsByCaseNoForSDLAC($case_no,$dist_code,$subDiv_code);
            if($caseCount == 0)
            {
                echo json_encode(array(
                    'responseType' => 3,
            ...
            else
            {
                ...
                else
                {
                    ...
                    $mmnn = $this->SettlementMbSdoModel->updateSettlementAppDetailsByCaseNoUnderProposal($case_no,$updatePro);
                    ...
                    {
                        $this->db->trans_rollback();
                        log_message('error', '#MR000612: Insertion failed in settlement_proceeding for case no :'. $case_no);
                        echo json_encode(array(
                            'responseType' => 1,
                        ));
                        return;
                    ...
                    else
                    {
                        ...
                        {
                            $this->db->trans_rollback();
                            $this->session->set_flashdata('message', "Error #MRAPI00631: Rejected by SDLAC failed case no # $case_no");
                            redirect(base_url() . "index.php/home");
                        }
                        else
                        {
    ...
    public function removeMarkApplicationForSDLAC()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    $this->db->trans_rollback();
                    log_message('error', '#MR00963: Insertion failed in settlement_basic for case no :'. $case_no);
                    echo json_encode(array(
                        'responseType' => 1,
                    ));
                    return;
                ...
                else
                {
                    ...
                    $insPetProceed = [
                        ...
                        'date_entry' => date('Y-m-d h:i:s'),
                        'operation' => 'E',
                        'note_on_order' => 'Unmarked from SDLAC Recommended List',
                        'ip' => $this->utilityclass->get_client_ip(),
                        'office_from' => MB_SUB_DIV_COMM,
                        'office_to' => MB_SUB_DIV_COMM,
...
//                      'task' => 'Under SDLAC Consideration.'
                    ...
                    {
                        $this->db->trans_rollback();
                        log_message('error', '#MR00998: Insertion failed in settlement_proceeding for case no :'. $case_no);
                        echo json_encode(array(
                            'responseType' => 1,
                        ));
                        return;
    ...
    // Approve application by SDLAC SDO
    public function applicationApprovedBySdlac()
    {
        ...
        else
        {
            ...
            $caseCount      = $this->SettlementMbSdoModel->countSettlementApplicationDetailsByCaseNoForSDLAC($case_no,$dist_code,$subDiv_code);
            $caseCountInPro = $this->SettlementMbSdoModel->countSettlementAppDetailsByCaseNoUnderProposal($case_no);
            ...
            else
            {
                ...
                {
                    $this->db->trans_rollback();
                    log_message('error', '#MR001088: Insertion failed in settlement_basic for case no :'. $case_no);
                    echo json_encode(array(
                        'responseType' => 1,
                    ));
                    return;
                ...
                else
                {
                    ...
                    $this->SettlementMbSdoModel->updateSettlementAppDetailsByCaseNoUnderProposal($case_no,$updatePro);
                    ...
                    {
                        $this->db->trans_rollback();
                        log_message('error', '#MR001130: Insertion failed in settlement_proceeding for case no :'. $case_no);
                        echo json_encode(array(
                            'responseType' => 1,
                        ));
                        return;
                    ...
                    else
                    {
                        ...
                        {
                            $this->db->trans_rollback();
                            $this->session->set_flashdata('message', "Error #MRAPI001150: Forward to DC failed case no # $case_no");
                            redirect(base_url() . "index.php/home");
                        }
                        else
                        {
    ...
    public function viewPaymentReceivedAppDetailsByDc()
    {
        ...
        $subDiv_code = $this->session->userdata('subdiv_code');
        $this->utilityclass->checkUserAuthForCaseForSdo($case_no);
        $caseCount = $this->SettlementMbSdoModel->countSettlementPaymentReceivedAppDetailsByCaseNo($case_no,$dist_code,$subDiv_code);
        ...
        {
            $caseDetails = $this->SettlementMbSdoModel->getSettlementPaymentReceivedAppDetailsByCaseNo($case_no,$dist_code,$subDiv_code);
            ...
            $data['_view'] = 'settlementView/Sdo/Khas/payment_received_app_details_khas';
    ...
    public function orderToCoByDcForChithaUpdating()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    $this->db->trans_rollback();
                    log_message('error', '#MR001256: Insertion failed in settlement_basic for case no :'. $case_no);
                    echo json_encode(array(
                        'responseType' => 1,
                    ));
                    return;
                ...
                else
                {
                    ...
                    {
                        $this->db->trans_rollback();
                        log_message('error', '#MR001291: Insertion failed in settlement_proceeding for case no :'. $case_no);
                        echo json_encode(array(
                            'responseType' => 1,
                        ));
                        return;
                    ...
                    else
                    {
                        ...
                        {
                            $this->db->trans_rollback();
                            $this->session->set_flashdata('message', "Error #MRAPI001307: Order for chitha update failed case no # $case_no");
                            redirect(base_url() . "index.php/home");
                        }
                        else
                        {
    ...
    public function updateProposalHearingDateKhas()
    {
        ...
        else
        {
            ...
            $allCases      = $this->SettlementMbSdoModel->getAllAppInReportSendByDcToSdlacKhas($proposalNo);
    ...
    public function updateHearingDateGenerateNoticeKhas()
    {
        ...
        else
        {
            ...
            $proposalDetails = $this->SettlementMbSdoModel->getProposalDetailsById($proposalNo,$dist_code,$subDiv_code);
            if($proposalDetails == '')
            {
                echo json_encode(array(
                    'responseType' => 3,
            ...
            else
            {
                ...
                else
                {
                    ...
                    $this->db->trans_begin();
                    if($this->SettlementMbSdoModel->updateProposalListById($proposalNo,$dist_code,$subDiv_code,$updateProposalData)== 0)
                    {
                        $this->db->trans_rollback();
                        log_message('error', '#MR001466: Insertion failed in settlement_proposal_list');
                        echo json_encode(array(
                            'responseType' => 1,
                        ));
                        return;
                    ...
                    else
                    {
                        foreach ($allSelectedList as $row)
                        {
                            ...
                            {
                                $this->db->trans_rollback();
                                log_message('error', '#MR001506: Insertion failed in settlement_proceeding for case no :'. $case_no);
                                echo json_encode(array(
                                    'responseType' => 1,
                                ));
                                return;
...
//********************** COMMON **********************************
    ...
    public function SettlementKhasLandDc()
    {
        ...
        // todo
        $firstProceedingCount = $this->SettlementMbSdoModel->countAllPendingSettlementKhas($dist_code,$subDiv_code);
        $SDLACCommitteeCount  = $this->SettlementCommonDcModel->countAllSDLACCommitteeMember($dist_code,$user_code,$user_desig_code);
        $SDLACNoticeCount     = $this->SettlementMbSdoModel->countMarkAsSDLACSettlementKhas($dist_code,$subDiv_code);
        $SDLACReportCount     = $this->SettlementMbSdoModel->countAllProposalSendByDcToSdlacKhas($dist_code,$subDiv_code);
        $caseStatusCount      = 0;
        $sdlacMemberApproval  = $this->SettlementMbSdoModel->countSdlacStatusList($dist_code,$subDiv_code);
        $SDLACConsideration   = $this->SettlementMbSdoModel->countAllUnderConsiderationAppKhas($dist_code,$subDiv_code);

        // $reReportByCOCount    = $this->SettlementMbSdoModel->countReRevertedByCoApplicationKhas($dist_code,$subDiv_code);
        // $approvedListCount    = $this->SettlementMbSdoModel->countAllApproveAppBySdlacKhas($dist_code,$subDiv_code);
        // $rejectedListCount    = $this->SettlementMbSdoModel->countAllRejectAppByDcKhas($dist_code,$subDiv_code);
        // $finalVerifyCaseCount = $this->SettlementMbSdoModel->countAllCasesForFinalVerifyAppKhas($dist_code,$subDiv_code);
        // $revertedByDepartmentCount = $this->SettlementMbSdoModel->countRevertedByDeptApplicationKhas($dist_code,$subDiv_code);
        // $chithaUpdateOrderCount    = $this->SettlementMbSdoModel->countAllOrderChithaUpdateAppKhas($dist_code,$subDiv_code);

        $coRejectedListCount = $this->SettlementCommonDcModel->countCoRejectedCaseForSDO($dist_code,$subDiv_code,SETTLEMENT_KHAS_LAND_ID);

        $coModificationListCount = $this->SettlementPullModel->countCoModificationRequestCaseForSDO($dist_code,$subDiv_code,SETTLEMENT_KHAS_LAND_ID);

        $rejctedListCount = $this->SettlementCommonDcModel->rejectedCaseListSDO($dist_code,$subDiv_code, SETTLEMENT_KHAS_LAND_ID, MB_SUB_DIV_COMM);
        $revivalListCount = $this->SettlementCommonDcModel->revivalListCountSDO($dist_code, $subDiv_code, SETTLEMENT_KHAS_LAND_ID, MB_SUB_DIV_COMM);
        ...
        $data['_view'] = 'settlementView/Sdo/Khas/first_landing_page_dc_khas';
        $this->load->view('layouts/main', $data);
    ...
    public function viewAllKhasFirstProceedingDCCaseList()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $subDiv_code = $this->session->userdata('subdiv_code');
        $pendingCase = $this->SettlementMbSdoModel->getAllPendingSettlementKhas($dist_code,$subDiv_code);
        ...
        $data['_view'] = 'settlementView/Sdo/Khas/first_proceeding_case_dc_khas';
        $this->load->view('layouts/main', $data);

    }


    //  settlement application details KHAS SDO
    public function getSettlementKhasApplicationDetails()
    {
        ...
        else
        {
            ...
            $caseDetails = $this->SettlementMbSdoModel->getSettlementApplicationDetailsByCaseNo($case_no,$dist_code,$subDiv_code);
            $proceedings = $this->SettlementMbSdoModel->getSettlementProceeding($case_no);
            $data['caseCount']   = $caseCount;
            $data['caseDetails'] = $caseDetails;
            $data['proceedings'] = $proceedings;
            ...
            $data['_view'] = 'settlementView/Sdo/Khas/settlement_app_details_khas';
            $this->load->view('layouts/main', $data);
        }
    }
    ...
    public function viewAllMarkAsSDLACListForDCKhas()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $subDiv_code = $this->session->userdata('subdiv_code');
        $pendingCase = $this->SettlementMbSdoModel->getMarkAsSDLACSettlementKhas($dist_code,$subDiv_code);
        ...
        $data['_view'] = 'settlementView/Sdo/Khas/mark_as_sdlac_case_dc_khas';
        $this->load->view('layouts/main', $data);
    ...
    public function getAllProposalListSdlacKhas()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $subDiv_code = $this->session->userdata('subdiv_code');
        $pendingCase = $this->SettlementMbSdoModel->getAllProposalSendByDcToSdlacKhas($dist_code,$subDiv_code);
        ...
        $data['_view'] = 'settlementView/Sdo/Khas/proposal_list_send_to_sdlac_khas';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getAllUnderConSdlacKhas()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $subDiv_code = $this->session->userdata('subdiv_code');
        $pendingCase = $this->SettlementMbSdoModel->getAllUnderConSettlementKhas($dist_code,$subDiv_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'settlementView/Sdo/Khas/under_consideration_case_dc_khas';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getAllApplicationInReportSendByDcToSdlacKhas()
    {
        ...
        $dist_code   = $this->session->userdata('dist_code');
        $subDiv_code = $this->session->userdata('subdiv_code');
        $pendingCase = $this->SettlementMbSdoModel->getAllAppInReportSendByDcToSdlacKhas($proposal_no);
        $proposalDetails = $this->SettlementMbSdoModel->getProposalDetailsById($proposal_no,$dist_code,$subDiv_code);
        ...
        $data['_view'] = 'settlementView/Sdo/Khas/send_to_sdlac_case_dc_khas';
        $this->load->view('layouts/main', $data);

    }


    // generate proposal notice KHAS SDO
    public function generateNoticeSendAllMarkAppToSDLACByDcKhas()
    {
        ...
        else
        {
            ...
            if(!empty($allSelectedList))
            {
                ...
                $this->db->trans_begin();
                if($this->SettlementMbSdoModel->saveProposalSDLACKhas($dataProSave) == 0)
                {
                    $this->db->trans_rollback();
                ...
                else
                {
                    ...
                    foreach ($allSelectedList as $row)
                    {
                        ...
                        if($this->SettlementMbSdoModel->saveProposalCaseListSDLACKhas($saveCaseList) == 0)
                        {
    ...
    // get all  approved list KHAS
    public function getAllApprovedBySDLACListKhas()
    {
        $subDiv_code = $this->session->userdata('subdiv_code');
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementMbSdoModel->getAllApproveAppBySdlacKhas($dist_code,$subDiv_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'settlementView/Sdo/Khas/approve_list_by_sdlac_khas';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function viewApprovedAppDetailsKhas()
    {
        ...
        else
        {
            ...
            $data['_view'] = 'settlementView/Sdo/Khas/settlement_app_details_only_view_khas';
            $this->load->view('layouts/main', $data);
        }
    }
    ...
    public function getAllRejectByDcListKhas()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $subDiv_code = $this->session->userdata('subdiv_code');
        $pendingCase = $this->SettlementMbSdoModel->getAllRejectAppByDcKhas($dist_code,$subDiv_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'settlementView/Sdo/Khas/rejected_list_by_dc_khas';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function viewRejectedAppDetailsKhas()
    {
        ...
        else
        {
            ...
            $data['_view'] = 'settlementView/Sdo/Khas/settlement_app_details_rejected_only_view_khas';
            $this->load->view('layouts/main', $data);
        }
    }
    ...
    public function getAllOrderChithaUpdateForDcAppKhas()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $subDiv_code = $this->session->userdata('subdiv_code');
        $pendingCase = $this->SettlementMbSdoModel->getAllOrderChithaUpdateAppKhas($dist_code,$subDiv_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'settlementView/Sdo/Khas/order_chitha_update_list_by_dc_khas';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getAllReReportAppByCOForDcAppKhas()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $subDiv_code = $this->session->userdata('subdiv_code');
        $pendingCase = $this->SettlementMbSdoModel->getReRevertedByCoApplicationKhas($dist_code,$subDiv_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'settlementView/Sdo/Khas/re_revert_by_co_list_khas';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getAllRevertedAppByDeptForDcAppKhas()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $subDiv_code = $this->session->userdata('subdiv_code');
        $pendingCase = $this->SettlementMbSdoModel->getRevertedByDeptApplicationKhas($dist_code,$subDiv_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'settlementView/Sdo/Khas/revert_by_dept_list_khas';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function applicationPaymentGenerateDcKhas()
    {
        ...
        else
        {
            ...
            $remarks   = $this->input->post('remarks');
            $user_code = $this->session->userdata('user_code');
            $caseCount = $this->SettlementMbSdoModel->caseForDcApprovalKhas($case_no,$dist_code,$subDiv_code);
            if($caseCount == 0)
            {
                echo json_encode(array(
                    'responseType' => 3,
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            $this->db->trans_rollback();
                            $this->session->set_flashdata('message', "Error #MRAPI002550: Order for chitha update failed case no # $case_no");
                            redirect(base_url() . "index.php/home");
                        }
                        else
                        {
    ...
    public function applicationRevertFromSDLACToCOKhas()
    {
        ...
        else
        {
            ...
            if($status == PRO_CASE_APPROVED_STATUS)
            {
                ...
                {
                    $this->db->trans_rollback();
                    log_message('error', '#MR002631: Update failed in settlement_proposal_cases for case no :'. $case_no);
                    echo json_encode(array(
                        'responseType' => 1,
                    ));
                    return;
                ...
                {
                    $this->db->trans_rollback();
                    log_message('error', '#MR002631: Update failed in settlement_basic for case no :'. $case_no);
                ...
                else
                {
                    ...
                    {
                        $this->db->trans_rollback();
                        log_message('error', '#MR002683: Insertion failed in settlement_proceeding for case no :'. $case_no);
                        echo json_encode(array(
                            'responseType' => 1,
                        ));
                        return;
                    ...
                    else
                    {
                        ...
                        {
                            $this->db->trans_rollback();
                            $this->session->set_flashdata('message', "Error #MRAPI002695: Reverted to CO failed case no # $case_no");
                            redirect(base_url() . "index.php/home");
                        }
                        else
                        {
            ...
            if($status == PRO_CASE_NOT_APPROVED_STATUS)
            {
                ...
                {
                    $this->db->trans_rollback();
                    log_message('error', '#MR002737: Insertion failed in settlement_proposal_cases_deleted for case no :'. $case_no);
                    echo json_encode(array(
                        'responseType' => 1,
                    ));
                    return;
                ...
                {
                    $this->db->trans_rollback();
                    log_message('error', '#MR002748: Deletion failed in settlement_proposal_cases for case no :'. $case_no);
                    echo json_encode(array(
                        'responseType' => 1,
                    ));
                    return;
                ...
                else
                {
                    ...
                    {
                        $this->db->trans_rollback();
                        log_message('error', '#MR002799: Insertion failed in settlement_proceeding for case no :'. $case_no);
                        echo json_encode(array(
                            'responseType' => 1,
                        ));
                        return;
                    ...
                    else
                    {
                        ...
                        {
                            $this->db->trans_rollback();
                            $this->session->set_flashdata('message', "Error #MRAPI002810: Reverted to CO failed case no # $case_no");
                            redirect(base_url() . "index.php/home");
                        }
                        else
                        {
    ...
    public function getAllCasesForFinalVerifyByDcKhas()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $subDiv_code = $this->session->userdata('subdiv_code');
        $allCase = $this->SettlementMbSdoModel->getAllCasesForFinalVerifyAppKhas($dist_code,$subDiv_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $allCase->result();
        $data['pendingCaseCount'] = $allCase->num_rows();

        $data['_view'] = 'settlementView/Sdo/Khas/final_verify_list_by_dc_khas';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function proposalForwardToDcForFinalVerifyKhas()
    {
        ...
        else
        {
            ...
            $dist_code   = $this->session->userdata('dist_code');
            $pendingCase = $this->SettlementMbSdoModel->getAllAppInReportSendByDcToSdlacKhas($proposal_no,$subDiv_code);
            ...
            foreach ($cases as $case)
            {
                ...
                if($case->case_status == 1)
                {
                    ...
                    if($this->SettlementMbSdoModel->updateSettlementAppDetailsByCaseNoUnderProposal($caseNo,$updateCase)== 0)
                    {
                        $this->db->trans_rollback();
                        echo json_encode(array(
                            'responseType' => 1,
                    ...
                    if($this->SettlementMbSdoModel->updateSettlementBasicData($caseNo,$dist_code,$subDiv_code,$updateBasicData)== 0)
                    {
                        $this->db->trans_rollback();
                        log_message('error', '#MR002968: Insertion failed in settlement_basic for case no :'. $caseNo);
                        echo json_encode(array(
                            'responseType' => 1,
                        ));
                        return;
                    ...
                    {
                        $this->db->trans_rollback();
                        log_message('error', '#MR003000: Insertion failed in settlement_proceeding for case no :'. $caseNo);
                        echo json_encode(array(
                            'responseType' => 1,
                        ));
                        return;
                    ...
                    {
                        $this->db->trans_rollback();
                        $this->session->set_flashdata('message', "Error #MRAPI003008: Forward to DC failed case no # $caseNo");
                        redirect(base_url() . "index.php/home");
                    }
                }
                ...
                if($case->case_status == 2)
                {
                    ...
                    {
                        $this->db->trans_rollback();
                        log_message('error', '#MR003054: Insertion failed in settlement_basic for case no :'. $caseNo);
                        echo json_encode(array(
                            'responseType' => 1,
                        ));
                        return;
                    ...
                    else
                    {
                        ...
                        $mmnn = $this->SettlementMbSdoModel->updateSettlementAppDetailsByCaseNoUnderProposal($caseNo,$updatePro);

                        if($mmnn == 0)
                        {
                            $this->db->trans_rollback();
                            log_message('error', '#MR002968: Insertion failed in settlement_proposal_cases for case no :'. $caseNo);
                            echo json_encode(array(
                                'responseType' => 1,
                            ));
                            return;
                        ...
                        {
                            $this->db->trans_rollback();
                            log_message('error', '#MR003106: Insertion failed in settlement_proceeding for case no :'. $caseNo);
                            echo json_encode(array(
                                'responseType' => 1,
                            ));
                            return;
                        ...
                        else
                        {
                            ...
                            {
                                $this->db->trans_rollback();
                                $this->session->set_flashdata('message', "Error #MRAPI003111: Rejected by SDLAC failed case no # $caseNo");
                                redirect(base_url() . "index.php/home");
                            }
                        }
                        //////proceeding end//////
            ...
            if($this->SettlementMbSdoModel->updateProposalListById($proposal_no,$dist_code,$subDiv_code,$dataUpdate)== 0)
            {
                $this->db->trans_rollback();
                log_message('error', '#MR003140: Insertion failed in settlement_proposal_list ');
                echo json_encode(array(
                    'responseType' => 1,
                ));
                return;
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows) {
                ...
                $json[] = array(
                    ...
                    '<a class="btn btn-success" href="'.base_url().'index.php/SettlementMbSdo/getSettlementKhasApplicationDetails/?case='.$rows->case_no.'">Process</a>',
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows)
            {
                ...
                $json[] = array(
                    ...
                    '<a class="btn btn-success btn-sm" href="'.base_url().'index.php/SettlementMbSdo/getSettlementKhasApplicationDetails/?case='.$rows->case_no.'">View Application</a>'
                );
                ...
                $json[] = array (
                  ...
                  <div class="text-green" id="list_of_cases_'.$rows->id.'"></div>',
                    ...
                    '<a class="btn btn-sm btn-primary" target= "SDLACProposalNotice" href="'.base_url().'index.php/SettlementCommonDc/getProposalNotice/?case='.$rows->id.'">Print Notice</a>
                     ...
                     <a class="btn btn-sm btn-success" href="'.base_url().'index.php/SettlementMbSdo/getAllApplicationInReportSendByDcToSdlacKhas/?case='.$rows->id.'">
    ...
    public function getAllSdlacMemberApprovalProposalListKhas()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $subdiv_code = $this->session->userdata('subdiv_code');
        $pendingCase = $this->SettlementMbSdoModel->getSdlacApprovalProposalListKhas($dist_code,$subdiv_code);
        ...
        $data['_view'] = 'settlementView/Sdo/Khas/sdlac_approval_proposal_list_khas';
        $this->load->view('layouts/main', $data);
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows)
            {
                ...
                <div class="text-green" id="list_of_cases_'.$rows->id.'"></div>',
                    ...
                    <a class="rezaButt btn-success" href="'.base_url().'index.php/SettlementMbSdo/getSdlacMemberApproveProposalViewIndividualKhas/?case='.$rows->id.'">
                    '.'Process'.'</a>'
                );
    ...
    public function getSdlacMemberApproveProposalViewIndividualKhas()
    {
        ...
        $suv_div     = $this->session->userdata('subdiv_code');
        $proposal_no = $this->input->get('case');
        $proposalDetails  = $this->SettlementMbSdoModel->getSdlacApprovalProposalIndividualKhas($proposal_no,$dist_code,$suv_div);
        $reportDetails    = $this->SettlementMbSdoModel->getSdlacMemberReportDetailsKhas($proposal_no,$dist_code,$suv_div);
        $getMembersStatus = $this->SettlementMbSdoModel->getSdlacMemberStatus($dist_code, $proposal_no);
        ...
        $data['_view'] = 'settlementView/Sdo/Khas/sdlac_committee_report_details_khas';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function insertNomineeDetailOfSdlacMember()
    {
        ...
        else
        {
            ...
            if($insert != 1 || $insert != true) {
                log_message('warning', '#MR004239: Insertion failed in sdlac_nominee_list '.
                    $this->db->last_query());
                echo json_encode(array(
                    'responseType' => 1,
                    'message'      => '#MR004239: Something went wrong on adding nominee detail. 
