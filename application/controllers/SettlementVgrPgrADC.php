...
class SettlementVgrPgrADC extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('SettlementMb/SettlementVgrPgrAdcModel');
...
//********************** COMMON **********************************
    ...
    public function applicationRevertFromDCToCO()
    {
        ...
        else
        {
            ...
            $caseCount = $this->SettlementVgrPgrAdcModel->countSettlementApplicationDetailsByCaseNo($case_no,$dist_code);
            ...
            else
            {
                ...
                $this->db->trans_begin();
                if($this->SettlementVgrPgrAdcModel->updateSettlementBasicData($case_no,$dist_code,$updateData)== 0)
                {
                    $this->db->trans_rollback();
                    echo json_encode(array(
                        'responseType' => 1,
    ...
    // send all application to SDLAC ADC // meooo
    ...
    public function updateHearingDateGenerateNoticeVgrPgr()
    {
        ...
        else
        {
            ...
            $proposalDetails = $this->SettlementVgrPgrAdcModel->getProposalDetailsById($proposalNo,$dist_code);
            if($proposalDetails == '')
            {
                echo json_encode(array(
                    'responseType' => 3,
            ...
            {
                $allCases      = $this->SettlementVgrPgrAdcModel->getAllAppInReportSendByDcToSdlacVgrPgr($proposalNo);
                $allCasesCount = $allCases->num_rows();
                if($allCasesCount == 0)
                {
                    echo json_encode(array(
                ...
                else
                {
                    ...
                    $this->db->trans_begin();
                    if($this->SettlementVgrPgrAdcModel->updateProposalListById($proposalNo,$updateProposalData)== 0)
                    {
                        $this->db->trans_rollback();
                        echo json_encode(array(
                            'responseType' => 1,
...
//********************** COMMON **********************************
    ...
    public function SettlementVgrPgrLandDc()
    {
        ...
        $firstProceedingCount = $this->SettlementVgrPgrAdcModel->countAllPendingSettlementVgrPgr($dist_code);
        $SDLACCommitteeCount  = $this->SettlementCommonDcModel->countAllSDLACCommitteeMember($dist_code,$user_code,$user_desig_code);
        $SDLACNoticeCount     = $this->SettlementVgrPgrAdcModel->countMarkAsSDLACSettlementVgrPgr($dist_code);
        $SDLACReportCount     = $this->SettlementVgrPgrAdcModel->countAllProposalSendByDcToSdlacVgrPgr($dist_code);;
        $caseStatusCount      = 0;
        $SDLACConsideration   = $this->SettlementVgrPgrAdcModel->countAllUnderConsiderationAppVgrPgr($dist_code);
        $sdlacMemberApproval  = $this->SettlementVgrPgrAdcModel->countSdlacStatusList($dist_code);
        $circleCluster        = $this->SettlementVgrPgrAdcModel->getCircleClusters($dist_code);
        $clusterCaseReReport  = $this->SettlementVgrPgrAdcModel->clusterCaseReReport($dist_code);
        $coRejectedListCount  = $this->SettlementCommonDcModel->countCoRejectedCaseForADC($dist_code,SETTLEMENT_PGR_VGR_LAND_ID);
        $rejctedListCount     = $this->SettlementCommonDcModel->rejectedCaseList($dist_code,SETTLEMENT_PGR_VGR_LAND_ID, MB_ADD_DEPUTY_COMM);
        $revivalListCount     = $this->SettlementCommonDcModel->revivalListCount($dist_code,SETTLEMENT_PGR_VGR_LAND_ID, MB_ADD_DEPUTY_COMM);
        $vgrPgrRevertCount    = $this->SettlementCommonDcModel->countAllVgrPgrRevertedCaseForAdc($dist_code);
        $coModificationListCount = $this->SettlementPullModel->countCoModificationRequestCaseForADC($dist_code,SETTLEMENT_PGR_VGR_LAND_ID);
    ...
    public function viewAllVgrPgrFirstProceedingDCCaseList()
    {
        $this->checkingLoginUserAccessSingleUser();
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementVgrPgrAdcModel->getAllPendingSettlementVgrPgr($dist_code);
        ...
        $data['_view'] = 'settlementView/Adc/VgrPgr/first_proceeding_case_dc_vgr_pgr';
        $this->load->view('layouts/main', $data);
    ...
    public function getSettlementVgrPgrApplicationDetails()
    {
        ...
        else
        {
            ...
            $caseDetails = $this->SettlementVgrPgrAdcModel->getSettlementApplicationDetailsByCaseNo($case_no,$dist_code);
            $proceedings = $this->SettlementVgrPgrAdcModel->getSettlementProceeding($case_no);
            $data['caseCount']   = $caseCount;
            $data['caseDetails'] = $caseDetails;
            $data['proceedings'] = $proceedings;
            ...
            $data['_view'] = 'settlementView/Adc/VgrPgr/settlement_app_details_vgr_pgr';
            $this->load->view('layouts/main', $data);
        }
    }
    ...
    public function viewAllMarkAsSDLACListForDCVgrPgr()
    {
        $this->checkingLoginUserAccessSingleUser();
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementVgrPgrAdcModel->getMarkAsSDLACSettlementVgrPgr($dist_code);
        ...
        $data['location'] = $circleList;
        $data['_view']    = 'settlementView/Adc/VgrPgr/mark_as_sdlac_case_dc_vgr_pgr';
        $this->load->view('layouts/main', $data);
    ...
    public function getAllProposalListSdlacVgrPgr()
    {
        $this->checkingLoginUserAccessSingleUser();
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementVgrPgrAdcModel->getAllProposalSendByDcToSdlacVgrPgr($dist_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'settlementView/Adc/VgrPgr/proposal_list_send_to_sdlac_vgr_pgr';
        $this->load->view('layouts/main', $data);
    }


    // get all application send by dc to sdlac for report VGR PGR // meooo
    public function getAllApplicationInReportSendByDcToSdlacVgrPgr()
    {
        $this->checkingLoginUserAccessSingleUser();
        $proposal_no = $this->input->get('case');
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementVgrPgrAdcModel->getAllAppInReportSendByDcToSdlacVgrPgr($proposal_no);
        $proposalDetails = $this->SettlementVgrPgrAdcModel->getProposalDetailsById($proposal_no,$dist_code);
        ...
        $data['_view'] = 'settlementView/Adc/VgrPgr/send_to_sdlac_case_dc_pgr';
        $this->load->view('layouts/main', $data);
    ...
    public function forwardSdlac()
    {
        ...
        foreach($result as $res)
        {
            ...
            $proceeding_id = $this->SettlementCommonDcModel->getProceedingIdAdc($cluster_case_no);
            if($proceeding_id == null or $proceeding_id==0)
            {
                $proceeding_id=1;
            }
    ...
    // generate proposal notice ADC // meooo
    public function generateNoticeSendAllMarkAppToSDLACByDcVgrPgr()
    {
        ...
        else
        {
            ...
            if(!empty($allSelectedList))
            {
                ...
                $this->db->trans_begin();
                if($this->SettlementVgrPgrAdcModel->saveProposalSDLACKhas($dataProSave) == 0)
                {
                    $this->db->trans_rollback();
                    echo json_encode(array(
                        'responseType' => 1,
                ...
                else
                {
                    ...
                    foreach ($allSelectedList as $row)
                    {
                        ...
                        if($this->SettlementVgrPgrAdcModel->saveProposalCaseListSDLACKhas($saveCaseList) == 0)
                        {
                            $this->db->trans_rollback();
                            echo json_encode(array(
                                'responseType' => 1,
    ...
    public function viewApprovedAppDetailsVgrPgr()
    {
        ...
        $caseCount = $this->SettlementVgrPgrAdcModel->countSettlementAppDetailsByCaseNoOnlyView($case_no,$dist_code);
        if($caseCount == 0)
        {
            $this->SettlementVgrPgrLandDc();
        }
        else
        {
            ...
            $caseDetails = $this->SettlementVgrPgrAdcModel->getSettlementAppDetailsByCaseNoOnlyView($case_no,$dist_code);
            $proceedings = $this->SettlementVgrPgrAdcModel->getSettlementProceeding($case_no);
            $data['caseCount']   = $caseCount;
            $data['caseDetails'] = $caseDetails;
            $data['proceedings'] = $proceedings;
            $data['reservation'] = $this->SettlementVgrModel->getSettlementReservation($application_no);
            $data['_view'] = 'settlementView/Adc/VgrPgr/settlement_app_details_only_view_vgr_pgr';
            $this->load->view('layouts/main', $data);
        }
    }
    ...
    public function printNoticeReservation()
    {
        $this->checkingLoginUserAccessSingleUser();
        $case_no = $this->input->get('case_no');
        // getting the notice file link
        $data['print_data'] = $this->SettlementVgrModel->getNoticeDataReservation($case_no);
    ...
    public function generateGeneralNotice()
    {
        ...
        if ($this->form_validation->run() == FALSE)
        {
            echo json_encode(array(
                'responseType' => 1,
                'message' => '#ERR3253: Something went wrong! Contact admin...'
            ));
            return;
        }
        else
        {
            ...
            else if($this->session->userdata('user_desig_code') == 'SDO')
            {
            ...
            {
                $caseDetails     = $this->SettlementVgrPgrAdcModel->getSettlementApplicationDetailsByCaseNo($case_no,$dist_code);
                $get_dag_details = $this->SettlementVgrPgrAdcModel->getReservationDetails($case_no,$dist_code);
    ...
    public function generateGeneralNoticeReservation()
    {
        ...
        else
        {
            ...
            {
                $caseDetails     = $this->SettlementVgrPgrAdcModel->getSettlementApplicationDetailsByCaseNo($case_no,$dist_code);
                $get_dag_details = $this->SettlementVgrPgrAdcModel->getReservationDetailsReservation($case_no,$dist_code);
                $applicantDetail = $this->SettlementCommonDcModel->getApplicantDetails($case_no);
                $dist_name    = $this->UtilsModel->getDistrictNameByDistCode($caseDetails->dist_code);
                $circle_name  = $this->UtilsModel->getCircleDetailsByDistDivision($caseDetails->dist_code,$caseDetails->subdiv_code,$caseDetails->cir_code);
                $mouza_name   = $this->UtilsModel->getMouzaDetailsByDistDivisionCircle($caseDetails->dist_code,$caseDetails->subdiv_code,$caseDetails->cir_code,$caseDetails->mouza_pargona_code);
                ...
                foreach($get_dag_details as $dereserveRes)
                {
                    if(in_array($dereserveRes->dist_code, json_decode(BARAK_VALLEY)))
                    {
                        $deReserveArr .=  '<div>
                                            <span><b>'.$dereserveRes->dag_no.'</b>নং দাগৰ অংশ</span>
                                            <span><b>'.$dereserveRes->dag_area_b.'</b>বি:</span>
                                            <span><b>'.$dereserveRes->dag_area_k.'</b>ক:</span>
                                            <span><b>'.$dereserveRes->dag_area_lc.'</b>চ:</span>
                                            <span><b>'.$dereserveRes->dag_area_g.'</b>গ:</span>
                                        </div>';
                    }
                    else
                    {
                        $deReserveArr .=  '<div>
                                            <span><b>'.$dereserveRes->dag_no.'</b> নং দাগৰ অংশ</span>
                                            <span><b>'.$dereserveRes->dag_area_b.'</b> বিঘা</span>
                                            <span><b>'.$dereserveRes->dag_area_k.'</b> কঠা</span>
                                            <span><b>'.$dereserveRes->dag_area_lc.'</b> লেছা</span>
                                        </div>';
                    }
                }
    ...
    public function saveGeneralNoticeVgrPgr()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                $sql = $this->db->query('select * from settlement_notice where case_no = ? and notice_type = ?', array($case_no, 'GN'));
    ...
    public function saveGeneralNoticeVgrPgrReservation()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
        $this->load->library('form_validation');
        ...
        else
        {
            ...
            else
            {
                ...
                $sql = $this->db->query('select * from settlement_notice where case_no = ? and notice_type = ?', array($case_no, 'GNR'));
                ...
                $updateData = array(
                    // 'general_notice_dc' => 'y',
                    'notice_reservation_yn' => 'y',
                    'notice_generated_date_reservation' => date('Y-m-d H:i:s'),
                    'next_date_of_hearing_reservation' => $hearing_date,
                    'date_update' => date('Y-m-d H:i:s'),
                    //'dc_proceeding'     => 1,
                );
                if($this->SettlementVgrPgrAdcModel->updateSettlementBasicData($case_no,$dist_code,$updateData)== 0)
    ...
    public function viewGeneralNoticeVgrPgr()
    {
        $this->checkingLoginUserAccessSingleUser();
        $case_no = $this->input->get('case');
        $noticeDetails = $this->SettlementVgrPgrAdcModel->getGeneralNoticeDetails($case_no);
        ...
        $data['_view'] = 'settlementView/Adc/VgrPgr/general_notice_print';
    ...
    public function firstProceedingPaginationAPI()
    {
        ...
        if($query->num_rows() > 0)
        {
            ...
            foreach($result as $rows)
            {
                ...
                //                {
                //                    $view_generate_notice = '<a class="rezaButt buttPrimary" href="'.base_url().'index.php/SettlementVgrPgrADC/getSettlementVgrPgrApplicationDetails/?case='.$rows->case_no.'">
                //                            <i class="fa fa-cog" aria-hidden="true"></i> Process
                //                        </a>
                //                        <a target="_blank" class="rezaButt buttInfo" href="'.base_url().'index.php/SettlementVgrPgrADC/viewGeneralNoticeVgrPgr/?case='.$rows->case_no.'">
    ...
    public function secondProceedingSdlacRecommendedMarked()
    {
        ...
        $this->db->where_in('pending_at', MB_ADD_DEPUTY_COMM);
        $this->db->limit($length, $start);
        $query = $this->db->get();
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows)
            {
                $json[] = array(
                    ...
                    <a class="btn btn-success" href="'.base_url().'index.php/SettlementVgrPgrADC/viewSDLACListForDCVgrPgr?cid='.$rows->cluster_id.'">
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows) {
                ...
                $json[] = array (
                    ...
                    <a class="btn  btn-sm btn-success" href="'.base_url().'index.php/SettlementVgrPgrADC/getAllApplicationInReportSendByDcToSdlacVgrPgr/?case='.$rows->id.'">
    ...
    public function updateProposalHearingDateKhas()
    {
        ...
        else
        {
            ...
            $allCases      = $this->SettlementVgrPgrAdcModel->getAllAppInReportSendByDcToSdlacKhas($proposalNo);
            $allCasesCount = $allCases->num_rows();
        ...
        else
        {
            ...
            foreach ($cases as $case)
            {
                ...
                if($case->case_status == 1)
                {
                    ...
                    if($this->SettlementVgrPgrAdcModel->updateSettlementAppDetailsByCaseNoUnderProposal($caseNo,$updateCase)== 0)
                    {
                        $this->db->trans_rollback();
                        echo json_encode(array(
                            'responseType' => 1,
                    ...
                    if($this->SettlementVgrPgrAdcModel->updateSettlementBasicData($caseNo,$dist_code,$updateBasicData)== 0)
                    {
                        $this->db->trans_rollback();
                        echo json_encode(array(
                            'responseType' => 1,
                ...
                if($case->case_status == 2)
                {
                    ...
                    else
                    {
                        ...
                        $mmnn = $this->SettlementVgrPgrAdcModel->updateSettlementAppDetailsByCaseNoUnderProposal($caseNo,$updatePro);
            ...
            if($this->SettlementVgrPgrAdcModel->updateProposalListById($proposal_no,$dataUpdate)== 0)
            {
                $this->db->trans_rollback();
                echo json_encode(array(
                    'responseType' => 1,
    ...
    public function getAllSdlacMemberApprovalProposalListVgrPgr()
    {
        ...
        $dist_code   = $this->session->userdata('dist_code');
        $subdiv_code = $this->session->userdata('subdiv_code');
        $pendingCase = $this->SettlementVgrPgrAdcModel->getSdlacApprovalProposalListVgrPgr($dist_code);
        ...
        $data['_view'] = 'settlementView/Adc/VgrPgr/sdlac_approval_proposal_list_vgr_pgr';
        $this->load->view('layouts/main', $data);

    }


    // get all forth proceeding SDLAC Report with data
        ...
        if($query->num_rows() > 0)
        {
            ...
            foreach($result as $rows)
            {
                $json[] = array (
                    ...
                    <a class="rezaButt btn-success" href="'.base_url().'index.php/SettlementVgrPgrADC/getSdlacMemberApproveProposalViewIndividualVgrPgr/?case='.$rows->id.'">
    ...
    public function getSdlacMemberApproveProposalViewIndividualVgrPgr()
    {
        ...
        $dist_code   = $this->session->userdata('dist_code');
        $proposal_no = $this->input->get('case');
        $proposalDetails = $this->SettlementVgrPgrAdcModel->getSdlacApprovalProposalIndividualVgrPgr($proposal_no,$dist_code);

        // SDLAC/CDLAC Member report details
        $reportDetails   = $this->SettlementVgrPgrAdcModel->getSdlacMemberReportDetailsVgrPgr($proposal_no,$dist_code);

        $getMembersStatus = $this->SettlementVgrPgrAdcModel->getSdlacMemberStatus($dist_code, $proposal_no);
        ...
        $data['_view'] = 'settlementView/Adc/VgrPgr/sdlac_committee_report_details_vgr_pgr';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function sdlacRecommendedMarked()
    {
        ...
        $this->db->where_in('settlement_basic.pending_officer', $this->session->userdata('user_desig_code'));
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows)
            {
                ...
                {
                    $linkView = '<a class="btn btn-success btn-sm m-1" href="'.base_url().'index.php/SettlementVgrPgrADC/getSettlementVgrPgrApplicationDetails?case='.$rows->case_no.'">View Application</a>
                    
                    <a target="_blank" href="'.base_url().'index.php/SettlementVgrPgrADC/printNotice?case_no='.$rows->case_no.'" class="btn btn-info btn-sm m-1"><i class="fa fa-check-square-o" aria-hidden="true"></i>Print De-Reservation Notice</a>
                    
                    <button type="button" onclick="generateNotice(\''.$rows->case_no.'\')" id="generateNotice" class="m-1 btn btn-sm btn-danger">Re-Generate De-Reservation Notice</button>
                    
                    <a target="_blank" href="'.base_url().'index.php/SettlementVgrPgrADC/printNoticeReservation?case_no='.$rows->case_no.'" class="btn btn-info btn-sm m-1">
                    <i class="fa fa-check-square-o" aria-hidden="true"></i>
                    Print Reservation Notice
                    </a> 
                    
                    <button class="btn btn-danger btn-sm m-1" onclick="generateNoticeReservation(\''.$rows->case_no.'\')" id="generateNoticeReservation">
                    <i class="fa fa-check-square-o" aria-hidden="true"></i>
                    Re-Generate Reservation Notice
                    </button>';
                }
                elseif($user_desig_code == 'SDO')
                {
                    $linkView = '<a class="btn btn-success btn-sm" href="'.base_url().'index.php/SettlementVgrPgrSdo/getSettlementVgrPgrApplicationDetails?case='.$rows->case_no.'">View Application</a>
