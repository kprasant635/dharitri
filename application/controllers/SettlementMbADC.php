...
class SettlementMbADC extends CI_Controller
{
...
//********************** COMMON **********************************
    ...
    public function SettlementKhasLandDc()
    {
        ...
        // todo
        $firstProceedingCount = $this->SettlementMbADCModel->countAllPendingSettlementKhas($dist_code);
        $SDLACCommitteeCount  = $this->SettlementCommonDcModel->countAllSDLACCommitteeMember($dist_code,$user_code,$user_desig_code);
        $SDLACNoticeCount     = $this->SettlementMbADCModel->countMarkAsSDLACSettlementKhas($dist_code);
        $SDLACReportCount     = $this->SettlementMbADCModel->countAllProposalSendByDcToSdlacKhas($dist_code);
        $caseStatusCount      = 0;
        ...
        $data['_view'] = 'settlementView/Adc/Khas/first_landing_page_dc_khas';
        $this->load->view('layouts/main', $data);
    ...
    public function viewAllKhasFirstProceedingDCCaseList()
    {
        $this->checkingLoginUserAccessSingleUser();
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementMbADCModel->getAllPendingSettlementKhas($dist_code);
        ...
        $data['_view'] = 'settlementView/Adc/Khas/first_proceeding_case_dc_khas';
        $this->load->view('layouts/main', $data);
    ...
    public function viewAllMarkAsSDLACListForDCKhas()
    {
        $this->checkingLoginUserAccessSingleUser();
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementMbADCModel->getMarkAsSDLACSettlementKhas($dist_code);
        // $getDistrict = $this->SettlementMbDcModel->getLocationName($dist_code);
    ...
    public function getAllProposalListSdlacKhas()
    {
        $this->checkingLoginUserAccessSingleUser();
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementMbADCModel->getAllProposalSendByDcToSdlacKhas($dist_code);
    ...
    public function generateNoticeSendAllMarkAppToSDLACByDcKhas()
    {
        ...
        else
        {
            ...
            if(!empty($allSelectedList))
            {
                ...
                $this->db->trans_begin();
                if($this->SettlementMbADCModel->saveProposalSDLACKhas($dataProSave) == 0)
                {
                    $this->db->trans_rollback();
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows)
            {
                ...
                $json[] = array(
                    ...
                    '<a class="btn btn-success" href="'.base_url().'index.php/SettlementMbADC/getSettlementKhasApplicationDetails/?case='.$rows->case_no.'">
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows)
            {
                ...
                $json[] = array(
                    ...
                    '<a class="btn btn-success btn-sm" href="'.base_url().'index.php/SettlementMbADC/getSettlementKhasApplicationDetails/?case='.$rows->case_no.'">View Application</a>'
                );
