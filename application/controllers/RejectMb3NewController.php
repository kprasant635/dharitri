...
class RejectMb3NewController extends CI_Controller
{
    ...
    public function getRejectModal()
    {
        ...
        // var_dump($lm_rej_remark1); die;

        $sqlRes = "SELECT chitha_flag, reject_code,remark, remark_head FROM reject_master WHERE flag=? and service_code=? and remark_head is not null";
        $res = $this->db->query($sqlRes, array('1', $service_code))->result();
        ...
        {
            $basicRow = $this->db->query('select * from settlement_basic where case_no = ?', array($case_no));
            if($basicRow->num_rows() <= 0)
            {
                $data = array(
                    'responseType'   => 2,
                    'responseStatus' => 1,
                    'message'        => '#MRREJGEO01: Something went wrong! Contact admin',
                );
                echo json_encode($data);
                return;
            }

            $caseDetails = $basicRow->row();
            $caseApplIds = trim($caseDetails->applid);

            if(GEO_TAG_VERIFICATION_FOR_REJECT  == 1)
            {
                $supportive_document_sql = $this->db->query("SELECT * FROM supportive_document WHERE (case_no =? or applid =?) and file_name=? ", array($caseApplIds, $caseApplIds, GEO_TAG_PHOTO));
                if($supportive_document_sql->num_rows() <= 0)
                {
                    ...
                    $data = array(
                        'responseType'   => 2,
                        'responseStatus' => 1,
                        'message' => '#MRREJGEO02: Geo tag photo yet to be uploaded',
                    );
                    echo json_encode($data);
                    return;
                }
...
//                'rules' => 'trim|required|min_length[50]|callback_check_script|xss_clean',
        ...
        if ($params[1] != 1) {
            $this->db->trans_rollback();
            log_message('error', '#ERREJ990001: Updating failed in ' . $params[0] . ' and query is: ' . $this->db->last_query());
            $json = [
                'success' => false,
                'message' => '#ERREJ990001: Unable to process',
            ];
            echo json_encode($json);
            return;
        }
        ...
        if($this->db->affected_rows() <= 0 && $service_code !=TEA_SERVICE_CODE)
        {
            $this->db->trans_rollback();
...
// insert proceeding direct rejected dc adc ap case
    public function proceedingDCADCDirectRemarkAp($case_no,$rejectedReasonList,$remark)
    {
        $case_no = trim($case_no);
        ...
        $values = array(
            'case_no' => $case_no,
            'proceeding_id' => $proceeding_id,
            'co_order' => 'Rejected for: '.$rejectedReasonList,
            'status' => 'Rejected',
            'user_code' => $this->session->userdata('user_code'),
    ...
    public function proceedingRemarkAp($case_no,$rejectedReasonList,$remark)
    {
        $case_no = trim($case_no);
    ...
    public function rejectDirectTableServiceWise($case_no, $user_code, $service_code)
    {
        $case_no = trim($case_no);
...
// update on petition basic table for direct reject
    public function rejectDirectTableAp($case_no, $user_code)
    {
        $case_no = trim($case_no);
...
// update on settlement basic table
    ...
    // update on petition basic table for direct reject
    public function rejectDirectTableReclass($case_no, $user_code)
    {
        $case_no = trim($case_no);
    ...
    function getApplid($case_no)
    {
        ...
        $query = "SELECT basundhara FROM basundhar_application WHERE dharitree = '$case_no'";
...
//                'rules' => 'trim|required|min_length[50]|callback_check_script|xss_clean',
        ...
        if($service_code != SERVICE_CONVERSION_MB3)
        {
        ...
        // check pull request from here
        if($service_code != SERVICE_CONVERSION_MB3 && $service_code != RECLASS_ID)
        {
            $basic = $this->SettlementPullModel->getSettlementBasicDetails($case_no);
            if($basic->pull_request == 1)
            {
        ...
        if($service_code == SERVICE_CONVERSION_MB3)
        {
            $params = $this->rejectDirectTableAp($case_no, $user_code);
        }
        else if($service_code == RECLASS_ID)
        {
            $params = $this->rejectDirectTableReclass($case_no, $user_code);
        }
        else
        {
            $params = $this->rejectDirectTableServiceWise($case_no, $user_code, $service_code);
        }

        if ($params[1] != 1)
        {
            $this->db->trans_rollback();
            log_message('error', '#ERREJ780001: Updating failed in ' . $params[0] . ' and query is: ' . $this->db->last_query());
            $json = [
                'success' => false,
                'message' => '#ERREJ780001: Unable to process',
            ];
            echo json_encode($json);
            return;
        }
        ...
        if($service_code == SLIJE_ID)
        {
            ...
            if($basicRow->num_rows() <= 0)
            {
                $data = array(
                    'success' => false,
                    'status'  => 1,
                    'message' => '#MRREJGEO03: Something went wrong! Contact admin',
                );
                echo json_encode($data);
                return;
            }
            ...
            if(GEO_TAG_VERIFICATION_FOR_REJECT  == 1)
            {
                ...
                if($supportive_document_sql->num_rows() <= 0)
                {
                    ...
                    $data = array(
                        'success' => false,
                        'status'  => 1,
                        'message' => '#MRREJGEO04: Geo tag photo yet to be uploaded',
                    );
                    echo json_encode($data);
                    return;
                }
        ...
        foreach($diffArray as $difAr)
        {
            ...
            if ($insert != 1) {
                $this->db->trans_rollback();
                log_message('error', '#ERREJ0003: Insertion failed in rejected_remark and query is: ' . $this->db->last_query());
                $json = [
                    'success' => false,
        ...
        if($service_code == SERVICE_CONVERSION_MB3)
        {
            if ($desg == 'CO')
            {
                $proceeding = $this->proceedingRemarkAp($case_no, $rejectedReasonList,$remark);
            }
            else
            {
                $proceeding = $this->proceedingDCADCDirectRemarkAp($case_no, $rejectedReasonList,$remark);
            }
        ...
        if($service_code == SERVICE_CONVERSION_MB3)
        {
            $application_no = $this->getApplid($case_no)->basundhara;
        }
        else if($service_code == RECLASS_ID)
        {
        ...
        $rtps_status = $this->basundhara3Model->postApiBasundharaForRejectedCase2nd($application_no, $case_no, $rmk, $status, $task, $pen, $rejectCodeArray);
        if ($rtps_status != "y")
        {
            $this->db->trans_rollback();
    ...
    public function deleteRejectRemarkOnConfirm()
    {
        ...
        if($service_code == RECLASS_ID)
        {
            ...
            {
                $this->db->trans_rollback();
                log_message('error', '#ERREJ00013333422: Unable to update reclass_suite_basic: ' . $this->db->last_query());
                $json = [
                    'response' => 0,
                    'message' => '#ERREJ00013333422: Unable to process! Contact admin...',
                ];
                echo json_encode($json);
                return;
            }
