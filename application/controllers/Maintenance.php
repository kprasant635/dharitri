 ...
 */
/* Developed by Partha Sarathi Das 02/04/2019 */
...
class Maintenance extends CI_Controller {
    ...
    public function index() {
        redirect(base_url() . "index.php/Maintenance/Menu ");
    }
    
    public function Menu() {
        // $this->load->view('../views/header');
        // $this->load->view('../views/Maintenance/Menu');
        // $this->load->view('../views/footer');

        $data['_view'] = 'Maintenance/Menu';
        $this->load->view('layouts/main',$data);
    }

    // public function JamabandiStatus() {
    ...
    //         $this->db->query("Update chitha_basic set jama_yn = '' where dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$cir_code' "
    ...
    //             $this->session->set_flashdata('message', 'Sorry...Status Cannot be Changed !! Please Contact Help Desk !!');
    //             redirect(base_url() . "index.php/Maintenance");
    //             echo "Error Occured";
    //         } else {
    //             $this->db->trans_commit();
    //             $this->session->set_flashdata('message', "Jamabandi Status Changed for Re Updation.");
    //             redirect(base_url() . "index.php/Maintenance");
    //         }
    //     } else {
    ...
    //         $patta_types = $this->db->query("select type_code,patta_type from    patta_code where jamabandi='y' order by type_code")->result();
    ...
    //         // $this->load->view('../views/header');
    //         // $this->load->view('../views/Maintenance/SelectLocForJamabandi', $district);
    //         // $this->load->view('../views/footer');

    //         $district['_view'] = 'Maintenance/SelectLocForJamabandi';
    //         $this->load->view('layouts/main',$district);
    //     }
    // }
    
    function ExistingDagNo($patta_no, $patta_type_code, $dist_code, $subdiv_code, $circle_code, $mouza_pargona_code, $lot_no, $vill_townprt_code){
        	//$db=  $this->session->userdata('db');
        $q = "select dag_no as dag_no from    chitha_basic p where p.dist_code='$dist_code' and p.subdiv_code='$subdiv_code' and p.cir_code='$circle_code' and
         ...
         $string = '';
         $comma1 = ' এই পট্টাত অন্তর্ভূক্ত হৈ থকা দাগ : ';
         ...
         foreach ($data as $key=>$value) {
            ...
            $exist=array('val'=>$string);
            echo json_encode($exist);
        
        
    }

    public function PattadarMatching() {
			$db=  $this->session->userdata('db');
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            //var_dump($_POST);
            $dist_code = $this->input->post('dist_code');
            ...
            $patta_type_name = $this->db->query("select patta_type as patta_type_name from    patta_code where type_code = '$patta_type_code' ")->row()->patta_type_name;
...
//            p.mouza_pargona_code='$mouza_pargona_code' and p.vill_townprt_code='$vill_townprt_code' 
//            and d.lot_no='$lot_no' and trim(p.patta_no)='$patta_no' and d.p_flag!='1' and p.patta_type_code='$patta_type_code'")->result(); //
//            var_dump($data);
            
            $chitha_pattadar = "select pdar_id,pdar_name,pdar_father from    chitha_pattadar where dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$cir_code' "
                    . "and mouza_pargona_code='$mouza_pargona_code' and lot_no='$lot_no' and vill_townprt_code='$vill_townprt_code' and TRIM(patta_no)='$patta_no' "
                    . "and patta_type_code='$patta_type_code'";
            $data['chitha_pattadar'] = $this->db->query($chitha_pattadar)->result();
            
            $jama_pattadar = "select * from    jama_pattadar where dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$cir_code' "
                    . "and mouza_pargona_code='$mouza_pargona_code' and lot_no='$lot_no' and vill_townprt_code='$vill_townprt_code' and TRIM(patta_no)='$patta_no' "
                    . "and patta_type_code='$patta_type_code' order by cast(pdar_id as integer)";
            $data['jama_pattadar'] = $this->db->query($jama_pattadar)->result();
            
            //var_dump($data);
            // $this->load->helper('html');
            // $this->load->view('../views/header');
            // $this->load->view('../views/Maintenance/ChithaVsJamaPattadars', $data);
            // $this->load->view('../views/footer');

            $data['_view'] = 'Maintenance/ChithaVsJamaPattadars';
            $this->load->view('layouts/main',$data);
        } else {
            $dist_code = $this->session->userdata('dist_code');
            $subdiv_code = $this->session->userdata('subdiv_code');
            ...
            // $this->load->helper('html');
            // $this->load->view('../views/header');
            // $this->load->view('../views/Maintenance/SelectLocForPattadar', $district);
            // $this->load->view('../views/footer');

            $district['_view'] = 'Maintenance/SelectLocForPattadar';
            $this->load->view('layouts/main',$district);
        }
    }

    function DeletePattadarChitha($merge_id, $pdar_id, $patta_no, $patta_type_code, $dist_code, $subdiv_code, $circle_code, $mouza_pargona_code, $lot_no, $vill_townprt_code){
      	$db=  $this->session->userdata('db');
	  $this->db->trans_begin();
        $string = '';
        $pdar_id = TRIM($pdar_id);
        $merge_id = TRIM($merge_id);
        
        $dag_no = "select dag_no as dag_no from    chitha_dag_pattadar p where p.dist_code='$dist_code' and p.subdiv_code='$subdiv_code' and p.cir_code='$circle_code' and "
                ...
                . "and p.patta_type_code='$patta_type_code' and pdar_id = '$pdar_id' ";
         $get_dag_no = $this->db->query($dag_no)->result();
         
         foreach ($get_dag_no as $data) {
             $chick_new_id_exist = "select count(*) as count from    chitha_dag_pattadar p where p.dist_code='$dist_code' and p.subdiv_code='$subdiv_code' and p.cir_code='$circle_code' and "
                . "p.mouza_pargona_code='$mouza_pargona_code' and p.vill_townprt_code='$vill_townprt_code' and p.lot_no='$lot_no' and TRIM(p.patta_no)='$patta_no' "
                . "and p.patta_type_code='$patta_type_code' and pdar_id = '$merge_id' and dag_no = '$data->dag_no' ";
             
             $count = $this->db->query($chick_new_id_exist)->row()->count;
             
             if($count == '0'){
                 $this->db->query("Update chitha_dag_pattadar set pdar_id = '$merge_id'  where dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$circle_code' and "
                . "mouza_pargona_code='$mouza_pargona_code' and vill_townprt_code='$vill_townprt_code' and lot_no='$lot_no' and TRIM(patta_no)='$patta_no' "
                . "and patta_type_code='$patta_type_code' and pdar_id = '$pdar_id' and dag_no = '$data->dag_no' ");
             } else {
                 $this->db->query("Delete from    chitha_dag_pattadar where dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$circle_code' and "
                . "mouza_pargona_code='$mouza_pargona_code' and vill_townprt_code='$vill_townprt_code' and lot_no='$lot_no' and TRIM(patta_no)='$patta_no' "
                . "and patta_type_code='$patta_type_code' and pdar_id = '$pdar_id' and dag_no = '$data->dag_no' ");
             }
         }
         
         $this->db->query("Delete from    chitha_pattadar where dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$circle_code' and "
                . "mouza_pargona_code='$mouza_pargona_code' and vill_townprt_code='$vill_townprt_code' and lot_no='$lot_no' and TRIM(patta_no)='$patta_no' "
                . "and patta_type_code='$patta_type_code' and pdar_id = '$pdar_id' ");
    ...
    function UpdatePattadarChitha(){
		//	$db=  $this->session->userdata('db');
        //var_dump($_GET);
        $this->db->trans_begin();
        $dist_code = $this->input->get('dist_code_new');
        $subdiv_code = $this->input->get('subdiv_code_new');
        $circle_code = $this->input->get('circle_code_new');
        $mouza_pargona_code = $this->input->get('mouza_code_new');
        $lot_no = $this->input->get('lot_no_new');
        $vill_townprt_code = $this->input->get('village_new');
        $patta_no = TRIM($this->input->get('patta_no'));
        $patta_type_code = $this->input->get('patta_type_code');
        
        $pdar_id = TRIM($this->input->get('pdar_id'));
        $new_pdar_id = TRIM($this->input->get('new_pdar_id'));
        $new_pdar_name = $this->input->get('new_pdar_name');
        $new_pdar_father = $this->input->get('new_pdar_father');
        
        $dag_no = "select * from    chitha_dag_pattadar p where p.dist_code='$dist_code' and p.subdiv_code='$subdiv_code' and p.cir_code='$circle_code' and "
                . "p.mouza_pargona_code='$mouza_pargona_code' and p.vill_townprt_code='$vill_townprt_code' and p.lot_no='$lot_no' and TRIM(p.patta_no)='$patta_no' "
                . "and p.patta_type_code='$patta_type_code' and pdar_id = '$pdar_id' ";
         $get_dag_no = $this->db->query($dag_no)->result();
         ...
         foreach ($get_dag_no as $data) {
             $chick_new_id_exist = "select count(*) as count from    chitha_dag_pattadar p where p.dist_code='$dist_code' and p.subdiv_code='$subdiv_code' and p.cir_code='$circle_code' and "
                . "p.mouza_pargona_code='$mouza_pargona_code' and p.vill_townprt_code='$vill_townprt_code' and p.lot_no='$lot_no' and TRIM(p.patta_no)='$patta_no' "
                . "and p.patta_type_code='$patta_type_code' and pdar_id = '$new_pdar_id' and dag_no = '$data->dag_no' ";
             
             $count = $this->db->query($chick_new_id_exist)->row()->count;
             
             if($count == '0'){
                 $this->db->query("Update chitha_dag_pattadar set pdar_id = '$new_pdar_id' where dist_code='$dist_code' "
                         . "and subdiv_code='$subdiv_code' and cir_code='$circle_code' and mouza_pargona_code='$mouza_pargona_code' and vill_townprt_code='$vill_townprt_code' and "
                         . "lot_no='$lot_no' and TRIM(patta_no)='$patta_no' and patta_type_code='$patta_type_code' and pdar_id = '$pdar_id' and dag_no = '$data->dag_no' ");
             }
         }
         
        //  $this->db->query("Update chitha_pattadar set pdar_id = '$new_pdar_id',pdar_name = '$new_pdar_name', pdar_father = '$new_pdar_father' where dist_code='$dist_code' and "
        //          . "subdiv_code='$subdiv_code' and cir_code='$circle_code' and mouza_pargona_code='$mouza_pargona_code' and vill_townprt_code='$vill_townprt_code' and "
        //          . "lot_no='$lot_no' and TRIM(patta_no)='$patta_no' and patta_type_code='$patta_type_code' and pdar_id = '$pdar_id' ");

        $table = 'chitha_pattadar';

        $params = [
            'pdar_id'     => $new_pdar_id,
            'pdar_name'   => $new_pdar_name,
            'pdar_father' => $new_pdar_father,
        ];
        ...
        $where = [
            ...
            'lot_no'             => $lot_no,
            'patta_no'           => trim($patta_no), // Equivalent to TRIM() in SQL
            'patta_type_code'    => $patta_type_code,
            'pdar_id'            => $pdar_id,
        ];
    ...
    function DeletePattadarJama($pdar_id, $patta_no, $patta_type_code, $dist_code, $subdiv_code, $circle_code, $mouza_pargona_code, $lot_no, $vill_townprt_code){
			$db=  $this->session->userdata('db');
        $this->db->trans_begin();
        $string = '';
        $pdar_id = TRIM($pdar_id);
        
        $this->db->query("Delete from    jama_pattadar where dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$circle_code' and "
                . "mouza_pargona_code='$mouza_pargona_code' and vill_townprt_code='$vill_townprt_code' and lot_no='$lot_no' and TRIM(patta_no)='$patta_no' "
                . "and patta_type_code='$patta_type_code' and pdar_id = '$pdar_id' ");
    ...
    function UpdatePattadarJama(){
        ...
        $this->db->query("Update jama_pattadar set pdar_id = '$new_pdar_id',pdar_name = '$new_pdar_name', pdar_father = '$new_pdar_father' where dist_code='$dist_code' and "
                 . "subdiv_code='$subdiv_code' and cir_code='$circle_code' and mouza_pargona_code='$mouza_pargona_code' and vill_townprt_code='$vill_townprt_code' and "
                 . "lot_no='$lot_no' and TRIM(patta_no)='$patta_no' and patta_type_code='$patta_type_code' and pdar_id = '$pdar_id' ");
    ...
    // function TransferPattadarToJama(){
	// 		$db=  $this->session->userdata('db');
    //     //var_dump($_GET);
    //     $this->db->trans_begin();
    //     $dist_code = $this->input->get('dist_code_new');
    ...
    //     $pattadar_details = $this->db->query("select * from    chitha_pattadar p join chitha_dag_pattadar d on p.dist_code = d.dist_code and p.subdiv_code = d.subdiv_code and 
    //         p.cir_code = d.cir_code and p.lot_no = d.lot_no and p.vill_townprt_code = d.vill_townprt_code and p.mouza_pargona_code = d.mouza_pargona_code and
    //         p.pdar_id = d.pdar_id and trim(p.patta_no) = trim(d.patta_no) where p.dist_code='$dist_code' and p.subdiv_code='$subdiv_code' and p.cir_code='$circle_code' and
    //         p.mouza_pargona_code='$mouza_pargona_code' and p.vill_townprt_code='$vill_townprt_code' and d.lot_no='$lot_no' and trim(p.patta_no)='$patta_no' and "
    //             . "d.pdar_id = '$pdar_id' and p.patta_type_code='$patta_type_code' ")->row(); 

    //     //var_dump($pattadar_details);
    //     if(!empty($new_pdar_id)){
    //         $chick_new_id_exist = "select count(*) as count from    jama_pattadar p where p.dist_code='$dist_code' and p.subdiv_code='$subdiv_code' and p.cir_code='$circle_code' and
    //         p.mouza_pargona_code='$mouza_pargona_code' and p.vill_townprt_code='$vill_townprt_code' and p.lot_no='$lot_no' and trim(p.patta_no)='$patta_no' and "
    //             . "p.pdar_id = '$new_pdar_id' and p.patta_type_code='$patta_type_code' ";
             
    //          $count = $this->db->query($chick_new_id_exist)->row()->count;
             
    //          if($count > '0'){
    //              $new_max_exist = $this->db->query("select MAX(cast(pdar_id as integer)+1) as pdar_id from    jama_pattadar p where p.dist_code='$dist_code' and p.subdiv_code='$subdiv_code' and "
    //                      . "p.cir_code='$circle_code' and p.mouza_pargona_code='$mouza_pargona_code' and p.vill_townprt_code='$vill_townprt_code' and p.lot_no='$lot_no' and "
    //                      . "trim(p.patta_no)='$patta_no' and p.patta_type_code='$patta_type_code' ")->row()->pdar_id;
                 
    //              $new_pdar_id = $new_max_exist;
    //          }
    ...
    //             'patta_no' =>$patta_no, 
    //             'pdar_id' =>$new_pdar_id, 
    //             'pdar_name' =>$pattadar_details->pdar_name, 
    //             'pdar_father' =>$pattadar_details->pdar_father, 
    //             'pdar_add1' =>$pattadar_details->pdar_add1, 
    //             'pdar_add2' =>$pattadar_details->pdar_add2,
    //             'pdar_land_b' =>$pattadar_details->dag_por_b, 
    //             'pdar_land_k' =>$pattadar_details->dag_por_k, 
    //             'pdar_land_lc' =>$pattadar_details->dag_por_lc,
    //             'pdar_land_g' =>$pattadar_details->dag_por_g,
    //             'p_flag' =>$pattadar_details->p_flag, 
    //             'user_code' =>$pattadar_details->user_code, 
    //             'entry_date' =>$pattadar_details->date_entry, 
    //             'entry_mode' =>$pattadar_details->operation, 
    //             'new_pdar_name' =>$pattadar_details->new_pdar_name, 
    //             'pdar_gender' =>$pattadar_details->pdar_gender, 
    //             'pdar_minor_yn' =>$pattadar_details->pdar_minor_yn
    //         );
    //         //var_dump($jama_pattadar);
    //         $this->db->insert('jama_pattadar', $jama_pattadar);  //*********************
    //     }
...
////////////////////////////////////////////////////////////////////////
    ...
    public function get_all_junk_dags() {
        ...
        $query1 = $this->db->query("select * from    chitha_basic where dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$cir_code' and TRIM(dag_no) ~* '[^a-z0-9]' ");
...
//public function modifydagpatta($pattatypeCode,$dist_code,$subdiv_code,$circle_code,$mouza_code,$lot_no,$vill_code) {
    ...
    public function delete_dags_info() {
        ...
        echo json_encode("Error:MAINTENANCE#2500002-Plz Contact System Admin");
        die;
		//	$db=  $this->session->userdata('db');
        $dist_code = $this->input->post('dist_code');
        $subdiv_code = $this->input->post('subdiv_code');
...
//     }
  ...
  //       $this->load->view('../views/footer');
        ...
        $data['_view'] = 'utility/delete_half_donecase_o';
        $this->load->view('layouts/main',$data);
    }

    public function delete_half_done() {
		//$db=  $this->session->userdata('db');
        $case_no['case'] = $this->input->post('case_no');
        // $this->load->helper('html');
        // $this->load->view('../views/header');
        // $this->load->view('../views/utility/half_done_delete_report', $case_no);
        // $this->load->view('../views/footer');

        $case_no['_view'] = 'utility/half_done_delete_report';
        $this->load->view('layouts/main',$case_no);
...
//                . "and mouza_Pargona_code = '$mouza_pargona_code' and lot_no='$lot_no' and vill_townprt_code='$vill_code'";
    ...
    public function searchpattano() {
        ...
        $district['_view'] = 'utility/searchpattano';
        $this->load->view('layouts/main',$district);
    }
...
//        return $arrData["data"];
  ...
  //       $this->load->view('header');
    ...
    public function DeleteCol8Order() {
        ...
        $data['_view'] = 'utility/Delete_message';
        $this->load->view('layouts/main',$data);
    }
    ...
    public function select_locationCol31() {
        ...
        $data['_view'] = 'utility/select_locationCol31';
        $this->load->view('layouts/main',$data);
    }
    ...
    public function select_locationCol31_SthalatLagat() {
        ...
        $data['_view'] = 'utility/select_locationCol31_SthalatLagat';
        $this->load->view('layouts/main',$data);
    }
    ...
    public function JamaWasilSingleLoc() {
        ...
        $data['_view'] = 'utility/JamaWasilSingleLoc';
        $this->load->view('layouts/main',$data);
    }
    ...
    public function JamaWasilLoc() {
        ...
        // $this->load->view('../views/footer');
        $data['_view'] = 'utility/JamaWasilLoc';
        $this->load->view('layouts/main',$data);
    }
    ...
    public function PeriodicPattaLoc() {
        ...
        $data['_view'] = 'utility/PeriodicPattaLoc';
        $this->load->view('layouts/main',$data);
    }
    ...
    public function PeriodicPattaLocAll() {
        ...
        $data['_view'] = 'utility/PeriodicPattaLocAll';
        $this->load->view('layouts/main',$data);
    }
  ...
  //       $this->load->view('header');
    ...
    public function getLandAreaJSON($dist_code, $subdiv_code, $circle_code, $mouza_code, $lot_no, $vill_code, $patta_type, $patta_no, $dag_no) {
		$db=  $this->session->userdata('db');
        $data = $this->db->query(""
                . "Select dag_area_b,dag_area_k,dag_area_lc  from    Chitha_Basic where Dist_code='$dist_code' and Subdiv_code='$subdiv_code' and  patta_type_code='$patta_type' and TRIM(patta_no) = '$patta_no' and Cir_code='$circle_code' and Mouza_Pargona_code='$mouza_code' and Lot_No='$lot_no' "
...
//                . "cir_code = '$cir_code' and date(lm_date) >='$define_date'";
 ...
 //        }
    ...
    function modifyserial(){
       ...
       // return;
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $data['circle_code']=$circle_code = $this->input->post('circle_code');
            $data['mouza_pargona_code']=$mouza_pargona_code = $this->input->post('mouza_code');
            $data['lot_no']=$lot_no = $this->input->post('lot_no');
            $data['vill_townprtcode']=$vill_townprtcode = $this->input->post('vill_code');
            ...
            $q="Select * from jama_pattadar where dist_code = '$dist_code' and  subdiv_code='$subdiv_code' and cir_code='$circle_code' 
            and mouza_pargona_code='$mouza_pargona_code' and lot_no='$lot_no' and 
            vill_townprt_code='$vill_townprtcode' and patta_type_code='$patta_type_code' and patta_no='$patta_no' order by cast (pdar_id as integer) ";
            $data['pdarlist']=$this->db->query($q)->result();
            $data['_view'] = 'Maintenance/listofpattadar_main';
            $this->load->view('layouts/main',$data);
    ...
    function pattadarserial(){
		...
		$q="SElect * from    jama_pattadar where dist_code = '$dist_code' and  subdiv_code='$subdiv_code' and cir_code='$cir_code' 
		and mouza_pargona_code='$mouza_code' and lot_no='$lot_no' and 
		vill_townprt_code='$vill_code' and patta_type_code='$patta_code' and patta_no='$patta_no' order by cast (pdar_id as integer) ";
		$data['pdarlist']=$this->db->query($q)->result();
		//var_dump($data);
		// $this->load->helper('html');
		// $this->load->view('../views/header');
		...
		//	var_dump($val);
		$q="Update jama_pattadar set pdar_sl_no='$val[0]',entry_mode='P' where dist_code = '$dist_code' and  subdiv_code='$subdiv_code' and cir_code='$cir_code' 
		and mouza_pargona_code='$mouza_code' and lot_no='$lot_no' and 
		vill_townprt_code='$vill_code' and patta_type_code='$patta_code' and patta_no='$patta_no' and pdar_id='$p' ";
		$this->db->query($q);
		}	
		redirect('/utility/pattadarserial');
	}

    function updatepattadarslNew(){
        // echo "<pre>";
        // var_dump($_POST);exit;
        $dist_code = $this->input->post('dist_code');
        $subdiv_code = $this->input->post('subdiv_code');
        ...
        $patta_no = $this->input->post('patta_no');
        $patta_code = $this->input->post('patta_type_code');
        $pdar_sl_no=$this->input->post('pdar_sl_no');
        //var_dump($pdar_sl_no);
        foreach($pdar_sl_no as $p=>$val){
            //  var_dump($val);
        ...
        redirect('/Maintenance/modifyserial');
    }
