...
class ChithaFlag extends CI_Controller
{
    ...
    public function locationDetails()
    {
        ...
        //Area Mapping =====
        $villages = $this->mutationmodel->getVillageCodeJSONChithaFlag($dist_code, $subdiv_code, $cir_code, $mouza_pargona_code, $lot_no);
    ...
    public function partialmapping()
    {
        ...
        //changes done for adding new feature in 18072024-------
        $zone_details = $this->ChithaFlagModel->getAllListedMappingCategory();
    ...
    public function saveMappingWithChitha(){
        ...
        $industrial_corridor = $this->input->post('IC');
        $nh_acquisition = $this->input->post('NH');
        $asom_mala_acquisition = $this->input->post('AM');
        $gramdan = $this->input->post('GD');
        $bhoodan = $this->input->post('BD');
    	...
    	foreach ($dag_array as $key => $value) {
    		...
    		$dataInsert = array(
                ...
                'industrial_corridor' => isset($industrial_corridor) ? $industrial_corridor : null,
                'nh_acquisition' => isset($nh_acquisition) ? $nh_acquisition : null,
                'asom_mala_acquisition' => isset($asom_mala_acquisition) ? $asom_mala_acquisition : null,
                'gramdan'     => isset($gramdan) ? $gramdan : null,
                'bhoodan'     => isset($bhoodan) ? $bhoodan : null,
		    	'user_code' => $this->session->userdata('user_code'),
                ...
                if(!empty($checkValResult) && $checkValResult[0]->dag_no != null){
                    $where_final = array(
                         ...
                         $data_final=array(
                            ...
                            'industrial_corridor' => $value->industrial_corridor,
                            'nh_acquisition' => $value->nh_acquisition,
                            'asom_mala_acquisition' => $value->asom_mala_acquisition,
                            'gramdan' => $value->gramdan,
                            'bhoodan' => $value->bhoodan,
                            'status' => 'F',
                            'updation_date_time' =>date('Y-m-d H:i:s'),
                            'remarks' => $value->remarks,
                            'approval_user_code' => $this->session->userdata('user_code')
                         );
                    ...
                    $value->status = "F";
                    $value->approval_user_code = $this->session->userdata('user_code');
        ...
        if (!empty($results)) {
            ...
            foreach ($data_rows as $key => $rows) {
                ...
                $json[] = array(
                    ...
                    $rows->industrial_corridor == 8 ? "Yes" : "N/A",
                    $rows->nh_acquisition == 9 ? "Yes" : "N/A",
                    $rows->asom_mala_acquisition == 10 ? "Yes" : "N/A",
                    $rows->gramdan == 11 ? "Yes" : "N/A",
                    $rows->bhoodan == 12 ? "Yes" : "N/A",
                    $rows->land_type
                );
            }
        ...
        if (!empty($results)) {
            ...
            foreach ($data_rows as $key => $rows) {
                ...
                $json[] = array(
                    ...
                    $rows->gramdan == 11 ? "Yes" : "N/A",
                    $rows->bhoodan == 12 ? "Yes" : "N/A",
                    $rows->bhoodan == 13 ? "Yes" : "N/A",
                    $rows->bhoodan == 14 ? "Yes" : "N/A",
                    $rows->land_type
                );
            }
