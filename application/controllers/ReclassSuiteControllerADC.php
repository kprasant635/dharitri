...
class ReclassSuiteControllerADC extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        if(($user_desig_code != MB_ADD_DEPUTY_COMM) || ($user_desig_code != MB_DEPUTY_COMM))
        {
...
//********************** COMMON **********************************
    ...
    public function checkCaseInModificationRequest($caseNo)
    {
        ...
        $basic = $this->reclassPullModel->getSettlementBasicDetails($caseNo);
    ...
    function randomFileName()
    {
        ...
        if($this->SettlementMbADCModel->checkDuplicateFileNameInProposal($new_case_no) != 0)
        {
            $this->randomFileName();
        }
        else
    ...
    public function applicationRevertFromDCToCO()
    {
        ...
        else
        {
            ...
            $this->utilityclass->checkUserAuthForCaseForAdcReclass($case_no);
            ...
            else
            {
                ...
                $this->db->trans_begin();
                if($this->reclassSuiteADCModel->updateSettlementBasicData($case_no,$dist_code,$updateData)== 0)
                {
                    $this->db->trans_rollback();
                    echo json_encode(array(
                        'responseType' => 1,
    ...
    public function applicationRejectedBySdlac()
    {
        ...
        else
        {
            ...
            $caseCount = $this->SettlementMbADCModel->countSettlementApplicationDetailsByCaseNoForSDLAC($case_no,$dist_code);
            if($caseCount == 0)
            {
                echo json_encode(array(
                    'responseType' => 3,
            ...
            else
            {
                ...
                $this->db->trans_begin();
                if($this->SettlementMbADCModel->updateSettlementBasicData($case_no,$dist_code,$updateData)== 0)
                {
                    $this->db->trans_rollback();
                ...
                else
                {
                    ...
                    $mmnn = $this->SettlementMbADCModel->updateSettlementAppDetailsByCaseNoUnderProposal($case_no,$updatePro);
    ...
    public function sendAllMarkAppToSDLACByDc()
    {
        ...
        else
        {
            ...
            if(MEETING_PROPOSAL_DLC_NOTICE_HOLD == 1)
            {
                if(date('Y-m-d H:i:s',strtotime(MEETING_PROPOSAL_DLC_NOTICE_DATE)) < date('Y-m-d H:i:s',strtotime($hearingDate)))
                {
                    echo json_encode(array(
                        'responseType' => 4,
                        'response'     => 4,
                        'message'      => 'Maximum Date of processing '.MEETING_PROPOSAL_DLC_NOTICE_DATE_SHOW
                    ));
                    return;
                }
            }
            ...
            if(!empty($allSelectedList))
            {
                foreach ($allSelectedList as $row)
                {
                    ...
                    $caseCount = $this->reclassSuiteADCModel->countSettlementApplicationDetailsByCaseNo($case_no,$dist_code);
                    $caseIdSdlacProposal = $this->SettlementCommonDcModel->countSettlementApplicationByCaseNoInSdlacProList($case_no);
                    $basic = $this->reclassSuiteADCModel->getRecBasicDetails($case_no);
                ...
                $commMembers  = $this->ReclassCommonDcModel->getSelectedMembersFromUsers($this->session->userdata('dist_code'),$allSelectedMember);
            ...
            else
            {
                ...
                else
                {
                    ...
                    $insPetProceed = [
                        ...
                        'date_entry'    => date('Y-m-d h:i:s'),
                        'operation'     => 'E',
                        'note_on_order' => 'Recommended for DLC',
                        'ip'            => $this->utilityclass->get_client_ip(),
                        'office_from'   => MB_ADD_DEPUTY_COMM,
                        'office_to'     => MB_ADD_DEPUTY_COMM,
                        'task'          => 'Recommended for DLC'
                    ];
                    $insertProceeding = $this->db->insert('settlement_proceeding', $insPetProceed);
            ...
            else
            {
                ...
                else
                {
                    ...
                    $insPetProceed = [
                        ...
                        'date_entry'    => date('Y-m-d h:i:s'),
                        'operation'     => 'E',
                        'note_on_order' => 'Unmarked from DLC List',
                        'ip'            => $this->utilityclass->get_client_ip(),
                        'office_from'   => MB_ADD_DEPUTY_COMM,
                        'office_to'     => MB_ADD_DEPUTY_COMM,
                        'task'          => 'Unmarked from DLC List'
                    ];
                    $insertProceeding = $this->db->insert('settlement_proceeding', $insPetProceed);
                    if($insertProceeding != 1)
                    {
    ...
    public function applicationApprovedBySdlac()
    {
        ...
        else
        {
            ...
            $caseCount      = $this->SettlementMbADCModel->countSettlementApplicationDetailsByCaseNoForSDLAC($case_no,$dist_code);
            $caseCountInPro = $this->SettlementMbADCModel->countSettlementAppDetailsByCaseNoUnderProposal($case_no);
            ...
            else
            {
                ...
                else
                {
                    ...
                    $this->SettlementMbADCModel->updateSettlementAppDetailsByCaseNoUnderProposal($case_no,$updatePro);
    ...
    public function viewPaymentReceivedAppDetailsByDc()
    {
        ...
        $dist_code = $this->session->userdata('dist_code');
        $this->utilityclass->checkUserAuthForCaseForAdc($case_no);
        $caseCount = $this->SettlementMbADCModel->countSettlementPaymentReceivedAppDetailsByCaseNo($case_no,$dist_code);
        ...
        {
            $caseDetails = $this->SettlementMbADCModel->getSettlementPaymentReceivedAppDetailsByCaseNo($case_no,$dist_code);
    ...
    public function updateProposalHearingDateKhas()
    {
        ...
        else
        {
            ...
            $allCases      = $this->SettlementMbADCModel->getAllAppInReportSendByDcToSdlacKhas($proposalNo);
    ...
    public function updateHearingDateGenerateNoticeKhas()
    {
        ...
        else
        {
            ...
            $proposalDetails = $this->SettlementMbADCModel->getProposalDetailsById($proposalNo,$dist_code);
            if($proposalDetails == '')
            {
                echo json_encode(array(
                    'responseType' => 3,
            ...
            else
            {
                ...
                else
                {
                    ...
                    $this->db->trans_begin();
                    if($this->SettlementMbADCModel->updateProposalListById($proposalNo,$updateProposalData)== 0)
                    {
                        $this->db->trans_rollback();
                        echo json_encode(array(
                            'responseType' => 1,
...
//********************** COMMON **********************************
    ...
    // 1st landing page KHAS ADC
    public function ReclassSuiteLandDc()
    {
        $dist_code = $this->session->userdata('dist_code');
        $user_code = $this->session->userdata('user_code');
        $user_desig_code = $this->session->userdata('user_desig_code');

        $noticegenerateCount = $this->reclassSuiteADCModel->countAllPendingReclassSuiteforNoticePart($dist_code);
        $noteofactionCount = $this->reclassSuiteADCModel->countAllPendingReclassSuiteforNoteActionPart($dist_code);
        $firstProceedingCount = $this->reclassSuiteADCModel->countAllPendingReclassSuite($dist_code);
        $SDLACCommitteeCount  = $this->SettlementCommonDcModel->countAllSDLACCommitteeMember($dist_code,$user_code,$user_desig_code);
        $SDLACNoticeCount     = $this->reclassSuiteADCModel->countMarkAsSDLACSettlementReclass($dist_code);
        $SDLACReportCount     = $this->reclassSuiteADCModel->countAllProposalSendByDcToSdlacReclasss($dist_code);
        $caseStatusCount      = 0;
        $sdlacMemberApproval  = $this->SettlementMbADCModel->countSdlacStatusList($dist_code);
        $SDLACConsideration   = $this->SettlementMbADCModel->countAllUnderConsiderationAppKhas($dist_code);

        // $reReportByCOCount    = $this->SettlementMbADCModel->countReRevertedByCoApplicationKhas($dist_code);
        // $approvedListCount    = $this->SettlementMbADCModel->countAllApproveAppBySdlacKhas($dist_code);
        // $rejectedListCount    = $this->SettlementMbADCModel->countAllRejectAppByDcKhas($dist_code);
        // $finalVerifyCaseCount = $this->SettlementMbADCModel->countAllCasesForFinalVerifyAppKhas($dist_code);
        // $revertedByDepartmentCount = $this->SettlementMbADCModel->countRevertedByDeptApplicationKhas($dist_code);
        // $chithaUpdateOrderCount    = $this->SettlementMbADCModel->countAllOrderChithaUpdateAppKhas($dist_code);

        $coRejectedListCount = '';//$this->SettlementCommonDcModel->countCoRejectedCaseForADC($dist_code,SETTLEMENT_KHAS_LAND_ID);
        $coModificationListCount = '';//$this->SettlementPullModel->countCoModificationRequestCaseForADC($dist_code,SETTLEMENT_KHAS_LAND_ID);

        $rejctedListCount = '';//$this->SettlementCommonDcModel->rejectedCaseList($dist_code,SETTLEMENT_KHAS_LAND_ID, MB_ADD_DEPUTY_COMM);

        $revivalListCount = '';//$this->SettlementCommonDcModel->revivalListCount($dist_code,SETTLEMENT_KHAS_LAND_ID, MB_ADD_DEPUTY_COMM);
        ...
        $data['noticegenerateCount'] = $noticegenerateCount;
        $data['noteofactionCount'] = $noteofactionCount;
        ...
        $data['_view'] = 'reclass_suite/Adc/first_landing_page_dc_reclass';
        $this->load->view('layouts/main', $data);
    ...
    public function viewAllReclassFirstProceedingDCCaseList()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->reclassSuiteADCModel->getAllPendingReclassSuite($dist_code);
        ...
        $data['_view'] = 'reclass_suite/Adc/first_proceeding_case_dc_reclass';
        $this->load->view('layouts/main', $data);
    ...
    public function getReclassSuiteApplicationDetails()
    {
        $case_no = $this->input->get('case');
        $dist_code = $this->session->userdata('dist_code');
        $this->utilityclass->checkUserAuthForCaseForAdcReclass($case_no);
        ...
        else
        {
            ...
            $caseDetailsCheckCount = $this->reclassSuiteADCModel->getReclassSuiteAppDetails($case_no,$dist_code);

            $data['caseDetailsCheckCount'] = $caseDetailsCheckCount;
            $data['caseDetailsCheck']      = $caseDetailsCheck;
            ...
            $data['_view'] = 'reclass_suite/Adc/reclass_app_details';
            $this->load->view('layouts/main', $data);
        }
    }
    ...
    public function viewAllMarkAsSDLACListForDCReclass()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->reclassSuiteADCModel->getMarkAsSDLACSettlementReclass($dist_code);
        ...
        $data['location'] = $circleList;
        $data['_view']    = 'reclass_suite/Adc/mark_as_sdlac_case_dc_reclass';
        $this->load->view('layouts/main', $data);
    ...
    public function getAllProposalListSdlacReclass()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->reclassSuiteADCModel->getAllProposalSendByDcToSdlacReclass($dist_code);

        $data['dist_code']        = $dist_code;
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'reclass_suite/Adc/proposal_list_send_to_sdlac_reclass';
        $this->load->view('layouts/main', $data);
    }
    ...
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementMbADCModel->getAllUnderConSettlementKhas($dist_code);
    ...
    public function getAllApplicationInReportSendByDcToSdlacReclass()
    {
        $proposal_no = $this->input->get('case');
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->reclassSuiteADCModel->getAllAppInReportSendByDcToSdlacRecls($proposal_no);
        $proposalDetails = $this->reclassSuiteADCModel->getProposalDetailsById($proposal_no,$dist_code);
        ...
        $data['_view'] = 'reclass_suite/Adc/send_to_sdlac_case_dc_reclass';
        $this->load->view('layouts/main', $data);

    }

    // generate proposal notice KHAS ADC
    public function generateNoticeSendAllMarkAppToSDLACByDcReclass()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
        $this->load->library('form_validation');
        ...
        else
        {
            ...
            if(!empty($allSelectedList))
            {
                ...
                $this->db->trans_begin();
                if($this->reclassSuiteADCModel->saveProposalSDLACReclass($dataProSave) == 0)
                {
                    $this->db->trans_rollback();
                ...
                else
                {
                    ...
                    {
                        $case_no = $row;
                        $this->utilityclass->checkUserAuthForCaseForAdcWithRollbackReclass($case_no);
                        $modificationRequest = $this->checkCaseInModificationRequestWithSession($case_no);
                        if($modificationRequest == 1)
                        {
                            echo json_encode(array(
                        ...
                        if($this->SettlementMbADCModel->saveProposalCaseListSDLACKhas($saveCaseList) == 0)
                        {
//                            $this->SettlementMbADCModel->deleteProposalSDLAC($proposalId);
    ...
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementMbADCModel->getAllApproveAppBySdlacKhas($dist_code);
    ...
    public function viewApprovedAppDetailsKhas()
    {
        ...
        $caseCount = $this->SettlementMbADCModel->countSettlementAppDetailsByCaseNoOnlyView($case_no,$dist_code);
        if($caseCount == 0)
        {
            $this->SettlementKhasLandDc();
        }
        else
        {
            ...
            $caseDetails = $this->SettlementMbADCModel->getSettlementAppDetailsByCaseNoOnlyView($case_no,$dist_code);
            $proceedings = $this->SettlementMbADCModel->getSettlementProceeding($case_no);
            $data['caseCount']   = $caseCount;
            $data['caseDetails'] = $caseDetails;
            $data['proceedings'] = $proceedings;
    ...
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementMbADCModel->getAllRejectAppByDcKhas($dist_code);
    ...
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementMbADCModel->getAllOrderChithaUpdateAppKhas($dist_code);
    ...
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementMbADCModel->getReRevertedByCoApplicationKhas($dist_code);
    ...
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->SettlementMbADCModel->getRevertedByDeptApplicationKhas($dist_code);
    ...
    public function applicationPaymentGenerateDcKhas()
    {
        ...
        else
        {
            ...
            $caseCount = $this->SettlementMbADCModel->caseForDcApprovalKhas($case_no,$dist_code);
            if($caseCount == 0)
            {
                echo json_encode(array(
                    'responseType' => 3,
    ...
    public function getAllCasesForFinalVerifyByDcKhas()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $allCase = $this->SettlementMbADCModel->getAllCasesForFinalVerifyAppKhas($dist_code);
    ...
    public function proposalForwardToDcForFinalVerifyKhas()
    {
        ...
        else
        {
            ...
            $dist_code   = $this->session->userdata('dist_code');
            $pendingCase = $this->SettlementMbADCModel->getAllAppInReportSendByDcToSdlacKhas($proposal_no);
            ...
            foreach ($cases as $case)
            {
                ...
                if($case->case_status == 1)
                {
                    ...
                    if($this->SettlementMbADCModel->updateSettlementAppDetailsByCaseNoUnderProposal($caseNo,$updateCase)== 0)
                    {
                        $this->db->trans_rollback();
                    ...
                    if($this->SettlementMbADCModel->updateSettlementBasicData($caseNo,$dist_code,$updateBasicData)== 0)
                    {
                        $this->db->trans_rollback();
                ...
                if($case->case_status == 2)
                {
                    ...
                    else
                    {
                        ...
                        $mmnn = $this->SettlementMbADCModel->updateSettlementAppDetailsByCaseNoUnderProposal($caseNo,$updatePro);
            ...
            if($this->SettlementMbADCModel->updateProposalListById($proposal_no,$dataUpdate)== 0)
            {
                $this->db->trans_rollback();
    ...
    public function firstProceedingPaginationAPI()
    {
        ...
        $valid_columns = array(
            0   => 'reclass_suite_basic.submission_date',
        );
        if(!isset($valid_columns[$col])){
            $order = 'reclass_suite_basic.submission_date';
        } else {
        ...
        $this->db->where('reclass_suite_basic.pending_officer', MB_ADD_DEPUTY_COMM);
        $this->db->where("(reclass_suite_basic.adc_code = '" . $this->session->userdata('user_code') . "' OR reclass_suite_basic.adc_code IS NULL)", null, false);
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows) {
                ...
                $json[] = array(
                    ...
                    '<a class="btn btn-success" href="'.base_url().'index.php/ReclassSuiteControllerADC/getReclassSuiteApplicationDetails/?case='.$rows->case_no.'">
    ...
    public function secondProceedingSdlacRecommendedMarked()
    {
        ...
        $this->db->select('*');
        $this->db->from('reclass_suite_basic');
        $this->db->join('(select distinct on(case_no) case_no,is_urban  from reclass_dag_details) t', 'reclass_suite_basic.case_no = t.case_no');
        $this->db->join('settlement_ap_lmnote', 'reclass_suite_basic.case_no = settlement_ap_lmnote.case_no');
        $this->db->where('reclass_suite_basic.service_code', $service);
        $this->db->where('reclass_suite_basic.dist_code', $dist_code);
        $this->db->where('reclass_suite_basic.status', MB_MARK_AS_SDLAC);
        $this->db->where('reclass_suite_basic.pending_officer', MB_ADD_DEPUTY_COMM);
        $this->db->where("(reclass_suite_basic.adc_code = '" . $this->session->userdata('user_code') . "' OR reclass_suite_basic.adc_code IS NULL)", null, false);
        ...
        if(!empty($approvedBy))
        {
            ...
            {
                // $this->db->where("(trim(reclass_suite_basic.approve_by)='GOVT' or
                //             (trim(reclass_suite_basic.approve_by) is null and (t.is_urban='Y' or (t.is_urban='N' and settlement_ap_lmnote.falls_und_gmc='YES'))))" );

                $this->db->where("(trim(reclass_suite_basic.approve_by)='GOVT')" );
            }
            if($approvedBy == 2) // DC
            {
                // $this->db->where("(trim(reclass_suite_basic.approve_by)='DC' or
                //             (trim(reclass_suite_basic.approve_by) is null and (t.is_urban='N' and settlement_ap_lmnote.falls_und_gmc='NO')))" );

                $this->db->where("(trim(reclass_suite_basic.approve_by)='DC')" );
            }
        }
        $this->db->limit($length, $start);
        $query = $this->db->get();
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows)
            {
                ...
                $json[] = array(
                    ...
                    '<a class="btn btn-success btn-sm" href="'.base_url().'index.php/ReclassSuiteControllerADC/getReclassSuiteApplicationDetails/?case='.$rows->case_no.'">View Application</a>'
                );
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows) {
                ...
                $json[] = array (
                    ...
                    <a class="btn btn-sm btn-dark" target= "SDLACProposalNotice" href="'.base_url().'index.php/ReclassSuiteCommonDc/downloadCasesWithProposalId/?case='.$rows->id.'">Download</a>
       
                    <a class="btn btn-sm btn-success" href="'.base_url().'index.php/ReclassSuiteControllerADC/getAllApplicationInReportSendByDcToSdlacReclass/?case='.$rows->id.'">
                      '.$this->lang->line('process').'</a>'
    ...
    public function sdlacReportOnlineApprove()
    {
        ...
        $json = [
            'response' => 2,
            'message'  => 'This Proposal would be available under process (MB3) DLC  Minutes',
        ];
        echo json_encode($json);
    }
    ...
    public function getAllSdlacMemberApprovalProposalListKhas()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $subdiv_code = $this->session->userdata('subdiv_code');
        $pendingCase = $this->SettlementMbADCModel->getSdlacApprovalProposalListKhas($dist_code);
    ...
    public function getAllSdlacMemberApprovalProposalListDataKhas()
    {
        ...
        if(!empty($by_case_no)) { //join table settlement_proposal_cases
            ...
            $this->db->where('settlement_proposal_list.created_by', MB_ADD_DEPUTY_COMM);
            $this->db->where('settlement_proposal_list.final_verify_status', 0);
            $this->db->where('settlement_proposal_list.user_code',$userCode);
    ...
    public function getSdlacMemberApproveProposalViewIndividualKhas()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $proposal_no = $this->input->get('case');
        $proposalDetails  = $this->SettlementMbADCModel->getSdlacApprovalProposalIndividualKhas($proposal_no,$dist_code);
        $reportDetails    = $this->SettlementMbADCModel->getSdlacMemberReportDetailsKhas($proposal_no,$dist_code);
        $getMembersStatus = $this->SettlementMbADCModel->getSdlacMemberStatus($dist_code, $proposal_no);
    ...
    public function getSettlementKhasApplicationDetails()
    {
        ...
        else
        {
            ...
            $data['_view'] = 'settlementView/Adc/Khas/settlement_app_details_khas';
            $this->load->view('layouts/main', $data);
        }
    }


    public function viewAllReclassNoticeGenerateDCCaseList()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->reclassSuiteADCModel->getAllPendingReclassSuitePartition($dist_code);
        ...
        $data['_view'] = 'reclass_suite/Adc/notice_generate_case_dc_reclass';
        $this->load->view('layouts/main', $data);

    }

    public function viewAllReclassReNoticeGenerateDCCaseList()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->reclassSuiteADCModel->getAllPendingReclassSuitePartitionReNotice($dist_code);
        ...
        $data['_view'] = 'reclass_suite/Adc/notice_re_generate_case_dc_reclass';
        $this->load->view('layouts/main', $data);

    }


    public function viewAllReclassNoteofActionDCCaseList()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->reclassSuiteADCModel->getAllPendingReclassSuitePartitionNoteAction($dist_code);
        ...
        $data['_view'] = 'reclass_suite/Adc/note_of_action_case_dc_reclass';
        $this->load->view('layouts/main', $data);
    ...
    public function noticegeneratePaginationAPI()
    {
        ...
        $this->db->where('reclass_suite_basic.service_code', $service);
        $this->db->where('reclass_suite_basic.dist_code', $dist_code);
        $this->db->where('reclass_suite_basic.status', 'AN');
        $this->db->where('reclass_suite_basic.co_partition_enable', 'Y');
        $this->db->where('reclass_suite_basic.notice_generated_yn', null);
        $this->db->where('reclass_suite_basic.next_date_of_hearing', null);
        $this->db->where('reclass_suite_basic.note_action_yn', null);
        $this->db->where('reclass_suite_basic.pending_officer', MB_ADD_DEPUTY_COMM);
        $this->db->where("(reclass_suite_basic.adc_code = '" . $this->session->userdata('user_code') . "' OR reclass_suite_basic.adc_code IS NULL)", null, false);
        $this->db->limit($length, $start);
        $query = $this->db->get();
    ...
    public function noticeRegeneratePaginationAPI()
    {
        $service     = $this->input->post('service');
        $by_case_no  = $this->input->post('case_no');
        $remark_cat  = $this->input->post('remark_cat');
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows) {
                ...
                $json[] = array(
                    ...
                    '<a class="btn btn-success" href="'.base_url().'index.php/ReclassSuiteControllerADC/getReclassSuiteApplicationDetailsRegenerateNotice/?case='.$rows->case_no.'">
                        '.$this->lang->line('process').'</a>'
    ...
    //pagination of first proceeding
    public function noteofactionPaginationAPI()
    {
        $service     = $this->input->post('service');
        $by_case_no  = $this->input->post('case_no');
        $remark_cat  = $this->input->post('remark_cat');
        ...
        $this->db->where('reclass_suite_basic.status', 'AN');
        $this->db->where('reclass_suite_basic.co_partition_enable', 'Y');
        $this->db->where('reclass_suite_basic.notice_generated_yn is not NULL');
        $this->db->where('reclass_suite_basic.next_date_of_hearing is not NULL');
        $this->db->where('reclass_suite_basic.note_action_yn', null);
        $this->db->where('reclass_suite_basic.pending_officer', MB_ADD_DEPUTY_COMM);
        $this->db->where("(reclass_suite_basic.adc_code = '" . $this->session->userdata('user_code') . "' OR reclass_suite_basic.adc_code IS NULL)", null, false);
        $this->db->limit($length, $start);
    ...
    public function generatePartitionNotice()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
        $this->load->library('form_validation');
        ...
        else
        {
            ...
            {
                $pb = $data['pb']= $caseDetails = $this->reclassSuiteADCModel->getSettlementApplicationDetailsByCaseNo($case_no, $dist_code);
                ...
                $sql = "SELECT * FROM  reclass_dag_details WHERE dist_code = '$pb->dist_code' and subdiv_code = '$pb->subdiv_code' and cir_code = '$pb->cir_code' and mouza_pargona_code = '$pb->mouza_pargona_code'"
                    . " and lot_no = '$pb->lot_no' and vill_townprt_code = '$pb->vill_townprt_code'  and Petition_no = '$pb->petition_no' and co_is_partition = 'Y'";
                ...
                foreach($data['dag'] as $dag_row)
                {
                    ...
                    $dag_row->next_date_of_hearing = $hearing_date;
                ...
                $this->load->view('reclass_suite/AST/printpetitioner_reclass', $data);
    ...
    // save general notice
    public function saveGeneralNoticeReclass()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
        $this->load->library('form_validation');
        ...
        else
        {
            ...
            {
                $caseDetails = $this->reclassSuiteADCModel->getSettlementApplicationDetailsByCaseNo($case_no, $dist_code);
                $applicantDetails = $this->reclassSuiteADCModel->getAllApplicant($case_no);
                ...
                if ($sqlCheck->num_rows() <= 0)
                {
                    ...
                    $notice_no = "MB3/GN/" . date('Y') . "/".RECLASS_SUITE."/" . $caseDetails->petition_no;
                    ...
                    echo json_encode(array(
                        'responseType' => 2,
                        'redirect'     => base_url().'index.php/ReclassSuiteControllerADC/viewAllReclassNoticeGenerateDCCaseList',
                    ));
                    return;
                }
                else
    ...
    public function randomFileNameGeneral()
    {
        ...
        if ($this->reclassSuiteADCModel->checkDuplicateFileNameInGeneral($new_case_no) != 0) {
            $this->randomFileName();
        } else {
            return $new_case_no;
        }
    ...
    public function SaveNoteofActionReclass()
    {
        ...
        $doc = $this->reclassSuiteADCModel->getSupportiveNoticeDoc($case_no);
        ...
        $insertArr = [
            ...
            'operation' => 'E',
            'ip' => $this->utilityclass->get_client_ip(),
            'office_from' => $pending_officer,
            'office_to'   => $pending_officer,
            'task'        => 'Note Of Action Submit',
        ];
        ...
        else
        {
            ...
            $rmk    = 'Note Of Action Submitted by '.$pending_officer;
            $status = 'M';
            $task   = $pending_officer;
            $pen    = $pending_officer;
            $case   = $case_no;
            $rtps_status = $this->SettlementApiModel->postApiBasundharaMb3($application_no,$case,$rmk,$status,$task,$pen);
            $rtps_status = json_decode($rtps_status);
    ...
    // Revert from adc to co
    public function applicationForwardFromADCToDC()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
        $this->load->library('form_validation');
            ...
            else
            {
                ...
                else
                {
                    ...
                    $insPetProceed = [
                        ...
                        'date_entry' => date('Y-m-d h:i:s'),
                        'operation'  => 'E',
                        'note_on_order' => 'Forward to DC from ADC for final order',
                        'ip' => $this->utilityclass->get_client_ip(),
                        'office_from' => MB_ADD_DEPUTY_COMM,
                        'office_to'   => MB_DEPUTY_COMM,
                        'task'        => 'Forward to DC'
    ...
    // 1st landing page KHAS ADC
    public function ReclassSuiteFinalLandDc()
    {
        $dist_code = $this->session->userdata('dist_code');
        $user_code = $this->session->userdata('user_code');
        $user_desig_code = $this->session->userdata('user_desig_code');
        ...
        $finalOrderDcCount = $this->reclassSuiteADCModel->countAllPendingReclassSuiteforFinalOrderDC($dist_code);
        ...
        $data['dist_code']            = $dist_code;
        $data['finalOrderDcCount']    = $finalOrderDcCount;

        $data['_view'] = 'reclass_suite/Dc/first_landing_page_dc_reclass_final';
        $this->load->view('layouts/main', $data);

    }

    public function viewAllReclassFinalOrderDCCaseList()
    {
        $dist_code   = $this->session->userdata('dist_code');
        $pendingCase = $this->reclassSuiteADCModel->getAllPendingReclassSuiteFinalorderDC($dist_code);
        ...
        $data['_view'] = 'reclass_suite/Dc/final_order_case_dc_reclass';
        $this->load->view('layouts/main', $data);

    }

    //pagination of first proceeding
    public function finalorderDcPaginationAPI()
    {
        $service     = $this->input->post('service');
        $by_case_no  = $this->input->post('case_no');
        $remark_cat  = $this->input->post('remark_cat');
        ...
        $this->db->where('reclass_suite_basic.service_code', $service);
        $this->db->where('reclass_suite_basic.dist_code', $dist_code);
        $this->db->where('reclass_suite_basic.status', 'W');
        $this->db->where_in('reclass_suite_basic.pending_officer', MB_DEPUTY_COMM);
        $this->db->limit($length, $start);
        $query = $this->db->get();
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows) {
                ...
                $json[] = array(
                    ...
                    '<a class="btn btn-success" href="'.base_url().'index.php/ReclassSuiteControllerADC/getReclassSuiteApplicationDetailsDCFinal/?case='.$rows->case_no.'">
                        '.$this->lang->line('process').'</a>'
    ...
    // settlement application details KHAS ADC
    public function getReclassSuiteApplicationDetailsDCFinal()
    {
        $case_no = $this->input->get('case');
        $dist_code = $this->session->userdata('dist_code');
        $this->checkUserAuthForCaseForDcReclass($case_no);
        //$this->checkCaseInModificationRequest($case_no);
        $application_no = $this->input->get('case');
        $basic= $this->reclassModel->getSettlementBasic($application_no);
        $applicants_buyers = $this->reclassModel->getAllApplicantBuyers($application_no);
        ...
        $caseCount = $this->reclassSuiteADCModel->countSettlementApplicationDetailsByCaseNoDCFinal($case_no,$dist_code);
        if($caseCount == 0)
        {
            $this->SettlementKhasLandDc();
        }
        else
        {
            ...
            $caseDetails = $this->reclassSuiteADCModel->getSettlementApplicationDetailsByCaseNoFinalDC($case_no,$dist_code);
            ...
            $data['_view'] = 'reclass_suite/Dc/reclass_app_details_final_order';
            $this->load->view('layouts/main', $data);
        }
    }
    ...
    // settlement application details KHAS ADC
    public function getReclassSuiteApplicationDetailsRegenerateNotice()
    {
        $case_no = $this->input->get('case');
        $dist_code = $this->session->userdata('dist_code');
        $this->utilityclass->checkUserAuthForCaseForAdcReclass($case_no);
        ...
        else
        {
            ...
            $data['_view'] = 'reclass_suite/Adc/reclass_app_details_re_notice';
            $this->load->view('layouts/main', $data);
        }
    }
    ...
    // check Auth user ADC
    public function checkUserAuthForCaseForDcReclass($case_no)
    {
        ...
        $sql = "SELECT * FROM reclass_suite_basic WHERE case_no = '$case_no' AND dist_code = '$ses_dist' and status='W' and pending_officer = '$ses_user' ";
    ...
    // Revert from adc to co
    public function applicationForwardFromDCToCO()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
        $this->load->library('form_validation');
        $this->form_validation->set_rules('caseNo', 'Case Number', 'trim|required');
        ...
        else
        {
            ...
            else
            {
                ...
                $this->db->trans_begin();
                if($this->reclassSuiteADCModel->updateSettlementBasicDataFromDC($case_no,$dist_code,$updateData)== 0)
                {
                    $this->db->trans_rollback();
                    echo json_encode(array(
                        'responseType' => 1,
                ...
                else
                {
                    ...
                    $insPetProceed = [
                        ...
                        'date_entry' => date('Y-m-d h:i:s'),
                        'operation'  => 'E',
                        'note_on_order' => 'Forwarded to CO from DC for chitha correction',
                        'ip' => $this->utilityclass->get_client_ip(),
    ...
    public function applicationRevertFromDCToCONoDLC()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
        $this->load->library('form_validation');
