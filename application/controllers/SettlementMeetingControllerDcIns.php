<?php
class SettlementMeetingControllerDcIns extends CI_Controller
{
    ...
    public function meetingLandPage()
    {
        ...
        $checkDsc = $this->SettlementMeetingDcInsModel->countDscRegistrationWithDistCode($dist_code);

        $data['location'] = $circleList;
        $data['checkDsc'] = $checkDsc;

        $data['_view'] = 'SettlementView/Dc/pending_meeting_list_for_approval_ins';
    ...
    public function listOfPendingMeetingIds()
    {
        ...
        $this->db->where('proposal_meeting_list.mb_status', 3);
        $this->db->where_in('proposal_meeting_list.meeting_type_ins', MB_3_SERVICE_CODE_ALLOW_FOR_MEETING);
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows)
            {
                ...
                else
                {
                    ...
                    {
//                        $digitalMinutes    = '<b style="color: #FF5252;">Coming Soon </b>'.'<br>';
                    ...
                    {
                        $digitalMinutes = '<a class="rezaButt btn btn-sm " style="color: #FFF; background-color: #9C27B0;" href="'.base_url().'index.php/SettlementMeetingControllerDcIns/getPendingProposalsAgainstMeetingId/?meetingId='.$rows->id.'"><i class="fa fa-download" aria-hidden="true"></i> &nbsp;Digital Minutes</a>';
                    }
                    ...
                    {
                        $uploadedMinutes ='<a class="rezaButt btn btn-sm" style="background-color :#03A9F4" target="SdlacMinutes" href="'.base_url().'index.php/SettlementMeetingControllerDcIns/viewSdlacUploadedMinute/?meetingId='.$rows->id.'"><i class="fa fa-download" aria-hidden="true"></i> &nbsp;Uploaded Minutes</a>';
                    }
                    ...
                    $attendance = '<a class="rezaButt btn btn-sm " style="background-color :#3F51B5" target="SdlacAttendance" href="'.base_url().'index.php/SettlementMeetingControllerDcIns/viewSdlacAttendance/?meetingId='.$rows->id.'"><i class="fa fa-download" aria-hidden="true"></i> &nbsp;Attendance</a>';
                }

                $viewMeeting ='<a class="rezaButt btn btn-sm " style="background-color :#4CAF50" href="'.base_url().'index.php/SettlementMeetingControllerDcIns/getPendingProposalsAgainstMeetingId/?meetingId='.$rows->id.'"><i class="fa fa-eye" aria-hidden="true"></i> &nbsp;View Detail</a>';

//                if($rows->meeting_type_ins == SLIJE_ID)
//                {
//                    $meeting_service = '<br>(Institution)';
//                }
//                else if($rows->meeting_type_ins == BHODDAN_SERVICE_CODE)
//                {
//                    $meeting_service = '<br>(Bhoodan)';
//                }
...
//                    '<b>'. $rows->meeting_name .$meeting_service.'</b>',
    ...
    public function meetingApprovedLandPage()
    {
        ...
        $data['_view'] = 'SettlementView/Dc/approve_meeting_list_for_approval_ins';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getPendingProposalsAgainstMeetingId()
    {
        ...
        $meeting = $this->SettlementMeetingDcInsModel->getPendingMeetingDetailByMeetingIDMbTin($meetingId)->row();

        $meetingName = trim($meeting->meeting_name);
        if($meetingName == '')
        {
            $this->session->set_flashdata('error', "Meeting Not Found !");
            redirect(base_url() . "index.php/SettlementMeetingControllerDcIns/meetingLandPage");
        }

        //list of proposals against meeting id
        $proposalDetail = $this->SettlementMeetingDcInsModel->getProposalDetailAgainstMeetingId($dist_code,
            $meetingId)->result();

        //list of SDLAC/CDLAC Member report
        $sdlacReport = $this->SettlementMeetingDcInsModel->sdlacMemberReportDetail($dist_code,
            $meetingId)->result();
        ...
        $data['_view'] = 'SettlementView/Dc/pending_proposals_against_meeting_id_ins';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getApprovedProposalsAgainstMeetingId()
    {
        ...
        $meeting = $this->SettlementMeetingDcInsModel->getPendingMeetingDetailByMeetingID(
            $meetingId)->row();
        $meetingName = trim($meeting->meeting_name);
        if($meetingName == '')
        {
            $this->session->set_flashdata('error', "Meeting Not Found !");
            redirect(base_url() . "index.php/SettlementMeetingControllerDcIns/meetingApprovedLandPage");
        }
        ...
        $data['_view'] = 'SettlementView/Dc/approved_proposals_against_meeting_id_ins';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function viewCasesAgainstProposalNo()
    {
        ...
        //list of cases
        $cases = $this->SettlementMeetingDcInsModel->getCaseDetailsByProposalNos($proposalNo);
    ...
    public function serviceAbbreviation($scode)
    {
        ...
        if($scode == BHODDAN_SERVICE_CODE) {
            $service_abbreviation = BHODDAN_PREFIX;
        }
        if($scode == SLIJE_ID) {
            $service_abbreviation = SLIJE_ANNOTATION;
        }
    ...
    {
        $meetingId = trim($this->input->get('meetingId'));
        $meetingDetails = $this->SettlementMeetingDcInsModel->getPendingMeetingDetailByMeetingID($meetingId)->row();
    ...
    public function generateDigitalMinutesDc()
    {
        ...
        else
        {
            ...
            if($checkMeeting == 1)
            {
                ...
                $memoName     = $distEngName.'/MEMO-MB/'.date("Y").'/'.$meetingId;
                ...
                {
                    foreach ($proposals as $proId)
                    {
                        $proposal_id = trim($proId->id);
                        ...
                        $reservationDetailData = $this->getReservationDetailsVGRPGR($cluster_id,MB_FINAL_APPROVED_BY_DC);
                ...
                $sdlacReport = $this->SettlementMeetingDcInsModel->sdlacMemberReportDetailOnlyUserCode($dist_code,
                    $meetingId)->result();
                ...
                foreach ($allMembers as $mem)
                {
                    ...
                    {
                        $nominee = $this->SettlementMeetingDcInsModel->sdlacMemberReportDetailWithMeetingIdUserCode
                ...
                {
                    $userMp      = $this->SettlementCommonDcModel->getUsersMpCopyToForSdo($dist_code,$subDivCode);
                    $userMpCount = $userMp->num_rows();
                    $userMp      = $userMp->result();

                    $userMla      = $this->SettlementCommonDcModel->getUsersMlaCopyToForSdo($dist_code,$subDivCode);
                    $userMlaCount = $userMla->num_rows();
                    $userMla      = $userMla->result();

                    $userSdlac      = $this->SettlementCommonDcModel->getUsersSdlacCopyToForSdo($dist_code,$subDivCode);
                    $userSdlacCount = $userSdlac->num_rows();
                    $userSdlacList  = $userSdlac->result();
                }
                ...
                else
                {
                    ...
                    echo json_encode(array(
                        ...
                        'meeting_remarks_show' => $meetingDetails->meeting_remarks
                    ));
    ...
    public function generateProposalCases($proposals, $meetingId)
    {
        ...
        try
        {
            ...
            foreach($result as $r)
            {
                ...
                else if($locations->service_code == BHODDAN_SERVICE_CODE)
                {
            ...
            $serviceNames[39] = BHODDAN_SERVICE_NAME;
    ...
    public function digitalSignAndSavePdf()
    {
        ...
        else
        {
            ...
            $fileName    = $distEngName.'_SDLAC_MB_'.$dist_code.'_'.date("Y").'_'.$meetingId;
    ...
    public function signedAndFinalApproveByDC()
    {
        ...
        try
        {
            ...
            $getProposalsList = $this->SettlementMeetingDcInsModel->getProposalDetailAgainstMeetingId($dist_code, $meetingId)->result();
            ...
            foreach($getProposalsList as $prop)
            {
                ...
                $proposalDetails = $this->SettlementMeetingDcInsModel->getProposalDetailsById($proposal_no,$dist_code);
                ...
                {
                    $pendingCase = $this->SettlementMeetingDcInsModel->getCaseDetailsByProposalNos($proposal_no);
                    ...
                    foreach ($cases as $case)
                    {
                        ...
                        if (!in_array(trim($basic['service_code']), MB_3_SERVICE_CODE_ALLOW_FOR_PROPOSAL))
                        {
                            echo 'Case Number '. $case->case_no . 'is not belongs to MB3';
                            die;
                        }
                        ...
                        {
                            $this->db->trans_rollback();
                            log_message('error', '#MR0001199: Few cases under this proposal has already 
                            been approved. Proposal# : '.$proposal_no.' Case# : '.$case_no);
                            echo json_encode(array(
                                'responseType' => 1,
                                'message'      => '#MR0001199: Unable to process for final approval. 
                        ...
                        {
                            if(trim($basic['service_code']) == SLIJE_ID)
                            {


                                if($basic['approve_by'] == '' || $basic['approve_by'] == NULL)
                                {
                                    echo json_encode(array(
                                        'responseType' => 1,
                                        'message'      => '#MR0001760: Approved by office not found. 
                                                            Kindly contact system administrator !!!!',
                                    ));
                                    return;
                                }
                            ...
                            else if($basic['service_code'] == BHODDAN_SERVICE_CODE)
                            {
                                if($basic['approve_by'] == '')
                                {
                                    if(strtoupper($dag->is_urban) == 'Y' || (strtoupper($dag->is_urban) == 'N' && strtoupper($urbanByLm->falls_und_gmc) == YES))
                                    {
                                        ...
                                        {
                                            $this->db->trans_rollback();
                                            log_message('error', '#MRY0002132: Updation failed in settlement_basic '
                                                .$this->db->last_query());
                                            echo json_encode(array(
                                                'responseType' => 1,
                                                'message'      => '#MRY0002132: Unable to process for final approval. 
                                                                     !! Kindly contact system administrator',
                                            ));
                                            return;
                                        }
                                    ...
                                    {
                                        $wedLandStatus = $this->SettlementCommonDcModel->caseUnderDeptOrDCByWetLand($case_no);
                                        if($wedLandStatus == 1)
                                        {
                                            $finalApprovedBy = 1; // dept
                                ...
                                else if($basic['approve_by'] != '')
                                {
                                    ...
                                    if($basic['approve_by'] == 'DC')
                                    {
                                        ...
                                        if(strtoupper($dag->is_urban) == 'Y' || (strtoupper($dag->is_urban) == 'N' && strtoupper($urbanByLm->falls_und_gmc) == YES))
                                        {
                                            ...
                                            {
                                                $this->db->trans_rollback();
                                                log_message('error', '#MRU0002161: Updation failed in settlement_basic '
                                                    .$this->db->last_query());
                                                echo json_encode(array(
                                                    'responseType' => 1,
                                                    'message'      => '#MRU0002161: Unable to process for final approval. 
                                                                     !! Kindly contact system administrator',
                                                ));
                                                return;
                                            }
                                        }
                                        elseif($wedLandStatus == 1)
                                        {
                                            $finalApprovedBy = 1; // dept
                                            // update basic
                                            $updateBasicForWedL = [
                            ...
                            else
                            {
                                echo json_encode(array(
                                    'responseType' => 1,
                                    'message'      => '#MR000170984: There is some error. 
                                                            Kindly contact system administrator !!!!',
                                ));
                                return;
                            }
                    ...
                    if (isset($allCasesUrbanByProposal) && count($allCasesUrbanByProposal)>0)
                    {
                        ...
                        if ($count_updateData != $count_updateData_db)
                        {
                            ...
                            echo json_encode(array(
                                'responseType' => 1,
                                'message'      => '#MR0002702: Unable to batch update settlement_basic  final approval. 
                                        Kindly contact system administrator !!!!',
                            ));
                            return;
                        }
                    ...
                    if (isset($allCasesRuralByProposal) && count($allCasesRuralByProposal)>0)
                    {
                        ...
                        if ($count_updateData != $count_updateData_db)
                        {
                            ...
                            echo json_encode(array(
                                'responseType' => 1,
                                'message'      => '#MR0002732: Unable to batch update settlement_basic  final approval. 
                                        Kindly contact system administrator !!!!',
                            ));
                            return;
                        }
            ...
            {
                $this->db->trans_rollback();
                log_message('error', '#MR0002880: Updation failed in proposal_meeting_list '.
                    $this->db->last_query());
                echo json_encode(array(
                    'responseType' => 1,
                    'message'      => '#MR0002880: Unable to process for final approval. 
                        Kindly contact system administrator !!!!',
                ));
                return;
            }
            ...
            if (isset($allAPICasesRural) && count($allAPICasesRural)>0)
            {
                ...
                $rtps_status = $this->SettlementApiModel->applicationStatusUpdateBulkMb3($allAPICasesRuralIds,'NA',$rmk,$status,$task,$pen);
                if($rtps_status!="y")
                {
                    $this->db->trans_rollback();
    ...
    public function holdDeptCaseForwardTimeOutIns($case_no)
    {
        $case_no = trim($case_no);
        $endDate = HOLD_CASES_FORWARD_TO_DEPT_BY_DC_INSTITUTE;
    ...
    {
        $case_no = trim($case_no);
        $endDate = HOLD_CASES_FORWARD_TO_DEPT_BY_DC_BHOODAN;
        $today   = date('Y-m-d H:i:s');
        $finalApprovedBy = 2; // dc
        if(strtotime($endDate) < strtotime($today))
        {
    ...
    public function verifyArea()
    {
        ...
        try
        {
            ...
            foreach($getProposalsList as $prop)
            {
                ...
                if($final_verify_status == 1)
                {
                    ...
                    foreach ($cases as $case)
                    {
                        ...
                        $pullCheck = $this->SettlementPullModel->getSettlementBasicDetails($case_no);

                        if($pullCheck->service_code == SLIJE_ID)
                        {
                            $deptCheck = $this->holdDeptCaseForwardTimeOutIns($case_no);
                        }
                        else
                        {
                        ...
                        if($pullCheck->service_code == SLIJE_ID)
                        {
                            $insAppDetails = $this->SettlementMeetingDcInsModel->getSingleAppInReportSendByDcToSdlacIns($case_no);

                            if($insAppDetails->ins_cat_type_co == 12)
                            {
                                if($insAppDetails->under_ngo_trust_localbodies == '' || $insAppDetails->under_ngo_trust_localbodies == NULL)
                                {
                                    $ngoTrust[] = $case_no;
                                }

                                if($insAppDetails->purpose_land_allot_co == 'Educational')
                                {
                                    if($insAppDetails->under_venture_school == '' || $insAppDetails->under_venture_school == NULL)
                                    {
                                        $ventureSchool[] = $case_no;
                                    }
                                }
                            }
            ...
            {
                $error_ngo = '';
                foreach ($ngoTrust as $errorPro)
                {
                    $error_ngo = $error_ngo.$errorPro.',';
                }
                log_message('error', '#MRNGO01162: Institution falls under category of NGOs, Trusts, Local Bodies,  Associations, Societies ?
                                       Not Set for case no  '.$error_ngo);
                echo json_encode(array(
                    'responseType' => 101,
                    'message'      => '#MRVEN01162: Institution falls under category of NGOs, Trusts, Local Bodies,  Associations, Societies ?
                                       Not Set for case no .'.$error_ngo.' !!!',
                ));
                return;
            ...
            // set venture
            if(count($ventureSchool) > 0)
            {
                $error_venture = '';
                foreach ($ventureSchool as $errorPro)
                {
                    $error_venture = $error_venture.$errorPro.',';
                }
                log_message('error', '#MRVEN01162: Is the Educational Institution a Non Provincialised Venture School  ?
                                       Not Set for Case No '.$error_venture);
                echo json_encode(array(
                    'responseType' => 101,
                    'message'      => '#MRVEN01162: Is the Educational Institution a Non Provincialised Venture School  ?
                                       Not Set for Case No  .'.$error_venture.' !!!',
                ));
                return;
    ...
    public function deleteProposalWithZeroCasesForPendingMeetingByDc()
    {
        ...
        {
            $pro = trim($singlePro);
            $proposalCount = $this->SettlementCommonDcModel->countSettlementProposalListMbTin($pro);
            ...
            $meetingDetails = $this->SettlementCommonDcModel->getMeetingDetailByMeetingIdMbTin(trim($proposalDetails->proposal_meeting_id));
    ...
    public function getAllPendingMeetingForDigitalResigning()
    {
        ...
        $data['_view'] = 'SettlementView/Dc/pending_meeting_for_digital_resigning_ins';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getResigningProposalsAgainstMeetingId()
    {
        ...
        $data['_view'] = 'SettlementView/Dc/pending_resigning_proposals_against_meeting_id_ins';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function generateResignDigitalMinutesDc()
    {
        ...
        else
        {
            ...
            if($checkMeeting == 1)
            {
                ...
                {
                    $userMp = $this->SettlementCommonDcModel->getUsersMpCopyToForSdo($dist_code,$subDivCode,$proposalCreatedBy);
                    $userMpCount = $userMp->num_rows();
                    $userMp = $userMp->result();

                    $userMla = $this->SettlementCommonDcModel->getUsersMlaCopyToForSdo($dist_code,$subDivCode,$proposalCreatedBy);
                    $userMlaCount = $userMla->num_rows();
                    $userMla = $userMla->result();

                    $userSdlac = $this->SettlementCommonDcModel->getUsersSdlacCopyToForSdo($dist_code,$subDivCode,$proposalCreatedBy);
                    $userSdlacCount = $userSdlac->num_rows();
                    $userSdlacList  = $userSdlac->result();
                }
    ...
    public function digitalResignAndSavePdf()
    {
        ...
        else
        {
            ...
            if($checkMeeting == 1)
            {
                ...
                $dist_name   = $this->UtilsModel->getEngDistrictNameByDistCode($dist_code);
                $distEngName = substr($dist_name->locname_eng, 0, 3);
                $fileName    = $distEngName.'_SDLAC_MB_'.$dist_code.'_'.date('Y', strtotime($meetingDetails->meeting_date)).'_'.$meetingId;
    ...
    public function revertedMeetingByDepartmentForDC()
    {
        ...
        $sqlForRevertList = 'select distinct pml.meeting_name,pml.signed_minute,pml.id,pml.id as meeting_id,pml.meeting_venue,pml.meeting_date from settlement_basic sb  
                                ...
                                join proposal_meeting_list pml on spl.proposal_meeting_id = pml.id
                                    ...
                                    and sb.service_code IN (\'39\', \'45\')';
        $query1 = $this->db->query($sqlForRevertList,array($dist_code,1,MB_DEPUTY_COMM,MB_DEPARTMENT));
        $total_records = $query1->num_rows();
        $meetings      = $query1->result();
        $data['meetingCount'] = $total_records;
        $data['meetings'] = $meetings;
        $data['_view'] = 'SettlementView/Dc/reverted_meeting_list_by_department_dc_ins';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getRevertedProposalsAgainstMeetingId()
    {
        ...
        $data['_view'] = 'SettlementView/Dc/reverted_dept_proposals_against_meeting_id_ins';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getRevertedCasesDeptAgainstMeetingId()
    {
        ...
        $sqlForRevertList = 'select distinct sb.case_no, sb.service_code,(select note_on_order from settlement_proceeding sp where sp.case_no = sb.case_no 
                             ...
                             and sb.pending_officer = ? and sb.service_code IN (\'39\', \'45\')';
        ...
        $data['_view'] = 'SettlementView/Dc/reverted_cases_by_department_dc_ins';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getAllDeptRevertedCases()
    {
        ...
        $sqlForRevertList = "select string_agg(distinct sb.case_no::text, ',') from settlement_basic sb  
                                ...
                                join settlement_proposal_list spl on spc.proposal_id = spl.id
                                join proposal_meeting_list pml on spl.proposal_meeting_id = pml.id
                                    where sb.dist_code=? and sb.dept_revert=? and sb.service_code IN ('39', '45')
    ...
    public function finalDeptRevertedCaseRevertToCO()
    {
        ...
        else
        {
            ...
            $sqlForRevertList = 'select distinct sb.case_no from settlement_basic sb  
                                 ...
                                 join proposal_meeting_list pml on spl.proposal_meeting_id = pml.id
                                 where sb.dist_code=? and sb.dept_revert=? 
                                 and sb.pending_officer=? and sb.from_office=? and pml.id =?  and sb.service_code IN (\'39\', \'45\')';
            $query1 = $this->db->query($sqlForRevertList,array($dist_code,1,MB_DEPUTY_COMM,MB_DEPARTMENT,$meeting_id));
            $arrayM = $query1->result();

            $meetingDetails  = $this->SettlementCommonDcModel->getMeetingDetailByMeetingIdMbTin(trim($meeting_id));
    ...
    public function getVgrPgrCasesForRevertDc($meetingId)
    {
        ...
        if($checkMeeting == 1)
        {
            ...
            if (trim($meetingDetails->vgr_pgr_status) == 1)
            {
                $proposals = $this->SettlementMeetingDcInsModel->getProposalDetailAgainstMeetingId($dist_code,$meetingId)->result();

                $allCases = [] ;
                foreach ($proposals as $singlePro)
                {
                    $cases = $this->SettlementMeetingDcInsModel->getCaseDetailsByProposalNos($singlePro->proposal_id);
    ...
    public function saveVgrPgrCasesForRevertDcData()
    {
        ...
        else
        {
            ...
            $meeting = $this->SettlementMeetingDcInsModel->getPendingMeetingDetailByMeetingID($meetingId);
    ...
    public function getVgrPgrRevertedCaseDetailsForDc()
    {
        ...
        if($caseId == '' || $caseId == NULL)
        {
            ...
            redirect(base_url() . "index.php/SettlementMeetingControllerDcIns/getAllVgrPgrRevertedCaseByDept");
        }
        ...
        $meetingDetails  = $this->SettlementCommonDcModel->getMeetingDetailByMeetingId(trim($revertDetails->meeting_id));
    ...
    public function revertedVgrCaseRevertToCoByDc()
    {
        ...
        $meetingDetails = $this->SettlementCommonDcModel->getMeetingDetailByMeetingId(trim($revertedCaseDetails->meeting_id));
        if($meetingDetails->vgr_pgr_status != 1)
        {
            echo json_encode([
                'responseType' => 1,
    ...
    public function revertedVgrCaseForwardToDeptByDc()
    {
        ...
        if(trim($revertedCaseDetails->meeting_pending_at) == MB_DEPARTMENT && $meetingDetails->digital_sign_status == 1)
        {
            ...
            $this->db->where('case_no', $case_no);
            $this->db->where('dist_code', $dist_code);
            $this->db->update('settlement_basic', $updateBas);
            if($this->db->affected_rows() != 1)
            {
                $this->db->trans_rollback();
                log_message('error', '#MRVGRR005526: Updation failed in settlement_basic 
                    for case_no : '.$case_no. ' and query is '. $this->db->last_query());
                $json = [
                    'responseType' => 1,
    ...
    public function getOrderNameDateDcCodeByCaseNo()
    {
        ...
        $proposalDetails = $this->SettlementCommonDcModel->getProposalDetailsByProId(trim($getProposalID->proposal_id));
        ...
        $caseInMeeting = $this->SettlementCommonDcModel->getMeetingDetailByMeetingId(trim($proposalDetails->proposal_meeting_id));
...
//        die();
    ...
    public function pullBackCasesFromDepartmentForDCIns()
    {
        ...
        $data['_view'] = 'SettlementView/Dc/pull_back_cases_from_dept_ins';
        $this->load->view('layouts/main', $data);
    ...
    public function pullBackCasesWithDeptPaginationAPIIns()
    {
        $service       = SLIJE_ID;
    ...
    // get revert back meeting details
    public function getPullBackCaseDetailsIns()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
        $this->load->library('form_validation');
    ...
    public function finalPullBackRevertToCoSubmitIns()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
        $this->load->library('form_validation');
        ...
        else
        {
            ...
            if($checkCase == 1)
            {
                ...
                {
                    $meetingDetails = $this->SettlementCommonDcModel->getMeetingDetailByMeetingIdMbTin(trim($meetingDetail->id));
                    if($meetingDetails->digital_sign_status != 1)
                    {
                        echo json_encode(array(
                            'responseType' => 1,
                    ...
                    $this->db->where(['id' => $meetingDetail->id, 'dist_code' => $dist_code]);
                    $this->db->update('proposal_meeting_list', $updateMeetingID);
                    if($this->db->affected_rows() <=  0)
                    {
                        $this->db->trans_rollback();
    ...
    // ** Bhoodan /
    // pull back dept pending cases
    public function pullBackCasesFromDepartmentForDCBhoodan()
    {
        ...
        $data['location'] = $circleList;
        $data['_view'] = 'SettlementView/Dc/pull_back_cases_from_dept_bhoodan';
        $this->load->view('layouts/main', $data);
    ...
    // Ajax for pull back case list
    public function pullBackCasesWithDeptPaginationAPIBhoodan()
    {
    ...
    // get revert back meeting details
    public function getPullBackCaseDetailsBhoodan()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
        $this->load->library('form_validation');
        ...
        else
        {
            ...
            $checkCase = $this->db->select()
                ->where('case_no', $appId)
                ->where('dist_code', $dist_code)
                ->where('service_code', BHODDAN_SERVICE_CODE)
                ->where('status', 'W')
                ->where('pending_officer', 'DPT')
                ->where("(add_cases_to_memo != 'Y' OR add_cases_to_memo IS NULL)", NULL, false)
                ->get('settlement_basic')
    ...
    // final pull back submit & revert back to co
    public function finalPullBackRevertToCoSubmitBhoodan()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
        $this->load->library('form_validation');
                ...
                else
                {
                    ...
                    foreach ($userSdlacList as $user)
                    {
                        ...
                        if($user->user_level == 7)
                        {
                            if ($indexM == 0)
                            {
                                $municipalName = $user->user_name;
                            }
                            else
                            {
                                $municipalName = $municipalName . ", " . $user->user_name;
                            }
                            $indexM++;
                        }
                        if($user->user_level == 8)
    ...
    public function digitalSignAndSavePdfHardCode()
    {
        ...
        else
        {
            ...
            $dist_name   = $this->UtilsModel->getEngDistrictNameByDistCode($dist_code);
            $distEngName = substr($dist_name->locname_eng, 0, 3);
            $fileName    = $distEngName.'_SDLAC_MB_'.date("Y").'_'.$meetingId;
    ...
    // Hard Code signed and final approved meeting by DC
    public function signedAndFinalApproveByDCHardCode()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
        $this->load->library('form_validation');
        ...
        else
        {
            ...
            if($checkMeeting == 1)
            {
                ...
                if($this->db->affected_rows() <=  0)
                {
                    ...
                    echo json_encode(array(
                        'responseType' => 1,
                        'message'      => '#ERR1273: Unable to process for final approval. 
                                        Kindly contact system administrator !!!!',
                    ));
                    return;
                }
    ...
    public function DepartmentApproved()
    {
        ...
        $data['getPaymentNoticeCo'] = $this->SettlementMeetingDcInsModel->DepartmentApprovedCaseList($service_code);
        $data['select_data'] = $this->SettlementCommonModel->locationSelect($service_code, $status);
        $data['_view'] = 'settlement_mb/payment_notice_co_new_ins_dc';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getListofPaymentNoticeCases()
    {
        ...
        $this->db->where('a.pending_officer', MB_DEPUTY_COMM);
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $rows) {
                ...
                $paymentNoticeLink = '<a type="button" href="' . base_url() . 'index.php/SettlementMeetingControllerDcIns/generatePaymentNoticeCo?case=' . $rows->case_no . '" class="lmreportmut btn-sm btn btn-primary mt-1">
                Payment Notice</a>';
    ...
    public function generatePaymentNoticeCo()
    {
        if(isset($_GET['case'])){
            ...
            // $this->db->trans_begin();
            // $settlement_premium_insertion = $this->SettlementCommonModel->premiumReCalculationForIns($case_no);
