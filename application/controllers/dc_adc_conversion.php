...
class dc_adc_conversion extends CI_Controller {
    ...
    public function GoToDC_ADC() {
        ...
        if ($process == '1') {
            $config['total_rows'] = $this->COofficeConversionModel->countPendingConversionSecondCases_dc($user_code);
            $cases['cases'] = $this->COofficeConversionModel->getPendingConversionSecondCases_dc($user_code)->result();
        } elseif ($process == '2') {
            $config['total_rows'] = $this->COofficeConversionModel->countPendingConversionSecondCases_adc($user_code);
            $cases['cases'] = $this->COofficeConversionModel->getPendingConversionSecondCases_adc($user_code)->result();
        } elseif ($process == '3') {
            $config['total_rows'] = $this->COofficeConversionModel->countPendingConversionFirstCases_dc($user_code);
            $cases['cases'] = $this->COofficeConversionModel->getPendingConversionFirstCases_dc($user_code)->result();
        } elseif ($process == '4') {
            $config['total_rows'] = $this->COofficeConversionModel->countPendingConversionFirstCases_adc($user_code);
            $cases['cases'] = $this->COofficeConversionModel->getPendingConversionFirstCases_adc($user_code)->result();
        } elseif ($process == '5') {
            $config['total_rows'] = $this->COofficeConversionModel->countPendingConversionSecondCases_adc_dpt($user_code);
            $cases['cases'] = $this->COofficeConversionModel->getPendingConversionSecondCases_adc_dpt($user_code)->result();
        } elseif ($process == '6') {
            $config['total_rows'] = $this->COofficeConversionModel->countAppConversionSecondCases_adc_dpt($user_code);
            $cases['cases'] = $this->COofficeConversionModel->getAppConversionSecondCases_adc_dpt($user_code)->result();
        }
        else if($process == '7') {
            $config['total_rows'] = $this->COofficeConversionModel->countRevertCasesFromDpt($user_code);
            $cases['cases'] = $this->COofficeConversionModel->getRevertCasesFromDpt($user_code)->result();
        }
        ...
        $cases['_view'] = 'dc_adc_office_conversion/dc_adc_conversion_cases';
        $this->load->view('layouts/main',$cases);
    }

    public function PassToAdc() {
        $case_no = $this->input->get('case_no');
        $authorization = $this->AuthorizationModel->isAuthorized(SERVICE_CONVERSION, 'DC', $case_no);
        if($authorization['status'] == 'n') {
            //ERRCONVADCSECONDTOADC001
            log_message('error', $authorization['messages'] . '. Error: ERRCONVADCSECONDTOADC001');
            $this->session->set_flashdata('message', $authorization['messages'].'. Error: ERRCONVADCSECONDTOADC001');
        ...
        $adc_name = $this->db->query("Select users.username, loginuser_table.user_code, users.user_desig_code from    users, loginuser_table where users.dist_code = loginuser_table.dist_code "
                    . "and users.user_code = loginuser_table.user_code and users.user_desig_code = 'ADC' and users.dist_code='$dist_code' and "
                    . "users.subdiv_code='00' and users.cir_code='00' and loginuser_table.dis_enb_option = 'E' and loginuser_table.priv = 'adm'")->row();
        
        $this->db->trans_begin();
        $this->db->query("UPDATE Petition_Basic SET add_off_desig = '$adc_name->user_desig_code', add_off_name = '$adc_name->username', status='P', co_user_code = '$adc_name->user_code' WHERE case_no = '$case_no' and "
        ...
        if ($this->db->affected_rows() == 0) {
            ...
            log_message('error', '#ERRCONVDC0001: Updation failed in Petition_Basic Case No ' . $case_no);
            $data = array(
                'error' => "#ERRCONVDC0001: Registration of Petition basic failed for case no : " . $case_no,
            );
        ...
        else
        {
            ...
            $this->session->set_flashdata('message', "Case no # $case_no has been forwarded to Additional Deputy Commissioner");
            redirect(base_url() . "index.php/home");
        }
    }
    ...
    public function FirstProceeding() {
        ...
        if($user_desig_code == 'ADC') {
            $data['post_url'] = 'index.php/dc_adc_conversion/FirstProceedingADC_save';
        }
        else if($user_desig_code == 'DC') {
            $data['post_url'] = 'index.php/dc_adc_conversion/FirstProceedingDC_save';
        }
    ...
    public function FirstProceedingDC_save() {
        ...
        if($formValidation['status'] == 'n') {
            ...
            redirect(base_url('index.php/dc_adc_conversion/GoToDC_ADC?pro=3'));
        }
    ...
    public function FirstProceedingADC_save() {
        ...
        if($formValidation['status'] == 'n') {
            ...
            redirect(base_url('index.php/dc_adc_conversion/GoToDC_ADC?pro=4'));
        }
    ...
    public function SecondProceeding() {
        ...
        $user_desig_code = $this->session->userdata('user_desig_code');
        if($user_desig_code == 'ADC') {
            $data['post_url'] = 'index.php/dc_adc_conversion/RegenerateADC';
        }
        else if($user_desig_code == 'DC') {
            $data['post_url'] = 'index.php/dc_adc_conversion/RegenerateDC';
        }
        $data['_view'] = 'dc_adc_office_conversion/Second_Proceeding';
        $this->load->view('layouts/main',$data);
    }
    ...
    public function RegenerateDC() {
        ...
        if($formValidation['status'] == 'n') {
            if($_POST['order_type'] == 'continuehearing') {
                ...
                redirect(base_url('index.php/dc_adc_conversion/GoToDC_ADC?pro=1'));
            }
        ...
        if ($re_co_note == 'Y') {
            ...
            if ($order_type == 'finalhukum') {
                ...
                $this->db->query("UPDATE Petition_Basic SET add_off_desig = 'CO', status = 'W' WHERE "
                            . "case_no = '$case_no' and dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$cir_code' and "
                            . "mouza_pargona_code = '$mouza_pargona_code' and lot_no = '$lot_no' and vill_townprt_code = '$vill_townprt_code'");
                ...
                if ($prepare_premium == 'Y') {
                    ...
                    $this->db->query("UPDATE petition_basic SET user_code = '$bo_name->user_code', proceeding_yn = NULL, co_order_conv_premium = 'Y', co_order_conv_date = '$date_entry', co_order_conv_notice = 'Y' "
                    ...
                    if ($this->db->affected_rows() == 0) {
                        ...
                        $data = array(
                            'error' => "#ERRCONVDC0012: Registration of Petition basic for case no : " . $case_no,
                        );
                        echo json_encode($data);
                        return false;
                    }
                    
                    // $this->db->query("UPDATE Petition_Basic SET user_code = '$bo_name->user_code' WHERE "
                    //         . "case_no = '$case_no' and dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$cir_code' and "
                    //         . "mouza_pargona_code = '$mouza_pargona_code' and lot_no = '$lot_no' and vill_townprt_code = '$vill_townprt_code'");
                    ...
                    $insert_ppd1 = $this->db->insert('petition_proceeding_dc_adc', $proceeding_data);
                    if($insert_ppd1 != 1){
                        $this->db->trans_rollback();
                ...
                elseif ($frwd_dept == 'Y') {
                    ...
                    $this->db->query("UPDATE Petition_Basic SET add_off_desig = 'DPT', status = 'W', user_code = 'DPT1' WHERE "
                    . "case_no = '$case_no' and dist_code='$dist_code' and subdiv_code='$subdiv_code' and cir_code='$cir_code' and "
                    . "mouza_pargona_code = '$mouza_pargona_code' and lot_no = '$lot_no' and vill_townprt_code = '$vill_townprt_code'");
    ...
    public function RegenerateADC() {
        ...
        //authorization
        if($_POST['order_type'] == 'continuehearing') {
            $authorization = $this->AuthorizationModel->isAuthorized(SERVICE_CONVERSION, 'ADC', $_POST['case_no'], CONV_ADC_SECOND);
            if($authorization['status'] == 'n') {
        ...
        if ($order_type == 'finalhukum') {
            ...
            if ($prepare_premium == 'Y') {
                ...
                if($formValid['status'] == 'n') {
                    //ERRCONVADCSECOND0006
                    $this->db->trans_rollback();
                    log_message('error', 'Message: '. $formValid['message'] .', Data: '. json_encode($formValid['data']) .'. Error: ERRCONVADCSECOND0006');
                    $this->session->set_flashdata('message', $formValid['message'] .' Error: ERRCONVADCSECOND0006');
                    redirect(base_url('index.php/dc_adc_conversion/GoToDC_ADC?pro=2'));
    ...
    public function FourthProceedingCo() {
        ...
        //syntax validation
        $requestResponse = checkRequestSpecChar($_POST, [], [], ['co_name' => true]);
        if($requestResponse['status'] == 'n') {
...
//        $this->load->view('../views/footer');
        ...
        if($ulpinCheckFlag ==1 && $compareCheckFlag == 'Y' && ENABLED_BLOCKCHAIN == 1 && in_array($this->session->userdata('dist_code'),json_decode(ENABLED_BLOCKCHAIN_FOR_DIST)))
        {
            ...
            redirect(base_url() . "index.php/dc_adc_conversion/TestFinalForm?ulpin=" . $ulpin . "&chain_revenue=" . $chain_revenue . "&chain_local_tax=" . $chain_local_tax . "&old_ulpin=" . $old_ulpin. "&ulpinCheckFlag=" . $ulpinCheckFlag . "&compareCheckFlag=" . $compareCheckFlag);
        }
        redirect(base_url() . "index.php/dc_adc_conversion/TestFinalForm");
    }
...
//                    $saperating_bigha = $remaining_lessa-($b*100);
        ...
        if($petition_basic->bo_note_yn == 'Y' && $petition_basic->pay_notice_gen_yn == null) {
            $data['post_url'] = 'index.php/dc_adc_conversion/FinalSaveTest';
        }
        else if($petition_basic->bo_note_yn == null && $petition_basic->pay_notice_gen_yn == 'Y') {
            $data['post_url'] = 'index.php/dc_adc_conversion/FinalSaveTestCo';
        }
        // var_dump($data['post_url']);die;
    ...
    //    die();
       ...
       ///////////REVALIDATE-DATA//////////////
       $sql="Select * from t_chitha_rmk_convorder where ord_no=?";
       $rmk_conv_data=$this->db->query($sql,array($case_no));
       if($rmk_conv_data->num_rows()==0){
            $this->db->trans_rollback();
            log_message('error', '#ERRCONVDC0055: NO DATA FOUND IN T-CHITHA-RMK-CONV-ORDER :'. $case_no);
            $json = [
                'message'=>"#ERRCONVDC0055: Failed to in T-CHITHA-RMK-CONV-ORDER for Case No : ".$case_no
            ];
            echo json_encode($json);
            return false;
       }
       ...
       else
       {
           ...
           if($ulpinCheckFlag ==1 && $compareCheckFlag == 'Y' && ENABLED_BLOCKCHAIN == 1 && in_array($this->session->userdata('dist_code'),json_decode(ENABLED_BLOCKCHAIN_FOR_DIST)))
           {
               ...
               redirect(base_url() . "index.php/dc_adc_conversion/select_full_or_partial?ulpin=" . $ulpin . "&chain_revenue=" . $chain_revenue . "&chain_local_tax=" . $chain_local_tax . "&old_ulpin=" . $old_ulpin . "&old_patta_type=" . $old_patta_type . "&new_patta_type=" . $new_patta_type . "&new_patta_no=" . $new_patta_no . "&old_patta_no=" . $old_patta_no . "&new_patta_no=" . $new_patta_no . "&old_dag_no=" . $old_dag_no . "&new_revenue=" . $new_revenue . "&new_local_tax=" . $new_local_tax . "&new_dag_no=" . $new_dag_no. "&ulpinCheckFlag=" . $ulpinCheckFlag . "&compareCheckFlag=" . $compareCheckFlag);
           }

           redirect(base_url() . "index.php/dc_adc_conversion/select_full_or_partial");
       }
    ...
    public function FinalSaveTest() {
        ...
        foreach ($pattadar_details as $p) {
            ...
            $query = "select p.pdar_id,p.pdar_name,p.pdar_father,p.pdar_add1,p.pdar_add2,p.pdar_add3,p.pdar_guard_reln from    chitha_pattadar p join 
                    chitha_dag_pattadar d on p.dist_code = d.dist_code and p.subdiv_code = d.subdiv_code and p.cir_code = d.cir_code 
                    and p.lot_no = d.lot_no and p.vill_townprt_code = d.vill_townprt_code and p.mouza_pargona_code = d.mouza_pargona_code and
                    p.pdar_id = d.pdar_id and trim(p.patta_no) = trim(d.patta_no) and p.patta_type_code=d.patta_type_code where p.dist_code='$dist_code' and p.subdiv_code='$subdiv_code' and p.cir_code='$cir_code' and
                    p.mouza_pargona_code='$mouza_pargona_code' and p.vill_townprt_code='$vill_townprt_code' 
                    and d.lot_no='$lot_no' and d.dag_no='$dag' and trim(p.patta_no)='$patta_no' 
                    and p.patta_type_code='$patta_type' and p.pdar_id='$pdar_id'";
    ...
    public function select_full_or_partial() {
        ...
        { 
            ...
            if ($result->make_mdb == 'F') {
                // echo "full";
                redirect(base_url() . "index.php/dc_adc_conversion/updateChithaConversionForFullConversion?ulpin=" . $ulpin . "&chain_revenue=" . $chain_revenue . "&chain_local_tax=" . $chain_local_tax . "&old_ulpin=" . $old_ulpin . "&old_patta_type=" . $old_patta_type . "&new_patta_type=" . $new_patta_type . "&new_patta_no=" . $new_patta_no . "&old_patta_no=" . $old_patta_no . "&new_patta_no=" . $new_patta_no . "&old_dag_no=" . $old_dag_no . "&new_revenue=" . $new_revenue . "&new_local_tax=" . $new_local_tax. "&ulpinCheckFlag=" . $ulpinCheckFlag . "&compareCheckFlag=" . $compareCheckFlag);
            } else {
                // echo "partial";
                redirect(base_url() . "index.php/dc_adc_conversion/updateChithaConversionForPartialConversion?ulpin=" . $ulpin . "&chain_revenue=" . $chain_revenue . "&chain_local_tax=" . $chain_local_tax . "&old_ulpin=" . $old_ulpin . "&old_patta_type=" . $old_patta_type . "&new_patta_type=" . $new_patta_type . "&new_patta_no=" . $new_patta_no . "&old_patta_no=" . $old_patta_no . "&new_patta_no=" . $new_patta_no . "&old_dag_no=" . $old_dag_no . "&new_revenue=" . $new_revenue . "&new_local_tax=" . $new_local_tax . "&new_dag_no=" . $new_dag_no. "&ulpinCheckFlag=" . $ulpinCheckFlag . "&compareCheckFlag=" . $compareCheckFlag);
            }
        }
        ...
        if ($result->make_mdb == 'F') {
            // echo "full";
            redirect(base_url() . "index.php/dc_adc_conversion/updateChithaConversionForFullConversion");
        } else {
            // echo "partial";
            redirect(base_url() . "index.php/dc_adc_conversion/updateChithaConversionForPartialConversion");
        }
    ...
    public function updateChithaConversionForFullConversion() {
        ...
        //                    . " vill_townprt_code='$order->vill_townprt_code' and dag_no='$order->dag_no' ";
            ...
            foreach ($pattdars as $p) {
                ...
                if ($chitha_basic_update == FALSE) {
                    ...
                    $where = [
                        ...
                        'vill_townprt_code'  => $ord->vill_townprt_code,
                        'dag_no'             => $ord->dag_no,
                        'patta_no'           => trim($ord->patta_no), // replicates TRIM(patta_no)=trim(...)
                ...
                $where = [
                    ...
                    'pdar_id'            => $ord->pdar_id,
                    'patta_type_code'    => $ord->patta_type_code,
                    'patta_no'           => trim($ord->patta_no), // Apply trim here in PHP
                ];
            ...
            else
            {
                ...
                $task='CO';
                $pen='NA';
                ////////////////FINAL-CHECK-DATA-VALIDATION///////////////////////
                $sql="Select * from t_chitha_rmk_convorder where ord_no=?";
                $rmk_conv_data=$this->db->query($sql,array($case_no));
                if($rmk_conv_data->num_rows()==0){
                    $this->db->trans_rollback();
                    log_message('error', '#ERRCONVDC0058: NO DATA FOUND IN T-CHITHA-RMK-CONV-ORDER :'. $case_no);
                    $this->session->set_flashdata('message', "Error in Processing. Please try Again. Error Code(#ERRCONVDC0058)");
                    redirect(base_url() . "index.php/home");
                }
                $sql="Select * from chitha_rmk_ordbasic where ord_no=?";
                $rmk_ord=$this->db->query($sql,array($case_no));
                if($rmk_ord->num_rows()==0){
                    $this->db->trans_rollback();
                    log_message('error', '#ERRCONVDC0057: NO DATA FOUND IN CHITHA-RMK-ORDBASIC :'. $case_no);
                    $this->session->set_flashdata('message', "Error in Processing. Please try Again. Error Code(#ERRCONVDC0057)");
                    redirect(base_url() . "index.php/home");
                }
                /////////////////////////////////
            ...
            foreach ($pattdars as $p) {
                ...
                if ($chitha_basic_update == FALSE) {
                    ...
                    $where = [
                        ...
                        'vill_townprt_code'  => $ord->vill_townprt_code,
                        'dag_no'             => $ord->dag_no,
                        'patta_no'           => trim($ord->patta_no),  // PHP trim replicates SQL TRIM()
                        'patta_type_code'    => $ord->patta_type_code,
                    ];

                    // Execute the update using your model
                    ...
                    $landclass_query = "select land_class_code from    petition_lm_note  where dist_code='$ord->dist_code' and"
                            ...
                            . " and lot_no='$ord->lot_no' and mouza_pargona_code='$ord->mouza_pargona_code' and "
                            . " vill_townprt_code='$ord->vill_townprt_code' and dag_no='$ord->dag_no' "
                            . " and co_reject is null order by date_entry desc limit 1";
                    //echo $landclass_query;
                    $landclasscode = $this->db->query($landclass_query)->row()->land_class_code;
    ...
    public function DepartmentReportSubmit(){
        ...
        if($formValidation['status'] == 'n') {
            ...
            redirect(base_url('index.php/dc_adc_conversion/GoToDC_ADC?pro=5'));
        }
    ...
    public function DepartmentReverted() {
        ...
        $bo_details = $this->db->query("Select * from petition_bo_note where dist_code=? and subdiv_code=? and cir_code=? and lot_no=? and vill_townprt_code=? and mouza_pargona_code=? and petition_no=?", [$petitionBasic->dist_code, $petitionBasic->subdiv_code, $petitionBasic->cir_code, $petitionBasic->lot_no, $petitionBasic->vill_townprt_code, $petitionBasic->mouza_pargona_code, $petitionBasic->petition_no])->row();
        ...
        if (!empty($bo_details)) {
            ...
            $boname = $this->db->query("select * from users where user_code=? and dist_code = ?", [$bo_details->bo_code, $bo_details->dist_code])->row();
            $data['bo_boname'] = $boname->username;
        }
        ...
        $data['bo_details_final'] = $bo_details;
        // $data['bo_details_final'] = $this->db->query("Select * from    petition_bo_note where dist_code='$petition_basic->dist_code' and subdiv_code='$petition_basic->subdiv_code' "
        //         . "and cir_code='$petition_basic->cir_code' and lot_no='$petition_basic->lot_no' and vill_townprt_code='$petition_basic->vill_townprt_code' and "
        //         . "mouza_pargona_code='$petition_basic->mouza_pargona_code' and petition_no='$petition_basic->petition_no'")->result();
        ...
        $data['_view'] = 'dc_adc_office_conversion/departmentReverted';
        $this->load->view('layouts/main',$data);
    ...
    public function DepartmentRevertToCo() {
        ...
        $dataProceeding = [
            ...
            'subdiv_code'=>$petitionBasic->subdiv_code,
            'cir_code'=>$petitionBasic->cir_code,
            'co_order'=>$proceedingDcAdc->co_order,
            'note_on_order'=>$proceedingDcAdc->note_on_order,
            'user_code'=>$user_code,
            'status'=>'Pending',
            'operation' => 'E',
            'date_entry' => date('Y-m-d H:i:s'),
