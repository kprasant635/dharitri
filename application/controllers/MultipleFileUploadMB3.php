...
class MultipleFileUploadMB3 extends CI_Controller
{
    ...
    public function getLRAFiles()
    {
        ...
        $get_application_id = $this->db->query("SELECT * FROM basundhar_application WHERE dharitree= ?", [$case_no])->row();
        // Query files for the case

        $sql   = "SELECT * FROM $table WHERE (case_no = ? or case_no = ?)";
        $query = $this->db->query($sql, [$get_application_id->basundhara, $case_no]);
        ...
        if ($query && $query->num_rows() > 0) {
            foreach ($query->result_array() as $file) {
                ...
                // Add the file data along wx`x`ith the path URL
                $file['file_path'] = "MultipleFileUploadMB3/viewfile/{$hashId}";
