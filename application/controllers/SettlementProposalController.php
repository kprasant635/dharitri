...
class SettlementProposalController extends CI_Controller
{
    ...
    public function commonProposalListView()
    {
        ...
        $data['countPendingCase'] = $this->SettlementCommonDcModel->getPendingProposalsReadyForMeeting($dist_code,MB_ADD_DEPUTY_COMM);
        ...
        $data['_view'] = 'SettlementView/Adc/common_proposal_list_forward_to_dc';
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows) {
                ...
                <div class="text-green" id="list_of_cases_'.$rows->id.'"></div>',
                    ...
                    <a class="btn btn-xs btn-success" style=" padding: 7px!important; margin-bottom: 5px!important" target= "SDLACProposalNotice" href="'.base_url().'index.php/SettlementProposalController/proposalEditOnSdlacMinutesAdc/?case='.$rows->id.'">
    ...
    public function sendProposalsToDcMinute()
    {
        ...
        if($this->SettlementCommonDcModel->checkProposalMatchWithMbThreeWithMbTwo($proposals) != 0)
        {
            $json = [
                'response' => 1,
                'message'  => 'Some of the Proposals Mismatch with MB 3.0 !',
            ];
            echo json_encode($json);
            return false;
        }
        ...
        // var_dump($allSelectedMember); die;
        ...
        // var_dump($proposalDetails); die;

        if($userMpCount == 0 OR $userMlaCount == 0 OR $userSdlacCount == 0)
        {
            $json = [
                'response' => 1,
                'message'  => '#ERR1018: Minutes Copy to Members are incomplete ! Kindly Add Members For Minutes Copy To ',
            ];
            echo json_encode($json);
            return false;
        }
    ...
    public function revertMeetingListForAdc()
    {
        ...
        $pendingCase              = $this->SettlementCommonDcModel->getRevertedMeetingList($dist_code,$createdBy);
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'SettlementView/Adc/reverted_meeting_list_adc';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getProposalUnderRevertedMeetingForAdc()
    {
        ...
        $pendingMeeting      = $this->SettlementCommonDcModel->checkRevertedMeetingExistOrNotWithMeetingIdAdcMB2($meetingId,$dist_code);
        $pendingMeetingCount = $pendingMeeting->num_rows();

        if($pendingMeetingCount == 0)
        {
            $this->session->set_flashdata('error', "Reverted Meeting by DC Not Found !");
            redirect(base_url() . "index.php/SettlementProposalController/revertMeetingListForAdc");
            $this->pendingProposalList();
        }
        else
        {
            ...
            $data['_view'] = 'SettlementView/Adc/reverted_proposal_by_dc_list';
            $this->load->view('layouts/main', $data);
        }
    }
    ...
    public function caseListUnderRevertedMeetingForAdc()
    {
        ...
        $proposalDetails = $this->SettlementCommonDcModel->getRevertedProposalDetailsByIdAdc($proposal_no,$dist_code);
        ...
        $data['_view'] = 'settlementView/Adc/reverted_application_by_proposal';
        $this->load->view('layouts/main', $data);
    ...
    public function sendRevertedProposalsToDcMinuteAdc()
    {
        ...
        if($userMpCount == 0 OR $userMlaCount == 0 OR $userSdlacCount == 0)
        {
            $json = [
                'response' => 1,
                'message'  => '#ERR3208: Minutes Copy to Members are incomplete ! Kindly Add Members For Minutes Copy To ',
            ];
            echo json_encode($json);
            return false;
        }
    ...
    public function forwardedMeetingListForAdc()
    {
        ...
        $pendingCase              = $this->SettlementCommonDcModel->getForwardedMeetingListAdc($dist_code,$createdBy);
        $data['cases']            = $pendingCase->result();
        $data['pendingCaseCount'] = $pendingCase->num_rows();

        $data['_view'] = 'SettlementView/Adc/forwarded_meeting_list_adc';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getForwardedMeetingDetailsByIdAdc()
    {
        ...
        $meeting = $this->SettlementCommonDcModel->getForwardedMeetingDetailByMeetingIDAdc(
        ...
        {
            $this->session->set_flashdata('error', "Meeting Not Found !");
            redirect(base_url() . "index.php/SettlementProposalController/forwardedMeetingListForAdc");
        }
        ...
        $data['_view'] = 'SettlementView/Adc/forwarded_proposals_against_meeting_id_adc';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function proposalEditOnSdlacMinutesAdc()
    {
        ...
        if($proposalDetails->meeting_create_status != 1)
        {
            ...
            redirect(base_url() . "index.php/SettlementProposalController/commonProposalListView");
        }
        ...
        $data['_view'] = 'settlementView/Adc/edit_proposal_from_sdlac_minutes_adc';
        $this->load->view('layouts/main', $data);
    }
