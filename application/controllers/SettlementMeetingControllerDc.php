...
class SettlementMeetingControllerDc extends CI_Controller
{
    ...
    public function meetingLandPage()
    {
        ...
        $data['_view'] = 'SettlementView/Dc/pending_meeting_list_for_approval';
    ...
    public function listOfPendingMeetingIds()
    {
        ...
        $this->db->where('proposal_meeting_list.meeting_type_ins', null);
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($result as $rows)
            {
                ...
                $checkServiceCode = $this->SettlementMeetingDcModel->checkServiceCodeFromMeetingToProposal($rows->id);
                ...
                if($checkServiceCode == 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            $uploadedMinutes ='<a class="rezaButt btn btn-sm" style="background-color :#03A9F4" target="SdlacMinutes" href="'.base_url().'index.php/SettlementMeetingControllerDc/viewSdlacUploadedMinute/?meetingId='.$rows->id.'"><i class="fa fa-download" aria-hidden="true"></i> &nbsp;Uploaded Minutes</a>';
                        }
                        ...
                        $attendance = '<a class="rezaButt btn btn-sm " style="background-color :#3F51B5" target="SdlacAttendance" href="'.base_url().'index.php/SettlementMeetingControllerDc/viewSdlacAttendance/?meetingId='.$rows->id.'"><i class="fa fa-download" aria-hidden="true"></i> &nbsp;Attendance</a>';
                    }
                }
                else
                {
                    $digitalMinutes    = '<b style="color: #FF5252;">There are some inconsistencies in MB 2 and MB 3 under this meeting.</b>'.'<br>';
                    $revertBackMeeting = '';
                    $uploadedMinutes   = '';
                    $attendance        = '';
                }
                ...
                $viewMeeting ='<a class="rezaButt btn btn-sm " style="background-color :#4CAF50" href="'.base_url().'index.php/SettlementMeetingControllerDc/getPendingProposalsAgainstMeetingId/?meetingId='.$rows->id.'"><i class="fa fa-eye" aria-hidden="true"></i> &nbsp;View Detail</a>';
    ...
    public function meetingApprovedLandPage()
    {
        ...
        $total_records = $query1->num_rows();
        $meetingList      = $query1->result();


        $meetings = [];
        foreach ($meetingList as $meeting)
        {
            $checkServiceCode = $this->SettlementMeetingDcModel->checkServiceCodeFromMeetingToProposal($meeting->id);
        ...
        $data['_view'] = 'SettlementView/Dc/approve_meeting_list_for_approval';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getPendingProposalsAgainstMeetingId()
    {
        ...
        $meeting = $this->SettlementMeetingDcModel->getPendingMeetingDetailByMeetingID(
            $meetingId)->row();
        ...
        $proposalDetail = $this->SettlementMeetingDcModel->getProposalDetailAgainstMeetingId($dist_code,
            $meetingId)->result();
        ...
        $data['_view'] = 'SettlementView/Dc/pending_proposals_against_meeting_id';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getApprovedProposalsAgainstMeetingId()
    {
        ...
        {
            $this->session->set_flashdata('error', "Meeting Not Found !");
            redirect(base_url() . "index.php/SettlementMeetingControllerDc/meetingApprovedLandPage");
        }
        ...
        $data['_view'] = 'SettlementView/Dc/approved_proposals_against_meeting_id';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function viewCasesAgainstProposalNo()
    {
        ...
        $json = [
            ...
            'proposal_name' => $this->SettlementMeetingDcModel->getProposalNameByProposalNo($proposalNo),
        ];
        echo json_encode($json);
        return;
    }
    ...
    public function chithaReserveAreaCheckWithCaseNo($application_no)
    {
        ...
        if (!in_array($basic->service_code, MB_2_SERVICE_CODE_ALLOW_FOR_PROPOSAL))
        {
            echo 'Case Number '. $application_no . 'is not belongs to MB2';
            die;
        }
    ...
    {
        $meetingId = trim($this->input->get('meetingId'));
        $meetingDetails = $this->SettlementMeetingDcModel->getPendingMeetingDetailByMeetingID($meetingId)->row();
    ...
    public function digitalSignAndSavePdf()
    {
        ...
        else
        {
            ...
            $fileName    = $distEngName.'_SDLAC_'.$dist_code.'_'.date("Y").'_'.$meetingId;
            ...
            if($checkMeeting == 1)
            {
                if(MB2_DIGITAL_SIGN_LIVE == 1)
                {
                    include 'vendor/mpdf/vendor/autoload.php';
                    $mpdf=new \Mpdf\Mpdf();
                    if(MB2_DIGITAL_SIGN_DRAFT_MODE == 1)
                    {
                        $waterMark = 'Mission Basundhara';
                    }
                    else
                    {
                        $waterMark = 'DRAFT';
    ...
    public function signedAndFinalApproveByDC()
    {
        ...
        try
        {
            ...
            $getProposalsList = $this->SettlementMeetingDcModel->getProposalDetailAgainstMeetingId($dist_code, $meetingId)->result();
            ...
            foreach($getProposalsList as $prop)
            {
                ...
                $proposalDetails = $this->SettlementMeetingDcModel->getProposalDetailsById($proposal_no,$dist_code);
                ...
                if($final_verify_status == 1)
                {
                    ...
                    foreach ($cases as $case)
                    {
                        ...
                        if (!in_array(trim($basic['service_code']), MB_2_SERVICE_CODE_ALLOW_FOR_PROPOSAL))
                        {
                            echo 'Case Number '. $case->case_no . 'is not belongs to MB2';
                            die;
                        }
            ...
            if (isset($allAPICasesRural) && count($allAPICasesRural)>0)
            {
                ...
                $rtps_status = $this->SettlementApiModel->applicationStatusUpdateBulk($allAPICasesRuralIds,'NA',$rmk,$status,$task,$pen);
                if($rtps_status!="y")
                {
                    $this->db->trans_rollback();
    ...
    {
        $case_no = trim($case_no);
        $endDate = HOLD_CASES_FORWARD_TO_DEPT_BY_DC;
    ...
    public function verifyArea()
    {
        ...
        try
        {
            ...
            if($wetCase != NULL)
            {
                $errorWetShow = ' Some cases have been marked as wetland ! (Case No. -  '.$wetCase .')';

                log_message('error', '#MR000313: '.$errorWetShow .') !');
                echo json_encode(array(
                    'responseType' => 101,
                    'message'      => '#MR000313: Unable to process for final approval .'.$errorWetShow.' !!!',
                ));
                return;
            }
    ...
    public function deleteProposalWithZeroCasesForPendingMeetingByDc()
    {
        ...
        {
            $pro = trim($singlePro);
            $proposalCount = $this->SettlementCommonDcModel->countSettlementProposalList($pro);
    ...
    public function revertBackMeetingToAdcSdo()
    {
        ...
        else
        {
            ...
            if($checkMeeting == 1)
            {
                ...
                //list of proposals against meeting id
                $getProposal = $this->SettlementMeetingDcModel->getProposalDetailAgainstMeetingId($dist_code,$meetingId);
    ...
    public function getAllPendingMeetingForDigitalResigning()
    {
        ...
        $data['_view'] = 'SettlementView/Dc/pending_meeting_for_digital_resigning';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getResigningProposalsAgainstMeetingId()
    {
        ...
        $data['_view'] = 'SettlementView/Dc/pending_resigning_proposals_against_meeting_id';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function digitalResignAndSavePdf()
    {
        ...
        else
        {
            ...
            if($checkMeeting == 1)
            {
                ...
                $dist_name   = $this->UtilsModel->getEngDistrictNameByDistCode($dist_code);
                $distEngName = substr($dist_name->locname_eng, 0, 3);
                $fileName    = $distEngName.'_SDLAC_'.$dist_code.'_'.date('Y', strtotime($meetingDetails->meeting_date)).'_'.$meetingId;
    ...
    public function revertedMeetingByDepartmentForDC()
    {
        ...
        $sqlForRevertList = 'select distinct pml.meeting_name,pml.signed_minute,pml.id as meeting_id,pml.meeting_venue,pml.meeting_date from settlement_basic sb  
                                ...
                                join proposal_meeting_list pml on spl.proposal_meeting_id = pml.id
                                    ...
                                    and sb.service_code IN (\'13\', \'14\', \'15\', \'16\', \'17\', \'18\')';
        ...
        $data['meetingCount'] = $total_records;
        $data['meetings'] = $meetings;
        $data['_view'] = 'SettlementView/Dc/reverted_meeting_list_by_department_dc';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getRevertedProposalsAgainstMeetingId()
    {
        ...
        $meeting = $this->SettlementMeetingDcModel->getPendingMeetingDetailByMeetingID($meetingId)->row();
        $meetingName = trim($meeting->meeting_name);
        if($meetingName == '')
        {
            $this->session->set_flashdata('error', "Meeting Not Found !");
            redirect(base_url() . "index.php/SettlementMeetingControllerDc/revertedMeetingByDepartmentForDC");
        }
        ...
        $data['_view'] = 'SettlementView/Dc/reverted_dept_proposals_against_meeting_id';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function getRevertedCasesDeptAgainstMeetingId()
    {
        ...
        $data['_view'] = 'SettlementView/Dc/reverted_cases_by_department_dc';
        $this->load->view('layouts/main', $data);
    }
    ...
    public function finalDeptRevertedCaseRevertToCO()
    {
        ...
        else
        {
            ...
            $sqlForRevertList = 'select distinct sb.case_no from settlement_basic sb  
                                 ...
                                 join proposal_meeting_list pml on spl.proposal_meeting_id = pml.id
                                 where sb.dist_code=? and sb.dept_revert=? 
                                 and sb.pending_officer=? and sb.from_office=? and pml.id =? 
                                 and sb.service_code IN (\'13\', \'14\', \'15\', \'16\', \'17\', \'18\')';
            $query1 = $this->db->query($sqlForRevertList,array($dist_code,1,MB_DEPUTY_COMM,MB_DEPARTMENT,$meeting_id));
            $arrayM = $query1->result();

            $meetingDetails  = $this->SettlementPullModel->getMeetingDetailByMeetingIDPull(trim($meeting_id));
    ...
    public function getVgrPgrCasesForRevertDc($meetingId)
    {
        ...
        if($checkMeeting == 1)
        {
            ...
            if (trim($meetingDetails->vgr_pgr_status) == 1)
            {
                $proposals = $this->SettlementMeetingDcModel->getProposalDetailAgainstMeetingId($dist_code,$meetingId)->result();

                $allCases = [] ;
                foreach ($proposals as $singlePro)
                {
                    $cases = $this->SettlementMeetingDcModel->getCaseDetailsByProposalNos($singlePro->proposal_id);
    ...
    public function saveVgrPgrCasesForRevertDcData()
    {
        ...
        else
        {
            ...
            $meeting = $this->SettlementMeetingDcModel->getPendingMeetingDetailByMeetingID($meetingId);
    ...
    public function getVgrPgrRevertedCaseDetailsForDc()
    {
        ...
        if($caseId == '' || $caseId == NULL)
        {
            ...
            redirect(base_url() . "index.php/SettlementMeetingControllerDc/getAllVgrPgrRevertedCaseByDept");
        }
...
//        die();
    ...
    public function digitalSignAndSavePdfHardCode()
    {
        ...
        else
        {
            ...
            $dist_name   = $this->UtilsModel->getEngDistrictNameByDistCode($dist_code);
            $distEngName = substr($dist_name->locname_eng, 0, 3);
            $fileName    = $distEngName.'_SDLAC_'.date("Y").'_'.$meetingId;
    ...
    vill_townprt_code = sd.vill_townprt_code
                ...
                sa.case_no,(SELECT applid  FROM settlement_basic WHERE case_no=sa.case_no) AS applid,
                (SELECT service_code FROM settlement_basic WHERE case_no=sa.case_no) AS service_code,
    ...
    // not used anymore
    public function signedAndFinalApproveByDC_old()
    {
        $_POST = json_decode(file_get_contents("php://input"), true);
        $this->load->library('form_validation');
        ...
        else
        {
            ...
            if($checkMeeting == 1)
            {
                ...
                foreach($getProposalsList as $prop)
                {
                    ...
                    if($final_verify_status == 1)
                    {
                        ...
                        if($caseCount == 0)
                        {
                            ...
                            log_message('error', '#MR0001162: Cases not found for proposal id '.$proposal_no);
                            echo json_encode(array(
                                'responseType' => 1,
                                'message'      => '#MR0001162: Unable to process for final approval. 
                                      Kindly contact system administrator !!!!' ,
                            ));
                            return;
                        }
                        else
                        {
                            foreach ($cases as $case)
                            {
                                ...
                                if($caseCount == 0)
                                    // if ($basic['status'] != MB_SEND_TO_SDLAC || !in_array($basic['pending_officer'],[MB_DEPUTY_COMM,MB_ADD_DEPUTY_COMM,MB_SUB_DIV_COMM]))
                                {
                                    $this->db->trans_rollback();
                                    log_message('error', '#MR0001189: Few cases under this proposal has already 
                                    been approved. Proposal# : '.$proposal_no.' Case# : '.$case_no);
                                ...
                                if(trim($basic['final_status']) == MB_APPROVED_BY_SDLAC)
                                {
                                    ...
                                    if($finalApprovedBy == 1) // approve by dept
                                    {
                                        ...
                                        {
                                            $this->db->trans_rollback();
                                            log_message('error', '#MR0001232: Updation failed in settlement basic '
                                                .$this->db->last_query());
                                            echo json_encode(array(
                                                'responseType' => 1,
                                                'message'      => '#MR0001232: Unable to process for final approval. 
                                                Kindly contact system administration !!!',
                                            ));
                                            return;
                                        }
                                        else
                                        {
                                            ...
                                            {
                                                $this->db->trans_rollback();
                                                log_message('error', '#MR0001250: Updation failed in settlement_proposal_cases '
                                                    .$this->db->last_query());
                                                echo json_encode(array(
                                                    'responseType' => 1,
                                                    'message'      => '#MR0001250: Unable to process for final approval. 
                                                  Kindly contact system administration !!!',
                                                ));
                                                return;
                                            }
                                            ...
                                            {
                                                $this->db->trans_rollback();
                                                log_message('error', '#MR0001287: Insertion failed in settlement_proceeding '
                                                    .$this->db->last_query());
                                                echo json_encode(array(
                                                    'responseType' => 1,
                                                    'message'      => '#MR0001287: Unable to process for final approval. 
                                                  Kindly contact system administration !!!',
                                                ));
                                                return;
                                            }
                                    ...
                                    else if($finalApprovedBy == 2) // approve by DC
                                    {
                                        ...
                                        {
                                            $this->db->trans_rollback();
                                            log_message('error', '#MR0001318: Updation failed in settlement_basic '
                                                .$this->db->last_query());
                                            echo json_encode(array(
                                                'responseType' => 1,
                                                'message'      => '#MR0001318: Unable to process for final approval. 
                                                Few cases under this meeting has already been approved !!',
                                            ));
                                            return;
                                        }
                                        else
                                        {
                                            ...
                                            {
                                                $this->db->trans_rollback();
                                                log_message('error', '#MR0001337: Updation failed in settlement_proposal_cases : '
                                                    .$this->db->last_query());
                                                echo json_encode(array(
                                                    'responseType' => 1,
                                                    'message'      => '#MR0001337: Unable to process for final approval. 
                                                    Few cases under this meeting has already been approved !!!!',
                                                ));
                                                return;
                                            }
                                            ...
                                            {
                                                $this->db->trans_rollback();
                                                log_message('error', '#MR0001374: Insertion failed in settlement_proceeding '
                                                    .$this->db->last_query());
                                                echo json_encode(array(
                                                    'responseType' => 1,
                                                    'message'      => '#MR0001374: Unable to process for final approval. 
                                                    Few cases under this meeting has already been approved !!!!',
                                                ));
                                                return;
                                            }
                                        ...
                                        if($enc_check->num_rows() > 0)
                                        {
                                            ...
                                            if($sql_land_bank->num_rows() > 0) {
                                                ...
                                                if($VLBresponse->responseType != 1) {
                                                    ...
                                                    $this->db->trans_rollback();
                                                    log_message('error', '#MR0001411: Insertion failed in landbank '
                                                        .$this->db->last_query());
                                                    echo json_encode(array(
                                                        'responseType' => 1,
                                                        'message'      => '#MR0001411: Unable to process for final approval. 
                                                Few cases under this meeting has already been approved !!',
                                                    ));
                                                    return;
                                                }
                                ...
                                elseif(trim($basic['final_status']) == MB_DISMISS)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        // if (trim($rtps_status)!="y" || $rtps_status == null || $rtps_status == '')
                                        // {
                                        //     $this->db->trans_rollback();
                                        //     log_message('error', '#MR0001555: Issue in API Call'
                                        //         .$this->db->last_query());
                            ...
                            log_message('error','Total time taken='.(microtime(true)-$final_st_time));
                            // $base64PDFData = $pdfData;
                            // $uploadpath   = SIGNPDF_UPLOAD_DIR;
                            // file_put_contents($uploadpath.$fileName.".pdf", base64_decode($base64PDFData));
                            // $signDateTime = date('Y-m-d H:i:s');
                ...
                //     }
                //     $this->db->trans_rollback();
                //     log_message('error', '#MR0001202: Applied area exceeds the limit (Case No. -  '.$case_str .') !');
                //     echo json_encode(array(
                //         'responseType' => 1,
                //         'message'      => '#MR0001202: Unable to process for final approval.
                //                         Total Area Recommended for Settlement can’t exceed available Area in Chitha (Case No. -  '.$case_str .') !!!',
                //     ));
                //     return;
