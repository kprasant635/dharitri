<?php
class SettlementTenantUrban extends CI_Controller
{
    ...
    public function valid_date_1899($date)
    {
        // Convert input date to a timestamp
        $timestamp = strtotime($date);
        
        // Check if the date is valid and after 31st December 1899
        if ($timestamp && $timestamp > strtotime('1899-12-31')) {
            return true; // Validation passed
        } else {
            $this->form_validation->set_message('valid_date_1899', 'The {field} must be a valid date after 1899.');
    ...
    public function settlementTenantRegistration($review_flag = false)
    {
        ...
        {
            $district['_view'] = 'SettlementView/SettlementTenantViewUrban';
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST')
        {
            ...
            if($_POST['lm_note'] == '2')
            {
                if(isset($_POST['rejected_reasons']))
                {
                    ...
                    $validation_bypass_array = $this->getValidationBypass(SETTLEMENT_TENANT_URBAN_ID);
            ...
            if($validation_bypass == 0)
            {
                ...
                $this->form_validation->set_rules('cont_possessing', 'Continious Possessing', 'trim|required');
                // $this->form_validation->set_rules('period_possession_lm', 'Period of Possession from Date', 'trim|required');
                $this->form_validation->set_rules(
                    'period_possession_lm',
                    'Period of Possession from Date',
                    'trim|required|callback_valid_date_1899'
                );
                ...
                if($this->input->post('nature_possession') == 'Others'){
                ...
                $this->form_validation->set_rules('date_notification', 'Date of notification vide which the area was included', 'trim|required');
                $this->form_validation->set_rules('cont_possession', 'Whether applicant(s) have been in continuous possession', 'trim|required');
            ...
            else
            {
                ...
                if($validation_bypass == 0)
                {
                        ...
                        for($i = 0; $i < $fileCount; $i++)
                        {
                            ...
                            if ($this->upload->do_upload('file'))
                            {
                                $document= array(
                                    ...
                                    'mut_type'   => SETTLEMENT_TENANT_URBAN_ID,
                                );
                    ...
                    $responseMasterObj = $this->SettlementCommonModel->lmRejectedValidationBypassFalse(SETTLEMENT_TENANT_URBAN_ID);
                    ...
                    $lmnote = array(
                        ...
                        'case_no' => $case_no,
                        'status' => 'W',
                        'land_used_by_occupants' => ($this->input->post('nature_possession') == 'Others') ? $this->input->post('land_used_by_occupants') : null,
                        ...
                        'cont_possessing' => $this->input->post('cont_possessing'),
                        'litigation' => $this->input->post('litigation'),
                        'is_tribal_belt' => $this->input->post('is_tribal_belt'),
                        'contravention' => ($this->input->post('is_tribal_belt') == 'NO') ? $this->input->post('contravention') :null,
                        ...
                        // 'lm_tenant_report' => json_encode($lra_report_new),
                        'date_notification'       => date('Y-m-d', strtotime($this->input->post('date_notification'))), 
                        'tenancy_record'          => $this->input->post('tenancy_record'), 
                        'cont_possession'         => $this->input->post('cont_possession'), 
                    );
                    ...
                    $dag_per_lessa_revenue = ($zonal_bigha / $total_dag_lessa);
                    $final_amount = ($total_app_lessa * $dag_per_lessa_revenue);
                    $landlordAmount = ceil($final_amount * 50);
                    ...
                    $prem_zonal = $this->utilityclass->getZonalValue($basic['dist_code'],$basic['uuid'],$this->input->post('dag_no'));
                    $prem_area = $this->input->post('total_lessa'.$this->input->post('dag_no'));
                    $prem_concession = "YES";

                    $percentage = 10;
                    $zonal_lessa = $prem_zonal / $area_in_bigha;
                    $premium = $prem_area * $zonal_lessa;
                    $govtamount = ceil($premium * $percentage / 100);

                    $sumMbAmount = $govtamount + $landlordAmount;
                    ...
                    $fmd=array(
                        ...
                        'uuid'            => $basic['uuid'],
                        'dag_no'          => $this->input->post('dag_no'),
                        'zonal_valuation' => $this->input->post('zonal_valuation_prem'.$this->input->post('dag_no')),
                        'area_name'       => null,
                        'land_type'       => null,
                        'rate_type'       => null,
                        'rate'            => null,
                        'concession'      => null,
                        'amount_dag'      => $this->input->post('amount'.$this->input->post('dag_no')),
                        'final_amount'    => $sumMbAmount,
                        'due_amount'      => $sumMbAmount,
                        'total_lessa'     => $this->input->post('total_lessa'.$this->input->post('dag_no')),
                        'is_full_pay'     => $this->input->post('paymode'),
                        'is_final'        => 1,
                        'date_entry'      => date('Y-m-d h:i:s'),
                        'govt_share'      => $govtamount,
                        'landlord_share'  => $landlordAmount
                    );
                    ...
                    $approved_by = $this->input->post('approval'.$this->input->post('dag_no'));
                    
                    //   var_dump("backend:".$sumMbAmount."---frontend:".$this->input->post('finalamount')); die;
                    // premium verify 2 start ******************
                    if($sumMbAmount != $this->input->post('finalamount')){
                        // var_dump("Amount mismatch!!!"); die;
                      $this->db->trans_rollback();
                      $this->session->set_flashdata('message', "Error #ERRAM1895: Settlement Application not submitted case no # $application_no");
                    ...
                    if($finaldueamount != $this->input->post('totaldue')){
                      ...
                      $this->db->trans_rollback();
                      $this->session->set_flashdata('message', "Error #ERRAM1912: Settlement Application not submitted case no # $case_no");
    ...
    public function secondProceeding()
    {
        ...
        if($_SERVER['REQUEST_METHOD'] != 'POST')
        {
            $district['_view'] = 'SettlementView/Lm/Tenant/SettlementTenantViewUrban';
            $this->load->view('layouts/main', $district);
        }
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST')
        {
            ...
            if($distCode == NULL)
            {
                redirect(base_url(). 'index.php/home/SettlementKhasLandLm?service='.SETTLEMENT_TENANT_URBAN_ID);
            }
            if($case_no == NULL)
            {
                redirect(base_url(). 'index.php/home/SettlementKhasLandLm?service='.SETTLEMENT_TENANT_URBAN_ID);
            ...
            if($validation_bypass == 0)
            {
                ...
                // $this->form_validation->set_rules('landslide', 'Area Under cover landslide prone', 'trim|required');
                // $this->form_validation->set_rules('erosion', 'Land falls under erosion', 'trim|required');
                $this->form_validation->set_rules('cont_possessing', 'Continiously possessing', 'trim|required');
                $this->form_validation->set_rules(
                    'period_possession_lm',
                    'Period of Possession from Date',
                    'trim|required|callback_valid_date_1899'
            ...
            else
            {
                ...
                if($validation_bypass == 0)
                {
                    ...
                    $lmnote = array(
                        ...
                        'bhumiputra_confirmation'   => $this->input->post('bhumiputra_confirmation_lm'),
                        'lm_rejected_remarks' => json_encode($responseMasterObj->reject_remarks),
                        'date_notification'       => $this->input->post('date_notification'), 
                        'tenancy_record'          => $this->input->post('tenancy_record'), 
                        'cont_possession'         => $this->input->post('cont_possession'), 
                    );
                    $this->db->where('case_no', $case_no);
                    ...
                    if ($is_prem_update == 'YES') {
                        ...
                        // var_dump("landlord:".$landlordAmount."---govtamount:".$govtamount);
                        // var_dump("backend:".$sumMbAmount."---frontend:".$this->input->post('finalamount')); die;
                        // premium verify 2 start ******************
                        if($sumMbAmount != $this->input->post('finalamount')){
                            // var_dump("Amount mismatch!!!"); die;
    ...
    public function addApplicantDetails()
    {
        ...
        $mime = mime_content_type($_FILES['noc_file']['tmp_name']);
        $exp  = explode("/",$mime);
        $onlyExtension  = $exp[1];

        if (isset($_FILES['noc_file']) && $_FILES['noc_file']['error'] === 0) {
          $uploadDir = UPLOAD_BASE.'tenant_noc_files/';
          if (!is_dir($uploadDir)) {
          ...
          $filename = basename($_FILES['noc_file']['name']);
          $targetPath = $uploadDir . $fileRename;

          // var_dump($_FILES['noc_file']['tmp_name']); die;

          if(!move_uploaded_file($_FILES['noc_file']['tmp_name'], $targetPath)){
            $this->db->trans_rollback();
            log_message('error', '#ERR4448: Document upload failed ' . $case_no);
            $data = [
                'responseType' => 0,
                'msg'          => "#ERR4448: Failed to upload document : " . $case_no,
            ];
            echo json_encode($data);
            return false;
          }
          else {
            // insert into supportive document
            $uploadNoc = [
              'case_no'         => $case_no,
              'file_name'       => 'NOC_DOC',
              'user_code'       => $this->session->userdata('user_code'),
              'fetch_file_name' => 'NOC_DOC',
              'file_type'       => $_FILES['noc_file']['type'],
            ...
            // save data in attachment file
            $addMoreDocQuery = $this->db->insert('supportive_document', $uploadNoc );
            log_message('error', '#ADD_TENANT_APPL: insert in supportive_document' . $this->db->last_query());
            // echo $this->db->last_query();
            // var_dump($addMoreDocQuery);

            if($addMoreDocQuery != 1)
            {
              $this->db->trans_rollback();
              log_message('error', '#ERR4475: Document upload failed ' . $case_no);
              $data = [
                  'responseType' => 0,
                  'msg'          => "#ERR4475: Failed to upload document : " . $case_no,
              ];
              echo json_encode($data);
              return false;
            }
        ...
        $addApplicantDetailsArr = [
          ...
          'pdar_name'          => trim($this->input->post('applicant_name_ass')),
          'eng_pdar_name'      => trim($this->input->post('applicant_name_eng')),
          'pdar_guardian'      => trim($this->input->post('guardian_name_ass')),
          'eng_pdar_guardian'  => trim($this->input->post('guardian_name_eng')),
          'pdar_rel_guar'      => trim($this->input->post('relation')),
          'pdar_gender'        => trim($this->input->post('gender')),
          'dob'                => trim($this->input->post('dob')),
          'marital_status'     => trim($this->input->post('marital_status')),
          'pdar_mobile'        => trim($this->input->post('mobile')),
          'pdar_add1'          => trim($this->input->post('per_address')),
          'pdar_add2'          => trim($this->input->post('pre_address')),
        ];

        $addsSetApplicant = $this->db->insert('settlement_applicant', $addApplicantDetailsArr);
        log_message('error', '#ADD_TENANT_APPL: insert in settlement_applicant' . $this->db->last_query());
        ...
        // var_dump($id); 
        $addApplicantDetailsArr['relation_name'] = $this->utilityclass->get_relation_id($this->input->post('relation'));
        if ($this->input->post('gender') == "1") {
            $gender = 'Male';
        } else if ($this->input->post('gender') == "2") {
            $gender = 'Female';
        } else if ($this->input->post('gender') == "3") {
            $gender = 'Others';
        }
        ...
        if ($addsSetApplicant != 1) {
            $this->db->trans_rollback();
            log_message('error', '#ERR4531: Insert failed in settlement_applicant ' . $case_no);
            $data = [
                'responseType' => 0,
                'msg'          => "#ERR4531: Insert failed in settlement_applicant : " . $case_no,
            ];
            echo json_encode($data);
            return false;
        }
