<?php
class BhoodanController extends CI_Controller
{
    ...
    public function coReSubmitLmCases()
    {
        ...
        $chitha_data['_view'] = 'Bhoodan/CO/co_resubmit_lm_cases';
    ...
    public function getValidationBypass($service_code)
    {
        ...
        foreach(json_decode(VALIDATION_BYPASS_BHOODAN) as $cons_reasons)
        {
    ...
    // application landing on LRA to get auto register
    public function applicationBhoodanRegistration($review_flag = false) {
        ...
        if(!$recordExist)
        {
            ...
            foreach ($district['applicants'] as $setl)
            {
                if ($get_aadhaar_photo != 'n' && $setl->is_applicant == '1' && IS_PRODUCTION == 1)
                {
                    ...
                    $aadhaar_file_to_write_base64 = fopen($aadhar_path, "w") or die("Unable to open file! 1");
        ...
        try
        {
            ...
            $basic                 = $this->BhoodanModel->getSettlementBasic($case_no);
            $applicants_buyers     = $this->BhoodanModel->getAllApplicantBuyers($case_no);
            $applicants_owners     = $this->BhoodanModel->getAllApplicantOwners($case_no);
            $applicants_encroacher = $this->BhoodanModel->getAllApplicantEncroacher($case_no);
            $applicants_riotee_nok = $this->BhoodanModel->getAllApplicantRioteeNok($case_no);
            $dags                  = $this->BhoodanModel->getSettlementDag($case_no);
            $lmnotes               = $this->BhoodanModel->getSettlementTenantLmNote($case_no);
            $proceedings           = $this->BhoodanModel->getSettlementProceeding($case_no);
            $dhardocuments         = $this->BhoodanModel->getDocuments($case_no);
            $nominee               = $this->BhoodanModel->getAllNomineeDetail($case_no);

            // premium
            $lmdata['s_area']      = $this->SettlementCommonModel->getPremiumArea();
            $premiumData           = $this->db->query("Select * from settlement_premium where case_no='$case_no' 
                                  and is_final=1")->row();
            ...
            foreach($lmdata['applicants_buyers'] as $adhar_photo):
                if($adhar_photo->is_applicant == 1 &&  IS_PRODUCTION == 1):
                    if (trim($adhar_photo->identity_type) == 'AADHAAR'):
                        ...
                        $open_adhar_file = fopen($adhar_photo_link, "r") or die("Unable to open file! 3");
        ...
        $getJsonBackup = $this->BhoodanModel->getJsonDataFromBackup($case_no);
        ...
        if($_SERVER['REQUEST_METHOD'] != 'POST'){
            $lmdata['_view'] = 'Bhoodan/LM/bhoodanLmView';
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            foreach($applicants_encroacher as $encroacher_prem){
                $revenue[] = $this->db->query("Select dag_revenue, dag_area_b, dag_area_k, dag_area_lc, dag_area_g, 
                        dag_area_kr, dag_no from chitha_basic where dist_code='$d' and "
                    . "subdiv_code='$s' and cir_code='$c' and mouza_pargona_code='$m' and lot_no='$l' and vill_townprt_code='$v' and dag_no='$encroacher_prem->dag_no'")->result();
                $lmdata['revenue']=$revenue;
            }
            ...
            if($_POST['lm_note'] == '2')
            {
                ...
                {
                    $validation_bypass_array = $this->getValidationBypass(BHODDAN_SERVICE_CODE);
            ...
            if ($this->form_validation->run() == false)
            {
                ...
                $lmdata['_view'] = 'Bhoodan/BhoodanView';
                $this->load->view('layouts/main',$lmdata);
            }
            else
            {
                ...
                if ($applicants_encroacher == true)
                {
                    foreach ($applicants_encroacher as $enc_applicant)
                    {
                        ...
                        {
                            $this->db->trans_rollback();
                            log_message('error', '#ERR2277: Updation failed in settlement_applicant RTPS Case No '.$application_no);
                            $data = array(
                                'error'=>"#ERR2277: Registration of Settlement failed for case no : ".$application_no
                            );
                            echo json_encode($data);
                            return false;
                        }
                ...
                {
                    $this->db->trans_rollback();
                    log_message('error', '#ERR2351: Updation failed in settlement_basic RTPS Case No '.$application_no);
                    $data = array(
                        'error'=>"#ERR2351: Registration of Settlement failed for case no : ".$application_no
                    );
                    echo json_encode($data);
                    return false;
                }
                ...
                if($validation_bypass == 0)
                {
                    ...
                    foreach ($lmdata['dags'] as $dags_doc)
                    {
                        ...
                        if(FILE_UPLOAD_REQUIRE_IN_DHARITREE == 1 && IS_PRODUCTION == 1)  //
                        {
                            ...
                            if(!move_uploaded_file($trace_map_file['tmp_name'], $trace_upload_path)){
                                log_message('error', '#ERRADDDOC00711001: Unable to move trace map file for case no '.$case_no);
                                $this->db->trans_rollback();
                    ...
                    $responseMasterObj = $this->SettlementCommonModel->lmRejectedValidationBypassFalse(BHODDAN_SERVICE_CODE);
                    ...
                    if ($insLmnote != 1) {
                        $this->db->trans_rollback();
                        log_message('error', '#ERR2027: Insertion failed in settlement_ap_lmnote RTPS Case No '.$application_no);
                        $data = array(
                            'error'=>"#ERR2027: Registration of Settlement failed for case no : ".$application_no
                        );
                        echo json_encode($data);
                        return false;
                    }
                ...
                //////proceeding start//////
                $proceeding_id = $this->db->query("Select max(proceeding_id)+1 as c from settlement_proceeding 
                            where case_no='$case_no' ")->row()->c;
                ...
                if ($insertProceeding != 1) {
                    $this->db->trans_rollback();
                    log_message('error', '#ERR3038: Insertion failed in settlement_proceeding for case no :'. $case_no);
                    $json = [
                        'errorMessage'=>"#ERR3038: Failed to forward the case for Case No : ".$case_no
                    ];
                    echo json_encode($json);
                    return false;
                }
                ...
                else
                {
                    ...
                    if (trim($rtps_status)!="y") {
                        $this->db->trans_rollback();
                        $this->session->set_flashdata('message', "Error #ERR3065: Settlement Application not submitted case no # $case_no");
                        redirect(base_url() . "index.php/home");
                    } else {
                        $this->db->trans_commit();
                    }
    ...
    function revertedCases()
    {
        ...
        if (!empty($cases['cases'])){
            ...
            }else if($cases['cases'][0]->service_code=="39"){
                $cases['service_controller']="BhoodanController";
            }
        }
    ...
    public function secondProceeding()
    {
        ...
        //                     $open_adhar_file = fopen($adhar_photo_link, "r") or die("Unable to open file! 5");
        //                     $read_adhar_file = fread($open_adhar_file, filesize($adhar_photo_link));
        //                     fclose($open_adhar_file);
        //                     // decoding the base64 encoding file variable
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            if($distCode == NULL)
            {
                redirect(base_url(). 'index.php/home/SettlementKhasLandLm?service='.BHODDAN_SERVICE_CODE);
            }
            if($case_no == NULL)
            {
                redirect(base_url(). 'index.php/home/SettlementKhasLandLm?service='.BHODDAN_SERVICE_CODE);
            ...
            foreach($lmdata['applicants_buyers'] as $adhar_photo):
                if($adhar_photo->is_applicant == 1):
                    if (trim($adhar_photo->identity_type) == 'AADHAAR'):
                        ...
                        $open_adhar_file = fopen($adhar_photo_link, "r") or die("Unable to open file! 6");
                        $read_adhar_file = fread($open_adhar_file, filesize($adhar_photo_link));
                        fclose($open_adhar_file);
                        // decoding the base64 encoding file variable
            ...
            if($validation_bypass == 0)
            {
                ...
                if(count($geo_dag_no_array) < count($dag_no_array))
                {
    ...
    public function getChithaApproveAppliedAppAreaCalculation()
    {
        ...
        else
        {
            ...
            $case      = $this->BhoodanModel->getAppDetailsByCaseNoDistCode($case_no, $dist_code);
            ...
            if($case->num_rows() > 0)
            {
                ...
                $curl_handle = curl_init();
                curl_setopt($curl_handle, CURLOPT_URL, API_LINK_MB3 . "getTotalVillageAppliedArea");
                curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, true);
                curl_setopt($curl_handle, CURLOPT_SSL_VERIFYPEER, false);
                curl_setopt($curl_handle, CURLOPT_CUSTOMREQUEST, "POST");
                curl_setopt($curl_handle, CURLOPT_SSL_VERIFYHOST, 2);
            ...
            foreach ($query->result() as $rows) {
                ...
                else
                {
                    ...
                    $khas_link = '<a type="button" href="' . base_url() . 'index.php/SettlementKhasCo/settlementKhasCo?case=' . $rows->case_no . '" class="lmreportmut btn-sm btn btn-primary">
                        write report</a>';
                    $bhoddan_link = '<a type="button" href="' . base_url() . 'index.php/SettlementKhasCo/settlementKhasCo?case=' . $rows->case_no . '" class="lmreportmut btn-sm btn btn-primary">
                        write report</a>';
